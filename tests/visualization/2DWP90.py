...
axes[0,0].set_title('Large-scale zonal wind u [m/s]')
pplot00 = axes[0,0].contourf(xx, zz, uLS,
                             cmap = 'RdBu_r',
                             vmin =  -0.044,
                             vmax = 0.044,
                             levels = np.arange(-0.044, 0.0441, 0.004))
plt.colorbar(pplot00, ax = axes[0,0],
             ticks = np.arange(-0.044, 0.0441, 0.004)[1::2])

axes[0,1].set_title('Large-scale meridional wind v [m/s]')
pplot01 = axes[0,1].contourf(xx, zz, vLS,
                             cmap = 'RdBu_r',
                             vmin = -0.02,
                             vmax = 0.02,
                             levels = np.arange(-0.02, 0.021, 0.002))
plt.colorbar(pplot01, ax = axes[0,1],
             ticks = np.arange(-0.02, 0.021, 0.002)[1::2])

axes[1,0].set_title('Large-scale vertical wind w [m/s]')
pplot10 = axes[1,0].contourf(xx, zz, wLS,
                             cmap = 'RdBu_r',
                             vmin = -12e-5,
                             vmax = 12e-5,
                             levels = np.arange(-12e-5, 12.1e-5, 2e-5))
plt.colorbar(pplot10, ax = axes[1,0],
             ticks = np.arange(-12e-5, 12.1e-5, 2e-5)[1::2])

axes[1,1].set_title('Large-scale tracer distribution')
pplot11 = axes[1,1].contourf(xx, zz, tLS,
                             cmap = 'RdBu_r',
                             vmin =  -1.2,
                             vmax = 1.2,
                             levels = np.arange(-1.2, 1.3, 0.1))
plt.colorbar(pplot11, ax = axes[1,1],
             ticks = np.arange(-1.2, 1.3, 0.1)[1::3])
...
if data_path != reference_path:
	...
	axes[0,0].set_title('Large-scale zonal wind u [m/s]')
	pplot00 = axes[0,0].contourf(xx, zz, uLS-uLSref,
									cmap = 'RdBu_r',
									vmin =  -0.044,
									vmax = 0.044,
									levels = np.arange(-0.044, 0.0441, 0.004))
	...
	axes[0,1].set_title('Large-scale meridional wind v [m/s]')
	pplot01 = axes[0,1].contourf(xx, zz, vLS-vLSref,
									cmap = 'RdBu_r',
									vmin = -0.02,
									vmax = 0.02,
									levels = np.arange(-0.02, 0.021, 0.002))
	...
	axes[1,0].set_title('Large-scale vertical wind w [m/s]')
	pplot10 = axes[1,0].contourf(xx, zz, wLS-wLSref,
									cmap = 'RdBu_r',
									vmin = -12e-5,
									vmax = 12e-5,
									levels = np.arange(-12e-5, 12.1e-5, 2e-5))
	...
	axes[1,1].set_title('Large-scale tracer distribution')
	pplot11 = axes[1,1].contourf(xx, zz, tLS-tLSref,
									cmap = 'RdBu_r',
									vmin =  -1.2,
									vmax = 1.2,
									levels = np.arange(-1.2, 1.3, 0.1))
