...
# Import data.
data = ModelOutput(data_path * "coldBubble")
reference = ModelOutput(reference_path * "coldBubble")

println("Time: ", data.variables["t"][end], " s")

# Set grid.
x = data.variables["x"][257:447] .* 0.001
z = data.variables["z"][:] .* 0.001
x = [xi for xi in x, j in 1:size(z)[1]]
z = [zj for i in 1:size(x)[1], zj in z]

# Get density fluctuations.
thetap = data.groups["atmvar"].variables["thetap"][257:447, 1, :, end]
thetapr = reference.groups["atmvar"].variables["thetap"][257:447, 1, :, end]
...
savefig(data_path * "/results/coldBubble.png")
...
if data_path != reference_path
  ...
  savefig(data_path * "/results/coldBubble_difference.png")
end
