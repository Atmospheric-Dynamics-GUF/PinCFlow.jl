...
# Import data.
data = ModelOutput(data_path * "hotBubble3D")
reference = ModelOutput(reference_path * "hotBubble3D")
...
thetap = data.groups["atmvar"].variables["thetap"][:, 17, :, end]
thetapr = reference.groups["atmvar"].variables["thetap"][:, 17, :, end]
...
savefig(data_path * "/results/hotBubble3D.png")
...
if data_path != reference_path
  ...
  savefig(data_path * "/results/hotBubble3D_difference.png")
end
