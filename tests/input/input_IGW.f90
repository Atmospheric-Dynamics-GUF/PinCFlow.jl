...
&domain
  ...
  lx_dim(1)                  =                  3.0E+5 ! Maximum of x
  ly_dim(0)                  =                  0.0E+0 ! Minimum of y
  ly_dim(1)                  =                  1.0E+3 ! Maximum of y
  lz_dim(0)                  =                  0.0E+0 ! Minimum of z
  lz_dim(1)                  =                  1.0E+4 ! Maximum of z
...
&outputList
  ...
  nOutput                    =                       1 ! Output every nOutput
                                                       ...
                                                       ! outputType =
                                                       ! 'timeStep'
  maxIter                    =                       5 ! Stop after maxIter
                                                       ! time steps for
                                                       ! outputType =
                                                       ! 'timeStep'
  outputTimeDiff             =                  3.0E+3 ! Output every
                                                       ! outputTimeDiff seconds
                                                       ! for outputType =
                                                       ! 'time'
  maxTime                    =                  3.0E+3 ! Stop after maxTime
                                                       ! seconds for outputType
                                                       ! = 'time'
  restart                    =                       F ! Restart the model from
                                                       ! state in previous
                                                       ! simulation
  thetaOffset                =                       T ! Subtract background
                                                       ! potential temperature
  rhoOffset                  =                       F ! Subtract background
                                                       ! density
...
&testCaseList
  testCase                   =      'SkamarockKlemp94' ! Predefined test case
&end
...
&wavePacket
  ...
  lambdaX_dim                =                  3.0E+5 ! Wavelength in x (0.0
                                                       ! for infinite
                                                       ! wavelength)
...
&LagrangeRayTracing
  ...
  yrmin_dim                  =                  0.0E+0 ! Backward bound of
                                                       ! initial rays
  yrmax_dim                  =                  5.0E+4 ! Forward bound of
                                                       ! initial rays
  zrmin_dim                  =                  3.0E+3 ! Bottom bound of
                                                       ! initial rays
  zrmax_dim                  =                  7.0E+4 ! Top bound of initial
                                                       ! rays
  ...
  amp_wkb                    =                  5.0E-1 ! Relative amplitude of
                                                       ! wave packet
  wlrx_init                  =                  1.0E+5 ! Wavelength in x of
                                                       ! wave packet
  wlry_init                  =                  0.0E+0 ! Wavelength in y of
                                                       ! wave packet
...
&baroclinic_LC
  ...
  Sponge_Rel_Bal_Type        =                   'env' ! Relaxation state:
                                                       ...
                                                       ! balance, 'env' =
                                                       ! geostrophic balance
  ta_hs_dim                  =                3.456E+6 ! Thermal-relaxation
                                                       ! time scale outside
                                                       ! tropical boundary
                                                       ! layer (zero means
                                                       ! infinity)
  ts_hs_dim                  =                3.456E+5 ! Thermal-relaxation
                                                       ! time scale in tropical
                                                       ! boundary layer (zero
                                                       ! means infinity)
  tf_hs_dim                  =                 8.64E+4 ! Boundary-layer
                                                       ! Rayleigh-damping  time
                                                       ! scale
  sigb_hs                    =                  7.0E-1 ! Sigma of boundary-
                                                       ! layer top
&end
...
&solverList
  cfl                        =                  9.0E-1 ! CFL number
  cfl_wave                   =                  9.0E-1 ! WKB CFL number
  dtMax_dim                  =                  1.0E+4 ! Maximum time step
  ...
  DySmaScheme                =                       F ! Dynamic Smagorinsky
                                                       ! scheme
  dtWave_on                  =                       F ! Limit time step by
                                                       ! inverse buoyancy
                                                       ! frequency
  ...
  dens_relax                 =                       F ! Heating by density
                                                       ! relaxation
  shap_dts_fac               =                  0.0E+0 ! Shapiro-filter damping
                                                       ! time
...
&atmosphereList
  ...
  backgroundFlow_dim(1)      =                  2.0E+1 ! Initial wind
...
&boundaryList
  ...
  sponge_uv                  =                       F ! Sponge layer for
                                                       ...
                                                       ! unifiedSponge =
                                                       ! .false.
  spongeHeight               =                  3.0E-1 ! Relative height of
                                                       ! lower sponge layer
                                                       ! edge (scale height for
                                                       ! unifiedSponge = .true.
                                                       ! and spongeType =
                                                       ! 'exponential')
  spongeAlphaZ_dim           =                  1.4E-4 ! Maximum relaxation
