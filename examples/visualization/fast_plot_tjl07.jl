...
"""
Compute the coarse-grid fraction of exceedances for a 2D slice.

Inputs:
- X, Z: 2D coordinate arrays (same shape as F)
- F: 2D field values

Keyword args:
- thresh: threshold for exceedance (F > thresh)
- nxc, nzc: number of coarse bins in x and z

Returns: (xc, zc, frac)
- xc, zc: 1D arrays of coarse-cell centers (length nxc, nzc)
- frac: nxc × nzc matrix of exceedance fractions (NaN where no samples)
"""
function coarse_fraction(
	X::AbstractArray,
	Z::AbstractArray,
	F::AbstractArray;
	thresh::Real = 0.0,
...
"""
Build a simple PDF over fraction values in [0,1]. Returns (centers, pdf).
"""
...
data = h5open("/home/dolaptch/PF/runs/tjl07/pincflow_output.h5")  #LES
data2 = h5open("/home/dolaptch/PF/runs/tjl08/pincflow_output.h5") # RT
data3 = h5open("/home/dolaptch/PF/runs/tjl09/pincflow_output.h5") # RT resolved sgs q_v
...
x2c = [xi for xi in x2c, iy in 1:size(z2c)[2], iz in 1:size(z2c)[3]]

iy = 1
tidx = 3 #length(data["w"][1, 1, 1, :])  # for a 4D array, 4th dimension is often "time"
...
subplot(132)
h2 = pcolormesh(x2[:, iy, :], z2[:, iy, :], fld3[:, iy, :]; cmap = "Blues")
colorbar(h2)
contour(x2[:, iy, :], z2[:, iy, :], fld3[:, iy, :], levels = [0.0], colors = "k", linewidths = 0.5)
#clim(vlow, vup)
ylim(yaxis_limits)
title("RT")
...
pc = pcolormesh(x2c[:, iy, :], z2c[:, iy, :], clc[:, iy, :]; cmap="Blues", vmin=0.0, vmax=1.0)
...
Z = z2[:, iy, :]
F = fld3[:, iy, :]
xc, zc, frac = coarse_fraction(X, Z, F; thresh = thresh, nxc = nxc, nzc = nzc)
pc = pcolormesh(xc, zc, frac'; cmap="Blues", vmin=0.0, vmax=1.0)
colorbar(pc)
 title("RT res.")
...
# Compute coarse fractions and bin centers using the helper
xc, zc, frac = coarse_fraction(X, Z, F; thresh = thresh, nxc = nxc, nzc = nzc)

pc = pcolormesh(xc, zc, frac'; cmap="Blues", vmin=0.0, vmax=1.0)
colorbar(pc)
xlabel("x (km)"); ylabel("z (km)"); title("LES Fraction > $(thresh) in coarse cells (RT)")
...
# (moved helper functions coarse_fraction and fraction_pdf above so they are available before first use)
