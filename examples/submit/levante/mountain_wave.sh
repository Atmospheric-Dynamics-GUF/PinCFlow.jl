...
# srun --cpu_bind=verbose --distribution=block:cyclic julia --project --check-bounds=no examples/submit/mountain_wave.jl ${scratch} 1>${scratch}/run.log 2>&1

# Run the model on interactive partition.
mpiexec -n 64 julia --project --check-bounds=no examples/submit/mountain_wave.jl ${scratch} 1>${scratch}/run.log 2>&1
