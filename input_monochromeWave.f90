...
&grid
  ...
  nby = 3,
  nbz = 3,
  lx_dim = -4000.0, 4000.0,       ! domain lenths in m
  ly_dim = 0.0, 6000.0, 
  lz_dim = 0.0, 6000.0,
...
&modelList
  ...
  model = "Boussinesq"    ! pseudo_incompressibl / Boussinesq / WKB
  vert_theta = 89.5 !deg    angle of rotation about y
...
&solverList
  ...
  cfl = 0.9
  dtMax_dim = 3.0 !s              ! max time step in s
...
&atmosphereList
  ...
  f_Coriolis_dim = 1.36672e-4 ! 1/s       Coriolis parameter
...
&boundaryList
  ...
  vFluxCorr = .false.
  wFluxCorr = .false.
  thetaFluxCorr = .true.   ! replace vertical theta flux by CDS at k=1, k=nz-1
  nbCellCorr = 1
...
&outputList
  ...
  outputTimeDiff =  30.0  !s    ! output every ... seconds
  maxTime = 300.0 !s          ! stop after maxTime seconds
  ...
  varOut = 1,1,1, 1,1,1   ! 1 = output, 0 = no output 
...
&testCaseList
  ...
  testCase = "monochromeWave"
  ! Boussinesq: uniform_theta / Robert_Bubble / wavePacket / monochromeWave
  ! Pseudo-incompressible: agnesiMountain

&end

&monochromeWave

  lambda_dim = 6000.0 !m       vertical wave length
