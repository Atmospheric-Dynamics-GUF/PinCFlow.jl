...
&solverList
  ...
  cfl = 0.5
  dtMax_dim = 30.0 !s              ! max time step in s
  ...
  timeScheme = "RungeKutta3"      ! RungeKutta3 / Euler / TVD-RungeKutta3
...
&atmosphereList
  ...
  !                     uniform    -> no stratification (Boussinesq)
  
  theta0_dim = 300 ! K      
  !                     background pot. temp. in K for 'isentropic'
  !                     ground pot. temp. in K for 'const-N' atmosphere
  !                     background pot temp in K for Boussinesq
  
  Temp0_dim = 300  ! K  background temperature in K for 'isothermal' 
  
  press0_dim =  101325.0 !Pa  ground pressure (at z=0) in Pa
  
  N_BruntVaisala_dim = 0.01   ! Brunt-Vaisala frequency for 
  !                             "const-N" atmosphere in 1/s
  backgroundFlow_dim =  0.0, 0.0, 0.0 !m/s background flow
...
&topographyList
  ...
  topography0 = .false.   ! set w explicitly at k=1
...
&outputList
  ...
  maxTime = 600.0 !s          ! stop after maxTime seconds
...
!----------------
!    bubbles: hotBubble, coldBubble, hotBubble3D -> bubble
!             Robter_Bubble -> Robert_Bubble
!----------------
&Robert_Bubble
