...
&domain
  ...
  nby = 2,
  nbz = 2,
  lx_dim =    0.0,  1.e7 ! domain lenths in m
  ly_dim =   -2.e7, 2.e7
  lz_dim =    0.0,  16.e4
...
&solverList
  ...
  timeScheme = "semiimplicit" !"LS_Will_RK3" !"semiimplicit"      ! LS_Will_RK3 -> Williamson / Euler /
  ...
  shap_dts_dim = 5.e3 !0. !5.e3             ! horizontal-Shapiro-filter damping time 
...
&outputList
  ...
  outputTimeDiff =  5.e4   ! output every ... seconds
  maxTime = 1.e6 !6.e6 !1.e5 !3.e6           ! stop after maxTime seconds
...
&baroclinic_LC
  ...
  ts_hs_dim = 3.456e5           ! thermal-relaxation time scale in
                                ! tropical boundary layer (s)
                                ! zero means infinty
  tf_hs_dim = 8.54e4            ! boundary-layer Rayleigh-damping time 
                                ! scale (s)
                                ! zero means infinty
  sigb_hs = 0.7                 ! sigma of boundary-layer top

  ! used both by Kuehnlein et al (2012) and Held & Suarez (1994):
  jwdth_dim = 5.e6              ! half (!) jet width (m)

  add_ptptb = .true. !.false. !.true.            ! add local potential-temperature 
                                ! perturbation
