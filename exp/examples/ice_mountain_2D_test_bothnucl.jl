# settings in IcePredictands: S_issr = 1.5, N_in = 10
...
grid = GridNamelist(;
    resolved_topography = (x, y) -> 
        x^2 <= (l0)^2 ?
        h0 / 2 * (1 + cos(pi / l0 * x)) : 0.0
)

ice = IceNamelist(;
	ice_setup = IceOn(),
#	ice_test_case = MultipleWavePackets(),
    ice_test_case = PinCFlow.Types.NamelistTypes.NoIceTestCase(), 
	dt_ice = 2.0,
	nscx = 1,
	nscy = 1,
	nscz = 1,
...
output =
    OutputNamelist(; output_variables = (:w, :u, :n_het, :qv, :q_het, :n_hom, :q_hom, :iaux1, :thetap, :pip, :rhop), 
    output_steps = false,
	output_interval = 50.0,
	tmax = 300.0,
    output_file = "ice_mountain_2D_test_bothnucl.h5")
