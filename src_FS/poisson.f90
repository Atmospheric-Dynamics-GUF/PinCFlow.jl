...
contains
  ...
  subroutine Corrector( var,flux,dMom,dt,errFlagBicg,nIter,m,opt,diffTermSwitch)
  !UAE
    ! -------------------------------------------------
    ...
    call calc_RHS(rhs, var, flux, dt, onlyinfo,diffTermSwitch)
    !UAE
    ...
    if (detailedinfo) call calc_RHS( rhs,var,flux,dt,detailedinfo,diffTermSwitch)
    !UAE
  ...
  subroutine calc_RHS( b,var,flux,dt,onlyinfo,diffTermSwitch)
  !UAE
    ...
    case( "pseudo_incompressible" ) 
       ...
       do k = 0,nz
          coef_t(k) = tRef/turb_dts * dz**2 *rhoStrat(k)
       end do
       ...
       do k = 1,nz
          do j  = 1,ny
             do i = 1,nx
                ...
                hD = flux(i,j,k-1,3,5) ! downward
                hU = flux(i,j,k,3,5)   ! upward
                ! hD = coef_t(k-1) &
                ...
                if (testCase == 'coldBubble' .and. diffTermSwitch == 1) then
                   b(i,j,k) = bu + bv + bw + heat(i,j,k) + fluxDiff 
                else 
          ...
          do k = 1,nz
             do j = 0,ny
                do i = 1,nx
                   ...
                   dv = - dt/(facu*facv + (0.5*(f_cor_nd(j)+f_cor_nd(j+1))*dt)**2) &
                          * (- 0.5*(f_cor_nd(j)+f_cor_nd(j+1))*dt * pGradX + facu * pGradY)
       ...
       do k = 1,nz
          do j = 1,ny
             do i = 1,nx
                ...
                acontr = 0.25 * dxy * pStrat(k)**2/rhoEdge &
                         * (-0.5*(f_cor_nd(j)+f_cor_nd(j+1))*dt)/(facu*facv + (0.5*(f_cor_nd(j)+f_cor_nd(j+1))*dt)**2)
                ...
                acontr = dy2 * pStrat(k)**2/rhoEdge &
                         * facu/(facu*facv + (0.5*(f_cor_nd(j)+f_cor_nd(j+1))*dt)**2)
