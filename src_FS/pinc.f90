program pinc_prog
  ...
  integer :: diffTermSwitch !for coldBubble test case
...
!FSE
        ...
        if (heatingONK14 .or. TurbScheme .or. rayTracer) then
        ! !if (heating) then
         ! begin FS_July2020 -> to be consistent with BK19 si-scheme; auskommentiert
          !  if (model == 'Boussinesq') then
          !     print*, "main:ONeill+Klein2014 heating only for &
          !            & pseudo-incompressible dyn."
          !     stop
          ...
          ! call BGstate_update(var,flux,0.5*dt,RKstage,dPStrat,drhoStrat, &
          !                    & "impl",diffTermSwitch)
        ! end FS_July2020 -> to be consistent with BK19 si-scheme
           !call Corrector ( var, flux, dMom, 0.5*dt, errFlagBicg, nIterBicg, &
           !    & RKstage, "expl",diffTermSwitch)
           CONTINUE
        else
          call Corrector ( var, flux, dMom, 0.5*dt, errFlagBicg, nIterBicg, &
               & RKstage, "expl",diffTermSwitch)
        ...
        !if (heating) then
           ...
           call BGstate_update(var,flux,dt,RKstage,dPStrat,drhoStrat, &
                           & "impl",diffTermSwitch) ! FS_July2020 -> to be consistent with BK19 si-scheme (0.5*dt -> dt)
           call set_spongeLayer(var, dt, "ref") !UA 200413
           call Corrector ( var, flux, dMom, dt, errFlagBicg, nIterBicg, &
                & RKstage, "impl",diffTermSwitch) ! FS_July2020 -> to be consistent with BK19 si-scheme (0.5*dt -> dt)
        else
           call Corrector ( var, flux, dMom, 0.5*dt, errFlagBicg, nIterBicg, &
                       & RKstage, "impl",diffTermSwitch) 
       ...
       450    continue
           ...
           if( correctMomentum ) then
              ...
              call Corrector ( var, flux, dMom, dt_Poisson, errFlagBicg, &
                             & nIterBicg, RKstage, "expl" ,diffTermSwitch)
              !UAE
