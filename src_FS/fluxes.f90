...
!   variables for the turbulence scheme      
    ...
    do k = 0,nz
       do j = 1,ny
          do i = 1,nx
             ...
             if (testCase == 'coldBubble')then
                flux(i,j,k,3,5) = rhoStrat(k)*coef_t*( Pstrat(k+1)/rhoStrat(k+1) - Pstrat(k)/rhoStrat(k) ) / dz
             else
  ...
  subroutine massFlux_old (vara,var,flux,fluxmode)
    !---------------------------------------------------------------------
    ! computes the mass flux at all cell edges using reconstructed values
    ! fluxmode = lin => linear flux, advecting velocities prescribed in 
    !                   vara
...
!-----------------------------------------------------------------------!
 ...
 subroutine momentumFlux_old (vara,var,flux,fluxmode)
  ...
  end subroutine momentumFlux_old
