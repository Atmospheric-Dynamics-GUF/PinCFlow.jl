  ...
  subroutine timestep (var,dt,errFlag)
    ...
    !-------------------------------------------
       ...
       case( "Boussinesq", "pseudo_incompressible")
          ...
          !UAB
          !dtWave = pi/(NN+small)
          dtWave = 1./(NN+small)!1.7/(NN+small) !FS -> to be consistent with Rieper et al. (2013)
          !UAE
...
!---------------------------------------------------------------------
  ...
  !UAB 200413
  subroutine BGstate_update(var,flux,dt,m,q_P,q_rho,int_mod, diffTermSwitch)
  !UAE 200413
  ...
  ! in/out variables
    ...
    integer, intent(in) :: diffTermSwitch 
    ...
    do k = 1,nz!1,nz
       ...
       if(int_mod == "expl")then
          ...
          ! hD = coef_t(k-1) &
          !           * (Pstrat(k)/rhoStrat(k) - Pstrat(k-1)/rhoStrat(k-1))/dz
          ! hU =  coef_t(k) &
          !           * (Pstrat(k+1)/rhoStrat(k+1) - Pstrat(k)/rhoStrat(k))/dz
          hD = flux(1,1,k-1,3,5) ! downward
          hU = flux(1,1,k,3,5)   ! upward
          
          ! convective part
          fluxDiff =  (hU-hD)/dz
          
          ! update: q(m-1) -> q(m)  
          if (testCase == 'coldBubble' .and. diffTermSwitch == 1) then
             q_P(k) = alpha(m) * q_P(k) - dt*divPw(k) - dt*S_bar(k) + dt*fluxDiff
          else
          ...
          Pstrat(k) = Pstrat(k) + beta(m) * q_P(k) !FS_PIold
          ...
          rhostrat(k) = rhostrat(k) + beta(m) * q_rho(k) !FS_PIold
       else if (int_mod == "impl")then
          ...
          if (testCase == 'coldBubble' .and. diffTermSwitch == 1) then
             PStrat(k) = PStrat(k) - dt*divPw(k) - dt*S_bar(k) + dt*fluxDiff !FS_PIold
          else
             PStrat(k) = PStrat(k) - dt*divPw(k) - dt*S_bar(k) !FS_PIold
          end if
          rhoStrat(k) = rhoStrat(k) - dt*divrhow(k) !FS_PIold
