    ...
    case( "var" )
       ...
       if( updateMass ) then
          ...
          if (testCase == "baroclinic_LC") then 
             if (fluctuationMode) then
                var(1:nx,1:ny,0,1) &
                = - var(1:nx,1:ny,1,1) + var_env(1:nx,1:ny,1,1) - rhoStrat(1) &
                  + var_env(1:nx,1:ny,0,1) - rhoStrat(0)

                var(1:nx,1:ny,nz+1,1) &
                = - var(1:nx,1:ny,nz,1) &
                  + var_env(1:nx,1:ny,nz,1) - rhoStrat(nz) &
                  + var_env(1:nx,1:ny,nz+1,1) - rhoStrat(nz+1) 
               else
                var(1:nx,1:ny,0,1) &
                = - var(1:nx,1:ny,1,1) + var_env(1:nx,1:ny,1,1) &
                  + var_env(1:nx,1:ny,0,1)

                var(1:nx,1:ny,nz+1,1) &
                = - var(1:nx,1:ny,nz,1) + var_env(1:nx,1:ny,nz,1) &
                  + var_env(1:nx,1:ny,nz+1,1)
             end if
           else
             do k = 1,nbz
                var(:,:,-k+1,1) = -var(:,:,k,1)
          ...
          if (timeScheme == "semiimplicit" .or. auxil_equ) then
             ...
             if (testCase == "baroclinic_LC") then 
                var(1:nx,1:ny,0,6) &
                = - var(1:nx,1:ny,1,6) + var_env(1:nx,1:ny,1,1) - rhoStrat(1) &
                  + var_env(1:nx,1:ny,0,1) - rhoStrat(0)
       ...
       if( correctMomentum ) then
          ...
          if (testCase == "baroclinic_LC") then 
             ! z = 0
             var(1:nx,1:ny,0,5) &
             = var(1:nx,1:ny,1,5) - var_env(1:nx,1:ny,1,5) + var_env(1:nx,1:ny,0,5)

             ! z = zMax
             var(1:nx,1:ny,nz+1,5) &
             = var(1:nx,1:ny,nz,5) - var_env(1:nx,1:ny,nz,5) + var_env(1:nx,1:ny,nz+1,5)  
            else
             ! z = 0
             var(:,:,0,5) = var(:,:,1,5)       
