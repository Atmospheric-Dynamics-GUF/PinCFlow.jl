...
!---------------------------------------------------------------------
  ...
  !subroutine momentumPredictor (var,flux,force,dt,q,m,mmp_mod,int_mod, &
    ...
    real, dimension(-nbx:nx+nbx,-nby:ny+nby,-nbz:nz+nbz) :: heat,rhopw_bar
    ...
    if (mmp_mod == "tot" .or. mmp_mod == "lhs") then
       ...
       if (int_mod == "expl") then
          do k = k0,k1
             ...
             do j = 1,ny
                do i = 1,nx
                   ...
                   if (spongeLayer) then
                      wAst = wAst - dt*0.5*(kr_sp(j,k)+kr_sp(j,k+1))*wvert
                   end if
          ...
          if (heatingONK14 .or. TurbScheme .or. rayTracer) then
             call heat_w0(var,var,flux,dt,heat,S_bar,w_0,rhopw_bar)
            else
             heat = 0.
             S_bar = 0.
             w_0 = 0.
...
!---------------------------------------------------------------------
  ...
  subroutine BGstate_update(var0,var,flux,dt,m,q_P,q_rho,int_mod,heating_switch)
  ...
  ! in/out variables
    ...
    real, dimension(-nbz:nz+nbz) :: divPw, divrhow, rhopw_bar !UA
    ...
    do k = 1,nz-1
       do j = 1,ny
          do i = 1,nx
             sum_local(k) &
             = sum_local(k) &
               + var0(i,j,k,4) * 0.5*(var0(i,j,k,1) + var0(i,j,k+1,1))
          end do
       end do
    end do
    call mpi_allreduce(sum_local(1),sum_global(1),&
    ...
    ! also take into account that the vertical density-fluctuation flux 
    ! is on half levels while heating etc is located on full levels

    sum_n &
    = sum_n &
      - S_bar(1)/PStrat(1) - 0.5*rhopw_bar(1) * g_ndim/(gamma*press0(1))
    sum_d = sum_d +  1./(gamma*press0(1))

    do k = 2,nz-1
       sum_n &
       = sum_n &
         - S_bar(k)/PStrat(k) &
         - 0.5*(rhopw_bar(k) + rhopw_bar(k-1)) * g_ndim/(gamma*press0(k))
       sum_d = sum_d +  1./(gamma*press0(k))
    end do
    ...
    = sum_n &
      - S_bar(nz)/PStrat(nz) &
      - 0.5*rhopw_bar(nz-1) * g_ndim/(gamma*press0(nz))
    sum_d = sum_d +  1./(gamma*press0(nz))
    ...
    = dz &
      *(- S_bar(1)/Pstrat(1) &
        - (dptopdt + g_ndim * 0.5*rhopw_bar(1))/(gamma*press0(1)))
    ...
    do k = 2,nz-1
       ...
       = w_0(k-1) &
         + dz &
           *(- S_bar(k)/Pstrat(k) &
             - (dptopdt &
                + g_ndim * 0.5*(rhopw_bar(k) + rhopw_bar(k-1))) &
                  /(gamma*press0(k)))
!    end do
    end do
    ...
    do k=1,nz
       ...
       divrhow(k) &
            = (  rhoStratTilde(k)*w_0(k) + rhopw_bar(k) &
            - rhoStratTilde(k-1)*w_0(k-1) - rhopw_bar(k-1))/dz
    end do
