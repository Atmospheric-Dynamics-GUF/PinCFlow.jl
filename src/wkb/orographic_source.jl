function orographic_source!(
  ...
  for jy in j0:j1, ix in i0:i1
    ...
    for iwm in 1:nwm
      ...
      (omi, wnk, wnl, wnm, wad) = orographic_wave_mode(
...
function orographic_source!(state::State, dt::AbstractFloat)
  ...
  for jy in j0:j1, ix in i0:i1
    ...
    for i_sfc in 1:n_sfc
      ...
      # Compute intrinsic frequency, wavenumbers and wave-action density.
      (omir, wnrk, wnrl, wnrm, wadr) = orographic_wave_mode(
        ratio * topography_spectrum[iwm, ix, jy],
        k_spectrum[iwm, ix, jy],
        l_spectrum[iwm, ix, jy],
        uavg,
      ...
      if steady_state
        ...
        if wadr != 0.0
          ...
          if iray > 0 && zr + 0.5 * dzr > ztildetfc[ix, jy, kz]
            ...
            if zr - 0.5 * dzr < ztildetfc[ix, jy, kz] || kz2 == 1
              ...
              rays.z[nrlc, ix, jy, kz + 1] =
                zr + 0.5 * dzr - 0.5 * rays.dzray[nrlc, ix, jy, kz + 1]
          ...
          elseif iray < 0
            ...
            iray = nray[ix, jy, kz]
            if iray > nray_wrk
              error("Error in orographic_source: iray > nray_wrk!")
            end
            ir_sfc[i_sfc, ix, jy] = iray
          end
