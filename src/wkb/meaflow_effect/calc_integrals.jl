function calc_integrals!(state::State, mode::Transient)
  (; domain, grid) = state
  ...
  for kzrv in (k0 - nbz):(k1 + nbz),
    ...
    for iray in 1:nray[ixrv, jyrv, kzrv]
      ...
      cgirz = -wnrm * (omir^2.0 - f_cor_nd^2.0) / (omir * (wnrh^2.0 + wnrm^2.0))

      ixmin, ixmax, jymin, jymax = ixminmax_jyminmax(state, xr, yr, dxr, dyr)
      ...
      for ix in ixmin:ixmax
        ...
        for jy in jymin:jymax
          if sizey > 1
            dyr = (
              min((yr + dyr * 0.5), (ly[0] + (jy + jo) * dy)) -
              max((yr - dyr * 0.5), (ly[0] + (jy + jo - 1) * dy))
            )
          ...
          kzmin = kztildetfc(ix, jy, zr - dzr * 0.5, domain, grid)
          kzmax = kztilfetfc(ix, jy, zr + dzr * 0.5, domain, grid)
          ...
          for kz in kzmin:kzmax
            ...
            wadr = fcpspx * fcpspy * fcpspz * rays.dens[iray, ixrv, iyrv, izrv]
            ...
            if sizex > 1
              if f_cor_nd != 0.0
                integrals.uu[ix, jy, kz] =
                  ...
                  wadr * (
                    wnrk * cgirx -
                    (wnrk * cgirx + wnrl * cgiry) / (1.0 - (omir / f_cor_nd)^2)
                  )
              else
                integrals.uu[ix, jy, kz] =
                  integrals.uu[ix, jy, kz] + wadr * wnrk * cgirx
              end
            end
            ...
            if sizey > 1
              if f_cor_nd != 0.0
                ...
                integrals.vv[ix, jy, kz] =
                  integrals.vv[ix, jy, kz] + wadr * wnrl * cgiry
              end
            end
            ...
            if f_cor_nd != 0.0
              integrals.etx[ix, jy, kz] =
                ...
                (rhostrattfc[ix, jy, kz] * g_ndim * omir * (wnrh^2 + wnrm^2))
            ...
            integrals.e[ix, jy, kz] = integrals.e[ix, jy, kz] + wadr * omir
          end
        end
      end
    end
  ...
  if f_cor_nd != 0.0
    for kz in k0:k1, jy in j0:j1, ix in i0:i1
      ...
      integrals.vtheta[ix, jy, kz] =
        -thetastrattfc[ix, jy, kz] / f_cor_nd * integrals.etx[ix, jy, kz]
    end
  end
end
