...
function correctorStep(semi, dt, opt, facray, facprs)
    ...
    @. var.exner[0:nx+1, 0:ny+1, 0:nz+1] =
        var.exner[0:nx+1, 0:ny+1, 0:nz+1] + dp[0:nx+1, 0:ny+1, 0:nz+1]
            ...
            for j = 1:ny
                for i = 0:nx
                    ...
                    if (spongeLayer && sponge_uv)
                        facu = facu + dt * 0.5 * (kr_sp_tfc[i, j, k] + kr_sp_tfc[i+1, j, k])
                    end
        ...
        for k = 1:nz
            for j = 1:ny
                for i = 1:nx
                    ...
                    rhow0 =
                        (
                            jac(i, j, k + 1) * (var.rho[i, j, k] + rhoStrat[i, j, k]) +
        ...
        for k = 1:nz
            for j = 1:ny
                for i = 1:nx
                    ...
                    fcscal_ru =
                        sqrt(pStrat(i + 1, j, k + 1)^2.0 / rhoStrat(i + 1, j, k + 1))
                    fcscal_rd =
                        sqrt(pStrat(i + 1, j, k - 1)^2.0 / rhoStrat(i + 1, j, k - 1))
                    fcscal_lu =
                        sqrt(pStrat(i - 1, j, k + 1)^2.0 / rhoStrat(i - 1, j, k + 1))
                    fcscal_ld =
                        sqrt(pStrat(i - 1, j, k - 1)^2.0 / rhoStrat(i - 1, j, k - 1))
                    fcscal_fu =
                        sqrt(pStrat(i, j + 1, k + 1)^2.0 / rhoStrat(i, j + 1, k + 1))
                    fcscal_fd =
                        sqrt(pStrat(i, j + 1, k - 1)^2.0 / rhoStrat(i, j + 1, k - 1))
                    fcscal_bu =
                        sqrt(pStrat(i, j - 1, k + 1)^2.0 / rhoStrat(i, j - 1, k + 1))
                    ...
                    fcscal_ruu =
                        sqrt(pStrat(i + 1, j, k + 2)^2.0 / rhoStrat(i + 1, j, k + 2))
                    fcscal_rdd =
                        sqrt(pStrat(i + 1, j, k - 2)^2.0 / rhoStrat(i + 1, j, k - 2))
                    fcscal_luu =
                        sqrt(pStrat(i - 1, j, k + 2)^2.0 / rhoStrat(i - 1, j, k + 2))
                    fcscal_ldd =
                        sqrt(pStrat(i - 1, j, k - 2)^2.0 / rhoStrat(i - 1, j, k - 2))
                    fcscal_fuu =
                        sqrt(pStrat(i, j + 1, k + 2)^2.0 / rhoStrat(i, j + 1, k + 2))
                    fcscal_fdd =
                        sqrt(pStrat(i, j + 1, k - 2)^2.0 / rhoStrat(i, j + 1, k - 2))
                    fcscal_buu =
                        sqrt(pStrat(i, j - 1, k + 2)^2.0 / rhoStrat(i, j - 1, k + 2))
        ...
        for k = 1:nz
            for j = 1:ny
                for i = 1:nx
                    ...
                    rhoEdgeR = 0.5 * (var.rho[i, j, k] + var.rho[i+1, j, k]) + rhoStratEdgeR
                    rhoEdgeL =
                        0.5 * (var.rho[i, j, k] + var.rho(i - 1, j, k)) + rhoStratEdgeL
                    rhoEdgeF =
                        0.5 * (var.rho[i, j, k] + var.rho(i, j + 1, k)) + rhoStratEdgeF
                    rhoEdgeB =
                        0.5 * (var.rho[i, j, k] + var.rho(i, j - 1, k)) + rhoStratEdgeB
                    rhoEdgeU =
                        (
                            jac(i, j, k + 1) * var.rho[i, j, k] +
                            jac[i, j, k] * var.rho(i, j, k + 1)
