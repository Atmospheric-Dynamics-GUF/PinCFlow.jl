program teclayout_program
  ...
  subroutine tecLayout(firstFile, lastFile, skipFile)
    ...
    ! grid
    write (unit = form, fmt = "(a,i1,a,i1,a,i1,a)") "(i", int(log10(real &
        (nx))) + 1, ",a,i", int(log10(real (ny))) + 1, ",a,i", int(log10(real &
        (nz))) + 1, ")" ! format for nx x ny x nz
    ...
    do i = 1, size(optVarOut)
      if (optVarOut(i) == 1) then
        nOutVar = nOutVar + 1
        if (i == 1) variablesChar = trim(variablesChar) // " ""p' [kPa]""  "
        if (i == 2) then
          if (thetaOffset) then
