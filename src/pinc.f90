program pinc_prog
  ...
  if(rayTracer) then
    ...
    call setup_wkb(ray, ray_var3D, var, diffusioncoeff, waveAmplitudes, &
        &dPhase, ray_varIce, ray_cloud = ray_cloud)

    if(include_ice) then
      !* uTime = 0 !set initial time
      call calc_ice(ray, var, ray_varIce, ray_cloud = ray_cloud)
      if(compute_cloudcover) then
        allocate(sc_ice_field(nxNSCX, nyNSCY, nzNSCZ), stat = allocstat)
  ...
  time_loop: do iTime = 1, maxIter
    ...
    if(include_ice .and. time * tRef .gt. timeIceIntegration) then
      ...
      if(master) then
        print *, 'maxval N, Q, Qv', real(maxval(var%ICE(:, :, :, inN)), 4), &
            ...
            &:, inQv)), 4) !, real(maxval(ray_cloud%si))
      end if
    end if !include_ice
