program pinc_prog
  ...
  ! 2) read input.f90
  call setup(var,var0,var1,varG,flux,flux0, &
       & force,source,dRho,dRhop,dMom,dTheta, &
       & dPStrat,drhoStrat,w_0,dIce,dTracer)

  call init_atmosphere      ! set atmospheric background state
  call init_output
  call initialise (var)     ! set initial conditions

  if (include_tracer) call setup_tracer(var) ! **IK**
  ...
  time_loop: do iTime = 1,maxIter
     ...
     if (TestCase == "hotBubble_heat" .or. TestCase == "hotBubble_heatedLayer")then
                 ...
                 do i = 1, nx
                    ...
                    if(lateralSponge) then
                       ...
                       if(x(i00 + i) <= xSponge0) then
                          alphaTFC(i, j, k) &
                               = alphaTFC(i, j, k) + spongeAlphaX &
                               * sin(0.5 * pi * (xSponge0 - x(i00 + i)) &
                               / (xSponge0 - lx(0))) ** 2.0
                       else if(x(i00 + i) >= xSponge1) then
                          alphaTFC(i, j, k) &
                               = alphaTFC(i, j, k) + spongeAlphaX &
                               * sin(0.5 * pi * (x(i00 + i) - xSponge1) &
                               / (lx(1) - xSponge1)) ** 2.0
                       end if
                       ...
                       if(y(j00 + j) <= ySponge0) then
                          alphaTFC(i, j, k) &
                               = alphaTFC(i, j, k) + spongeAlphaY &
                               * sin(0.5 * pi * (ySponge0 - y(j00 + j)) &
                               / (ySponge0 - ly(0))) ** 2.0
                       else if(y(j00 + j) >= ySponge1) then
                          alphaTFC(i, j, k) &
                               = alphaTFC(i, j, k) + spongeAlphaY &
                               * sin(0.5 * pi * (y(j00 + j) - ySponge1) &
                               / (ly(1) - ySponge1)) ** 2.0
                       end if
                    end if
                    ...
                    if(heightTFC(i, j, k) >= zSponge) then
                       alphaTFC(i, j, k) &
                            = alphaTFC(i, j, k) + spongeAlphaZ &
                            * sin(0.5 * pi * (heightTFC(i, j, k) &
     ...
     if (timeScheme == "semiimplicit") then
        ...
        do RKstage = 1, nStages
           ...
           if (include_tracer) then
              call tracerFlux(var0,var,flux,"lin")
           end if
        ...
        Runge_Kutta_Loop: do RKstage = 1, nStages
           ...
           if (     auxil_equ &
              ...
              if(model /= "pseudo_incompressible" &
                   .and. model /= "Boussinesq") then
                 stop "Auxiliary equation only ready for &
                      pseudo-incompressible and Boussinesq"
              end if
           ...
           if (updateTracer ) then
              call tracerFlux (var,var,flux,"nln")
           end if
...
10 if( master ) then   ! modified by Junhong Wei for MPI (20161103)
  ...
  !if (master) print *, "terminate will be skipped"
  call terminate (var,var0,var1,flux,force,source,dRho,dRhop,dMom,dTheta,dIce,dTracer)
