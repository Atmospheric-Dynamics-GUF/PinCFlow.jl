function massUpdate!(semi, ode, dt, upd_var, upd_mod, int_mod, m, facray)
    ...
    (; g_ndim, kappaInv, MaInv2) = equations
        ...
        elseif upd_mod == "rhs"
            if int_mod == "impl"
                ...
                for k in 1:nz, j in 1:ny, i in 1:nx
                    ...
                    rho += rhoStrat[i, j, k]
                    rhow += (jac[i, j, k + 1] * rhoStrat[i, j, k] +
                             jac[i, j, k] * rhoStrat[i, j, k + 1]) /
                            (jac[i, j, k] + jac[i, j, k + 1])
                    rhowm += (jac[i, j, k - 1] * rhoStrat[i, j, k] +
                              jac[i, j, k] * rhoStrat[i, j, k - 1]) /
                             (jac[i, j, k] + jac[i, j, k - 1])
                    ...
                    piREdgeU = (jac(i + 1, j, k + 1) * var.exner(i + 1, j, k) +
                                jac(i + 1, j, k) * var.exner(i + 1, j, k + 1)) /
                               (jac(i + 1, j, k) + jac(i + 1, j, k + 1))
                    piLEdgeU = (jac(i - 1, j, k + 1) * var.exner(i - 1, j, k) +
                                jac(i - 1, j, k) * var.exner(i - 1, j, k + 1)) /
                               (jac(i - 1, j, k) + jac(i - 1, j, k + 1))
                    piREdgeD = (jac(i + 1, j, k - 1) * var.exner(i + 1, j, k) +
                                jac(i + 1, j, k) * var.exner(i + 1, j, k - 1)) /
                               (jac(i + 1, j, k) + jac(i + 1, j, k - 1))
                    piLEdgeD = (jac(i - 1, j, k - 1) * var.exner(i - 1, j, k) +
                                jac(i - 1, j, k) * var.exner(i - 1, j, k - 1)) /
                               (jac(i - 1, j, k) + jac(i - 1, j, k - 1))
                    piFEdgeU = (jac(i, j + 1, k + 1) * var.exner(i, j + 1, k) +
                                jac(i, j + 1, k) * var.exner(i, j + 1, k + 1)) /
                               (jac(i, j + 1, k) + jac(i, j + 1, k + 1))
                    piBEdgeU = (jac(i, j - 1, k + 1) * var.exner(i, j - 1, k) +
                                jac(i, j - 1, k) * var.exner(i, j - 1, k + 1)) /
                               (jac(i, j - 1, k) + jac(i, j - 1, k + 1))
                    piFEdgeD = (jac(i, j + 1, k - 1) * var.exner(i, j + 1, k) +
                                jac(i, j + 1, k) * var.exner(i, j + 1, k - 1)) /
                               (jac(i, j + 1, k) + jac(i, j + 1, k - 1))
                    piBEdgeD = (jac(i, j - 1, k - 1) * var.exner(i, j - 1, k) +
                                jac(i, j - 1, k) * var.exner(i, j - 1, k - 1)) /
                               (jac(i, j - 1, k) + jac(i, j - 1, k - 1))
                    ...
                    buoy = 1.0 /
                           (facw + rhoStrat[i, j, k] / rho * bvsStrat[i, j, k] * dt^2) *
                           (-rhoStrat[i, j, k] / rho *
                            bvsStrat[i, j, k] *
                            dt *
                            jac[i, j, k] *
            ...
            elseif int_mod == "expl"
                for k in 1:nz, j in 1:ny, i in 1:nx
                    ...
                    wvrt = 0.5 * (vertWind(i, j, k, semi) + vertWind(i, j, k - 1, semi))
                    buoy = -g_ndim * rhop / rho
                    buoy -= dt * rhoStrat[i, j, k] / rho * bvsStrat[i, j, k] * wvrt
                    var.rhop[i, j, k] = -buoy * rho / g_ndim
                end
            else
...
function momentumPredictor!(semi, ode, dt, mmp_mod, int_mod, m, facray)
    ...
    (; alphaRK, betaRK, dMom, usave, rhoOld, vOld, uOld, rhopOld) = ode
    (; kappaInv, MaInv2, g_ndim, model) = equations
                ...
                for i in i0:i1
                    ...
                    if mmp_mod == "lhs"
                        ...
                        volForce = volForce +
                                   ...
                                   ((rhoOld[i, j, k] + rhoStrat[i, j, k]) * vC +
                                    (rhoOld[i + 1, j, k] + rhoStrat[i + 1, j, k]) * vR)
                        F = -fluxDiff + volForce
                    end
        ...
        if int_mod == "expl"
            for k in k0:k1, j in 1:ny, i in 1:nx
                ...
                rhoStratEdgeU = (jac[i, j, k + 1] * rhoStrat[i, j, k] +
                                 jac[i, j, k] * rhoStrat[i, j, k + 1]) /
                                (jac[i, j, k] + jac[i, j, k + 1])
                rho000 = rho000 + rhoStrat[i, j, k]
                rho001 = rho001 + rhoStrat[i, j, k + 1]
