                ...
                for k = 1:nz, j = 1:ny, i = 1:nx
                    ...
                    piGradZEdgeD =
                        kappaInv * MaInv2 * pEdgeD / rhowm * (
                            ...
                            met33EdgeD * (var.exner(i, j, k) - var.exner(i, j, k - 1)) / dz
                        )
                    ...
                    buoy =
                        1.0 / (facw + rhoStrat[i, j, k] / rho * bvsStrat[i, j, k] * dt^2) *
                        (
                            -rhoStrat[i, j, k] / rho *
                            bvsStrat[i, j, k] *
                            dt *
            ...
            for k = 1:nz
                for j = 1:ny
                    for i = i0:i1
                        ...
                        if k == 1
                            piUUEdgeR =
                                0.5 * (var.exner[i, j, k+2] + var.exner[i+1, j, k+2])
    ...
    if mmp_mod == "lhs"
        for k = k0:k1, j = 1:ny, i = 1:nx
            ...
            rhoM_1 =
                ...
                (jac[i, j, k] + jac[i, j, k+1])
            rhoM =
                (jac[i, j, k+1] * var.rho[i, j, k] + jac[i, j, k] * var.rho[i, j, k+1]) /
                (jac[i, j, k] + jac[i, j, k+1])

            rhoStratEdgeU =
                (jac[i, j, k+1] * rhoStrat[i, j, k] + jac[i, j, k] * rhoStrat[i, j, k+1]) /
                (jac[i, j, k] + jac[i, j, k+1])
    ...
    elseif mmp_mod == "rhs"
        ...
        if int_mod == "expl"
            for k = k0:k1, j = 1:ny, i = 1:nx
                ...
                # Compute values at cell edges.
                pEdgeU =
                    (jac[i, j, k+1] * pStrat[i, j, k] + jac[i, j, k] * pStrat[i, j, k+1]) /
                    (jac[i, j, k] + jac[i, j, k+1])
                met13EdgeU =
                    (
                        jac[i, j, k+1] * met[i, j, k, 1, 3] +
