...
function massUpdate_rhop_impl!(semi, ode, upd_mod, m)
    ...
    for k = 1:nz, j = 1:ny, i = 1:nx
        ...
        pEdgeU =
            (jac(i, j, k + 1) * pStratTFC[i, j, k] + jac(i, j, k) * pStratTFC[i, j, k+1]) /
            (jac(i, j, k) + jac(i, j, k + 1))
        pEdgeD =
            (jac(i, j, k - 1) * pStratTFC[i, j, k] + jac(i, j, k) * pStratTFC[i, j, k-1]) /
            (jac(i, j, k) + jac(i, j, k - 1))
        ...
        piGradZEdgeU =
            kappaInv * MaInv2 * pEdgeU / rhow * (
                0.5 * met13EdgeU * (var.pi[i+1, j, k] - var.pi[i-1, j, k]) / dx +
                0.5 * met23EdgeU * (var.pi[i, j+1, k] - var.pi[i, j-1, k]) / dy +
                met33EdgeU * (var.pi[i, j, k+1] - var.pi[i, j, k]) / dz
            )

        piGradZEdgeD =
            kappaInv * MaInv2 * pEdgeD / rhowm * (
                0.5 * met13EdgeD * (var.pi[i+1, j, k] - var.pi[i-1, j, k]) / dx +
                0.5 * met23EdgeD * (var.pi[i, j+1, k] - var.pi[i, j-1, k]) / dy +
                met33EdgeD * (var.pi[i, j, k] - var.pi[i, j, k-1]) / dz
            )
        ...
        buoy =
            1.0 / (facw + rhoStratTFC[i, j, k] / rho * bvsStratTFC[i, j, k] * dt^2) * (
...
function massUpdate_rhop_tot!(semi, ode, upd_mod, m)
    ...
    for k = 1:nz
        for j = 1:ny
            for i = 1:nx
                ...
                F = -fluxDiff + rhoStrat[i, j, k] / g_ndim * bvsStrat[i, j, k] * wvrt
