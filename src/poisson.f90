...
contains
          ...
          j_loop: do j = 1,ny
             i_loop: do i = 1,nx
                ...
                if(topography .and. hortot == "tot") then
                    ...
                    ! Correct operator.
                    Ls(i, j, k) = Ls(i, j, k) + ARU * sRU + ARD * sRD &
                                  + ALU * sLU + ALD * sLD + AFU * sFU &
                                  + AFD * sFD + ABU * sBU + ABD * sBD &
                                  + AUU * sUU + ADD * sDD + ARUU * sRUU &
                                  + ARDD * sRDD + ALUU * sLUU + ALDD * sLDD &
                                  + AFUU * sFUU + AFDD * sFDD + ABUU * sBUU &
                                  + ABDD * sBDD
               end if

                ! TFC FJ
                if (timeScheme == "semiimplicit" .and. .not. topography) &
                then
                   if (opt == 'impl') then
                      ! -------------------- A(i,j,k) ---------------------
  ...
  subroutine linearOperatorTestTFC(var, dt, opt, facray)
      ...
      real, dimension((- nbx):(nx + nbx), (- nby):(ny + nby), &
      (- nbz):(nz + nbz), nVar), intent(in) :: var
      ...
      real :: jacInv
      real :: pEdgeRDiv, pEdgeLDiv, pEdgeFDiv, pEdgeBDiv, pEdgeUDiv, &
              pEdgeDDiv
      real :: pEdgeRGra, pEdgeLGra, pEdgeFGra, pEdgeBGra, pEdgeUGra, &
              pEdgeDGra
      ...
      ac_tfc, ar_tfc, al_tfc, af_tfc, ab_tfc, au_tfc, ad_tfc, aru_tfc, &
      ard_tfc, alu_tfc, ald_tfc, afu_tfc, afd_tfc, abu_tfc, abd_tfc, &
      auu_tfc, add_tfc, aruu_tfc, ardd_tfc, aluu_tfc, aldd_tfc, afuu_tfc, &
      afdd_tfc, abuu_tfc, abdd_tfc
              ...
              do j = 1, ny
                  do i = 1, nx
                      ...
                      pEdgeBDiv = 0.5 * (jac(i, j, k) * pStratTFC(i, j, k) &
                                  + jac(i, j - 1, k) * pStratTFC(i, j - 1, k))
                      pEdgeUDiv = 0.5 * (jac(i, j, k) * pStratTFC(i, j, k) &
                                  + jac(i, j, k + 1) * pStratTFC(i, j, k + 1))
                      pEdgeDDiv = 0.5 * (jac(i, j, k) * pStratTFC(i, j, k) &
                                  + jac(i, j, k - 1) * pStratTFC(i, j, k - 1))
                      ...
                      rhoEdgeR = 0.5 * (var(i, j, k, 1) &
                                 + var(i + 1, j, k, 1) &
                                 + rhoStratTFC(i, j, k) &
                                 + rhoStratTFC(i + 1, j, k))
                      rhoEdgeL = 0.5 * (var(i, j, k, 1) &
                                 + var(i - 1, j, k, 1) &
                                 + rhoStratTFC(i, j, k) &
                                 + rhoStratTFC(i - 1, j, k))
                      rhoEdgeF = 0.5 * (var(i, j, k, 1) &
                                 + var(i, j + 1, k, 1) &
                                 + rhoStratTFC(i, j, k) &
                                 + rhoStratTFC(i, j + 1, k))
                      rhoEdgeB = 0.5 * (var(i, j, k, 1) &
                                 + var(i, j - 1, k, 1) &
                                 + rhoStratTFC(i, j, k) &
                                 + rhoStratTFC(i, j - 1, k))
                      rhoEdgeU = 0.5 * (var(i, j, k, 1) &
                                 + var(i, j, k + 1, 1) &
                                 + rhoStratTFC(i, j, k) &
                                 + rhoStratTFC(i, j, k + 1))
                      rhoEdgeD = 0.5 * (var(i, j, k, 1) &
                                 + var(i, j, k - 1, 1) &
                                 + rhoStratTFC(i, j, k) &
                                 + rhoStratTFC(i, j, k - 1))
                      ...
                      if(k == 1) then
                          AC = - jacInv / dx * (pEdgeRDiv / rhoEdgeR &
                               * pEdgeRGra * (1.0 / dx + 0.75 &
                               * met(i, j, k, 1, 3) / dz) + pEdgeLDiv &
                               / rhoEdgeL * pEdgeLGra * (1.0 / dx - 0.75 &
                               * met(i, j, k, 1, 3) / dz)) &
                               - jacInv / dy * (pEdgeFDiv / rhoEdgeF &
                               * pEdgeFGra * (1.0 / dy + 0.75 &
                               * met(i, j, k, 2, 3) / dz) + pEdgeBDiv &
                               / rhoEdgeB * pEdgeBGra * (1.0 / dy - 0.75 &
                               * met(i, j, k, 2, 3) / dz)) &
                               - jacInv / dz * pEdgeUDiv / rhoEdgeU &
                               * pEdgeUGra * met(i, j, k, 3, 3) / dz &
                               + jacInv / dz * pEdgeUDiv / rhoEdgeU * 0.5 &
                               * pStratTFC(i, j, k) / jac(i, j, k) &
                               * (chris(i, j, k, 1, 1) &
                               + chris(i, j, k, 2, 2) &
                               + 2.0 * chris(i, j, k, 1, 3) &
                               * met(i, j, k, 1, 3) &
                          ...
                          AC = - jacInv / dx * (pEdgeRDiv / rhoEdgeR &
                               ...
                               * pEdgeDGra * met(i, j, k, 3, 3) / dz &
                               - jacInv / dz * pEdgeDDiv / rhoEdgeD * 0.5 &
                               * pStratTFC(i, j, k) / jac(i, j, k) &
                               * (chris(i, j, k, 1, 1) &
                               + chris(i, j, k, 2, 2) &
                               + 2.0 * chris(i, j, k, 1, 3) &
                          ...
                          AC = - jacInv / dx * (pEdgeRDiv / rhoEdgeR &
                               * pEdgeRGra / dx + pEdgeLDiv / rhoEdgeL &
                               * pEdgeLGra / dx) &
                               - jacInv / dy * (pEdgeFDiv / rhoEdgeF &
                               * pEdgeFGra / dy + pEdgeBDiv / rhoEdgeB &
                               * pEdgeBGra / dy) &
                               - jacInv / dz * (pEdgeUDiv / rhoEdgeU &
                               * pEdgeUGra * met(i, j, k, 3, 3) / dz &
                               + pEdgeDDiv / rhoEdgeD &
                      ...
                      if(k == 1) then
                          ...
                          AR = jacInv / dx * pEdgeRDiv / rhoEdgeR &
                               * pEdgeRGra / dx &
                               + jacInv / dz * (pEdgeUDiv / rhoEdgeU &
                               * pEdgeUGra * met(i + 1, j, k, 1, 3) * 0.25 &
                               / dx - pEdgeDDiv / rhoEdgeD * pEdgeDGra &
                               * met(i + 1, j, k, 1, 3) * 0.25 / dx)
                      end if
                      ...
                      if(k == 1) then
                          ...
                          AL = jacInv / dx * pEdgeLDiv / rhoEdgeL &
                               * pEdgeLGra / dx &
                               - jacInv / dz * (pEdgeUDiv / rhoEdgeU &
                               * pEdgeUGra * met(i - 1, j, k, 1, 3) &
                               * 0.25 / dx &
                               - pEdgeDDiv / rhoEdgeD * pEdgeDGra &
                               * met(i - 1, j, k, 1, 3) * 0.25 / dx)
                      end if
                      ...
                      if(k == 1) then
                          ...
                          AF = jacInv / dy * pEdgeFDiv / rhoEdgeF &
                               * pEdgeFGra / dy &
                               + jacInv / dz * (pEdgeUDiv / rhoEdgeU &
                               * pEdgeUGra * met(i, j + 1, k, 2, 3) &
                               * 0.25 / dy &
                               - pEdgeDDiv / rhoEdgeD * pEdgeDGra &
                               * met(i, j + 1, k, 2, 3) * 0.25 / dy)
                      end if
                      ...
                      if(k == 1) then
                          AU = jacInv / dx * (pEdgeRDiv / rhoEdgeR &
                               * pEdgeRGra * met(i, j, k + 1, 1, 3) / dz &
                               - pEdgeLDiv / rhoEdgeL * pEdgeLGra &
                               * met(i, j, k + 1, 1, 3) / dz) &
                               + jacInv / dy * (pEdgeFDiv / rhoEdgeF &
                               * pEdgeFGra * met(i, j, k + 1, 2, 3) / dz &
                               - pEdgeBDiv / rhoEdgeB * pEdgeBGra &
                               * met(i, j, k + 1, 2, 3) / dz) &
                               + jacInv / dz * pEdgeUDiv / rhoEdgeU &
                               * pEdgeUGra * met(i, j, k + 1, 3, 3) / dz &
                               + jacInv / dz * pEdgeUDiv / rhoEdgeU &
                               * 0.5 * pStratTFC(i, j, k + 1) &
                               / jac(i, j, k + 1) &
                               * (chris(i, j, k + 1, 1, 1) &
                               + chris(i, j, k + 1, 2, 2) &
                               + 2.0 * chris(i, j, k + 1, 1, 3) &
                               * met(i, j, k + 1, 1, 3) &
                               + 2.0 * chris(i, j, k + 1, 2, 3) &
                               * met(i, j, k + 1, 2, 3))
                      else if(k == nz) then
                          AU = 0.0
                      else
                          AU = jacInv / dx * (pEdgeRDiv / rhoEdgeR &
                               * pEdgeRGra * met(i, j, k + 1, 1, 3) * 0.25 &
                               / dz - pEdgeLDiv / rhoEdgeL * pEdgeLGra &
                               * met(i, j, k + 1, 1, 3) * 0.25 / dz) &
                               + jacInv / dy * (pEdgeFDiv / rhoEdgeF &
                               * pEdgeFGra * met(i, j, k + 1, 2, 3) * 0.25 &
                               / dz - pEdgeBDiv / rhoEdgeB * pEdgeBGra &
                               * met(i, j, k + 1, 2, 3) * 0.25 / dz) &
                               + jacInv / dz * pEdgeUDiv / rhoEdgeU &
                               * pEdgeUGra * met(i, j, k + 1, 3, 3) / dz &
                               + jacInv / dz * pEdgeUDiv / rhoEdgeU * 0.5 &
                               * pStratTFC(i, j, k + 1) / jac(i, j, k + 1) &
                      ...
                      if(k == 1) then
                          ...
                          AD = - jacInv / dx * (pEdgeRDiv / rhoEdgeR &
                               * pEdgeRGra * met(i, j, k - 1, 1, 3) / dz &
                               - pEdgeLDiv / rhoEdgeL * pEdgeLGra &
                               * met(i, j, k - 1, 1, 3) / dz) &
                               - jacInv / dy * (pEdgeFDiv / rhoEdgeF &
                      ...
                      if(k == 1) then
                          ARU = jacInv / dx * pEdgeRDiv / rhoEdgeR &
                                ...
                                * met(i + 1, j, k + 1, 1, 3) / dz &
                                + jacInv / dz * pEdgeUDiv / rhoEdgeU &
                                * pEdgeUGra * met(i + 1, j, k + 1, 1, 3) &
                                * 0.25 / dx
                      else if(k == nz) then
                          ARU = 0.0
                      else
                          ARU = jacInv / dx * pEdgeRDiv / rhoEdgeR &
                                * pEdgeRGra * met(i + 1, j, k + 1, 1, 3) &
                                * 0.25 / dz &
                                + jacInv / dz * pEdgeUDiv / rhoEdgeU &
                                * pEdgeUGra * met(i + 1, j, k + 1, 1, 3) &
                                * 0.25 / dx
                      ...
                      if(k == 1) then
                          AFU = jacInv / dy * pEdgeFDiv / rhoEdgeF &
                                ...
                                * met(i, j + 1, k + 1, 2, 3) / dz &
                                + jacInv / dz * pEdgeUDiv / rhoEdgeU &
                                * pEdgeUGra * met(i, j + 1, k + 1, 2, 3) &
                                * 0.25 / dy
                      else if(k == nz) then
                          AFU = 0.0
                      else
                          AFU = jacInv / dy * pEdgeFDiv / rhoEdgeF &
                                * pEdgeFGra * met(i, j + 1, k + 1, 2, 3) &
                                * 0.25 / dz &
                                + jacInv / dz * pEdgeUDiv / rhoEdgeU &
                                * pEdgeUGra * met(i, j + 1, k + 1, 2, 3) &
                                * 0.25 / dy
                      ...
                      if(k == 1) then
                          AUU = - jacInv / dx * (pEdgeRDiv / rhoEdgeR &
                                * pEdgeRGra * 0.25 &
                                * met(i, j, k + 2, 1, 3) &
                                / dz - pEdgeLDiv / rhoEdgeL * pEdgeLGra &
                                * 0.25 * met(i, j, k + 2, 1, 3) / dz) &
                                - jacInv / dy * (pEdgeFDiv / rhoEdgeF &
                                * pEdgeFGra * 0.25 &
                                * met(i, j, k + 2, 2, 3) &
                                / dz - pEdgeBDiv / rhoEdgeB * pEdgeBGra &
                                * 0.25 * met(i, j, k + 2, 2, 3) / dz)
                      else
                          AUU = 0.0
                      end if
                      ...
                      if(k == nz) then
                          ADD = jacInv / dx * (pEdgeRDiv / rhoEdgeR &
                                * pEdgeRGra * 0.25 &
                                * met(i, j, k - 2, 1, 3) &
                                / dz - pEdgeLDiv / rhoEdgeL * pEdgeLGra &
                                * 0.25 * met(i, j, k - 2, 1, 3) / dz) &
          ...
          do k = 1, nz
              do j = 1, ny
                  do i = 1, nx
                      ...
                      ! Correct operator.
                      LsTest(i, j, k) = LsTest(i, j, k) + ARU * sRU &
                                        + ARD * sRD + ALU * sLU + ALD * sLD &
                                        + AFU * sFU + AFD * sFD + ABU * sBU &
                                        + ABD * sBD + AUU * sUU + ADD * sDD &
                                        + ARUU * sRUU + ARDD * sRDD &
                                        + ALUU * sLUU + ALDD * sLDD &
                                        + AFUU * sFUU + AFDD * sFDD &
                                        + ABUU * sBUU + ABDD * sBDD
             ...
             j_loop: do j = 1, ny
                i_loop: do i = 1, nx
                   ...
                   if(k == 1) then
                       ...
                       Ls(i, j, k) = jacInv / dx * pEdgeRDiv / rhoEdgeR &
                                     * pEdgeRGra * ((s(i + 1, j, k) &
                                     - s(i, j, k)) / dx + 0.25 &
                                     * (- met(i, j, k + 2, 1, 3) &
                                     * s(i, j, k + 2) &
                                     ...
                                     * s(i, j - 1, k + 1)) / dy &
                                     + (met(i, j, k + 1, 3, 3) &
                                     * s(i, j, k + 1) - met(i, j, k, 3, 3) &
                                     * s(i, j, k)) / dz) &
                                     + jacInv / dz * pEdgeUDiv / rhoEdgeU &
                                     * 0.5 * (pStratTFC(i, j, k) &
                                     / jac(i, j, k) * (chris(i, j, k, 1, 1) &
                                     + chris(i, j, k, 2, 2) + 2.0 &
                                     * chris(i, j, k, 1, 3) &
                                     * met(i, j, k, 1, 3) + 2.0 &
                                     * chris(i, j, k, 2, 3) &
               ...
               do j = 1, ny
                   do i = 1, nx
                       ...
                       pEdgeU = 0.5 * (jac(i, j, k) * pStratTFC(i, j, k) &
                                + jac(i, j, k + 1) * pStratTFC(i, j, k + 1))
                       pEdgeD = 0.5 * (jac(i, j, k) * pStratTFC(i, j, k) &
                                + jac(i, j, k - 1) * pStratTFC(i, j, k - 1))
                       ! Compute RHS.
                       bu = (pEdgeR * uR - pEdgeL * uL) / dx &
                            / jac(i, j, k) * Ma ** 2.0 * kappa
                       bv = (pEdgeF * vF - pEdgeB * vB) / dy &
                            / jac(i, j, k) * Ma ** 2.0 * kappa
                       bw = (pEdgeU * wU - pEdgeD * wD) / dz &
                            / jac(i, j, k) * Ma ** 2.0 * kappa
           ...
           do k = 1, nz
               do j = 1, ny
                   do i = 1, nx
                       ...
                       uR = 0.5 * (jac(i, j, k) + jac(i + 1, j, k)) &
                            * var(i, j, k, 2)
                       uL = 0.5 * (jac(i, j, k) + jac(i - 1, j, k)) &
                            * var(i - 1, j, k, 2)
                       vF = 0.5 * (jac(i, j, k) + jac(i, j + 1, k)) &
                            * var(i, j, k, 3)
                       vB = 0.5 * (jac(i, j, k) + jac(i, j - 1, k)) &
                            * var(i, j - 1, k, 3)
                       wU = 0.5 * (jac(i, j, k) + jac(i, j, k + 1)) &
                            * var(i, j, k, 4)
                       wD = 0.5 * (jac(i, j, k) + jac(i, j, k - 1)) &
                            * var(i, j, k - 1, 4)
                       ! Compute RHS.
                       bu = Ma ** 2.0 * kappa / theta00 * (uR - uL) / dx &
                            / jac(i, j, k)
                       bv = Ma ** 2.0 * kappa / theta00 * (vF - vB) / dy &
                            / jac(i, j, k)
                       bw = Ma ** 2.0 * kappa / theta00 * (wU - wD) / dz &
                            / jac(i, j, k)
                       b(i, j, k) = bu + bv + bw
                       bl2loc = bu ** 2.0 + bv ** 2.0 + bw ** 2.0
                       ! Compute check sum for solvability criterion.
...
!   achatze
  ...
  subroutine correctorStep( var, dMom, dt, RKstage, opt, facray,facprs)
    ...
    real :: dpEdgeR, dpUEdgeR, dpUUEdgeR, dpDEdgeR, dpDDEdgeR, &
            dpEdgeF, dpUEdgeF, dpUUEdgeF, dpDEdgeF, dpDDEdgeF, &
            dpREdgeU, dpLEdgeU, dpFEdgeU, dpBEdgeU, &
            dpREdgeD, dpLEdgeD, dpFEdgeD, dpBEdgeD
                ...
                do i = 0,nx
                   ...
                   if(topography) then
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           dpUUEdgeR = 0.5 * (jac(i, j, k + 2) &
                                       * met(i, j, k + 2, 1, 3) &
                                       * dp(i, j, k + 2) &
                                       + jac(i + 1, j, k + 2) &
                                       * met(i + 1, j, k + 2, 1, 3) &
                                       * dp(i + 1, j, k + 2))
                           dpUEdgeR = 0.5 * (jac(i, j, k + 1) &
                                      * met(i, j, k + 1, 1, 3) &
                                      * dp(i, j, k + 1) + jac(i + 1, j, k + 1) &
                                      * met(i + 1, j, k + 1, 1, 3) &
                                      * dp(i + 1, j, k + 1))
                           dpEdgeR = 0.5 * (jac(i, j, k) * met(i, j, k, 1, 3) &
                                     * dp(i, j, k) + jac(i + 1, j, k) &
                                     * met(i + 1, j, k, 1, 3) * dp(i + 1, j, k))
                           pGradX = kappaInv * MaInv2 / rhou * pEdgeR &
                                    * ((jac(i + 1, j, k) * dp(i + 1, j, k) &
                                    - jac(i, j, k) * dp(i, j, k)) / dx &
                                    + (- dpUUEdgeR + 4.0 * dpUEdgeR - 3.0 &
                                    * dpEdgeR) * 0.5 / dz)
                       else if(k == nz .and. zBoundary == "solid_wall") then
                           dpDDEdgeR = 0.5 * (jac(i, j, k - 2) &
                                       * met(i, j, k - 2, 1, 3) &
                                       * dp(i, j, k - 2) &
                                       + jac(i + 1, j, k - 2) &
                                       * met(i + 1, j, k - 2, 1, 3) &
                                       * dp(i + 1, j, k - 2))
                           dpDEdgeR = 0.5 * (jac(i, j, k - 1) &
                                      * met(i, j, k - 1, 1, 3) &
                                      * dp(i, j, k - 1) + jac(i + 1, j, k - 1) &
                                      * met(i + 1, j, k - 1, 1, 3) &
                                      * dp(i + 1, j, k - 1))
             ...
             do j = 0,ny
                do i = 1,nx
                   ...
                   if(topography) then
                       ...
                       ! Compute pressure difference gradient component.
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           dpUUEdgeF = 0.5 * (jac(i, j, k + 2) &
                                       * met(i, j, k + 2, 2, 3) &
                                       * dp(i, j, k + 2) &
                                       + jac(i, j + 1, k + 2) &
                                       * met(i, j + 1, k + 2, 2, 3) &
                                       * dp(i, j + 1, k + 2))
                           dpUEdgeF = 0.5 * (jac(i, j, k + 1) &
                                      * met(i, j, k + 1, 2, 3) &
                                      * dp(i, j, k + 1) + jac(i, j + 1, k + 1) &
                                      * met(i, j + 1, k + 1, 2, 3) &
                                      * dp(i, j + 1, k + 1))
                           dpEdgeF = 0.5 * (jac(i, j, k) * met(i, j, k, 2, 3) &
                                     * dp(i, j, k) + jac(i, j + 1, k) &
                                     * met(i, j + 1, k, 2, 3) * dp(i, j + 1, k))
                           pGradY = kappaInv * MaInv2 / rhov * pEdgeF &
                                    * ((jac(i, j + 1, k) * dp(i, j + 1, k) &
                                    - jac(i, j, k) * dp(i, j, k)) / dy &
                                    + (- dpUUEdgeF + 4.0 * dpUEdgeF - 3.0 &
                                    * dpEdgeF) * 0.5 / dz)
                       else if(k == nz .and. zBoundary == "solid_wall") then
                           dpDDEdgeF = 0.5 * (jac(i, j, k - 2) &
                                       * met(i, j, k - 2, 2, 3) &
                                       * dp(i, j, k - 2) &
                                       + jac(i, j + 1, k - 2) &
                                       * met(i, j + 1, k - 2, 2, 3) &
                                       * dp(i, j + 1, k - 2))
                           dpDEdgeF = 0.5 * (jac(i, j, k - 1) &
                                      * met(i, j, k - 1, 2, 3) &
                                      * dp(i, j, k - 1) + jac(i, j + 1, k - 1) &
                                      * met(i, j + 1, k - 1, 2, 3) &
                                      * dp(i, j + 1, k - 1))
                ...
                do i = 1,nx
                   ...
                   if (spongeLayer) then
                       if(topography .and. spongeTFC) then
                           ! TFC FJ
                           facw = facw + dt * 0.5 * (alphaTFC(i, j, k) &
                                  + alphaTFC(i, j, k + 1))
                       else
                           facw = facw + dt * 0.5*(kr_sp(j,k) + kr_sp(j,k+1))
                       end if
                   ...
                   if(topography) then
                       ...
                       pEdgeU = 0.5 * (pStratTFC(i, j, k) / jac(i, j, k) &
                                + pStratTFC(i, j, k + 1) / jac(i, j, k + 1))
                       bvsstw = 0.5 * (bvsStratTFC(i, j, k) &
                                + bvsStratTFC(i, j, k + 1))
                       dpREdgeU = 0.5 * (jac(i + 1, j, k) &
                                  * met(i + 1, j, k, 3, 1) * dp(i + 1, j, k) &
                                  + jac(i + 1, j, k + 1) &
                                  * met(i + 1, j, k + 1, 3, 1) &
                                  * dp(i + 1, j, k + 1))
                       dpLEdgeU = 0.5 * (jac(i - 1, j, k) &
                                  * met(i - 1, j, k, 3, 1) * dp(i - 1, j, k) &
                                  + jac(i - 1, j, k + 1) &
                                  * met(i - 1, j, k + 1, 3, 1) &
                                  * dp(i - 1, j, k + 1))
                       dpFEdgeU = 0.5 * (jac(i, j + 1, k) &
                                  * met(i, j + 1, k, 3, 2) * dp(i, j + 1, k) &
                                  + jac(i, j + 1, k + 1) &
                                  * met(i, j + 1, k + 1, 3, 2) &
                                  * dp(i, j + 1, k + 1))
                       dpBEdgeU = 0.5 * (jac(i, j - 1, k) &
                                  * met(i, j - 1, k, 3, 2) * dp(i, j - 1, k) &
                                  + jac(i, j - 1, k + 1) &
                                  * met(i, j - 1, k + 1, 3, 2) &
                                  * dp(i, j - 1, k + 1))
                       chris11EdgeU = 0.5 * (pStratTFC(i, j, k) &
                                      * chris(i, j, k, 1, 1) * dp(i, j, k) &
                                      + pStratTFC(i, j, k + 1) &
                                      * chris(i, j, k + 1, 1, 1) &
                                      * dp(i, j, k + 1))
                       chris22EdgeU = 0.5 * (pStratTFC(i, j, k) &
                                      * chris(i, j, k, 2, 2) * dp(i, j, k) &
                                      + pStratTFC(i, j, k + 1) &
                                      * chris(i, j, k + 1, 2, 2) &
                                      * dp(i, j, k + 1))
                       chris13EdgeU = 0.5 * (pStratTFC(i, j, k) &
                                      * chris(i, j, k, 1, 3) &
                                      * met(i, j, k, 1, 3) * dp(i, j, k) &
                                      + pStratTFC(i, j, k + 1) &
                                      * chris(i, j, k + 1, 1, 3) &
                                      * met(i, j, k + 1, 1, 3) &
                                      * dp(i, j, k + 1))
                       chris23EdgeU = 0.5 * (pStratTFC(i, j, k) &
                                      * chris(i, j, k, 2, 3) &
                                      * met(i, j, k, 2, 3) &
                                      * dp(i, j, k) + pStratTFC(i, j, k + 1) &
                                      * chris(i, j, k + 1, 2, 3) &
                                      * met(i, j, k + 1, 2, 3) &
                                      * dp(i, j, k + 1))
                       ! Compute pressure difference gradient component.
                       pGradZ = kappaInv * MaInv2 / rhoEdge * pEdgeU &
                                * ((dpREdgeU - dpLEdgeU) * 0.5 / dx &
                                + (dpFEdgeU - dpBEdgeU) * 0.5 / dy &
                                + (jac(i, j, k + 1) * met(i, j, k + 1, 3, 3) &
                                * dp(i, j, k + 1) - jac(i, j, k) &
                                * met(i, j, k, 3, 3) * dp(i, j, k)) / dz) &
                                + kappaInv * MaInv2 / rhoEdge * (chris11EdgeU &
                                + chris22EdgeU + 2.0 * (chris13EdgeU &
                                + chris23EdgeU))
                       ! Compute velocity correction.
                       dw = - facprs * dt / (facw + rhoStratEdgeU / rhoEdge &
                            ...
                            * rhoStratEdgeU / rhoEdge * bvsstw * dt ** 2.0 &
                            * (0.25 * (met(i, j, k, 1, 3) * (corX(i, j, k) &
                            + corX(i - 1, j, k)) + met(i, j, k + 1, 1, 3) &
                            * (corX(i, j, k + 1) + corX(i - 1, j, k + 1))) &
                            + 0.25 * (met(i, j, k, 2, 3) * (corY(i, j, k) &
                 ...
                 if(topography) then
                     ...
                     chris13EdgeU = 0.5 * (pStratTFC(i, j, k) &
                                    ...
                                    * met(i, j, k, 1, 3) * dp(i, j, k) &
                                    + pStratTFC(i, j, k + 1) &
                                    * chris(i, j, k + 1, 1, 3) &
                                    * met(i, j, k + 1, 1, 3) * dp(i, j, k + 1))
                     chris23EdgeU = 0.5 * (pStratTFC(i, j, k) &
                                    * chris(i, j, k, 2, 3) &
                                    * met(i, j, k, 2, 3) * dp(i, j, k) &
                                    + pStratTFC(i, j, k + 1) &
                                    * chris(i, j, k + 1, 2, 3) &
                                    * met(i, j, k + 1, 2, 3) * dp(i, j, k + 1))
                     ! Compute pressure difference gradient component.
                     pGradZ = kappaInv * MaInv2 / rhoEdge * pEdgeU &
                              * ((dpREdgeU - dpLEdgeU) * 0.5 / dx &
                              + (dpFEdgeU - dpBEdgeU) * 0.5 / dy &
                    ...
                    pGradZ = kappaInv * MaInv2 * pEdge / rhoEdge &
                             *(dp(i,j,k+1) - dp(i,j,k)) / dz
                    ! pGradZ = ( dp(i,j,k+1) - dp(i,j,k) ) / dz
                end if
             ...
             do j = 1,ny
                do i = 1,nx
                   ...
                   if(topography) then
                       ...
                       rhow0 = 0.5 * (var(i, j, k, 1) &
                               + var(i, j, k + 1, 1) &
                               + rhoStratTFC(i, j, k) &
                               + rhoStratTFC(i, j, k + 1))
                       rhowm = 0.5 * (var(i, j, k, 1) &
                               + var(i, j, k - 1, 1) &
                               + rhoStratTFC(i, j, k) &
                               + rhoStratTFC(i, j, k - 1))
                       ...
                       dpLEdgeU = 0.5 * (jac(i - 1, j, k) &
                                  ...
                                  * met(i - 1, j, k + 1, 1, 3) &
                                  * dp(i - 1, j, k + 1))
                       dpREdgeD = 0.5 * (jac(i + 1, j, k) &
                                  * met(i + 1, j, k, 1, 3) &
                                  * dp(i + 1, j, k) &
                                  + jac(i + 1, j, k - 1) &
                                  * met(i + 1, j, k - 1, 1, 3) &
                                  * dp(i + 1, j, k - 1))
                       dpLEdgeD = 0.5 * (jac(i - 1, j, k) &
                                  * met(i - 1, j, k, 1, 3) &
                                  * dp(i - 1, j, k) &
                                  + jac(i - 1, j, k - 1) &
                                  * met(i - 1, j, k - 1, 1, 3) &
                       ...
                       dpBEdgeU = 0.5 * (jac(i, j - 1, k) &
                                  ...
                                  * met(i, j - 1, k + 1, 2, 3) &
                                  * dp(i, j - 1, k + 1))
                       dpFEdgeD = 0.5 * (jac(i, j + 1, k) &
                                  * met(i, j + 1, k, 2, 3) &
                                  * dp(i, j + 1, k) &
                                  + jac(i, j + 1, k - 1) &
                                  * met(i, j + 1, k - 1, 2, 3) &
                                  * dp(i, j + 1, k - 1))
                       dpBEdgeD = 0.5 * (jac(i, j - 1, k) &
                                  * met(i, j - 1, k, 2, 3) &
                                  * dp(i, j - 1, k) &
                                  + jac(i, j - 1, k - 1) &
                                  * met(i, j - 1, k - 1, 2, 3) &
                       ...
                       chris11EdgeU = 0.5 * (pStratTFC(i, j, k) &
                                      ...
                                      * chris(i, j, k + 1, 1, 1) &
                                      * dp(i, j, k + 1))
                       chris11EdgeD = 0.5 * (pStratTFC(i, j, k) &
                                      * chris(i, j, k, 1, 1) &
                                      * dp(i, j, k) &
                                      + pStratTFC(i, j, k - 1) &
                                      * chris(i, j, k - 1, 1, 1) &
                       ...
                       chris22EdgeU = 0.5 * (pStratTFC(i, j, k) &
                                      ...
                                      * chris(i, j, k + 1, 2, 2) &
                                      * dp(i, j, k + 1))
                       chris22EdgeD = 0.5 * (pStratTFC(i, j, k) &
                                      * chris(i, j, k, 2, 2) &
                                      * dp(i, j, k) &
                                      + pStratTFC(i, j, k - 1) &
                                      * chris(i, j, k - 1, 2, 2) &
                       ...
                       chris13EdgeU = 0.5 * (pStratTFC(i, j, k) &
                                      ...
                                      * met(i, j, k + 1, 1, 3) &
                                      * dp(i, j, k + 1))
                       chris13EdgeD = 0.5 * (pStratTFC(i, j, k) &
                                      * chris(i, j, k, 1, 3) &
                                      * met(i, j, k, 1, 3) &
                                      * dp(i, j, k) &
                                      + pStratTFC(i, j, k - 1) &
                       ...
                       chris23EdgeU = 0.5 * (pStratTFC(i, j, k) &
                                      ...
                                      * met(i, j, k + 1, 2, 3) &
                                      * dp(i, j, k + 1))
                       chris23EdgeD = 0.5 * (pStratTFC(i, j, k) &
                                      * chris(i, j, k, 2, 3) &
                                      * met(i, j, k, 2, 3) &
                                      * dp(i, j, k) &
                                      + pStratTFC(i, j, k - 1) &
                       ...
                       pGradZEdgeU = kappaInv * MaInv2 * pEdgeU / rhow0 &
                                     ...
                                     * dp(i, j, k + 1) - jac(i, j, k) &
                                     * met(i, j, k, 3, 3) &
                                     * dp(i, j, k)) / dz) + kappaInv &
                                     * MaInv2 / rhow0 * (chris11EdgeU &
                                     + chris22EdgeU + 2.0 &
                                     * (chris13EdgeU + chris23EdgeU))
                       pGradZEdgeD = kappaInv * MaInv2 * pEdgeD / rhowm &
                                     * (0.5 * (dpREdgeD - dpLEdgeD) &
                                     / dx + 0.5 * (dpFEdgeD &
                                     - dpBEdgeD) / dy &
                                     + (jac(i, j, k) &
                                     * met(i, j, k, 3, 3) &
                                     * dp(i, j, k) - jac(i, j, k - 1) &
                                     * met(i, j, k - 1, 3, 3) &
                                     * dp(i, j, k - 1)) / dz) &
                                     + kappaInv * MaInv2 / rhowm &
                                     * (chris11EdgeD + chris22EdgeD &
                                     + 2.0 * (chris13EdgeD &
                                     + chris23EdgeD))
                       ...
                       db = - 1.0 / (facw + rhoStratTFC(i, j, k) / rho &
                            ...
                            * bvsStratTFC(i, j, k) * facprs * dt ** 2.0 &
                            * pGradZ + rhoStratTFC(i, j, k) / rho &
                            * bvsStratTFC(i, j, k) * dt * jac(i, j, k) * facw &
                            * 0.5 * (met(i, j, k, 1, 3) * (corX(i, j, k) &
                            + corX(i - 1, j, k)) + met(i, j, k, 2, 3) &
 ...
 !==============================================================
  ...
  subroutine val_PsIn(var, dt, opt, facray)
    ...
    real :: pEdgeRGra, pEdgeLGra, pEdgeFGra, pEdgeBGra, pEdgeUGra, &
            pEdgeDGra
    real :: rhoEdgeR, rhoEdgeL, rhoEdgeF, rhoEdgeB, rhoEdgeU, rhoEdgeD
    real :: pUEdgeRGra, pUEdgeLGra, pUEdgeFGra, pUEdgeBGra, &
            pDEdgeRGra, pDEdgeLGra, pDEdgeFGra, pDEdgeBGra
    real :: rhoStratEdgeR, rhoStratEdgeL, rhoStratEdgeF, rhoStratEdgeB, &
            rhoStratEdgeU, rhoStratEdgeD, &
            rhoUEdgeR, rhoUEdgeL, rhoUEdgeF, rhoUEdgeB, &
            rhoDEdgeR, rhoDEdgeL, rhoDEdgeF, rhoDEdgeB
    real :: bvsStratEdgeU, bvsStratEdgeD
    real :: facEdgeR, facEdgeL, facEdgeF, facEdgeB, facEdgeU, facEdgeD, &
            facUEdgeR, facUEdgeL, facUEdgeF, facUEdgeB, &
            facDEdgeR, facDEdgeL, facDEdgeF, facDEdgeB
    real :: impHorEdgeR, impHorEdgeL, impHorEdgeF, impHorEdgeB, &
            impHorUEdgeR, impHorUEdgeL, impHorUEdgeF, impHorUEdgeB, &
            impHorDEdgeR, impHorDEdgeL, impHorDEdgeF, impHorDEdgeB, &
            impVerEdgeU, impVerEdgeD
    real :: gEdgeR, gEdgeL, gEdgeF, gEdgeB, gEdgeU, gEdgeD, &
            gUEdgeR, gUEdgeL, gUEdgeF, gUEdgeB, &
            gDEdgeR, gDEdgeL, gDEdgeF, gDEdgeB
           ...
           do k = 1, nz
               ...
               do j = 1, ny
                   do i = 1, nx
                       ...
                       ! Compute P coefficients (divergence).
                       pEdgeRDiv = 0.5 * (jac(i, j, k) &
                                   * pStratTFC(i, j, k) &
                                   + jac(i + 1, j, k) &
                                   * pStratTFC(i + 1, j, k))
                       pEdgeLDiv = 0.5 * (jac(i, j, k) &
                                   * pStratTFC(i, j, k) &
                                   + jac(i - 1, j, k) &
                                   * pStratTFC(i - 1, j, k))
                       pEdgeFDiv = 0.5 * (jac(i, j, k) &
                                   * pStratTFC(i, j, k) &
                                   + jac(i, j + 1, k) &
                                   * pStratTFC(i, j + 1, k))
                       pEdgeBDiv = 0.5 * (jac(i, j, k) &
                                   * pStratTFC(i, j, k) &
                                   + jac(i, j - 1, k) &
                                   * pStratTFC(i, j - 1, k))
                       pEdgeUDiv = 0.5 * (jac(i, j, k) &
                                   * pStratTFC(i, j, k) &
                                   + jac(i, j, k + 1) &
                                   * pStratTFC(i, j, k + 1))
                       pEdgeDDiv = 0.5 * (jac(i, j, k) &
                                   * pStratTFC(i, j, k) &
                                   + jac(i, j, k - 1) &
                                   * pStratTFC(i, j, k - 1))

                       ! Compute P coefficients (pressure gradient).
                       pEdgeRGra = 0.5 * (pStratTFC(i, j, k) &
                                   / jac(i, j, k) &
                                   + pStratTFC(i + 1, j, k) &
                                   / jac(i + 1, j, k))
                       pEdgeLGra = 0.5 * (pStratTFC(i, j, k) &
                                   / jac(i, j, k) &
                                   + pStratTFC(i - 1, j, k) &
                                   / jac(i - 1, j, k))
                       pEdgeFGra = 0.5 * (pStratTFC(i, j, k) &
                                   / jac(i, j, k) &
                                   + pStratTFC(i, j + 1, k) &
                                   / jac(i, j + 1, k))
                       pEdgeBGra = 0.5 * (pStratTFC(i, j, k) &
                                   / jac(i, j, k) &
                                   + pStratTFC(i, j - 1, k) &
                                   / jac(i, j - 1, k))
                       pEdgeUGra = 0.5 * (pStratTFC(i, j, k) &
                                   / jac(i, j, k) &
                                   + pStratTFC(i, j, k + 1) &
                                   / jac(i, j, k + 1))
                       pEdgeDGra = 0.5 * (pStratTFC(i, j, k) &
                                   / jac(i, j, k) &
                                   + pStratTFC(i, j, k - 1) &
                                   / jac(i, j, k - 1))
                       pUEdgeRGra = 0.5 * (pStratTFC(i, j, k + 1) &
                                   / jac(i, j, k + 1) &
                                   + pStratTFC(i + 1, j, k + 1) &
                                   / jac(i + 1, j, k + 1))
                       pUEdgeLGra = 0.5 * (pStratTFC(i, j, k + 1) &
                                   / jac(i, j, k + 1) &
                                   + pStratTFC(i - 1, j, k + 1) &
                                   / jac(i - 1, j, k + 1))
                       pUEdgeFGra = 0.5 * (pStratTFC(i, j, k + 1) &
                                   / jac(i, j, k + 1) &
                                   + pStratTFC(i, j + 1, k + 1) &
                                   / jac(i, j + 1, k + 1))
                       pUEdgeBGra = 0.5 * (pStratTFC(i, j, k + 1) &
                                   / jac(i, j, k + 1) &
                                   + pStratTFC(i, j - 1, k + 1) &
                                   / jac(i, j - 1, k + 1))
                       pDEdgeRGra = 0.5 * (pStratTFC(i, j, k - 1) &
                                   / jac(i, j, k - 1) &
                                   + pStratTFC(i + 1, j, k - 1) &
                                   / jac(i + 1, j, k - 1))
                       pDEdgeLGra = 0.5 * (pStratTFC(i, j, k - 1) &
                                   / jac(i, j, k - 1) &
                                   + pStratTFC(i - 1, j, k - 1) &
                                   / jac(i - 1, j, k - 1))
                       pDEdgeFGra = 0.5 * (pStratTFC(i, j, k - 1) &
                                   / jac(i, j, k - 1) &
                                   + pStratTFC(i, j + 1, k - 1) &
                                   / jac(i, j + 1, k - 1))
                       pDEdgeBGra = 0.5 * (pStratTFC(i, j, k - 1) &
                                   / jac(i, j, k - 1) &
                                   + pStratTFC(i, j - 1, k - 1) &
                                   / jac(i, j - 1, k - 1))
                       ...
                       rhoStratEdgeR = 0.5 * (rhoStratTFC(i, j, k) &
                                       + rhoStratTFC(i + 1, j, k))
                       rhoStratEdgeL = 0.5 * (rhoStratTFC(i, j, k) &
                                       + rhoStratTFC(i - 1, j, k))
                       rhoStratEdgeF = 0.5 * (rhoStratTFC(i, j, k) &
                                       + rhoStratTFC(i, j + 1, k))
                       rhoStratEdgeB = 0.5 * (rhoStratTFC(i, j, k) &
                                       + rhoStratTFC(i, j - 1, k))
                       rhoStratEdgeU = 0.5 * (rhoStratTFC(i, j, k) &
                                       + rhoStratTFC(i, j, k + 1))
                       rhoStratEdgeD = 0.5 * (rhoStratTFC(i, j, k) &
                                       + rhoStratTFC(i, j, k - 1))
                       rhoEdgeR = 0.5 * (var(i, j, k, 1) &
                                  + var(i + 1, j, k, 1)) &
                                  + rhoStratEdgeR
                       ...
                       rhoEdgeD = 0.5 * (var(i, j, k, 1) &
                                  + var(i, j, k - 1, 1)) &
                                  + rhoStratEdgeD
                       rhoUEdgeR = 0.5 * (var(i, j, k + 1, 1) &
                                   + var(i + 1, j, k + 1, 1) &
                                   + rhoStratTFC(i, j, k + 1) &
                                   + rhoStratTFC(i + 1, j, k + 1))
                       rhoUEdgeL = 0.5 * (var(i, j, k + 1, 1) &
                                   + var(i - 1, j, k + 1, 1) &
                                   + rhoStratTFC(i, j, k + 1) &
                                   + rhoStratTFC(i - 1, j, k + 1))
                       rhoUEdgeF = 0.5 * (var(i, j, k + 1, 1) &
                                   + var(i, j + 1, k + 1, 1) &
                                   + rhoStratTFC(i, j, k + 1) &
                                   + rhoStratTFC(i, j + 1, k + 1))
                       rhoUEdgeB = 0.5 * (var(i, j, k + 1, 1) &
                                   + var(i, j - 1, k + 1, 1) &
                                   + rhoStratTFC(i, j, k + 1) &
                                   + rhoStratTFC(i, j - 1, k + 1))
                       rhoDEdgeR = 0.5 * (var(i, j, k - 1, 1) &
                                   + var(i + 1, j, k - 1, 1) &
                                   + rhoStratTFC(i, j, k - 1) &
                                   + rhoStratTFC(i + 1, j, k - 1))
                       rhoDEdgeL = 0.5 * (var(i, j, k - 1, 1) &
                                   + var(i - 1, j, k - 1, 1) &
                                   + rhoStratTFC(i, j, k - 1) &
                                   + rhoStratTFC(i - 1, j, k - 1))
                       rhoDEdgeF = 0.5 * (var(i, j, k - 1, 1) &
                                   + var(i, j + 1, k - 1, 1) &
                                   + rhoStratTFC(i, j, k - 1) &
                                   + rhoStratTFC(i, j + 1, k - 1))
                       rhoDEdgeB = 0.5 * (var(i, j, k - 1, 1) &
                                   + var(i, j - 1, k - 1, 1) &
                                   + rhoStratTFC(i, j, k - 1) &
                                   + rhoStratTFC(i, j - 1, k - 1))

                       ! Compute squared buoyancy frequency at edges.
                       bvsStratEdgeU = 0.5 * (bvsStratTFC(i, j, k) &
                                       + bvsStratTFC(i, j, k + 1))
                       bvsStratEdgeD = 0.5 * (bvsStratTFC(i, j, k) &
                                       + bvsStratTFC(i, j, k - 1))
                       ...
                       if(spongeLayer) then
                           if(sponge_uv) then
                               ...
                               facEdgeF = facEdgeF + 0.5 * dt * (kr_sp(j, k) &
                                          + kr_sp(j + 1, k))
                               facEdgeB = facEdgeB + 0.5 * dt * (kr_sp(j, k) &
                                          + kr_sp(j - 1, k))
                               facUEdgeR = facUEdgeR + dt * kr_sp(j, k + 1)
                               facUEdgeL = facUEdgeL + dt * kr_sp(j, k + 1)
                               facUEdgeF = facUEdgeF + 0.5 * dt &
                                           * (kr_sp(j, k + 1) &
                                           + kr_sp(j + 1, k + 1))
                               facUEdgeB = facUEdgeB + 0.5 * dt &
                                           * (kr_sp(j, k + 1) &
                                           + kr_sp(j - 1, k + 1))
                               facDEdgeR = facDEdgeR + dt * kr_sp(j, k - 1)
                               facDEdgeL = facDEdgeL + dt * kr_sp(j, k - 1)
                               facDEdgeF = facDEdgeF + 0.5 * dt &
                                           * (kr_sp(j, k - 1) &
                                           + kr_sp(j + 1, k - 1))
                               facDEdgeB = facDEdgeB + 0.5 * dt &
                                           * (kr_sp(j, k - 1) &
                                           + kr_sp(j - 1, k - 1))
                           end if
                           if(spongeTFC) then
                               facEdgeU = facEdgeU + 0.5 * dt &
                                          * (alphaTFC(i, j, k) &
                                          + alphaTFC(i, j, k + 1))
                               facEdgeD = facEdgeD + 0.5 * dt &
                                          * (alphaTFC(i, j, k) &
                                          + alphaTFC(i, j, k - 1))
                           else
                               facEdgeU = facEdgeU + 0.5 * dt * (kr_sp(j, k) &
                                          + kr_sp(j, k + 1))
                               facEdgeD = facEdgeD + 0.5 * dt * (kr_sp(j, k) &
                                          + kr_sp(j, k - 1))
                           end if
                       end if
                       if(testCase == "baroclinic_LC") then
                           if(background == "HeldSuarez") then
                               ...
                               facEdgeF = facEdgeF + 0.5 * dt &
                                          * (kv_hs(j, k) + kv_hs(j + 1, k))
                               facEdgeB = facEdgeB + 0.5 * dt &
                                          * (kv_hs(j, k) + kv_hs(j - 1, k))
                               ...
                               facDEdgeB = facDEdgeB + 0.5 * dt &
                                          * (kv_hs(j, k - 1) &
                                          + kv_hs(j - 1, k - 1))
                               facEdgeU = facEdgeU + 0.5 * dt * (kw_hs(k) &
                                          + kw_hs(k + 1))
                               facEdgeD = facEdgeD + 0.5 * dt * (kw_hs(k) &
                                          + kw_hs(k - 1))
                           end if
                       end if
                       ...
                       impVerEdgeU = 1.0 / (facEdgeU + rhoStratEdgeU &
                                    / rhoEdgeU * bvsStratEdgeU * dt ** 2.0)
                       impVerEdgeD = 1.0 / (facEdgeD + rhoStratEdgeD &
                                    / rhoEdgeD * bvsStratEdgeD * dt ** 2.0)

                       ! Compute gradient coefficients

                       ! G(i + 1 / 2)
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           gEdgeR = jacInv / dx * pEdgeRDiv &
                                    * impHorEdgeR * facEdgeR / rhoEdgeR &
                                    + jacInv / dz * pEdgeUDiv &
                                    * impVerEdgeU * rhoStratEdgeU &
                                    / rhoEdgeU * bvsStratEdgeU * dt &
                                    ** 2.0 * 0.25 * met(i, j, k, 1, 3) &
                                    * impHorEdgeR * facEdgeR / rhoEdgeR
                       else if(k == nz .and. zBoundary == "solid_wall") then
                           gEdgeR = jacInv / dx * pEdgeRDiv &
                                    * impHorEdgeR * facEdgeR / rhoEdgeR &
                                    - jacInv / dz * pEdgeDDiv &
                                    * impVerEdgeD * rhoStratEdgeD &
                                    / rhoEdgeD * bvsStratEdgeD * dt &
                                    ** 2.0 * 0.25 * met(i, j, k, 1, 3) &
                                    * impHorEdgeR * facEdgeR / rhoEdgeR
                       else
                           gEdgeR = jacInv / dx * pEdgeRDiv &
                       ...
                       ! G(i - 1 / 2)
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           gEdgeL = - jacInv / dx * pEdgeLDiv &
                                    * impHorEdgeL * facEdgeL / rhoEdgeL &
                                    + jacInv / dz * pEdgeUDiv &
                                    * impVerEdgeU * rhoStratEdgeU &
                                    / rhoEdgeU * bvsStratEdgeU * dt &
                                    ** 2.0 * 0.25 * met(i, j, k, 1, 3) &
                       ...
                       ! G(j + 1 / 2)
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           gEdgeF = jacInv / dy * pEdgeFDiv &
                                    * impHorEdgeF * facEdgeF / rhoEdgeF &
                                    + jacInv / dz * pEdgeUDiv &
                                    * impVerEdgeU * rhoStratEdgeU &
                                    / rhoEdgeU * bvsStratEdgeU * dt &
                                    ** 2.0 * 0.25 * met(i, j, k, 2, 3) &
                                    * impHorEdgeF * facEdgeF / rhoEdgeF
                       else if(k == nz .and. zBoundary == "solid_wall") then
                           gEdgeF = jacInv / dy * pEdgeFDiv &
                                    * impHorEdgeF * facEdgeF / rhoEdgeF &
                       ...
                       ! G(j - 1 / 2)
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           gEdgeB = - jacInv / dy * pEdgeBDiv &
                                    * impHorEdgeB * facEdgeB / rhoEdgeB &
                                    + jacInv / dz * pEdgeUDiv &
                                    * impVerEdgeU * rhoStratEdgeU &
                                    / rhoEdgeU * bvsStratEdgeU * dt &
                                    ** 2.0 * 0.25 * met(i, j, k, 2, 3) &
                       ...
                       if(k == nz .and. zBoundary == "solid_wall") then
                           ...
                           gEdgeU = jacInv / dz * pEdgeUDiv &
                                    * impVerEdgeU / rhoEdgeU
                       end if
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ...
                           gEdgeD = - jacInv / dz * pEdgeDDiv &
                                    * impVerEdgeD / rhoEdgeD
                       end if
                       ...
                       if(k == nz .and. zBoundary == "solid_wall") then
                           ...
                           gUEdgeR = jacInv / dz * pEdgeUDiv &
                                     * impVerEdgeU * rhoStratEdgeU &
                                     / rhoEdgeU * bvsStratEdgeU * dt &
                                     ** 2.0 * 0.25 &
                                     * met(i, j, k + 1, 1, 3) &
                                     * impHorUEdgeR * facUEdgeR &
                                     / rhoUEdgeR
                       end if
                       ...
                       if(k == nz .and. zBoundary == "solid_wall") then
                           ...
                           gUEdgeL = jacInv / dz * pEdgeUDiv &
                                     * impVerEdgeU * rhoStratEdgeU &
                                     / rhoEdgeU * bvsStratEdgeU * dt &
                                     ** 2.0 * 0.25 &
                                     * met(i, j, k + 1, 1, 3) &
                                     * impHorUEdgeL * facUEdgeL &
                                     / rhoUEdgeL
                       end if
                       ...
                       if(k == nz .and. zBoundary == "solid_wall") then
                           ...
                           gUEdgeF = jacInv / dz * pEdgeUDiv &
                                     * impVerEdgeU * rhoStratEdgeU &
                                     / rhoEdgeU * bvsStratEdgeU * dt &
                                     ** 2.0 * 0.25 &
                                     * met(i, j, k + 1, 2, 3) &
                                     * impHorUEdgeF * facUEdgeF &
                                     / rhoUEdgeF
                       end if
                       ...
                       if(k == nz .and. zBoundary == "solid_wall") then
                           ...
                           gUEdgeB = jacInv / dz * pEdgeUDiv &
                                     * impVerEdgeU * rhoStratEdgeU &
                                     / rhoEdgeU * bvsStratEdgeU * dt &
                                     ** 2.0 * 0.25 &
                                     * met(i, j, k + 1, 2, 3) &
                                     * impHorUEdgeB * facUEdgeB &
                                     / rhoUEdgeB
                       end if
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ...
                           gDEdgeR = - jacInv / dz * pEdgeDDiv &
                                     * impVerEdgeD * rhoStratEdgeD &
                                     / rhoEdgeD * bvsStratEdgeD * dt &
                                     ** 2.0 * 0.25 &
                                     * met(i, j, k - 1, 1, 3) &
                                     * impHorDEdgeR * facDEdgeR &
                                     / rhoDEdgeR
                       end if
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ...
                           gDEdgeL = - jacInv / dz * pEdgeDDiv &
                                     * impVerEdgeD * rhoStratEdgeD &
                                     / rhoEdgeD * bvsStratEdgeD * dt &
                                     ** 2.0 * 0.25 &
                                     * met(i, j, k - 1, 1, 3) &
                                     * impHorDEdgeL * facDEdgeL &
                                     / rhoDEdgeL
                       end if
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ...
                           gDEdgeF = - jacInv / dz * pEdgeDDiv &
                                     * impVerEdgeD * rhoStratEdgeD &
                                     / rhoEdgeD * bvsStratEdgeD * dt &
                                     ** 2.0 * 0.25 &
                                     * met(i, j, k - 1, 2, 3) &
                                     * impHorDEdgeF * facDEdgeF &
                                     / rhoDEdgeF
                       end if
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ...
                           gDEdgeB = - jacInv / dz * pEdgeDDiv &
                                     * impVerEdgeD * rhoStratEdgeD &
                                     / rhoEdgeD * bvsStratEdgeD * dt &
                                     ** 2.0 * 0.25 &
                                     * met(i, j, k - 1, 2, 3) &
                                     * impHorDEdgeB * facDEdgeB &
                                     / rhoDEdgeB
                       end if
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           AC = - gEdgeR * pEdgeRGra * (1.0 / dx + 0.75 &
                                * met(i, j, k, 1, 3) / dz) + gEdgeL &
                                * pEdgeLGra * (1.0 / dx - 0.75 &
                                * met(i, j, k, 1, 3) / dz) - gEdgeF &
                                * pEdgeFGra * (1.0 / dy + 0.75 &
                                * met(i, j, k, 2, 3) / dz) + gEdgeB &
                                * pEdgeBGra * (1.0 / dy - 0.75 &
                                * met(i, j, k, 2, 3) / dz) + gEdgeU &
                                * (- pEdgeUGra * met(i, j, k, 3, 3) / dz &
                                + 0.5 * pStratTFC(i, j, k) * jacInv &
                                * (chris(i, j, k, 1, 1) &
                                + chris(i, j, k, 2, 2) + 2.0 &
                                * chris(i, j, k, 1, 3) &
                                * met(i, j, k, 1, 3) + 2.0 &
                                * chris(i, j, k, 2, 3) &
                                * met(i, j, k, 2, 3))) - gUEdgeR &
                                * pUEdgeRGra * 0.25 * met(i, j, k, 1, 3) &
                                / dz - gUEdgeL * pUEdgeLGra * 0.25 &
                                * met(i, j, k, 1, 3) / dz - gUEdgeF &
                                * pUEdgeFGra * 0.25 * met(i, j, k, 2, 3) &
                                / dz - gUEdgeB * pUEdgeBGra * 0.25 &
                                * met(i, j, k, 2, 3) / dz
                       else if(k == 2 .and. zBoundary == "solid_wall") then
                           AC = - gEdgeR * pEdgeRGra / dx + gEdgeL &
                                * pEdgeLGra / dx - gEdgeF * pEdgeFGra &
                                / dy + gEdgeB * pEdgeBGra / dy + gEdgeU &
                                * (- pEdgeUGra * met(i, j, k, 3, 3) / dz &
                                + 0.5 * pStratTFC(i, j, k) * jacInv &
                                * (chris(i, j, k, 1, 1) &
                                + chris(i, j, k, 2, 2) + 2.0 &
                                ...
                                * pUEdgeFGra * 0.25 * met(i, j, k, 2, 3) &
                                / dz - gUEdgeB * pUEdgeBGra * 0.25 &
                                * met(i, j, k, 2, 3) / dz + gDEdgeR &
                                * pDEdgeRGra * met(i, j, k, 1, 3) &
                                / dz + gDEdgeL * pDEdgeLGra &
                                * met(i, j, k, 1, 3) / dz + gDEdgeF &
                                * pDEdgeFGra * met(i, j, k, 2, 3) &
                                / dz + gDEdgeB * pDEdgeBGra &
                                * met(i, j, k, 2, 3) / dz
                           ...
                           AC = - gEdgeR * pEdgeRGra / dx + gEdgeL &
                                ...
                                * chris(i, j, k, 2, 3) &
                                * met(i, j, k, 2, 3))) - gUEdgeR &
                                * pUEdgeRGra * met(i, j, k, 1, 3) &
                                / dz - gUEdgeL * pUEdgeLGra &
                                * met(i, j, k, 1, 3) / dz - gUEdgeF &
                                * pUEdgeFGra * met(i, j, k, 2, 3) &
                                / dz - gUEdgeB * pUEdgeBGra &
                                * met(i, j, k, 2, 3) / dz + gDEdgeR &
                                * pDEdgeRGra * 0.25 * met(i, j, k, 1, 3) &
                                / dz + gDEdgeL * pDEdgeLGra * 0.25 &
                                * met(i, j, k, 1, 3) / dz + gDEdgeF &
                                * pDEdgeFGra * 0.25 * met(i, j, k, 2, 3) &
                                / dz + gDEdgeB * pDEdgeBGra * 0.25 &
                                * met(i, j, k, 2, 3) / dz
                       else if(k == nz .and. zBoundary == "solid_wall") then
                           AC = - gEdgeR * pEdgeRGra * (1.0 / dx - 0.75 &
                                * met(i, j, k, 1, 3) / dz) + gEdgeL &
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           AR = gEdgeR * pEdgeRGra * (1.0 / dx - 0.75 &
                                * met(i + 1, j, k, 1, 3) / dz) + gEdgeU &
                                * pEdgeUGra * 0.25 &
                                * met(i + 1, j, k, 1, 3) / dx - gUEdgeR &
                                * pUEdgeRGra * 0.25 &
                                * met(i + 1, j, k, 1, 3) / dz
                       else if(k == 2 .and. zBoundary == "solid_wall") then
                           AR = gEdgeR * pEdgeRGra / dx + gEdgeU &
                                * pEdgeUGra * 0.25 &
                                * met(i + 1, j, k, 1, 3) / dx + gEdgeD &
                                * pEdgeDGra * 0.25 &
                                * met(i + 1, j, k, 1, 3) / dx - gUEdgeR &
                                * pUEdgeRGra * 0.25 &
                                * met(i + 1, j, k, 1, 3) / dz + gDEdgeR &
                                * pDEdgeRGra &
                                * met(i + 1, j, k, 1, 3) / dz
                       else if(k == nz - 1 .and. zBoundary == "solid_wall") then
                           AR = gEdgeR * pEdgeRGra / dx + gEdgeU &
                           ...
                           AR = gEdgeR * pEdgeRGra * (1.0 / dx + 0.75 &
                                * met(i + 1, j, k, 1, 3) / dz) + gEdgeD &
                                * pEdgeDGra * 0.25 &
                                * met(i + 1, j, k, 1, 3) / dx + gDEdgeR &
                                * pDEdgeRGra * 0.25 &
                                * met(i + 1, j, k, 1, 3) / dz
                       else
                           AR = gEdgeR * pEdgeRGra / dx + gEdgeU &
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           AL = - gEdgeL * pEdgeLGra * (1.0 / dx + 0.75 &
                                * met(i - 1, j, k, 1, 3) / dz) - gEdgeU &
                                * pEdgeUGra * 0.25 &
                                * met(i - 1, j, k, 1, 3) / dx - gUEdgeL &
                                * pUEdgeLGra * 0.25 &
                                * met(i - 1, j, k, 1, 3) / dz
                       else if(k == 2 .and. zBoundary == "solid_wall") then
                           AL = - gEdgeL * pEdgeLGra / dx - gEdgeU &
                                * pEdgeUGra * 0.25 &
                                * met(i - 1, j, k, 1, 3) / dx - gEdgeD &
                                * pEdgeDGra * 0.25 &
                                * met(i - 1, j, k, 1, 3) / dx - gUEdgeL &
                                * pUEdgeLGra * 0.25 &
                                * met(i - 1, j, k, 1, 3) / dz + gDEdgeL &
                                * pDEdgeLGra &
                                * met(i - 1, j, k, 1, 3) / dz
                       else if(k == nz - 1 .and. zBoundary == "solid_wall") then
                           AL = - gEdgeL * pEdgeLGra / dx - gEdgeU &
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           AF = gEdgeF * pEdgeFGra * (1.0 / dy - 0.75 &
                                * met(i, j + 1, k, 2, 3) / dz) + gEdgeU &
                                * pEdgeUGra * 0.25 &
                                * met(i, j + 1, k, 2, 3) / dy - gUEdgeF &
                                * pUEdgeFGra * 0.25 &
                                * met(i, j + 1, k, 2, 3) / dz
                       else if(k == 2 .and. zBoundary == "solid_wall") then
                           AF = gEdgeF * pEdgeFGra / dy + gEdgeU &
                                * pEdgeUGra * 0.25 &
                                * met(i, j + 1, k, 2, 3) / dy + gEdgeD &
                                * pEdgeDGra * 0.25 &
                                * met(i, j + 1, k, 2, 3) / dy - gUEdgeF &
                                * pUEdgeFGra * 0.25 &
                                * met(i, j + 1, k, 2, 3) / dz + gDEdgeF &
                                * pDEdgeFGra &
                                * met(i, j + 1, k, 2, 3) / dz
                       else if(k == nz - 1 .and. zBoundary == "solid_wall") then
                           AF = gEdgeF * pEdgeFGra / dy + gEdgeU &
                           ...
                           AF = gEdgeF * pEdgeFGra * (1.0 / dy + 0.75 &
                                * met(i, j + 1, k, 2, 3) / dz) + gEdgeD &
                                * pEdgeDGra * 0.25 &
                                * met(i, j + 1, k, 2, 3) / dy + gDEdgeF &
                                * pDEdgeFGra * 0.25 &
                                * met(i, j + 1, k, 2, 3) / dz
                       else
                           AF = gEdgeF * pEdgeFGra / dy + gEdgeU &
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           AB = - gEdgeB * pEdgeBGra * (1.0 / dy + 0.75 &
                                * met(i, j - 1, k, 2, 3) / dz) - gEdgeU &
                                * pEdgeUGra * 0.25 &
                                * met(i, j - 1, k, 2, 3) / dy - gUEdgeB &
                                * pUEdgeBGra * 0.25 &
                                * met(i, j - 1, k, 2, 3) / dz
                       else if(k == 2 .and. zBoundary == "solid_wall") then
                           AB = - gEdgeB * pEdgeBGra / dy - gEdgeU &
                                * pEdgeUGra * 0.25 &
                                * met(i, j - 1, k, 2, 3) / dy - gEdgeD &
                                * pEdgeDGra * 0.25 &
                                * met(i, j - 1, k, 2, 3) / dy - gUEdgeB &
                                * pUEdgeBGra * 0.25 &
                                * met(i, j - 1, k, 2, 3) / dz + gDEdgeB &
                                * pDEdgeBGra &
                                * met(i, j - 1, k, 2, 3) / dz
                       else if(k == nz - 1 .and. zBoundary == "solid_wall") then
                           AB = - gEdgeB * pEdgeBGra / dy - gEdgeU &
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           AU = gEdgeR * pEdgeRGra &
                                * met(i, j, k + 1, 1, 3) / dz + gEdgeL &
                                * pEdgeLGra &
                                * met(i, j, k + 1, 1, 3) / dz + gEdgeF &
                                * pEdgeFGra &
                                * met(i, j, k + 1, 2, 3) / dz + gEdgeB &
                                * pEdgeBGra &
                                * met(i, j, k + 1, 2, 3) / dz + gEdgeU &
                                * (pEdgeUGra * met(i, j, k + 1, 3, 3) &
                                / dz + 0.5 * pStratTFC(i, j, k + 1) &
                                / jac(i, j, k + 1) &
                                * (chris(i, j, k + 1, 1, 1) &
                                + chris(i, j, k + 1, 2, 2) + 2.0 &
                                * chris(i, j, k + 1, 1, 3) &
                                * met(i, j, k + 1, 1, 3) + 2.0 &
                                * chris(i, j, k + 1, 2, 3) &
                                * met(i, j, k + 1, 2, 3))) - gUEdgeR &
                                * pUEdgeRGra / dx + gUEdgeL * pUEdgeLGra &
                                / dx - gUEdgeF * pUEdgeFGra / dy &
                                + gUEdgeB * pUEdgeBGra / dy
                       else if(k == 2 .and. zBoundary == "solid_wall") then
                           AU = gEdgeR * pEdgeRGra * 0.25 &
                                * met(i, j, k + 1, 1, 3) / dz + gEdgeL &
                                * pEdgeLGra * 0.25 &
                                * met(i, j, k + 1, 1, 3) / dz + gEdgeF &
                                * pEdgeFGra * 0.25 &
                                ...
                                * pUEdgeRGra / dx + gUEdgeL * pUEdgeLGra &
                                / dx - gUEdgeF * pUEdgeFGra / dy &
                                + gUEdgeB * pUEdgeBGra / dy - gDEdgeR &
                                * pDEdgeRGra * 0.25 &
                                * met(i, j, k + 1, 1, 3) / dz - gDEdgeL &
                                * pDEdgeLGra * 0.25 &
                                * met(i, j, k + 1, 1, 3) / dz - gDEdgeF &
                                * pDEdgeFGra * 0.25 &
                                * met(i, j, k + 1, 2, 3) / dz - gDEdgeB &
                                * pDEdgeBGra * 0.25 &
                                * met(i, j, k + 1, 2, 3)
                       else if(k == nz - 1 .and. zBoundary == "solid_wall") then
                           AU = gEdgeR * pEdgeRGra * 0.25 &
                                ...
                                * chris(i, j, k + 1, 2, 3) &
                                * met(i, j, k + 1, 2, 3))) - gUEdgeR &
                                * pUEdgeRGra * (1.0 / dx - 0.75 &
                                * met(i, j, k + 1, 1, 3) / dz) + gUEdgeL &
                                * pUEdgeLGra * (1.0 / dx + 0.75 &
                                * met(i, j, k + 1, 1, 3) / dz) - gUEdgeF &
                                * pUEdgeFGra * (1.0 / dy - 0.75 &
                                * met(i, j, k + 1, 2, 3) / dz) + gUEdgeB &
                                * pUEdgeBGra * (1.0 / dy + 0.75 &
                                * met(i, j, k + 1, 2, 3) / dz)
                       else if(k == nz .and. zBoundary == "solid_wall") then
                           AU = 0.0
                       else
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ...
                           AD = - gEdgeR * pEdgeRGra * 0.25 &
                                ...
                                * met(i, j, k - 1, 2, 3) / dz - gEdgeB &
                                * pEdgeBGra * 0.25 &
                                * met(i, j, k - 1, 2, 3) / dz + gEdgeD &
                                * (- pEdgeDGra * met(i, j, k - 1, 3, 3) &
                                / dz + 0.5 * pStratTFC(i, j, k - 1) &
                                / jac(i, j, k - 1) &
                                * (chris(i, j, k - 1, 1, 1) &
                                + chris(i, j, k - 1, 2, 2) + 2.0 &
                                ...
                                * met(i, j, k - 1, 1, 3) + 2.0 &
                                * chris(i, j, k - 1, 2, 3) &
                                * met(i, j, k - 1, 2, 3))) - gDEdgeR &
                                * pDEdgeRGra * (1.0 / dx + 0.75 &
                                * met(i, j, k - 1, 1, 3) / dz) + gDEdgeL &
                                * pDEdgeLGra * (1.0 / dx - 0.75 &
                                * met(i, j, k - 1, 1, 3) / dz) - gDEdgeF &
                                * pDEdgeFGra * (1.0 / dy + 0.75 &
                                * met(i, j, k - 1, 2, 3) / dz) + gDEdgeB &
                                * pDEdgeBGra * (1.0 / dy - 0.75 &
                                * met(i, j, k - 1, 2, 3) / dz)
                       else if(k == nz - 1 .and. zBoundary == "solid_wall") then
                           AD = - gEdgeR * pEdgeRGra * 0.25 &
                                * met(i, j, k - 1, 1, 3) / dz - gEdgeL &
                                ...
                                * chris(i, j, k - 1, 2, 3) &
                                * met(i, j, k - 1, 2, 3))) - gDEdgeR &
                                * pDEdgeRGra / dx + gDEdgeL * pDEdgeLGra &
                                / dx - gDEdgeF * pDEdgeFGra / dy &
                                + gDEdgeB * pDEdgeBGra / dy + gUEdgeR &
                                * pUEdgeRGra * 0.25 &
                                * met(i, j, k - 1, 1, 3) / dz + gUEdgeL &
                                * pUEdgeLGra * 0.25 &
                                * met(i, j, k - 1, 1, 3) / dz + gUEdgeF &
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ARU = gEdgeR * pEdgeRGra &
                                 * met(i + 1, j, k + 1, 1, 3) / dz &
                                 + gEdgeU * pEdgeUGra * 0.25 &
                                 * met(i + 1, j, k + 1, 1, 3) / dx &
                                 + gUEdgeR * pUEdgeRGra / dx
                       else if(k == 2 .and. zBoundary == "solid_wall") then
                           ARU = gEdgeR * pEdgeRGra * 0.25 &
                                 * met(i + 1, j, k + 1, 1, 3) / dz &
                                 + gEdgeU * pEdgeUGra * 0.25 &
                                 * met(i + 1, j, k + 1, 1, 3) / dx &
                                 + gUEdgeR * pUEdgeRGra / dx - gDEdgeR &
                                 * pDEdgeRGra * 0.25 &
                                 * met(i + 1, j, k + 1, 1, 3) / dz
                       else if(k == nz - 1 .and. zBoundary == "solid_wall") then
                           ARU = gEdgeR * pEdgeRGra * 0.25 &
                                 * met(i + 1, j, k + 1, 1, 3) / dz &
                                 + gEdgeU * pEdgeUGra * 0.25 &
                                 * met(i + 1, j, k + 1, 1, 3) / dx &
                                 + gUEdgeR * pUEdgeRGra * (1.0 / dx &
                                 + 0.75 * met(i + 1, j, k + 1, 1, 3) &
                                 / dz)
                       else if(k == nz .and. zBoundary == "solid_wall") then
                           ARU = 0.0
                       else
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ...
                           ARD = - gEdgeR * pEdgeRGra * 0.25 &
                                 * met(i + 1, j, k - 1, 1, 3) / dz &
                                 + gEdgeD * pEdgeDGra * 0.25 &
                                 * met(i + 1, j, k - 1, 1, 3) / dx &
                                 + gDEdgeR * pDEdgeRGra * (1.0 / dx &
                                 - 0.75 * met(i + 1, j, k - 1, 1, 3) &
                                 / dz)
                       else if(k == nz - 1 .and. zBoundary == "solid_wall") then
                           ARD = - gEdgeR * pEdgeRGra * 0.25 &
                                 * met(i + 1, j, k - 1, 1, 3) / dz &
                                 + gEdgeD * pEdgeDGra * 0.25 &
                                 * met(i + 1, j, k - 1, 1, 3) / dx &
                                 + gDEdgeR * pDEdgeRGra / dx + gUEdgeR &
                                 * pUEdgeRGra * 0.25 &
                                 * met(i + 1, j, k - 1, 1, 3) / dz
                       else if(k == nz .and. zBoundary == "solid_wall") then
                           ARD = - gEdgeR * pEdgeRGra &
                                 * met(i + 1, j, k - 1, 1, 3) / dz &
                                 + gEdgeD * pEdgeDGra * 0.25 &
                                 * met(i + 1, j, k - 1, 1, 3) / dx &
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ALU = gEdgeL * pEdgeLGra &
                                 * met(i - 1, j, k + 1, 1, 3) / dz &
                                 - gEdgeU * pEdgeUGra * 0.25 &
                                 * met(i - 1, j, k + 1, 1, 3) / dx &
                                 - gUEdgeL * pUEdgeLGra / dx
                       else if(k == 2 .and. zBoundary == "solid_wall") then
                           ALU = gEdgeL * pEdgeLGra * 0.25 &
                                 * met(i - 1, j, k + 1, 1, 3) / dz &
                                 - gEdgeU * pEdgeUGra * 0.25 &
                                 * met(i - 1, j, k + 1, 1, 3) / dx &
                                 - gUEdgeL * pUEdgeLGra / dx - gDEdgeL &
                                 * pDEdgeLGra * 0.25 &
                                 * met(i - 1, j, k + 1, 1, 3) / dz
                       else if(k == nz - 1 .and. zBoundary == "solid_wall") then
                           ALU = gEdgeL * pEdgeLGra * 0.25 &
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ...
                           ALD = - gEdgeL * pEdgeLGra * 0.25 &
                                 * met(i - 1, j, k - 1, 1, 3) / dz &
                                 - gEdgeD * pEdgeDGra * 0.25 &
                                 * met(i - 1, j, k - 1, 1, 3) / dx &
                                 - gDEdgeL * pDEdgeLGra * (1.0 / dx &
                                 + 0.75 * met(i - 1, j, k - 1, 1, 3) &
                                 / dz)
                       else if(k == nz - 1 .and. zBoundary == "solid_wall") then
                           ALD = - gEdgeL * pEdgeLGra * 0.25 &
                                 * met(i - 1, j, k - 1, 1, 3) / dz &
                                 - gEdgeD * pEdgeDGra * 0.25 &
                                 * met(i - 1, j, k - 1, 1, 3) / dx &
                                 - gDEdgeL * pDEdgeLGra / dx + gUEdgeL &
                                 * pUEdgeLGra * 0.25 &
                                 * met(i - 1, j, k - 1, 1, 3) / dz
                       else if(k == nz .and. zBoundary == "solid_wall") then
                           ALD = - gEdgeL * pEdgeLGra &
                                 * met(i - 1, j, k - 1, 1, 3) / dz &
                                 - gEdgeD * pEdgeDGra * 0.25 &
                                 * met(i - 1, j, k - 1, 1, 3) / dx &
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           AFU = gEdgeF * pEdgeFGra &
                                 * met(i, j + 1, k + 1, 2, 3) / dz &
                                 + gEdgeU * pEdgeUGra * 0.25 &
                                 * met(i, j + 1, k + 1, 2, 3) / dy &
                                 + gUEdgeF * pUEdgeFGra / dy
                       else if(k == 2 .and. zBoundary == "solid_wall") then
                           AFU = gEdgeF * pEdgeFGra * 0.25 &
                                 * met(i, j + 1, k + 1, 2, 3) / dz &
                                 + gEdgeU * pEdgeUGra * 0.25 &
                                 * met(i, j + 1, k + 1, 2, 3) / dy &
                                 + gUEdgeF * pUEdgeFGra / dy - gDEdgeF &
                                 * pDEdgeFGra * 0.25 &
                                 * met(i, j + 1, k + 1, 2, 3) / dz
                       else if(k == nz - 1 .and. zBoundary == "solid_wall") then
                           AFU = gEdgeF * pEdgeFGra * 0.25 &
                                 * met(i, j + 1, k + 1, 2, 3) / dz &
                                 + gEdgeU * pEdgeUGra * 0.25 &
                                 * met(i, j + 1, k + 1, 2, 3) / dy &
                                 + gUEdgeF * pUEdgeFGra * (1.0 / dy &
                                 + 0.75 * met(i, j + 1, k + 1, 2, 3) &
                                 / dz)
                       else if(k == nz .and. zBoundary == "solid_wall") then
                           AFU = 0.0
                       else
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ...
                           AFD = - gEdgeF * pEdgeFGra * 0.25 &
                                 * met(i, j + 1, k - 1, 2, 3) / dz &
                                 + gEdgeD * pEdgeDGra * 0.25 &
                                 * met(i, j + 1, k - 1, 2, 3) / dy &
                                 + gDEdgeF * pDEdgeFGra * (1.0 / dy &
                                 - 0.75 * met(i, j + 1, k - 1, 2, 3) &
                                 / dz)
                       else if(k == nz - 1 .and. zBoundary == "solid_wall") then
                           AFD = - gEdgeF * pEdgeFGra * 0.25 &
                                 * met(i, j + 1, k - 1, 2, 3) / dz &
                                 + gEdgeD * pEdgeDGra * 0.25 &
                                 * met(i, j + 1, k - 1, 2, 3) / dy &
                                 + gDEdgeF * pDEdgeFGra / dy + gUEdgeF &
                                 * pUEdgeFGra * 0.25 &
                                 * met(i, j + 1, k - 1, 2, 3) / dz
                       else if(k == nz .and. zBoundary == "solid_wall") then
                           AFD = - gEdgeF * pEdgeFGra &
                                 * met(i, j + 1, k - 1, 2, 3) / dz &
                                 + gEdgeD * pEdgeDGra * 0.25 &
                                 * met(i, j + 1, k - 1, 2, 3) / dy &
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ABU = gEdgeB * pEdgeBGra &
                                 * met(i, j - 1, k + 1, 2, 3) / dz &
                                 - gEdgeU * pEdgeUGra * 0.25 &
                                 * met(i, j - 1, k + 1, 2, 3) / dy &
                                 - gUEdgeB * pUEdgeBGra / dy
                       else if(k == 2 .and. zBoundary == "solid_wall") then
                           ABU = gEdgeB * pEdgeBGra * 0.25 &
                                 * met(i, j - 1, k + 1, 2, 3) / dz &
                                 - gEdgeU * pEdgeUGra * 0.25 &
                                 * met(i, j - 1, k + 1, 2, 3) / dy &
                                 - gUEdgeB * pUEdgeBGra / dy - gDEdgeB &
                                 * pDEdgeBGra * 0.25 &
                                 * met(i, j - 1, k + 1, 2, 3) / dz
                       else if(k == nz - 1 .and. zBoundary == "solid_wall") then
                           ABU = gEdgeB * pEdgeBGra * 0.25 &
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ...
                           ABD = - gEdgeB * pEdgeBGra * 0.25 &
                                 * met(i, j - 1, k - 1, 2, 3) / dz &
                                 - gEdgeD * pEdgeDGra * 0.25 &
                                 * met(i, j - 1, k - 1, 2, 3) / dy &
                                 - gDEdgeB * pDEdgeBGra * (1.0 / dy &
                                 + 0.75 * met(i, j - 1, k - 1, 2, 3) &
                                 / dz)
                       else if(k == nz - 1 .and. zBoundary == "solid_wall") then
                           ABD = - gEdgeB * pEdgeBGra * 0.25 &
                                 * met(i, j - 1, k - 1, 2, 3) / dz &
                                 - gEdgeD * pEdgeDGra * 0.25 &
                                 * met(i, j - 1, k - 1, 2, 3) / dy &
                                 - gDEdgeB * pDEdgeBGra / dy + gUEdgeB &
                                 * pUEdgeBGra * 0.25 &
                                 * met(i, j - 1, k - 1, 2, 3) / dz
                       else if(k == nz .and. zBoundary == "solid_wall") then
                           ABD = - gEdgeB * pEdgeBGra &
                                 * met(i, j - 1, k - 1, 2, 3) / dz &
                                 - gEdgeD * pEdgeDGra * 0.25 &
                                 * met(i, j - 1, k - 1, 2, 3) / dy &
                                 - gDEdgeB * pDEdgeBGra / dy
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           AUU = - gEdgeR * pEdgeRGra * 0.25 &
                                 * met(i, j, k + 2, 1, 3) / dz - gEdgeL &
                                 * pEdgeLGra * 0.25 &
                                 * met(i, j, k + 2, 1, 3) / dz - gEdgeF &
                                 * pEdgeFGra * 0.25 &
                                 * met(i, j, k + 2, 2, 3) / dz - gEdgeB &
                                 * pEdgeBGra * 0.25 &
                                 * met(i, j, k + 2, 2, 3) / dz + gUEdgeR &
                                 * pUEdgeRGra * 0.25 &
                                 * met(i, j, k + 2, 1, 3) / dz + gUEdgeL &
                                 * pUEdgeLGra * 0.25 &
                                 * met(i, j, k + 2, 1, 3) / dz + gUEdgeF &
                                 * pUEdgeFGra * 0.25 &
                                 * met(i, j, k + 2, 2, 3) / dz + gUEdgeB &
                                 * pUEdgeBGra * 0.25 &
                                 * met(i, j, k + 2, 2, 3) / dz
                       else if((k == nz - 1 .or. k == nz) &
                       .and. zBoundary == "solid_wall") then
                           AUU = 0.0
                       else
                           AUU = gUEdgeR * pUEdgeRGra * 0.25 &
                                 * met(i, j, k + 2, 1, 3) / dz + gUEdgeL &
                                 * pUEdgeLGra * 0.25 &
                                 * met(i, j, k + 2, 1, 3) / dz + gUEdgeF &
                                 * pUEdgeFGra * 0.25 &
                       ...
                       .and. zBoundary == "solid_wall") then
                           ...
                           ADD = gEdgeR * pEdgeRGra * 0.25 &
                                 ...
                                 * met(i, j, k - 2, 2, 3) / dz + gEdgeB &
                                 * pEdgeBGra * 0.25 &
                                 * met(i, j, k - 2, 2, 3) / dz - gDEdgeR &
                                 * pDEdgeRGra * 0.25 &
                                 * met(i, j, k - 2, 1, 3) / dz - gDEdgeL &
                                 * pDEdgeLGra * 0.25 &
                                 * met(i, j, k - 2, 1, 3) / dz - gDEdgeF &
                                 * pDEdgeFGra * 0.25 &
                                 * met(i, j, k - 2, 2, 3) / dz - gDEdgeB &
                                 * pDEdgeBGra * 0.25 &
                                 * met(i, j, k - 2, 2, 3) / dz
                       else
                           ADD = - gDEdgeR * pDEdgeRGra * 0.25 &
                                 * met(i, j, k - 2, 1, 3) / dz - gDEdgeL &
                                 * pDEdgeLGra * 0.25 &
                                 * met(i, j, k - 2, 1, 3) / dz - gDEdgeF &
                                 * pDEdgeFGra * 0.25 &
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ARUU = - gEdgeR * pEdgeRGra * 0.25 &
                                  * met(i + 1, j, k + 2, 1, 3) / dz &
                                  + gUEdgeR * pUEdgeRGra * 0.25 &
                                  * met(i + 1, j, k + 2, 1, 3) / dz
                       else if((k == nz - 1 .or. k == nz) &
                       .and. zBoundary == "solid_wall") then
                           ARUU = 0.0
                       else
                           ARUU = gUEdgeR * pUEdgeRGra * 0.25 &
                                  * met(i + 1, j, k + 2, 1, 3) / dz
                       end if
                       ...
                       .and. zBoundary == "solid_wall") then
                           ...
                           ARDD = gEdgeR * pEdgeRGra * 0.25 &
                                  * met(i + 1, j, k - 2, 1, 3) / dz &
                                  - gDEdgeR * pDEdgeRGra * 0.25 &
                                  * met(i + 1, j, k - 2, 1, 3) / dz
                       else
                           ARDD = - gDEdgeR * pDEdgeRGra * 0.25 &
                                  * met(i + 1, j, k - 2, 1, 3) / dz
                       end if
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ALUU = - gEdgeL * pEdgeLGra * 0.25 &
                                  * met(i - 1, j, k + 2, 1, 3) / dz &
                                  + gUEdgeL * pUEdgeLGra * 0.25 &
                                  * met(i - 1, j, k + 2, 1, 3) / dz
                       else if((k == nz - 1 .or. k == nz) &
                       .and. zBoundary == "solid_wall") then
                           ALUU = 0.0
                       else
                           ALUU = gUEdgeL * pUEdgeLGra * 0.25 &
                                  * met(i - 1, j, k + 2, 1, 3) / dz
                       end if
                       ...
                       .and. zBoundary == "solid_wall") then
                           ...
                           ALDD = gEdgeL * pEdgeLGra * 0.25 &
                                  * met(i - 1, j, k - 2, 1, 3) / dz &
                                  - gDEdgeL * pDEdgeLGra * 0.25 &
                                  * met(i - 1, j, k - 2, 1, 3) / dz
                       else
                           ALDD = - gDEdgeL * pDEdgeLGra * 0.25 &
                                  * met(i - 1, j, k - 2, 1, 3) / dz
                       end if
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           AFUU = - gEdgeF * pEdgeFGra * 0.25 &
                                  * met(i, j + 1, k + 2, 2, 3) / dz &
                                  + gUEdgeF * pUEdgeFGra * 0.25 &
                                  * met(i, j + 1, k + 2, 2, 3) / dz
                       else if((k == nz - 1 .or. k == nz) &
                       .and. zBoundary == "solid_wall") then
                           AFUU = 0.0
                       else
                           AFUU = gUEdgeF * pUEdgeFGra * 0.25 &
                                  * met(i, j + 1, k + 2, 2, 3) / dz
                       end if
                       ...
                       .and. zBoundary == "solid_wall") then
                           ...
                           AFDD = gEdgeF * pEdgeFGra * 0.25 &
                                  * met(i, j + 1, k - 2, 2, 3) / dz &
                                  - gDEdgeF * pDEdgeFGra * 0.25 &
                                  * met(i, j + 1, k - 2, 2, 3) / dz
                       else
                           AFDD = - gDEdgeF * pDEdgeFGra * 0.25 &
                                  * met(i, j + 1, k - 2, 2, 3) / dz
                       end if
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           ABUU = - gEdgeB * pEdgeBGra * 0.25 &
                                  * met(i, j - 1, k + 2, 2, 3) / dz &
                                  + gUEdgeB * pUEdgeBGra * 0.25 &
                                  * met(i, j - 1, k + 2, 2, 3) / dz
                       else if((k == nz - 1 .or. k == nz) &
                       .and. zBoundary == "solid_wall") then
                           ABUU = 0.0
                       else
                           ABUU = gUEdgeB * pUEdgeBGra * 0.25 &
                                  * met(i, j - 1, k + 2, 2, 3) / dz
                       end if
                       ...
                       .and. zBoundary == "solid_wall") then
                           ...
                           ABDD = gEdgeB * pEdgeBGra * 0.25 &
                                  * met(i, j - 1, k - 2, 2, 3) / dz &
                                  - gDEdgeB * pDEdgeBGra * 0.25 &
                                  * met(i, j - 1, k - 2, 2, 3) / dz
                       else
                           ABDD = - gDEdgeB * pDEdgeBGra * 0.25 &
                                  * met(i, j - 1, k - 2, 2, 3) / dz
                       end if
...
!------------------------------------------------------------------
  ...
  subroutine correctorStepTestTFC(var, dMom, int_mod)
      ...
      real, dimension((- nbx):(nx + nbx), (- nby):(ny + nby), &
      (- nbz):(nz + nbz), nVar), intent(inout) :: var
      real, dimension((- nbx):(nx + nbx), (- nby):(ny + nby), &
      (- nbz):(nz + nbz), 3), intent(inout) :: dMom
      character(len = *), intent(in) :: int_mod

      real, dimension((- nbx):(nx + nbx), (- nby):(ny + nby), &
      (- nbz):(nz + nbz), nVar) :: var_tfc
      ...
      if(int_mod == "expl") then
          ...
          topography = .true.
          print*, "Corrector step difference (impl): ", &
          ...
          call correctorStep(var, dMom, 1.0, 1, "expl", 1.0, 1.0)
          topography = .true.
          print*, "Corrector step difference (expl): ", &
          maxval(abs(var_tfc - var))
      end if
