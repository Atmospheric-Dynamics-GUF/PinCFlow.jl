module poisson_module
      ...
      do k = 1, nz
        do j = 1, ny
          do i = 1, nx
            ...
            rhoEdgeR = 0.5 * (var(i, j, k, 1) + var(i + 1, j, k, 1) &
                + rhoStratTFC(i, j, k) + rhoStratTFC(i + 1, j, k))
            rhoEdgeL = 0.5 * (var(i, j, k, 1) + var(i - 1, j, k, 1) &
                + rhoStratTFC(i, j, k) + rhoStratTFC(i - 1, j, k))
            rhoEdgeF = 0.5 * (var(i, j, k, 1) + var(i, j + 1, k, 1) &
                + rhoStratTFC(i, j, k) + rhoStratTFC(i, j + 1, k))
            rhoEdgeB = 0.5 * (var(i, j, k, 1) + var(i, j - 1, k, 1) &
                + rhoStratTFC(i, j, k) + rhoStratTFC(i, j - 1, k))
            rhoEdgeU = 0.5 * (var(i, j, k, 1) + var(i, j, k + 1, 1) &
            ...
            if (k == 1) then
              ...
              AC = - jacInv / dx * (pEdgeRDiv / rhoEdgeR * pEdgeRGra / dx &
                  ...
                  * (pEdgeFDiv / rhoEdgeF * pEdgeFGra / dy + pEdgeBDiv &
                  ...
                  / rhoEdgeU * pEdgeUGra * met(i, j, k, 3, 3) / dz + pEdgeDDiv &
                  ...
                  * chris(i, j, k, 2, 3) * met(i, j, k, 2, 3)) - jacInv / dz &
            ...
            if (k == 1) then
              ...
              AF = jacInv / dy * pEdgeFDiv / rhoEdgeF * pEdgeFGra / dy &
                  + jacInv / dz * (pEdgeUDiv / rhoEdgeU * pEdgeUGra * met(i, j &
                  + 1, k, 2, 3) * 0.25 / dy - pEdgeDDiv / rhoEdgeD * pEdgeDGra &
            ...
            if (k == 1) then
              AU = jacInv / dx * (pEdgeRDiv / rhoEdgeR * pEdgeRGra * met(i, j, &
                  ...
                  * met(i, j, k + 1, 3, 3) / dz + jacInv / dz * pEdgeUDiv &
                  / rhoEdgeU * 0.5 * pStratTFC(i, j, k + 1) / jac(i, j, k + 1) &
              ...
              AU = jacInv / dx * (pEdgeRDiv / rhoEdgeR * pEdgeRGra * met(i, j, &
                  ...
                  * (pEdgeFDiv / rhoEdgeF * pEdgeFGra * met(i, j, k + 1, 2, 3) &
                  * 0.25 / dz - pEdgeBDiv / rhoEdgeB * pEdgeBGra * met(i, j, k &
                  + 1, 2, 3) * 0.25 / dz) + jacInv / dz * pEdgeUDiv / rhoEdgeU &
                  * pEdgeUGra * met(i, j, k + 1, 3, 3) / dz + jacInv / dz &
                  * pEdgeUDiv / rhoEdgeU * 0.5 * pStratTFC(i, j, k + 1) &
                  / jac(i, j, k + 1) * (chris(i, j, k + 1, 1, 1) + chris(i, j, &
                  k + 1, 2, 2) + 2.0 * chris(i, j, k + 1, 1, 3) * met(i, j, k &
                  + 1, 1, 3) + 2.0 * chris(i, j, k + 1, 2, 3) * met(i, j, k &
                  + 1, 2, 3))
            end if
            ...
            if (k == 1) then
              ...
              AD = - jacInv / dx * (pEdgeRDiv / rhoEdgeR * pEdgeRGra * met(i, &
                  ...
                  * met(i, j, k - 1, 3, 3) / dz - jacInv / dz * pEdgeDDiv &
                  / rhoEdgeD * 0.5 * pStratTFC(i, j, k - 1) / jac(i, j, k - 1) &
                  * (chris(i, j, k - 1, 1, 1) + chris(i, j, k - 1, 2, 2) + 2.0 &
                  * chris(i, j, k - 1, 1, 3) * met(i, j, k - 1, 1, 3) + 2.0 &
                  * chris(i, j, k - 1, 2, 3) * met(i, j, k - 1, 2, 3))
            else
              AD = - jacInv / dx * (pEdgeRDiv / rhoEdgeR * pEdgeRGra * met(i, &
                  ...
                  * pStratTFC(i, j, k - 1) / jac(i, j, k - 1) * (chris(i, j, k &
                  - 1, 1, 1) + chris(i, j, k - 1, 2, 2) + 2.0 * chris(i, j, k &
                  - 1, 1, 3) * met(i, j, k - 1, 1, 3) + 2.0 * chris(i, j, k &
          ...
          do i = 1, nx
            ...
            LsTest(i, j, k) = LsTest(i, j, k) + ARU * sRU + ARD * sRD + ALU &
                ...
                * sBD + AUU * sUU + ADD * sDD + ARUU * sRUU + ARDD * sRDD &
                + ALUU * sLUU + ALDD * sLDD + AFUU * sFUU + AFDD * sFDD + ABUU &
                * sBUU + ABDD * sBDD
            ...
            if (k == 1) then
              ...
              Ls(i, j, k) = jacInv / dx * pEdgeRDiv / rhoEdgeR * pEdgeRGra &
                  ...
                  * s(i + 1, j, k + 2) + 4.0 * met(i, j, k + 1, 1, 3) * s(i, &
                  j, k + 1) + 4.0 * met(i + 1, j, k + 1, 1, 3) * s(i + 1, j, k &
                  + 1) - 3.0 * met(i, j, k, 1, 3) * s(i, j, k) - 3.0 * met(i &
                  + 1, j, k, 1, 3) * s(i + 1, j, k)) / dz) - jacInv / dx &
                  * pEdgeLDiv / rhoEdgeL * pEdgeLGra * ((s(i, j, k) - s(i - 1, &
                  j, k)) / dx + 0.25 * (- met(i, j, k + 2, 1, 3) * s(i, j, k &
                  + 2) - met(i - 1, j, k + 2, 1, 3) * s(i - 1, j, k + 2) + 4.0 &
                  * met(i, j, k + 1, 1, 3) * s(i, j, k + 1) + 4.0 * met(i - 1, &
                  j, k + 1, 1, 3) * s(i - 1, j, k + 1) - 3.0 * met(i, j, k, 1, &
                  3) * s(i, j, k) - 3.0 * met(i - 1, j, k, 1, 3) * s(i - 1, j, &
                  k)) / dz) + jacInv / dy * pEdgeFDiv / rhoEdgeF * pEdgeFGra &
                  * ((s(i, j + 1, k) - s(i, j, k)) / dy + 0.25 * (- met(i, j, &
                  k + 2, 2, 3) * s(i, j, k + 2) - met(i, j + 1, k + 2, 2, 3) &
                  * s(i, j + 1, k + 2) + 4.0 * met(i, j, k + 1, 2, 3) * s(i, &
                  j, k + 1) + 4.0 * met(i, j + 1, k + 1, 2, 3) * s(i, j + 1, k &
                  + 1) - 3.0 * met(i, j, k, 2, 3) * s(i, j, k) - 3.0 * met(i, &
                  j + 1, k, 2, 3) * s(i, j + 1, k)) / dz) - jacInv / dy &
                  * pEdgeBDiv / rhoEdgeB * pEdgeBGra * ((s(i, j, k) - s(i, j &
                  - 1, k)) / dy + 0.25 * (- met(i, j, k + 2, 2, 3) * s(i, j, k &
                  + 2) - met(i, j - 1, k + 2, 2, 3) * s(i, j - 1, k + 2) + 4.0 &
                  * met(i, j, k + 1, 2, 3) * s(i, j, k + 1) + 4.0 * met(i, j &
                  - 1, k + 1, 2, 3) * s(i, j - 1, k + 1) - 3.0 * met(i, j, k, &
                  2, 3) * s(i, j, k) - 3.0 * met(i, j - 1, k, 2, 3) * s(i, j &
                  - 1, k)) / dz) + jacInv / dz * pEdgeUDiv / rhoEdgeU &
                  * pEdgeUGra * (0.25 * (met(i + 1, j, k, 3, 1) * s(i + 1, j, &
                  k) + met(i + 1, j, k + 1, 3, 1) * s(i + 1, j, k + 1) - met(i &
                  - 1, j, k, 3, 1) * s(i - 1, j, k) - met(i - 1, j, k + 1, 3, &
                  1) * s(i - 1, j, k + 1)) / dx + 0.25 * (met(i, j + 1, k, 3, &
                  2) * s(i, j + 1, k) + met(i, j + 1, k + 1, 3, 2) * s(i, j &
                  + 1, k + 1) - met(i, j - 1, k, 3, 2) * s(i, j - 1, k) &
                  - met(i, j - 1, k + 1, 3, 2) * s(i, j - 1, k + 1)) / dy &
                  + (met(i, j, k + 1, 3, 3) * s(i, j, k + 1) - met(i, j, k, 3, &
                  3) * s(i, j, k)) / dz) + jacInv / dz * pEdgeUDiv / rhoEdgeU &
                  * 0.5 * (pStratTFC(i, j, k) / jac(i, j, k) * (chris(i, j, k, &
                  1, 1) + chris(i, j, k, 2, 2) + 2.0 * chris(i, j, k, 1, 3) &
                  * met(i, j, k, 1, 3) + 2.0 * chris(i, j, k, 2, 3) * met(i, &
                  j, k, 2, 3)) * s(i, j, k) + pStratTFC(i, j, k + 1) / jac(i, &
                  j, k + 1) * (chris(i, j, k + 1, 1, 1) + chris(i, j, k + 1, &
                  2, 2) + 2.0 * chris(i, j, k + 1, 1, 3) * met(i, j, k + 1, 1, &
              ...
              Ls(i, j, k) = jacInv / dx * pEdgeRDiv / rhoEdgeR * pEdgeRGra &
                  ...
                  * ((s(i, j + 1, k) - s(i, j, k)) / dy + 0.25 * (met(i, j, k &
                  - 2, 2, 3) * s(i, j, k - 2) + met(i, j + 1, k - 2, 2, 3) &
                  * s(i, j + 1, k - 2) - 4.0 * met(i, j, k - 1, 2, 3) * s(i, &
                  j, k - 1) - 4.0 * met(i, j + 1, k - 1, 2, 3) * s(i, j + 1, k &
                  - 1) + 3.0 * met(i, j, k, 2, 3) * s(i, j, k) + 3.0 * met(i, &
                  j + 1, k, 2, 3) * s(i, j + 1, k)) / dz) - jacInv / dy &
                  ...
                  * s(i, j, k - 1)) / dz) - jacInv / dz * pEdgeDDiv / rhoEdgeD &
                  * 0.5 * (pStratTFC(i, j, k) / jac(i, j, k) * (chris(i, j, k, &
                  1, 1) + chris(i, j, k, 2, 2) + 2.0 * chris(i, j, k, 1, 3) &
                  * met(i, j, k, 1, 3) + 2.0 * chris(i, j, k, 2, 3) * met(i, &
                  j, k, 2, 3)) * s(i, j, k) + pStratTFC(i, j, k - 1) / jac(i, &
              ...
              Ls(i, j, k) = jacInv / dx * pEdgeRDiv / rhoEdgeR * pEdgeRGra &
                  ...
                  * (0.25 * (met(i + 1, j, k, 3, 1) * s(i + 1, j, k) + met(i &
                  + 1, j, k + 1, 3, 1) * s(i + 1, j, k + 1) - met(i - 1, j, k, &
                  3, 1) * s(i - 1, j, k) - met(i - 1, j, k + 1, 3, 1) * s(i &
                  - 1, j, k + 1)) / dx + 0.25 * (met(i, j + 1, k, 3, 2) * s(i, &
                  j + 1, k) + met(i, j + 1, k + 1, 3, 2) * s(i, j + 1, k + 1) &
                  - met(i, j - 1, k, 3, 2) * s(i, j - 1, k) - met(i, j - 1, k &
                  + 1, 3, 2) * s(i, j - 1, k + 1)) / dy + (met(i, j, k + 1, 3, &
                  3) * s(i, j, k + 1) - met(i, j, k, 3, 3) * s(i, j, k)) / dz) &
                  + jacInv / dz * pEdgeUDiv / rhoEdgeU * 0.5 * (pStratTFC(i, &
                  j, k) / jac(i, j, k) * (chris(i, j, k, 1, 1) + chris(i, j, &
                  k, 2, 2) + 2.0 * chris(i, j, k, 1, 3) * met(i, j, k, 1, 3) &
                  + 2.0 * chris(i, j, k, 2, 3) * met(i, j, k, 2, 3)) * s(i, j, &
                  k) + pStratTFC(i, j, k + 1) / jac(i, j, k + 1) * (chris(i, &
                  j, k + 1, 1, 1) + chris(i, j, k + 1, 2, 2) + 2.0 * chris(i, &
                  j, k + 1, 1, 3) * met(i, j, k + 1, 1, 3) + 2.0 * chris(i, j, &
                  k + 1, 2, 3) * met(i, j, k + 1, 2, 3)) * s(i, j, k + 1)) &
                  - jacInv / dz * pEdgeDDiv / rhoEdgeD * pEdgeDGra * (0.25 &
                  * (met(i + 1, j, k, 3, 1) * s(i + 1, j, k) + met(i + 1, j, k &
                  - 1, 3, 1) * s(i + 1, j, k - 1) - met(i - 1, j, k, 3, 1) &
                  * s(i - 1, j, k) - met(i - 1, j, k - 1, 3, 1) * s(i - 1, j, &
                  k - 1)) / dx + 0.25 * (met(i, j + 1, k, 3, 2) * s(i, j + 1, &
                  k) + met(i, j + 1, k - 1, 3, 2) * s(i, j + 1, k - 1) &
                  - met(i, j - 1, k, 3, 2) * s(i, j - 1, k) - met(i, j - 1, k &
                  - 1, 3, 2) * s(i, j - 1, k - 1)) / dy + (met(i, j, k, 3, 3) &
                  * s(i, j, k) - met(i, j, k - 1, 3, 3) * s(i, j, k - 1)) &
                  / dz) - jacInv / dz * pEdgeDDiv / rhoEdgeD * 0.5 &
                  * (pStratTFC(i, j, k) / jac(i, j, k) * (chris(i, j, k, 1, 1) &
                  + chris(i, j, k, 2, 2) + 2.0 * chris(i, j, k, 1, 3) * met(i, &
                  j, k, 1, 3) + 2.0 * chris(i, j, k, 2, 3) * met(i, j, k, 2, &
                  3)) * s(i, j, k) + pStratTFC(i, j, k - 1) / jac(i, j, k - 1) &
                  * (chris(i, j, k - 1, 1, 1) + chris(i, j, k - 1, 2, 2) + 2.0 &
                  * chris(i, j, k - 1, 1, 3) * met(i, j, k - 1, 1, 3) + 2.0 &
                  * chris(i, j, k - 1, 2, 3) * met(i, j, k - 1, 2, 3)) * s(i, &
                  j, k - 1))
        ...
        do k = 1, nz
          ...
          do j = 1, ny
            do i = 1, nx
              ...
              wU = var(i, j, k, 4) - w_0(k); wD = var(i, j, k - 1, 4) - w_0(k &
                  - 1)
              !UAE
        ...
        do k = 1, nz
          do j = 1, ny
            do i = 1, nx
              ...
              uR = 0.5 * (jac(i, j, k) + jac(i + 1, j, k)) * var(i, j, k, 2)
              uL = 0.5 * (jac(i, j, k) + jac(i - 1, j, k)) * var(i - 1, j, k, 2)
              vF = 0.5 * (jac(i, j, k) + jac(i, j + 1, k)) * var(i, j, k, 3)
              vB = 0.5 * (jac(i, j, k) + jac(i, j - 1, k)) * var(i, j - 1, k, 3)
              wU = 0.5 * (jac(i, j, k) + jac(i, j, k + 1)) * var(i, j, k, 4)
              wD = 0.5 * (jac(i, j, k) + jac(i, j, k - 1)) * var(i, j, k - 1, 4)
              ! Compute RHS.
          ...
          do j = 0, ny
            do i = 1, nx
              ...
              pGradX = kappaInv * MaInv2 * 0.25 * (pStrat(k) / rhou00 * (dp(i &
                  ...
                  j, k) - dp(i - 1, j, k)) / dx + pStrat(k) / rhou01 * (dp(i &
                  + 1, j + 1, k) - dp(i, j + 1, k)) / dx + pStrat(k) / rhoum1 &
              ...
              if (topography) then
                ...
                if (k == 1 .and. zBoundary == "solid_wall") then
                  ...
                  dpEdgeR = 0.5 * (jac(i, j, k) * met(i, j, k, 1, 3) * dp(i, &
                      j, k) + jac(i + 1, j, k) * met(i + 1, j, k, 1, 3) * dp(i &
                      + 1, j, k))
                ...
                pGradY = kappaInv * MaInv2 * 0.25 * (pStrat(k) / rhov0m &
                    * (dp(i, j, k) - dp(i, j - 1, k)) / dy + pStrat(k) &
                    ...
                    + pStrat(k) / rhov1m * (dp(i + 1, j, k) - dp(i + 1, j - 1, &
                    k)) / dy + pStrat(k) / rhov10 * (dp(i + 1, j + 1, k) &
                    - dp(i + 1, j, k)) / dy)
              ...
              if (k == 1 .and. zBoundary == "solid_wall") then
                dpUUEdgeR = 0.5 * (jac(i, j, k + 2) * met(i, j, k + 2, 1, 3) &
                    * dp(i, j, k + 2) + jac(i + 1, j, k + 2) * met(i + 1, j, k &
                    + 2, 1, 3) * dp(i + 1, j, k + 2))
                dpUEdgeR = 0.5 * (jac(i, j, k + 1) * met(i, j, k + 1, 1, 3) &
                    * dp(i, j, k + 1) + jac(i + 1, j, k + 1) * met(i + 1, j, k &
                    + 1, 1, 3) * dp(i + 1, j, k + 1))
                dpEdgeR = 0.5 * (jac(i, j, k) * met(i, j, k, 1, 3) * dp(i, j, &
                    k) + jac(i + 1, j, k) * met(i + 1, j, k, 1, 3) * dp(i + 1, &
                    j, k))
                pGradX = kappaInv * MaInv2 / rhou * pEdgeR * ((jac(i + 1, j, &
                    k) * dp(i + 1, j, k) - jac(i, j, k) * dp(i, j, k)) / dx + &
              ...
              if (topography) then
                ...
                pGradX = kappaInv * MaInv2 * 0.25 * (pStrat(k) / rhou00 &
                    * (dp(i + 1, j, k) - dp(i, j, k)) / dx + pStrat(k) &
                    ...
                    + pStrat(k) / rhou01 * (dp(i + 1, j + 1, k) - dp(i, j + 1, &
                    k)) / dx + pStrat(k) / rhoum1 * (dp(i, j + 1, k) - dp(i &
                    - 1, j + 1, k)) / dx)
              ...
              if (k == 1 .and. zBoundary == "solid_wall") then
                dpUUEdgeF = 0.5 * (jac(i, j, k + 2) * met(i, j, k + 2, 2, 3) &
                    * dp(i, j, k + 2) + jac(i, j + 1, k + 2) * met(i, j + 1, k &
                    + 2, 2, 3) * dp(i, j + 1, k + 2))
                dpUEdgeF = 0.5 * (jac(i, j, k + 1) * met(i, j, k + 1, 2, 3) &
                    * dp(i, j, k + 1) + jac(i, j + 1, k + 1) * met(i, j + 1, k &
                    + 1, 2, 3) * dp(i, j + 1, k + 1))
          ...
          do j = 1, ny
            do i = 1, nx
              ...
              if (topography) then
                ...
                rhow0 = 0.5 * (var(i, j, k, 1) + var(i, j, k + 1, 1) &
                    + rhoStratTFC(i, j, k) + rhoStratTFC(i, j, k + 1))
                rhowm = 0.5 * (var(i, j, k, 1) + var(i, j, k - 1, 1) &
                    + rhoStratTFC(i, j, k) + rhoStratTFC(i, j, k - 1))
                ...
                pGradZEdgeU = kappaInv * MaInv2 * pEdgeU / rhow0 * (0.5 &
                    * (dpREdgeU - dpLEdgeU) / dx + 0.5 * (dpFEdgeU - dpBEdgeU) &
                    / dy + (jac(i, j, k + 1) * met(i, j, k + 1, 3, 3) * dp(i, &
                    j, k + 1) - jac(i, j, k) * met(i, j, k, 3, 3) * dp(i, j, &
                    k)) / dz) + kappaInv * MaInv2 / rhow0 * (chris11EdgeU &
                    + chris22EdgeU + 2.0 * (chris13EdgeU + chris23EdgeU))
                pGradZEdgeD = kappaInv * MaInv2 * pEdgeD / rhowm * (0.5 &
                    * (dpREdgeD - dpLEdgeD) / dx + 0.5 * (dpFEdgeD - dpBEdgeD) &
                    / dy + (jac(i, j, k) * met(i, j, k, 3, 3) * dp(i, j, k) &
                    - jac(i, j, k - 1) * met(i, j, k - 1, 3, 3) * dp(i, j, k &
                ...
                !db = rhoStrat(k)/rho * bvsStrat(k) * dt**2 &
                db = rhoStrat(k) / rho * pStrat(k) / pStrat_0(k) * bvsStrat(k) &
                    * facprs * dt ** 2 / (facw + rhoStrat(k) / rho * pStrat(k) &
                    / pStrat_0(k) * bvsStrat(k) * dt ** 2) * pGradz
                !/(  facw &
                !  + rhoStrat(k)/rho * bvsStrat(k) * dt**2) &
              end if
          ...
          do j = 1, ny
            ...
            if (yloc > 0.5 * (ymax + ymin)) then
              ...
              tau_relax_sc = tau_relax / tref + (tau_relax_low / tref &
                  - tau_relax / tref) * (1.0 - 0.5 * (tanh((yloc / ymax - 0.1) &
                  / 0.02) - tanh((yloc / ymax - 0.9) / 0.02)))
            else
              tau_relax_sc = tau_relax / tref + (tau_relax_low / tref &
      ...
      do k = 1, nz
        do j = 1, ny
          do i = 1, nx
            ...
            heat(i, j, k) = heat(i, j, k) + rho * ((flux(i, j, k, 1, 5) &
                - flux(i - 1, j, k, 1, 5)) / dx + (flux(i, j, k, 2, 5) &
                - flux(i, j - 1, k, 2, 5)) / dy + (flux(i, j, k, 3, 5) &
                - flux(i, j, k - 1, 3, 5)) / dz)
          end do
        end do
      end do
            ...
            do i = 1, nx
              ...
              if (k == 1 .and. zBoundary == "solid_wall") then
                ...
                AU = jacInv / dx * (pEdgeRDiv / rhoEdgeR * pEdgeRGra * met(i, &
                    j, k + 1, 1, 3) * 0.25 / dz - pEdgeLDiv / rhoEdgeL &
                    * pEdgeLGra * met(i, j, k + 1, 1, 3) * 0.25 / dz) + jacInv &
                    / dy * (pEdgeFDiv / rhoEdgeF * pEdgeFGra * met(i, j, k &
                    + 1, 2, 3) * 0.25 / dz - pEdgeBDiv / rhoEdgeB * pEdgeBGra &
                    * met(i, j, k + 1, 2, 3) * 0.25 / dz) + jacInv / dz &
                    * pEdgeUDiv / rhoEdgeU * pEdgeUGra * met(i, j, k + 1, 3, &
                    3) / dz + jacInv / dz * pEdgeUDiv / rhoEdgeU * 0.5 &
                    * pStratTFC(i, j, k + 1) / jac(i, j, k + 1) * (chris(i, j, &
                    k + 1, 1, 1) + chris(i, j, k + 1, 2, 2) + 2.0 * chris(i, &
                    j, k + 1, 1, 3) * met(i, j, k + 1, 1, 3) + 2.0 * chris(i, &
                    j, k + 1, 2, 3) * met(i, j, k + 1, 2, 3))
              end if
              ...
              if (k == 1 .and. zBoundary == "solid_wall") then
                AUU = - jacInv / dx * (pEdgeRDiv / rhoEdgeR * pEdgeRGra * 0.25 &
                    * met(i, j, k + 2, 1, 3) / dz - pEdgeLDiv / rhoEdgeL &
                    * pEdgeLGra * 0.25 * met(i, j, k + 2, 1, 3) / dz) - jacInv &
                    / dy * (pEdgeFDiv / rhoEdgeF * pEdgeFGra * 0.25 * met(i, &
                    j, k + 2, 2, 3) / dz - pEdgeBDiv / rhoEdgeB * pEdgeBGra &
          ...
          do j = 1, ny
            do i = 1, nx
              ...
              rhoUEdgeL = 0.5 * (var(i, j, k + 1, 1) + var(i - 1, j, k + 1, 1) &
                  + rhoStratTFC(i, j, k + 1) + rhoStratTFC(i - 1, j, k + 1))
              rhoUEdgeF = 0.5 * (var(i, j, k + 1, 1) + var(i, j + 1, k + 1, 1) &
                  + rhoStratTFC(i, j, k + 1) + rhoStratTFC(i, j + 1, k + 1))
              rhoUEdgeB = 0.5 * (var(i, j, k + 1, 1) + var(i, j - 1, k + 1, 1) &
                  + rhoStratTFC(i, j, k + 1) + rhoStratTFC(i, j - 1, k + 1))
              rhoDEdgeR = 0.5 * (var(i, j, k - 1, 1) + var(i + 1, j, k - 1, 1) &
                  + rhoStratTFC(i, j, k - 1) + rhoStratTFC(i + 1, j, k - 1))
              rhoDEdgeL = 0.5 * (var(i, j, k - 1, 1) + var(i - 1, j, k - 1, 1) &
                  + rhoStratTFC(i, j, k - 1) + rhoStratTFC(i - 1, j, k - 1))
              rhoDEdgeF = 0.5 * (var(i, j, k - 1, 1) + var(i, j + 1, k - 1, 1) &
                  + rhoStratTFC(i, j, k - 1) + rhoStratTFC(i, j + 1, k - 1))
              rhoDEdgeB = 0.5 * (var(i, j, k - 1, 1) + var(i, j - 1, k - 1, 1) &
                  + rhoStratTFC(i, j, k - 1) + rhoStratTFC(i, j - 1, k - 1))
              ...
              if (k == nz .and. zBoundary == "solid_wall") then
                ...
                gUEdgeR = jacInv / dz * pEdgeUDiv * impVerEdgeU &
                    * rhoStratEdgeU / rhoEdgeU * bvsStratEdgeU * dt ** 2.0 &
                    * 0.25 * met(i, j, k + 1, 1, 3) * impHorUEdgeR * facUEdgeR &
                    / rhoUEdgeR
              end if
              ...
              if (k == nz .and. zBoundary == "solid_wall") then
                ...
                gUEdgeL = jacInv / dz * pEdgeUDiv * impVerEdgeU &
                    * rhoStratEdgeU / rhoEdgeU * bvsStratEdgeU * dt ** 2.0 &
                    * 0.25 * met(i, j, k + 1, 1, 3) * impHorUEdgeL * facUEdgeL &
                    / rhoUEdgeL
              end if
              ...
              if (k == nz .and. zBoundary == "solid_wall") then
                ...
                gUEdgeF = jacInv / dz * pEdgeUDiv * impVerEdgeU &
                    * rhoStratEdgeU / rhoEdgeU * bvsStratEdgeU * dt ** 2.0 &
                    * 0.25 * met(i, j, k + 1, 2, 3) * impHorUEdgeF * facUEdgeF &
                    / rhoUEdgeF
              end if
              ...
              if (k == nz .and. zBoundary == "solid_wall") then
                ...
                gUEdgeB = jacInv / dz * pEdgeUDiv * impVerEdgeU &
                    * rhoStratEdgeU / rhoEdgeU * bvsStratEdgeU * dt ** 2.0 &
                    * 0.25 * met(i, j, k + 1, 2, 3) * impHorUEdgeB * facUEdgeB &
                    / rhoUEdgeB
              end if
              ...
              if (k == 1 .and. zBoundary == "solid_wall") then
                ...
                gDEdgeR = - jacInv / dz * pEdgeDDiv * impVerEdgeD &
                    * rhoStratEdgeD / rhoEdgeD * bvsStratEdgeD * dt ** 2.0 &
                    * 0.25 * met(i, j, k - 1, 1, 3) * impHorDEdgeR * facDEdgeR &
                    / rhoDEdgeR
              end if
              ...
              if (k == 1 .and. zBoundary == "solid_wall") then
                ...
                gDEdgeL = - jacInv / dz * pEdgeDDiv * impVerEdgeD &
                    * rhoStratEdgeD / rhoEdgeD * bvsStratEdgeD * dt ** 2.0 &
                    * 0.25 * met(i, j, k - 1, 1, 3) * impHorDEdgeL * facDEdgeL &
                    / rhoDEdgeL
              end if
              ...
              if (k == 1 .and. zBoundary == "solid_wall") then
                ...
                gDEdgeF = - jacInv / dz * pEdgeDDiv * impVerEdgeD &
                    * rhoStratEdgeD / rhoEdgeD * bvsStratEdgeD * dt ** 2.0 &
                    * 0.25 * met(i, j, k - 1, 2, 3) * impHorDEdgeF * facDEdgeF &
                    / rhoDEdgeF
              end if
              ...
              if (k == 1 .and. zBoundary == "solid_wall") then
                ...
                gDEdgeB = - jacInv / dz * pEdgeDDiv * impVerEdgeD &
                    * rhoStratEdgeD / rhoEdgeD * bvsStratEdgeD * dt ** 2.0 &
                    * 0.25 * met(i, j, k - 1, 2, 3) * impHorDEdgeB * facDEdgeB &
                    / rhoDEdgeB
              end if
              ...
              if (k == 1 .and. zBoundary == "solid_wall") then
                AC = - gEdgeR * pEdgeRGra * (1.0 / dx + 0.75 * met(i, j, k, 1, &
                    ...
                    * met(i, j, k, 2, 3) / dz) + gEdgeB * pEdgeBGra * (1.0 &
                    ...
                    j, k) * jacInv * (chris(i, j, k, 1, 1) + chris(i, j, k, 2, &
                    2) + 2.0 * chris(i, j, k, 1, 3) * met(i, j, k, 1, 3) + 2.0 &
                ...
                AC = - gEdgeR * pEdgeRGra / dx + gEdgeL * pEdgeLGra / dx &
                    ...
                    * met(i, j, k, 1, 3) + 2.0 * chris(i, j, k, 2, 3) * met(i, &
                    j, k, 2, 3))) + gEdgeD * (pEdgeDGra * met(i, j, k, 3, 3) &
                    / dz + 0.5 * pStratTFC(i, j, k) * jacInv * (chris(i, j, k, &
                    1, 1) + chris(i, j, k, 2, 2) + 2.0 * chris(i, j, k, 1, 3) &
                    * met(i, j, k, 1, 3) + 2.0 * chris(i, j, k, 2, 3) * met(i, &
                    j, k, 2, 3))) - gUEdgeR * pUEdgeRGra * 0.25 * met(i, j, k, &
                    1, 3) / dz - gUEdgeL * pUEdgeLGra * 0.25 * met(i, j, k, 1, &
                    3) / dz - gUEdgeF * pUEdgeFGra * 0.25 * met(i, j, k, 2, 3) &
                    / dz - gUEdgeB * pUEdgeBGra * 0.25 * met(i, j, k, 2, 3) &
                    / dz + gDEdgeR * pDEdgeRGra * met(i, j, k, 1, 3) / dz &
                ...
                AC = - gEdgeR * pEdgeRGra / dx + gEdgeL * pEdgeLGra / dx &
                    ...
                    * met(i, j, k, 1, 3) + 2.0 * chris(i, j, k, 2, 3) * met(i, &
                    ...
                    1, 1) + chris(i, j, k, 2, 2) + 2.0 * chris(i, j, k, 1, 3) &
                    * met(i, j, k, 1, 3) + 2.0 * chris(i, j, k, 2, 3) * met(i, &
                    j, k, 2, 3))) - gUEdgeR * pUEdgeRGra * met(i, j, k, 1, 3) &
                    / dz - gUEdgeL * pUEdgeLGra * met(i, j, k, 1, 3) / dz &
                    - gUEdgeF * pUEdgeFGra * met(i, j, k, 2, 3) / dz - gUEdgeB &
                ...
                AC = - gEdgeR * pEdgeRGra * (1.0 / dx - 0.75 * met(i, j, k, 1, &
                    ...
                    * pStratTFC(i, j, k) * jacInv * (chris(i, j, k, 1, 1) &
                    + chris(i, j, k, 2, 2) + 2.0 * chris(i, j, k, 1, 3) &
                    * met(i, j, k, 1, 3) + 2.0 * chris(i, j, k, 2, 3) * met(i, &
                    j, k, 2, 3))) + gDEdgeR * pDEdgeRGra * 0.25 * met(i, j, k, &
                    1, 3) / dz + gDEdgeL * pDEdgeLGra * 0.25 * met(i, j, k, 1, &
                    3) / dz + gDEdgeF * pDEdgeFGra * 0.25 * met(i, j, k, 2, 3) &
                ...
                AC = - gEdgeR * pEdgeRGra / dx + gEdgeL * pEdgeLGra / dx &
                    ...
                    * met(i, j, k, 1, 3) + 2.0 * chris(i, j, k, 2, 3) * met(i, &
                    ...
                    3) / dz - gUEdgeF * pUEdgeFGra * 0.25 * met(i, j, k, 2, 3) &
                    / dz - gUEdgeB * pUEdgeBGra * 0.25 * met(i, j, k, 2, 3) &
                    / dz + gDEdgeR * pDEdgeRGra * 0.25 * met(i, j, k, 1, 3) &
              ...
              if (k == 1 .and. zBoundary == "solid_wall") then
                ...
                AU = gEdgeR * pEdgeRGra * 0.25 * met(i, j, k + 1, 1, 3) / dz &
                    ...
                    * pStratTFC(i, j, k + 1) / jac(i, j, k + 1) * (chris(i, j, &
                    ...
                    j, k + 1, 2, 3) * met(i, j, k + 1, 2, 3))) - gUEdgeR &
                    * pUEdgeRGra / dx + gUEdgeL * pUEdgeLGra / dx - gUEdgeF &
                    * pUEdgeFGra / dy + gUEdgeB * pUEdgeBGra / dy - gDEdgeR &
                    * pDEdgeRGra * 0.25 * met(i, j, k + 1, 1, 3) / dz &
                    - gDEdgeL * pDEdgeLGra * 0.25 * met(i, j, k + 1, 1, 3) &
                ...
                AU = gEdgeR * pEdgeRGra * 0.25 * met(i, j, k + 1, 1, 3) / dz &
                    ...
                    * pStratTFC(i, j, k + 1) / jac(i, j, k + 1) * (chris(i, j, &
                    ...
                    j, k + 1, 1, 3) * met(i, j, k + 1, 1, 3) + 2.0 * chris(i, &
                    j, k + 1, 2, 3) * met(i, j, k + 1, 2, 3))) - gUEdgeR &
                    * pUEdgeRGra * (1.0 / dx - 0.75 * met(i, j, k + 1, 1, 3) &
                    / dz) + gUEdgeL * pUEdgeLGra * (1.0 / dx + 0.75 * met(i, &
                    j, k + 1, 1, 3) / dz) - gUEdgeF * pUEdgeFGra * (1.0 / dy &
              ...
              if (k == 1 .and. zBoundary == "solid_wall") then
                ...
                AD = - gEdgeR * pEdgeRGra * 0.25 * met(i, j, k - 1, 1, 3) / dz &
                    ...
                    * (chris(i, j, k - 1, 1, 1) + chris(i, j, k - 1, 2, 2) &
                    ...
                    3))) - gDEdgeR * pDEdgeRGra * (1.0 / dx + 0.75 * met(i, j, &
                    k - 1, 1, 3) / dz) + gDEdgeL * pDEdgeLGra * (1.0 / dx &
                ...
                AD = - gEdgeR * pEdgeRGra * met(i, j, k - 1, 1, 3) / dz &
                    ...
                    * (chris(i, j, k - 1, 1, 1) + chris(i, j, k - 1, 2, 2) &
                    ...
                    + 2.0 * chris(i, j, k - 1, 2, 3) * met(i, j, k - 1, 2, &
                    3))) - gDEdgeR * pDEdgeRGra / dx + gDEdgeL * pDEdgeLGra &
                    / dx - gDEdgeF * pDEdgeFGra / dy + gDEdgeB * pDEdgeBGra / dy
              else
                AD = - gEdgeR * pEdgeRGra * 0.25 * met(i, j, k - 1, 1, 3) / dz &
                    - gEdgeL * pEdgeLGra * 0.25 * met(i, j, k - 1, 1, 3) / dz &
                    - gEdgeF * pEdgeFGra * 0.25 * met(i, j, k - 1, 2, 3) / dz &
  ...
  subroutine correctorStepTestTFC(var, dMom, int_mod)
    ...
    if (int_mod == "expl") then
      ...
      topography = .true.
      print *, "Corrector step difference (impl): ", maxval(abs(var_tfc - var))
      ...
      call correctorStep(var, dMom, 1.0, 1, "expl", 1.0, 1.0)
      topography = .true.
      print *, "Corrector step difference (expl): ", maxval(abs(var_tfc - var))
    end if
