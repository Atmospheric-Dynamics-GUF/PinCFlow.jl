  ...
  subroutine setBoundary_x_periodic(var,flux,option)
       ...
       if( updateTracer ) then
          ...
          if(verbose .and. master) then
             print*, "horizontalBoundary: &
                  & x-horizontal BC for tracer set."
          end if
       end if
    ...
    case( "varTilde" )
      ...
      if ( updateTracer ) then
         ...
         if(verbose .and. master) print*,"horizontalBoundary: &
              & x-horizontal BC for tracerTilde set."
      end if
  ...
  subroutine setBoundary_y_periodic(var,flux,option)
    ...
    case( "var" )
       ...
       if( updateTracer) then
          ...
          if( verbose .and. master) then
             print *, "horizontalBoundary: &
                  & y-horizontal BC for tracer set."
          end if
       end if
...
!-------------------------------------------------------------------------
  ...
  subroutine setBoundary_z_solidWall(var,flux,option)
    ...
    ! TFC FJ
    real :: met13EdgeRU, met23EdgeRU, met33EdgeRU, &
            chris11EdgeRU, chris13EdgeRU, chris12EdgeRU, chris23EdgeRU, &
            uEdgeRUU, uEdgeRU, uEdgeLU, uEdgeRU1, uEdgeRU2, wEdgeRU, &
            wEdgeRD, vEdgeRU
    real :: met13EdgeFU, met23EdgeFU, met33EdgeFU, &
            chris12EdgeFU, chris13EdgeFU, chris22EdgeFU, chris23EdgeFU, &
            vEdgeFUU, vEdgeFU, vEdgeBU, vEdgeFU1, vEdgeFU2, wEdgeFU, &
            wEdgeFD, uEdgeFU
             ...
             if (fluctuationMode) then
                 if(topography) then
                     ...
                     = - var(1:nx, 1:ny, 1, 1) + dens_env_pp(:, :, 1) &
                       - rhoStratTFC(1:nx, 1:ny, 1) + dens_env_pp(:, :, 0) &
                       - rhoStratTFC(1:nx, 1:ny, 0)

                     var(1:nx, 1:ny, nz + 1, 1) &
                     = - var(1:nx, 1:ny, nz, 1) + dens_env_pp(:, :, nz) &
                       - rhoStratTFC(1:nx, 1:ny, nz) &
                       + dens_env_pp(:, :, nz + 1) &
                       - rhoStratTFC(1:nx, 1:ny, nz + 1)
                 else
                    var(1:nx,1:ny,0,1) &
          ...
          if (timeScheme == "semiimplicit" .or. auxil_equ) then
             ...
             if (testCase == "baroclinic_LC") then
                 if(topography) then
                     ! TFC FJ
                     var(1:nx, 1:ny, 0, 6) &
                     = - var(1:nx, 1:ny, 1, 6) + dens_env_pp(:, :, 1) &
                       - rhoStratTFC(1:nx, 1:ny, 1) + dens_env_pp(:, :, 0) &
                       - rhoStratTFC(1:nx, 1:ny, 0)

                     var(1:nx, 1:ny, nz + 1, 6) &
                     = - var(1:nx, 1:ny, nz, 6) + dens_env_pp(:, :, nz) &
                       - rhoStratTFC(1:nx, 1:ny, nz) + dens_env_pp(:, :, nz + 1) &
                       - rhoStratTFC(1:nx, 1:ny, nz + 1)
                 else
                     var(1:nx,1:ny,0,6) &
             ...
             var(1:nx,1:ny,0,2) = var(1:nx,1:ny,1,2)
              ...
              if(topography .and. freeSlipTFC) then
                  ...
                  do i = 1, nx
                      do j = 1, ny
                          ! BC for u at z = 0.
                          met13EdgeRU = 0.25 * (met(i, j, 0, 1, 3) &
                                       + met(i + 1, j, 0, 1, 3) &
                                       + met(i, j, 1, 1, 3) &
                                       + met(i + 1, j, 1, 1, 3))
                          met23EdgeRU = 0.25 * (met(i, j, 0, 2, 3) &
                                       + met(i + 1, j, 0, 2, 3) &
                                       + met(i, j, 1, 2, 3) &
                                       + met(i + 1, j, 1, 2, 3))
                          met33EdgeRU = 0.25 * (met(i, j, 0, 3, 3) &
                                       + met(i + 1, j, 0, 3, 3) &
                                       + met(i, j, 1, 3, 3) &
                                       + met(i + 1, j, 1, 3, 3))
                          chris11EdgeRU = 0.25 *(chris(i, j, 0, 1, 1) &
                                          + chris(i + 1, j, 0, 1, 1) &
                                          + chris(i, j, 1, 1, 1) &
                                          + chris(i + 1, j, 1, 1, 1))
                          chris13EdgeRU = 0.25 * (met(i, j, 0, 1, 3) &
                                          * chris(i, j, 0, 1, 3) &
                                          + met(i + 1, j, 0, 1, 3) &
                                          * chris(i + 1, j, 0, 1, 3) &
                                          + met(i, j, 1, 1, 3) &
                                          * chris(i, j, 1, 1, 3) &
                                          + met(i + 1, j, 1, 1, 3) &
                                          * chris(i + 1, j, 1, 1, 3))
                          chris12EdgeRU = 0.25 *(chris(i, j, 0, 1, 2) &
                                          + chris(i + 1, j, 0, 1, 2) &
                                          + chris(i, j, 1, 1, 2) &
                                          + chris(i + 1, j, 1, 1, 2))
                          chris23EdgeRU = 0.25 * (met(i, j, 0, 1, 3) &
                                          * chris(i, j, 0, 2, 3) &
                                          + met(i + 1, j, 0, 1, 3) &
                                          * chris(i + 1, j, 0, 2, 3) &
                                          + met(i, j, 1, 1, 3) &
                                          * chris(i, j, 1, 2, 3) &
                                          + met(i + 1, j, 1, 1, 3) &
                                          * chris(i + 1, j, 1, 2, 3))
                          uEdgeRUU = 1.5 * var(i + 1, j, 1, 2) &
                                     - 0.5 * var(i + 1, j, 2, 2)
                          uEdgeRU = 1.5 * var(i, j, 1, 2) &
                                    - 0.5 * var(i, j, 2, 2)
                          uEdgeLU = 1.5 * var(i - 1, j, 1, 2) &
                                    - 0.5 * var(i - 1, j, 2, 2)
                          uEdgeRU1 = 1.5 * var(i, j + 1, 1, 2) &
                                     - 0.5 * var(i, j + 1, 2, 2)
                          uEdgeRU2 = 1.5 * var(i, j - 1, 1, 2) &
                                     - 0.5 * var(i, j - 1, 2, 2)
                          vEdgeRU = 1.5 * 0.25 * (var(i, j, 1, 3) &
                                    + var(i, j - 1, 1, 3) &
                                    + var(i + 1, j, 1, 3) &
                                    + var(i + 1, j - 1, 1, 3)) &
                                    - 0.5 * 0.25 * (var(i, j, 2, 3) &
                                    + var(i, j - 1, 2, 3) &
                                    + var(i + 1, j, 2, 3) &
                                    + var(i + 1, j - 1, 2, 3))
                          do k = 1, nbz
                              wEdgeRU = 0.5 * (var(i, j, k, 4) &
                                        + var(i + 1, j, k, 4))
                              wEdgeRD = 0.5 * (var(i, j, - k, 4) &
                                        + var(i + 1, j, - k, 4))
                              var(i, j, - k + 1, 2) &
                              = (2.0 * k - 1.0) * dz / met33EdgeRU &
                                * (met13EdgeRU * 0.5 * (uEdgeRUU - uEdgeLU) &
                                / dx &
                                + met23EdgeRU * 0.5 * (uEdgeRU1 - uEdgeRU2) &
                                / dy &
                                + met33EdgeRU * var(i, j, k, 2) &
                                / ((2.0 * k - 1.0) * dz) &
                                - met13EdgeRU * (wEdgeRU - wEdgeRD) &
                                / (2.0 * k * dz) &
                                - (chris11EdgeRU + chris13EdgeRU) * uEdgeRU &
                                - (chris12EdgeRU + chris23EdgeRU) * vEdgeRU)
                          end do
                          ! BC for u at z = H.
                          met13EdgeRU = 0.25 * (met(i, j, nz, 1, 3) &
                                       + met(i + 1, j, nz, 1, 3) &
                                       + met(i, j, nz + 1, 1, 3) &
                                       + met(i + 1, j, nz + 1, 1, 3))
                          met23EdgeRU = 0.25 * (met(i, j, nz, 2, 3) &
                                       + met(i + 1, j, nz, 2, 3) &
                                       + met(i, j, nz + 1, 2, 3) &
                                       + met(i + 1, j, nz + 1, 2, 3))
                          met33EdgeRU = 0.25 * (met(i, j, nz, 3, 3) &
                                       + met(i + 1, j, nz, 3, 3) &
                                       + met(i, j, nz + 1, 3, 3) &
                                       + met(i + 1, j, nz + 1, 3, 3))
                          chris11EdgeRU = 0.25 *(chris(i, j, nz, 1, 1) &
                                          + chris(i + 1, j, nz, 1, 1) &
                                          + chris(i, j, nz + 1, 1, 1) &
                                          + chris(i + 1, j, nz + 1, 1, 1))
                          chris13EdgeRU = 0.25 * (met(i, j, nz, 1, 3) &
                                          * chris(i, j, nz, 1, 3) &
                                          + met(i + 1, j, nz, 1, 3) &
                                          * chris(i + 1, j, nz, 1, 3) &
                                          + met(i, j, nz + 1, 1, 3) &
                                          * chris(i, j, nz + 1, 1, 3) &
                                          + met(i + 1, j, nz + 1, 1, 3) &
                                          * chris(i + 1, j, nz + 1, 1, 3))
                          chris12EdgeRU = 0.25 *(chris(i, j, nz, 1, 2) &
                                          + chris(i + 1, j, nz, 1, 2) &
                                          + chris(i, j, nz + 1, 1, 2) &
                                          + chris(i + 1, j, nz + 1, 1, 2))
                          chris23EdgeRU = 0.25 * (met(i, j, nz, 1, 3) &
                                          * chris(i, j, nz, 2, 3) &
                                          + met(i + 1, j, nz, 1, 3) &
                                          * chris(i + 1, j, nz, 2, 3) &
                                          + met(i, j, nz + 1, 1, 3) &
                                          * chris(i, j, nz + 1, 2, 3) &
                                          + met(i + 1, j, nz + 1, 1, 3) &
                                          * chris(i + 1, j, nz + 1, 2, 3))
                          uEdgeRUU = 1.5 * var(i + 1, j, nz, 2) &
                                     - 0.5 * var(i + 1, j, nz - 1, 2)
                          uEdgeRU = 1.5 * var(i, j, nz, 2) &
                                    - 0.5 * var(i, j, nz - 1, 2)
                          uEdgeLU = 1.5 * var(i - 1, j, nz, 2) &
                                    - 0.5 * var(i - 1, j, nz - 1, 2)
                          uEdgeRU1 = 1.5 * var(i, j + 1, nz, 2) &
                                     - 0.5 * var(i, j + 1, nz - 1, 2)
                          uEdgeRU2 = 1.5 * var(i, j - 1, nz, 2) &
                                     - 0.5 * var(i, j - 1, nz - 1, 2)
                          vEdgeRU = 1.5 * 0.25 * (var(i, j, nz, 3) &
                                    + var(i, j - 1, nz, 3) &
                                    + var(i + 1, j, nz, 3) &
                                    + var(i + 1, j - 1, nz, 3)) &
                                    - 0.5 * 0.25 * (var(i, j, nz - 1, 3) &
                                    + var(i, j - 1, nz - 1, 3) &
                                    + var(i + 1, j, nz - 1, 3) &
                                    + var(i + 1, j - 1, nz - 1, 3))
                          do k = 1, nbz
                              wEdgeRU = 0.5 * (var(i, j, nz + k, 4) &
                                        + var(i + 1, j, nz + k, 4))
                              wEdgeRD = 0.5 * (var(i, j, nz - k, 4) &
                                        + var(i + 1, j, nz - k, 4))
                              var(i, j, nz + k, 2) &
                              = - (2.0 * k - 1.0) * dz / met33EdgeRU &
                                * (met13EdgeRU * 0.5 * (uEdgeRUU - uEdgeLU) &
                          ...
                          ! BC for v at z = 0.
                          met13EdgeFU = 0.25 * (met(i, j, 0, 1, 3) &
                                       + met(i, j + 1, 0, 1, 3) &
                                       + met(i, j, 1, 1, 3) &
                                       + met(i, j + 1, 1, 1, 3))
                          met23EdgeFU = 0.25 * (met(i, j, 0, 2, 3) &
                                       + met(i, j + 1, 0, 2, 3) &
                                       + met(i, j, 1, 2, 3) &
                                       + met(i, j + 1, 1, 2, 3))
                          met33EdgeFU = 0.25 * (met(i, j, 0, 3, 3) &
                                       + met(i, j + 1, 0, 3, 3) &
                                       + met(i, j, 1, 3, 3) &
                                       + met(i, j + 1, 1, 3, 3))
                          chris12EdgeFU = 0.25 *(chris(i, j, 0, 1, 2) &
                                          + chris(i, j + 1, 0, 1, 2) &
                                          + chris(i, j, 1, 1, 2) &
                                          + chris(i, j + 1, 1, 1, 2))
                          chris13EdgeFU = 0.25 * (met(i, j, 0, 2, 3) &
                                          * chris(i, j, 0, 1, 3) &
                                          + met(i, j + 1, 0, 2, 3) &
                                          * chris(i, j + 1, 0, 1, 3) &
                                          + met(i, j, 1, 2, 3) &
                                          * chris(i, j, 1, 1, 3) &
                                          + met(i, j + 1, 1, 2, 3) &
                                          * chris(i, j + 1, 1, 1, 3))
                          chris22EdgeFU = 0.25 *(chris(i, j, 0, 2, 2) &
                                          + chris(i, j + 1, 0, 2, 2) &
                                          + chris(i, j, 1, 2, 2) &
                                          + chris(i, j + 1, 1, 2, 2))
                          chris23EdgeFU = 0.25 * (met(i, j, 0, 2, 3) &
                                          * chris(i, j, 0, 2, 3) &
                                          + met(i, j + 1, 0, 2, 3) &
                                          * chris(i, j + 1, 0, 2, 3) &
                                          + met(i, j, 1, 2, 3) &
                                          * chris(i, j, 1, 2, 3) &
                                          + met(i, j + 1, 1, 2, 3) &
                                          * chris(i, j + 1, 1, 2, 3))
                          vEdgeFUU = 1.5 * var(i, j + 1, 1, 3) &
                                     - 0.5 * var(i, j + 1, 2, 3)
                          vEdgeFU = 1.5 * var(i, j, 1, 3) &
                                    - 0.5 * var(i, j, 2, 3)
                          vEdgeBU = 1.5 * var(i, j - 1, 1, 3) &
                                    - 0.5 * var(i, j - 1, 2, 3)
                          vEdgeFU1 = 1.5 * var(i + 1, j, 1, 3) &
                                     - 0.5 * var(i + 1, j, 2, 3)
                          vEdgeFU2 = 1.5 * var(i - 1, j, 1, 3) &
                                     - 0.5 * var(i - 1, j, 2, 3)
                          uEdgeFU = 1.5 * 0.25 * (var(i, j, 1, 2) &
                                    + var(i - 1, j, 1, 2) &
                                    + var(i, j + 1, 1, 2) &
                                    + var(i - 1, j + 1, 1, 2)) &
                                    - 0.5 * 0.25 * (var(i, j, 2, 2) &
                                    + var(i - 1, j, 2, 2) &
                                    + var(i, j + 1, 2, 2) &
                                    + var(i - 1, j + 1, 2, 2))
                          do k = 1, nbz
                              wEdgeFU = 0.5 * (var(i, j, k, 4) &
                                        + var(i, j + 1, k, 4))
                              wEdgeFD = 0.5 * (var(i, j, - k, 4) &
                                        + var(i, j + 1, - k, 4))
                              var(i, j, - k + 1, 3) &
                              = (2.0 * k - 1.0) * dz / met33EdgeFU &
                                * (met13EdgeFU * 0.5 * (vEdgeFU1 - vEdgeFU2) &
                                / dx &
                                + met23EdgeFU * 0.5 * (vEdgeFUU - vEdgeBU) &
                                / dy &
                                + met33EdgeFU * var(i, j, k, 3) &
                                / ((2.0 * k - 1.0) * dz) &
                                - met23EdgeFU * (wEdgeFU - wEdgeFD) &
                                / (2.0 * k * dz) &
                                - (chris12EdgeFU + chris13EdgeFU) * uEdgeFU &
                                - (chris22EdgeFU + chris23EdgeFU) * vEdgeFU)
                          end do
                          ! BC for u at z = H.
                          met13EdgeFU = 0.25 * (met(i, j, nz, 1, 3) &
                                      + met(i, j + 1, nz, 1, 3) &
                                      + met(i, j, nz + 1, 1, 3) &
                                      + met(i, j + 1, nz + 1, 1, 3))
                          met23EdgeFU = 0.25 * (met(i, j, nz, 2, 3) &
                                      + met(i, j + 1, nz, 2, 3) &
                                      + met(i, j, nz + 1, 2, 3) &
                                      + met(i, j + 1, nz + 1, 2, 3))
                          met33EdgeFU = 0.25 * (met(i, j, nz, 3, 3) &
                                      + met(i, j + 1, nz, 3, 3) &
                                      + met(i, j, nz + 1, 3, 3) &
                                      + met(i, j + 1, nz + 1, 3, 3))
                          chris12EdgeFU = 0.25 *(chris(i, j, nz, 1, 2) &
                                          + chris(i, j + 1, nz, 1, 2) &
                                          + chris(i, j, nz + 1, 1, 2) &
                                          + chris(i, j + 1, nz + 1, 1, 2))
                          chris13EdgeFU = 0.25 * (met(i, j, nz, 2, 3) &
                                          * chris(i, j, nz, 1, 3) &
                                          + met(i, j + 1, nz, 2, 3) &
                                          * chris(i, j + 1, nz, 1, 3) &
                                          + met(i, j, nz + 1, 2, 3) &
                                          * chris(i, j, nz + 1, 1, 3) &
                                          + met(i, j + 1, nz + 1, 2, 3) &
                                          * chris(i, j + 1, nz + 1, 1, 3))
                          chris22EdgeFU = 0.25 *(chris(i, j, nz, 2, 2) &
                                          + chris(i, j + 1, nz, 2, 2) &
                                          + chris(i, j, nz + 1, 2, 2) &
                                          + chris(i, j + 1, nz + 1, 2, 2))
                          chris23EdgeFU = 0.25 * (met(i, j, nz, 2, 3) &
                                          * chris(i, j, nz, 2, 3) &
                                          + met(i, j + 1, nz, 2, 3) &
                                          * chris(i, j + 1, nz, 2, 3) &
                                          + met(i, j, nz + 1, 2, 3) &
                                          * chris(i, j, nz + 1, 2, 3) &
                                          + met(i, j + 1, nz + 1, 2, 3) &
                                          * chris(i, j + 1, nz + 1, 2, 3))
                          vEdgeFUU = 1.5 * var(i, j + 1, nz, 3) &
                                     - 0.5 * var(i, j + 1, nz - 1, 3)
                          vEdgeFU = 1.5 * var(i, j, nz, 3) &
                                    - 0.5 * var(i, j, nz - 1, 3)
                          vEdgeBU = 1.5 * var(i, j - 1, nz, 3) &
                                    - 0.5 * var(i, j - 1, nz - 1, 3)
                          vEdgeFU1 = 1.5 * var(i + 1, j, nz, 3) &
                                     - 0.5 * var(i + 1, j, nz - 1, 3)
                          vEdgeFU2 = 1.5 * var(i - 1, j, nz, 3) &
                                     - 0.5 * var(i - 1, j, nz - 1, 3)
                          uEdgeFU = 1.5 * 0.25 * (var(i, j, nz, 2) &
                                    + var(i - 1, j, nz, 2) &
                                    + var(i, j + 1, nz, 2) &
                                    + var(i - 1, j + 1, nz, 2)) &
                                    - 0.5 * 0.25 * (var(i, j, nz - 1, 2) &
                                    + var(i - 1, j, nz - 1, 2) &
                                    + var(i, j + 1, nz - 1, 2) &
                                    + var(i - 1, j + 1, nz - 1, 2))
                          do k = 1, nbz
                              wEdgeFU = 0.5 * (var(i, j, nz + k, 4) &
                                        + var(i, j + 1, nz + k, 4))
                              wEdgeFD = 0.5 * (var(i, j, nz - k, 4) &
                                        + var(i, j + 1, nz - k, 4))
                              var(i, j, nz + k, 3) &
                              = - (2.0 * k - 1.0) * dz / met33EdgeFU &
                                * (met13EdgeFU * 0.5 * (vEdgeFU1 - vEdgeFU2) &
