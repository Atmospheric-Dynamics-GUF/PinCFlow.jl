module type_module
  ...
  namelist / outputList / atmvarOut, prepare_restart, restart, iIn, runName, &
      &outputType, nOutput, maxIter, outputTimeDiff, maxTime, fancy_namelists
  !achatzb
  !achatze
  ...
  real :: dtMin_dim
  namelist / debuggingList / dtMin_dim
  ...
  namelist / solverList / cfl, dtMax_dim, tStepChoice, limiterType1
  ...
  namelist / poissonSolverList / tolPoisson, maxIterPoisson, preconditioner, &
      &dtau, maxIterADI, initialCleaning, correctMomentum, tolcrit
  ...
  namelist / atmosphereList / specifyReynolds, ReInv, mu_viscous_dim, &
      &background, Temp0_dim, press0_dim, backgroundFlow_dim, f_Coriolis_dim, &
      &corset
  ...
  namelist / topographyList / mountainHeight_dim, mountainWidth_dim, &
      &mountain_case, range_factor, spectral_modes, envelope_reduction, &
      &stretch_exponent
  ...
  namelist / boundaryList / spongeLayer, sponge_uv, spongeHeight, &
      &spongeAlphaZ_dim, spongeAlphaZ_fac, unifiedSponge, lateralSponge, &
      &spongeType, spongeOrder, cosmoSteps, relax_to_mean, relaxation_period, &
  ...
  subroutine write_namelists
    ...
    if(master) then
      ...
      call write_character("limiterType1", limiterType1, "Flux limiter &
          &('MCVariant')")
      write(90, "(a)") "&end"
      write(90, "(a)") ""
