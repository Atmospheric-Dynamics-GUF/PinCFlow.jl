...
\\begin{align*}
    ...
    & \\qquad \\qquad + \\left.\\frac{\\left(\\overline{\\rho} \\left\\langle u' w' \\right\\rangle\\right)_{k + 1} - \\left(\\overline{\\rho} \\left\\langle u' w' \\right\\rangle\\right)_{k - 1}}{2 J \\Delta \\widehat{z}}\\right] - \\rho_\\mathrm{r} \\frac{f}{\\overline{\\theta}} \\left\\langle \\theta' v' \\right\\rangle,\\\\
    \\left(\\frac{\\partial \\rho_\\mathrm{r} v_\\mathrm{b}}{\\partial t}\\right)_\\mathrm{w} & = - \\frac{\\rho_\\mathrm{r}}{\\overline{\\rho}} \\left[\\frac{\\left(\\overline{\\rho} \\left\\langle v' u' \\right\\rangle\\right)_{i + 1} - \\left(\\overline{\\rho} \\left\\langle v' u' \\right\\rangle\\right)_{i - 1}}{2 \\Delta \\widehat{x}} + G^{13} \\frac{\\left(\\overline{\\rho} \\left\\langle v' u' \\right\\rangle\\right)_{k + 1} - \\left(\\overline{\\rho} \\left\\langle v' u' \\right\\rangle\\right)_{k - 1}}{2 \\Delta \\widehat{z}}\\right.\\\\
    ...
    & \\qquad \\qquad + \\left.\\frac{\\left(\\overline{\\rho} \\left\\langle v' w' \\right\\rangle\\right)_{k + 1} - \\left(\\overline{\\rho} \\left\\langle v' w' \\right\\rangle\\right)_{k - 1}}{2 J \\Delta \\widehat{z}}\\right] + \\rho_\\mathrm{r} \\frac{f}{\\overline{\\theta}} \\left\\langle \\theta' u' \\right\\rangle,\\\\
...
where ``\\left(u_\\mathrm{b}, v_\\mathrm{b}, \\widehat{w}_\\mathrm{b}\\right)`` are the components of the transformed (i.e. terrain-following) resolved wind, ``\\rho_\\mathrm{r}`` is the resolved density (including the reference part ``\\overline{\\rho}``) and ``P_\\mathrm{r}`` is the resolved mass-weighted potential temperature (including the reference part ``\\overline{P}``). For a documentation of the fluxes, see [`PinCFlow.MSGWaM.MeanFlowEffect.compute_gw_integrals!`](@ref). Below `state.namelists.wkb.impact_altitude`, all tendencies are set to zero.
