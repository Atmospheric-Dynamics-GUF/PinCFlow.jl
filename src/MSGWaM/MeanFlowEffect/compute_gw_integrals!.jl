...
```math
\\begin{align*}
    \\bar{\\rho} \\left\\langle u' u' \\right\\rangle & = \\bar{\\rho} \\sum_{r, \\lambda, \\mu, \\nu} \\left(F u_\\mathrm{w} u_\\mathrm{w}^*\\right)_{r, i + \\lambda, j + \\mu, k + \\nu},\\\\
    \\bar{\\rho} \\left\\langle u' v' \\right\\rangle & = \\bar{\\rho} \\sum_{r, \\lambda, \\mu, \\nu} \\left(F u_\\mathrm{w} v_\\mathrm{w}^*\\right)_{r, i + \\lambda, j + \\mu, k + \\nu},\\\\
    \\bar{\\rho} \\left\\langle u' w' \\right\\rangle & = \\bar{\\rho} \\sum_{r, \\lambda, \\mu, \\nu} \\left(F u_\\mathrm{w} w_\\mathrm{w}^*\\right)_{r, i + \\lambda, j + \\mu, k + \\nu},\\\\
    \\bar{\\rho} \\left\\langle v' v' \\right\\rangle & = \\bar{\\rho} \\sum_{r, \\lambda, \\mu, \\nu} \\left(F v_\\mathrm{w} v_\\mathrm{w}^*\\right)_{r, i + \\lambda, j + \\mu, k + \\nu},\\\\
...
This method computes ``\\bar{\\rho} \\left\\langle u' w' \\right\\rangle``, ``\\bar{\\rho} \\left\\langle v' w' \\right\\rangle``, ``\\left\\langle \\theta' u' \\right\\rangle``, ``\\left\\langle \\theta' v' \\right\\rangle`` and ``\\mathcal{E}`` (see above for details).
...
This method computes the sums ``\\bar{\\rho} \\left\\langle u' w' \\right\\rangle`` and ``\\bar{\\rho} \\left\\langle v' w' \\right\\rangle`` (see above for details). In contrast to the multi-column and single-column modes, the steady-state mode uses the pseudo-momentum approximation
