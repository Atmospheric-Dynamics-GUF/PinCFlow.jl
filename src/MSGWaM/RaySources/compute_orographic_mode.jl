...
mountain-wave theory. The implemented formulas are as follows (where this method's arguments are represented by ``h_{\\mathrm{w}, \\alpha}``, ``k_{h, \\alpha}``, ``l_{h, \\alpha}``, ``u_h``, ``v_h``, ``\\bar{\\rho}_h``, ``N_h^2``, ``f`` and ``\\sigma``).
