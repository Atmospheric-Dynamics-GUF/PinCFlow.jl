...
Determine the index of the closest half-level above `z` at the horizontal position `(i, j)`.
...
This method is heavily used for interpolation to ray-volume positions. To ensure that the vertical boundary conditions are enforced and no out-of-bounds errors occur, the following constraints are set.
  ...
  - On MPI processes at the upper boundary of the domain, the returned index cannot be larger than `domain.k1`, on other processes, it cannot be larger than `domain.nzz - 1`.
