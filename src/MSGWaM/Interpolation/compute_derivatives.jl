...
function compute_derivatives(
    ...
    @ivy if (zctilde[i, j, ku] + zctilde[i + 1, j, ku]) / 2 <
            (hb[i, j] + hb[i + 1, j]) / 2 # minor bug fix (commit bd8249b)
        phid = 0.0
        phiu = 0.0
    elseif (zctilde[i, j, kd] + zctilde[i + 1, j, kd]) / 2 <
        ...
        if (zctilde[i, j, ku] + zctilde[i + 1, j, ku]) / 2 < lz # minor bug fix
            phid =
        ...
        elseif (zctilde[i, j, kd] + zctilde[i + 1, j, kd]) / 2 < lz # minor bug fix
            phid =
                (u[i, j, kd + 1] - u[i, j, kd]) / dz / (
                    jac[i, j, kd] * jac[i, j, kd + 1] /
                    (jac[i, j, kd] + jac[i, j, kd + 1]) +
