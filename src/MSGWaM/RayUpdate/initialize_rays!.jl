...
function initialize_rays!(state::State, testcase::WKBWavePacket)
    (; sizex, sizey, sizez) = state.namelists.domain
    (; testcase) = state.namelists.setting
    (; coriolis_frequency) = state.namelists.atmosphere
    (;
    ...
    (; lambdax_dim, lambday_dim, lambdaz_dim) = state.namelists.wavepacket
    ...
    # Set vertical index bounds.
    kz0 = max(k0, floor(Int, (zrmin - lz[1]) / dz) + k0 - ko)
    kz1 = min(k1, floor(Int, (zrmax - lz[1]) / dz) + k0 - ko)
    ...
    if fac_dk_init == 0.0
        dk_ini_nd = 0.0
    elseif lambdax_dim != 0.0
        dk_ini_nd = fac_dk_init * 2.0 * pi / lambdax_dim * lref
    elseif lambday_dim != 0.0
        dk_ini_nd = fac_dk_init * 2.0 * pi / lambday_dim * lref
    else
        error("Error in initialize_rays! (WKBWavePacket): both lambdax_dim 
        and lambday_dim = 0.0")
    end
    ...
    if fac_dl_init == 0.0
        dl_ini_nd = 0.0
    elseif lambday_dim != 0.0
        dl_ini_nd = fac_dl_init * 2.0 * pi / lambday_dim * lref
    elseif lambdax_dim != 0.0
        dl_ini_nd = fac_dl_init * 2.0 * pi / lambdax_dim * lref
    else
        error("Error in initialize_rays! (WKBWavePacket): both lambdax_dim 
        and lambday_dim = 0.0")
    end

    dm_ini_nd = fac_dm_init * 2.0 * pi / lambdaz_dim * lref
