...
!gagarinae
    ...
    ! TFC FJ
    real, dimension((- nbx):(sizeX + nbx), (- nby):(sizeY + nby)) :: x_rotated
    real :: x_rotated_center
    ...
    if(topography) then
        if(.not. allocated(pStratTFC)) then
            allocate(pStratTFC((- nbx):(nx + nbx),(- nby):(ny + nby),&
        ...
        if(.not. allocated(rhoStratTFC)) then
            allocate(rhoStratTFC((- nbx):(nx + nbx),(- nby):(ny + nby),&
            (- 1):(nz + 2)), stat = allocstat)
            if(allocstat /= 0) stop "atmosphere.f90: could not allocate &
            rhoStratTFC"
        end if
        ...
        if(.not. allocated(bvsStratTFC)) then
            allocate(bvsStratTFC((- nbx):(nx + nbx),(- nby):(ny + nby),&
            (- 1):(nz + 2)), stat = allocstat)
            if(allocstat /= 0) stop "atmosphere.f90: could not allocate &
            bvsStratTFC"
        end if
        ...
        if(.not. allocated(piStratTFC)) then
            allocate(piStratTFC((- nbx):(nx + nbx),(- nby):(ny + nby),&
            (- 1):(nz + 2)), stat = allocstat)
            if(allocstat /= 0) stop "atmosphere.f90: could not allocate &
            piStratTFC"
        end if
...
! FS: see now further below
    ...
    if(topography) then
        ...
        x_rotated_center = sqrt(x_center ** 2.0 + y_center ** 2.0)
        ...
        if(mountain_case == 2) then
            do i = - nbx, sizeX + nbx
                do j = - nby, sizeY + nby
                    x_rotated(i, j) = cos(0.25 * pi) * x(i) &
                                      + sin(0.25 * pi) * y(j)
                end do
            end do
        end if
        ...
        do j = - nby, ny + nby
            do i = - nbx, nx + nbx
                if(mountain_case == 1) then
                    ...
                    = 0.5 * mountainHeight &
                      * (1.0 + cos(k_mountainw * (x(i + i00) &
                      - x_center)))
                else if(mountain_case == 2) then
                    topography_surface(i, j) &
                    = 0.5 * mountainHeight &
                      * (1.0 + cos(k_mountainw &
                      * (x_rotated(i + i00, j + j00) &
                      - x_rotated_center)))
                else if(mountain_case == 3) then
                    topography_surface(i, j) &
                    = mountainHeight &
                      / (1.0 + ((x(i + i00) - x_center) ** 2.0) &
                      / (mountainWidth ** 2.0))
                else if(mountain_case == 4) then
                    topography_surface(i, j) &
                    = mountainHeight &
                      / (1.0 + ((x(i + i00) - x_center) ** 2.0 &
                    ...
                    = 0.5 * mountainHeight &
                      * (1.0 + cos(k_mountainw * (x(i + i00) &
                      - x_center))) * (cos(range_fac * k_mountainw &
                      * (x(i + i00) - x_center)))
                else if(mountain_case == 6) then
                    topography_surface(i, j) &
                    = mountainHeight &
                      * exp(- ((x(i + i00) - x_center) &
                      / mountainWidth) ** 2.0) * cos(5.0 * k_mountainw &
                  ...
                  do j = - nby, ny + nby
                      do k = - 1, nz + 2
                          if(heightTFC(i, j, k) <= z_tr) then
                              ...
                              ! Define rhoStratTFC.
                              rhoStratTFC(i, j, k) &
                              = pStratTFC(i, j, k) / thetaStratTFC(i, j, k)
                          else
                              ! Isothermal stratosphere.
                              ! Define pStratTFC.
                              pStratTFC(i, j, k) &
                              = p0 ** kappa * press_tr ** (1.0 / gamma) &
                                * exp(- sig /gamma / T_tr &
                                * (heightTFC(i, j, k) - z_tr))
                              ! Define thetaStratTFC.
                              thetaStratTFC(i, j, k) &
                              = theta_tr * exp(kappa * sig / T_tr &
                                * (heightTFC(i, j, k) - z_tr))
                              ! Define rhoStratTFC.
                              rhoStratTFC(i, j, k) &
                              = pStratTFC(i, j, k) / thetaStratTFC(i, j, k)
                 ...
                 do i = - nbx, nx + nbx
                     do j = - nby, ny + nby
                         do k = - 1, nz + 2
                             ! Define pStratTFC.
                             pStratTFC(i, j, k) &
                             = p0 * exp(- sig * heightTFC(i, j, k) / gamma / T0)
                             ! Define thetaStratTFC.
                             thetaStratTFC(i, j, k) &
                             = T0 * exp(kappa * sig / T0 * heightTFC(i, j, k))
                             ! Define rhoStratTFC.
                             rhoStratTFC(i, j, k) &
                             = pStratTFC(i, j, k) / thetaStratTFC(i, j, k)
                         end do
                 ...
                 do i = - nbx, nx + nbx
                     do j = - nby, ny + nby
                         do k = - 1, nz + 2
                             ...
                             ! Define pStratTFC.
                             pStratTFC(i, j, k) &
                             = p0 * (1.0 + FrInv2 * kappa * sig / N2 / theta0 &
                               * (term - 1.0)) ** power
                             ! Define thetaStratTFC.
                             thetaStratTFC(i, j, k) &
                             = theta0 * exp(Fr ** 2.0 * N2 * heightTFC(i, j, k))
                             ! Define rhoStratTFC.
                             rhoStratTFC(i, j, k) &
                             = pStratTFC(i, j, k) / thetaStratTFC(i, j, k)
                         end do
                  ...
                  do j = - nby, ny + nby
                      do k = - 1, nz + 2
                          ...
                          if(zk < z_tr_dim) then
                              T_bar = Temp0_dim - gamma_t * zk
                              if(gamma_t /= 0.0) then
                                  p_bar = press0_dim * (1.0 - gamma_t * zk &
                                          / Temp0_dim) ** pow_t
                              else
                                  p_bar = press0_dim * exp(- zk / (Rsp &
                                          * Temp0_dim / g))
                              end if
                          else
                              T_bar = Temp0_dim - gamma_t * z_tr_dim &
                                      - gamma_s * (zk - z_tr_dim)
                              if(gamma_s /= 0.0) then
                                  p_bar = p_t_b * (1.0 - gamma_s * (zk &
                                          - z_tr_dim) / T_c_b) ** pow_s
                              else
                                  p_bar = p_t_b * exp(- (zk - z_tr_dim) &
                                          / (Rsp * T_c_b / g))
                              end if
                          end if
                          ...
                          ! Define rhoStratTFC.
                          rhoStratTFC(i, j, k) &
                          = p_bar / (Rsp * T_bar) / rhoRef
                          ! Define pStratTFC.
                          pStratTFC(i, j, k) &
                          = rhoStratTFC(i, j, k) * thetaStratTFC(i, j, k)
                      end do
                  end do
              end do
          end if
...
!           end do
          ...
          if(topography) then
              do i = - nbx, nx + nbx
                  do j = - nby, ny + nby
                      ...
                      ! the surface.
                      do k = - nbz, 1
                          T_bar = max(tp_strato, tp_srf_trp - 0.5 &
                                  * tpdiffhor_tropo)
                          ! Define Exner pressure.
                          piStratTFC(i, j, k) &
                          = 1.0 + heightTFC(i, j, k) * kappa / T_bar
                          if(k == 1 .and. piStratTFC(i, j, k) <= 0.0) then
                              stop "ERROR: non-positive piStratTFC at k = 1"
                          end if
                          T_bar = max(tp_strato, piStratTFC(i, j, k) &
                                  * (tp_srf_trp - 0.5 * tpdiffhor_tropo &
                                  - 0.5 * ptdiffvert_tropo / kappa &
                                  * log(piStratTFC(i, j, k))))
                          ! Define pStratTFC.
                          pStratTFC(i, j, k) &
                          = piStratTFC(i, j, k) ** ((1.0 - kappa) / kappa)
                          ! Define thetaStratTFC.
                          thetaStratTFC(i, j, k) &
                          = T_bar / piStratTFC(i, j, k)
                          ! Define rhoStratTFC.
                          rhoStratTFC(i, j, k) &
                          = pStratTFC(i, j, k) / thetaStratTFC(i, j, k)
                      end do
                      ...
                      do k = 2, nz + nbz
                          ! Leapfrog step.
                          piStar = piStratTFC(i, j, k - 2) - 2.0 * dz &
                                   * jac(i, j, k - 1) * kappa &
                                   / thetaStratTFC(i, j, k - 1)
                          if(piStar <= 0.0) then
                              print*, "ERROR: non-positive piStar at k =", k
                              stop
                          end if
                          T_bar = max(tp_strato, piStar * (tp_srf_trp &
                                  - 0.5 * tpdiffhor_tropo - 0.5 &
                                  * ptdiffvert_tropo / kappa * log(piStar)))
                          thetaStar = T_bar / piStar
                          ! Trapezoidal step.
                          piStratTFC(i, j, k) &
                          = piStratTFC(i, j, k - 1) - 0.5 * dz * 0.5 &
                            * (jac(i, j, k) + jac(i, j, k - 1)) * (kappa &
                            / thetaStar + kappa / thetaStratTFC(i, j, k - 1))
                          if(piStratTFC(i, j, k) <= 0.0) then
                              print*, "ERROR: non-positive piStratTFC at &
                              k =", k
                              stop
                          end if
                          T_bar = max(tp_strato, piStratTFC(i, j, k) &
  ...
  subroutine terminate_atmosphere
    ...
    if(topography) then
        ...
        if(allocstat /= 0) stop "atmosphere.f90: could not dealloc &
        pStratTFC"
  ...
  subroutine setHalosOfField2D(field)
      ...
      if ( idim > 1 ) then
          ...
          call mpi_sendrecv(xSliceRight_send(i0,j0), sendcount, &
               ...
               & xSliceLeft_recv(i0,j0), recvcount, &
          ...
          call mpi_sendrecv(xSliceLeft_send(i0,j0), sendcount, &
               & mpi_double_precision, dest, tag, &
               & xSliceRight_recv(i0,j0), recvcount, &
               & mpi_double_precision, &
      ...
      if( jdim > 1 ) then
          ...
          call mpi_sendrecv(ySliceForw_send(i0,j0), sendcount, &
               & mpi_double_precision, dest, tag, &
               & ySliceBack_recv(i0,j0), recvcount, &
               & mpi_double_precision, &
          ...
          call mpi_sendrecv(ySliceBack_send(i0,j0), sendcount, &
               & mpi_double_precision, dest, tag, &
               & ySliceForw_recv(i0,j0), recvcount, &
               & mpi_double_precision, &
  ...
  subroutine output_topography
      ...
      ! Open file.
      if(master) then
          open(42, file = "topography.dat", form = "unformatted", &
          access = "direct", recl = sizeX * sizeY)
      end if
      ...
      do j = 1, ny
          ...
          call mpi_gather(topography_prc(1, j), nx, mpi_real, &
                          topography_mst(1, j), nx, mpi_real, &
                          0, comm, ierror)
      end do
          ...
          do j = 1, ny
              ...
              do j_prc = 1, nprocy
                  ...
                  do i_prc = 1, nprocx
                      do i = 1, nx
                          i_out = nx * (i_prc - 1) + i
                          i_mst = nprocy * nx * (i_prc - 1) &
                                  + (j_prc - 1) * nx + i
                          topography_out(i_out, j_out) &
                          = topography_mst(i_mst, j_mst)
                      end do
                  end do
              end do
          end do
  ...
  subroutine read_topography
      ...
      do j = 1, ny
         ! Distribute data over all processors.
         call mpi_scatter(topography_mst(1, j), nx, mpi_real, &
                          topography_prc(1, j), nx, mpi_real, 0, comm, &
                          ierror)
          do i = 1, nx
              ! Non-dimensionalize.
  ...
  function met(i, j, k, mu, nu)
      ...
      if((mu == 1 .and. nu == 3) .or. (mu == 3 .and. nu == 1)) then
          met = (topography_surface(i + 1, j) &
                - topography_surface(i - 1, j)) / (2.0 * dx) &
                * (z(k) - (lz(1) - lz(0))) / ((lz(1) - lz(0)) &
                - topography_surface(i, j))
      else if((mu == 2 .and. nu == 3) .or. (mu == 3 .and. nu == 2)) then
          met = (topography_surface(i, j + 1) &
                - topography_surface(i, j - 1)) / (2.0 * dy) &
                * (z(k) - (lz(1) - lz(0))) / ((lz(1) - lz(0)) &
                - topography_surface(i, j))
      else if(mu == 3 .and. nu == 3) then
          met = ((lz(1) - lz(0)) / ((lz(1) - lz(0)) &
                - topography_surface(i, j))) ** 2.0 &
                + ((z(k) - (lz(1) - lz(0))) / ((lz(1) - lz(0)) &
                - topography_surface(i, j))) ** 2.0 &
                * (((topography_surface(i + 1, j) &
                - topography_surface(i - 1, j)) / (2.0 * dx)) ** 2.0 &
                + ((topography_surface(i, j + 1) &
                - topography_surface(i, j - 1)) / (2.0 * dy)) ** 2.0)
      end if
  ...
  function chris(i, j, k, mu, nu)
      ...
      if(mu == 1 .and. nu == 1) then
          chris = - (topography_surface(i - 1, j) &
                  - 2.0 * topography_surface(i, j) &
                  + topography_surface(i + 1, j)) / (dx ** 2.0) &
                  * (z(k) - (lz(1) - lz(0))) / ((lz(1) - lz(0)) &
                  - topography_surface(i, j))
      else if((mu == 1 .and. nu == 2) .or. (mu == 2 .and. nu == 1)) then
          chris = - (topography_surface(i + 1, j + 1) &
                  - topography_surface(i - 1, j + 1) &
                  - topography_surface(i + 1, j - 1) &
                  + topography_surface(i - 1, j - 1)) / (4.0 * dx * dy) &
                  * (z(k) - (lz(1) - lz(0))) / ((lz(1) - lz(0)) &
                  - topography_surface(i, j))
      else if(mu == 2 .and. nu == 2) then
          chris = - (topography_surface(i, j - 1) &
                  - 2.0 * topography_surface(i, j) &
                  + topography_surface(i, j + 1)) / (dy ** 2.0) &
                  * (z(k) - (lz(1) - lz(0))) / ((lz(1) - lz(0)) &
                  - topography_surface(i, j))
  ...
  function heightTFC(i, j, k)
      ...
      heightTFC = z(k) * ((lz(1) - lz(0))- topography_surface(i, j)) &
                  / (lz(1) - lz(0)) + topography_surface(i, j)
  ...
  function vertWindTFC(i, j, k, var)
      ...
      real :: vertWindTFC
      real :: uEdgeR, uUEdgeR, uEdgeL, uUEdgeL, &
              vEdgeF, vUEdgeF, vEdgeB, vUEdgeB, &
              wEdgeU
      ...
      vertWindTFC = trafoTFC(i, j, k, uEdgeR, uUEdgeR, uEdgeL, uUEdgeL, &
                    vEdgeF, vUEdgeF, vEdgeB, vUEdgeB, wEdgeU, "car")

  end function vertWindTFC

  function trafoTFC(i, j, k, uEdgeR, uUEdgeR, uEdgeL, uUEdgeL, &
           vEdgeF, vUEdgeF, vEdgeB, vUEdgeB, wEdgeU, wind)
      ...
      real :: jacEdgeU
      real :: uC, uU, vC, vU
      real :: metEdgeR, metUEdgeR, metEdgeL, metUEdgeL, &
              metEdgeF, metUEdgeF, metEdgeB, metUEdgeB
      ...
      case(1)
          ...
          if(wind == "car") then
              trafoTFC = - 0.5 * (jac(i, j, k) * met(i, j, k, 1, 3) &
                         * uC + jac(i, j, k + 1) &
                         * met(i, j, k + 1, 1, 3) * uU) - 0.5 &
                         * (jac(i, j, k) * met(i, j, k, 2, 3) * vC &
      ...
      case(2)
          ...
          vU = 0.5 * (vUEdgeF + vUEdgeB)
          if(wind == "car") then
              trafoTFC = jacEdgeU * (- 0.5 * (met(i, j, k, 1, 3) &
                         * uC + met(i, j, k + 1, 1, 3) &
                         * uU) - 0.5 * (met(i, j, k, 2, 3) &
                         * vC + met(i, j, k + 1, 2, 3) &
                         * vU) + wEdgeU)
          else if(wind == "tfc") then
              trafoTFC = 0.5 * (met(i, j, k, 1, 3) &
                         * uC + met(i, j, k + 1, 1, 3) &
                         * uU) + 0.5 * (met(i, j, k, 2, 3) &
                         * vC + met(i, j, k + 1, 2, 3) &
                         * vU) + wEdgeU / jacEdgeU
      ...
      case(3)
          ...
          jacEdgeU = 0.5 * (jac(i, j, k) + jac(i, j, k + 1))
          metEdgeR = 0.5 * (jac(i, j, k) * met(i, j, k, 1, 3) &
                      + jac(i + 1, j, k) * met(i + 1, j, k, 1, 3))
          metUEdgeR = 0.5 * (jac(i, j, k + 1) * met(i, j, k + 1, 1, 3) &
                      + jac(i + 1, j, k + 1) * met(i + 1, j, k + 1, 1, 3))
          metEdgeL = 0.5 * (jac(i, j, k) * met(i, j, k, 1, 3) &
                      + jac(i - 1, j, k) * met(i - 1, j, k, 1, 3))
          metUEdgeL = 0.5 * (jac(i, j, k + 1) * met(i, j, k + 1, 1, 3) &
                      + jac(i - 1, j, k + 1) * met(i - 1, j, k + 1, 1, 3))
          metEdgeF = 0.5 * (jac(i, j, k) * met(i, j, k, 2, 3) &
                      + jac(i, j + 1, k) * met(i, j + 1, k, 2, 3))
          metUEdgeF = 0.5 * (jac(i, j, k + 1) * met(i, j, k + 1, 2, 3) &
                      + jac(i, j + 1, k + 1) * met(i, j + 1, k + 1, 2, 3))
          metEdgeB = 0.5 * (jac(i, j, k) * met(i, j, k, 2, 3) &
                      + jac(i, j - 1, k) * met(i, j - 1, k, 2, 3))
          metUEdgeB = 0.5 * (jac(i, j, k + 1) * met(i, j, k + 1, 2, 3) &
                      + jac(i, j - 1, k + 1) * met(i, j - 1, k + 1, 2, 3))
          if(wind == "car") then
              trafoTFC = - 0.25 * (metEdgeR * uEdgeR + metUEdgeR &
                         * uUEdgeR + metEdgeL * uEdgeL + metUEdgeL &
                         * uUEdgeL) - 0.25 * (metEdgeF * vEdgeF &
                         + metUEdgeF * vUEdgeF + metEdgeB * vEdgeB &
                         + metUEdgeB * vUEdgeB) + jacEdgeU * wEdgeU
          else if(wind == "tfc") then
              trafoTFC = (0.25 * (metEdgeR * uEdgeR + metUEdgeR &
                         * uUEdgeR + metEdgeL * uEdgeL + metUEdgeL &
                         * uUEdgeL) + 0.25 * (metEdgeF * vEdgeF &
                         + metUEdgeF * vUEdgeF + metEdgeB * vEdgeB &
                         + metUEdgeB * vUEdgeB) + wEdgeU) / jacEdgeU
          end if
      ...
      case(4)
          ...
          metUEdgeR = 0.5 * (met(i, j, k + 1, 1, 3) &
                      + met(i + 1, j, k + 1, 1, 3))
          metEdgeL = 0.5 * (met(i, j, k, 1, 3) + met(i - 1, j, k, 1, 3))
          metUEdgeL = 0.5 * (met(i, j, k + 1, 1, 3) &
                      + met(i - 1, j, k + 1, 1, 3))
          metEdgeF = 0.5 * (met(i, j, k, 2, 3) + met(i, j + 1, k, 2, 3))
          metUEdgeF = 0.5 * (met(i, j, k + 1, 2, 3) &
                      + met(i, j + 1, k + 1, 2, 3))
          metEdgeB = 0.5 * (met(i, j, k, 2, 3) + met(i, j - 1, k, 2, 3))
          metUEdgeB = 0.5 * (met(i, j, k + 1, 2, 3) &
                      + met(i, j - 1, k + 1, 2, 3))
          if(wind == "car") then
              trafoTFC = jacEdgeU * (- 0.25 * (metEdgeR * uEdgeR &
                         + metUEdgeR * uUEdgeR + metEdgeL * uEdgeL &
                         + metUEdgeL * uUEdgeL) - 0.25 * (metEdgeF &
                         * vEdgeF + metUEdgeF * vUEdgeF + metEdgeB &
                         * vEdgeB + metUEdgeB * vUEdgeB) + wEdgeU)
          else if(wind == "tfc") then
              trafoTFC = 0.25 * (metEdgeR * uEdgeR &
                         + metUEdgeR * uUEdgeR + metEdgeL * uEdgeL &
                         + metUEdgeL * uUEdgeL) + 0.25 * (metEdgeF &
                         * vEdgeF + metUEdgeF * vUEdgeF + metEdgeB &
                         * vEdgeB + metUEdgeB * vUEdgeB) + wEdgeU &
      ...
      case(5)
          ...
          jacEdgeU = 0.5 * (jac(i, j, k) + jac(i, j, k + 1))
          metEdgeRU = 0.25 * (jac(i, j, k) * met(i, j, k, 1, 3) &
                      + jac(i + 1, j, k) * met(i + 1, j, k, 1, 3) &
                      + jac(i, j, k + 1) * met(i, j, k + 1, 1, 3) &
                      + jac(i + 1, j, k + 1) * met(i + 1, j, k + 1, 1, 3))
          metEdgeLU = 0.25 * (jac(i, j, k) * met(i, j, k, 1, 3) &
                      + jac(i - 1, j, k) * met(i - 1, j, k, 1, 3) &
                      + jac(i, j, k + 1) * met(i, j, k + 1, 1, 3) &
                      + jac(i - 1, j, k + 1) * met(i - 1, j, k + 1, 1, 3))
          metEdgeFU = 0.25 * (jac(i, j, k) * met(i, j, k, 2, 3) &
                      + jac(i, j + 1, k) * met(i, j + 1, k, 2, 3) &
                      + jac(i, j, k + 1) * met(i, j, k + 1, 2, 3) &
                      + jac(i, j + 1, k + 1) * met(i, j + 1, k + 1, 2, 3))
          metEdgeBU = 0.25 * (jac(i, j, k) * met(i, j, k, 2, 3) &
                      + jac(i, j - 1, k) * met(i, j - 1, k, 2, 3) &
                      + jac(i, j, k + 1) * met(i, j, k + 1, 2, 3) &
                      + jac(i, j - 1, k + 1) * met(i, j - 1, k + 1, 2, 3))
          ...
          if(wind == "car") then
              trafoTFC = - 0.5 * (metEdgeRU * uEdgeRU - metEdgeLU &
                         * uEdgeLU) - 0.5 * (metEdgeFU * vEdgeFU &
                         - metEdgeBU * vEdgeBU) + jacEdgeU * wEdgeU
          else if(wind == "tfc") then
              trafoTFC = (0.5 * (metEdgeRU * uEdgeRU - metEdgeLU &
                         * uEdgeLU) + 0.5 * (metEdgeFU * vEdgeFU &
                         - metEdgeBU * vEdgeBU) + wEdgeU) / jacEdgeU
          end if
      ...
      case(6)
          ...
          jacEdgeU = 0.5 * (jac(i, j, k) + jac(i, j, k + 1))
          metEdgeRU = 0.25 * (met(i, j, k, 1, 3) &
                      + met(i + 1, j, k, 1, 3) + met(i, j, k + 1, 1, 3) &
                      + met(i + 1, j, k + 1, 1, 3))
          metEdgeLU = 0.25 * (met(i, j, k, 1, 3) &
                      + met(i - 1, j, k, 1, 3) + met(i, j, k + 1, 1, 3) &
                      + met(i - 1, j, k + 1, 1, 3))
          metEdgeFU = 0.25 * (met(i, j, k, 2, 3) &
                      + met(i, j + 1, k, 2, 3) + met(i, j, k + 1, 2, 3) &
                      + met(i, j + 1, k + 1, 2, 3))
          metEdgeBU = 0.25 * (met(i, j, k, 2, 3) &
                      + met(i, j - 1, k, 2, 3) + met(i, j, k + 1, 2, 3) &
                      + met(i, j - 1, k + 1, 2, 3))
          uEdgeRU = 0.5 * (uEdgeR + uUEdgeR)
          uEdgeLU = 0.5 * (uEdgeL + uUEdgeL)
          ...
          vEdgeBU = 0.5 * (vEdgeB + vUEdgeB)
          if(wind == "car") then
              trafoTFC = jacEdgeU * (- 0.5 * (metEdgeRU * uEdgeRU &
                         - metEdgeLU * uEdgeLU) - 0.5 * (metEdgeFU &
                         * vEdgeFU - metEdgeBU * vEdgeBU) + wEdgeU)
          else if(wind == "tfc") then
              trafoTFC = 0.5 * (metEdgeRU * uEdgeRU &
                         - metEdgeLU * uEdgeLU) + 0.5 * (metEdgeFU &
                         * vEdgeFU - metEdgeBU * vEdgeBU) + wEdgeU &
                         / jacEdgeU
          end if
  ...
  function stressTensTFC(i, j, k, mu, nu, var)
      ...
      met13EdgeU = 0.5 * (jac(i, j, k) * met(i, j, k, 1, 3) &
                   + jac(i, j, k + 1) * met(i, j, k + 1, 1, 3))
      met13EdgeD = 0.5 * (jac(i, j, k) * met(i, j, k, 1, 3) &
                   + jac(i, j, k - 1) * met(i, j, k - 1, 1, 3))
      met23EdgeU = 0.5 * (jac(i, j, k) * met(i, j, k, 2, 3) &
                   + jac(i, j, k + 1) * met(i, j, k + 1, 2, 3))
      met23EdgeD = 0.5 * (jac(i, j, k) * met(i, j, k, 2, 3) &
                   + jac(i, j, k - 1) * met(i, j, k - 1, 2, 3))
      ...
      wR = 0.5 * (vertWindTFC(i + 1, j, k, var) &
           + vertWindTFC(i + 1, j, k - 1, var))
      wL = 0.5 * (vertWindTFC(i - 1, j, k, var) &
           + vertWindTFC(i - 1, j, k - 1, var))
      wF = 0.5 * (vertWindTFC(i, j + 1, k, var) &
           + vertWindTFC(i, j + 1, k - 1, var))
      wB = 0.5 * (vertWindTFC(i, j - 1, k, var) &
           + vertWindTFC(i, j - 1, k - 1, var))

      if(mu == 1 .and. nu == 1) then
          stressTensTFC = (2.0 * (jacEdgeR * var(i, j, k, 2) &
                          - jacEdgeL * var(i - 1, j, k, 2)) / dx &
                          + (jac(i, j, k + 1) * met(i, j, k + 1, 1, 3) &
                          * uU - jac(i, j, k - 1) &
                          * met(i, j, k - 1, 1, 3) * uD) / dz - 2.0 &
                          / 3.0 * ((jacEdgeR * var(i, j, k, 2) - jacEdgeL &
                          * var(i - 1, j, k, 2)) / dx + (jacEdgeF &
                          * var(i, j, k, 3) - jacEdgeB &
                          * var(i, j - 1, k, 3)) / dy + (jacEdgeU &
                          * var(i, j, k, 4) - jacEdgeD &
                          * var(i, j, k - 1, 4)) / dz)) / jac(i, j, k)
      else if((mu == 1 .and. nu == 2) .or.(mu == 2 .and. nu == 1)) then
          stressTensTFC = (0.5 * (jac(i, j + 1, k) * uF &
                          - jac(i, j - 1, k) * uB) / dy + 0.5 &
                          * (jac(i, j, k + 1) * met(i, j, k + 1, 2, 3) &
                          * uU - jac(i, j, k - 1) &
                          * met(i, j, k - 1, 2, 3) * uD) / dz + 0.5 &
                          * (jac(i + 1, j, k) * vR - jac(i - 1, j, k) &
                          * vL) / dx + 0.5 * (jac(i, j, k + 1) &
                          * met(i, j, k + 1, 1, 3) * vU - &
                           jac(i, j, k - 1) * met(i, j, k - 1, 1, 3) &
                           * vD) / dz) / jac(i, j, k)
      else if((mu == 1 .and. nu == 3) .or.(mu == 3 .and. nu == 1)) then
          stressTensTFC = (0.5 * (uU - uD) / dz + 0.5 &
                          * (jac(i + 1, j, k) * wR - jac(i - 1, j, k) &
                          * wL) / dx + (met13EdgeU &
                          * vertWindTFC(i, j, k, var) - met13EdgeD &
                          * vertWindTFC(i, j, k - 1, var)) / dz) &
                          / jac(i, j, k)
      else if(mu == 2 .and. nu == 2) then
          stressTensTFC = (2.0 * (jacEdgeF * var(i, j, k, 3) &
                          - jacEdgeB * var(i, j - 1, k, 3)) / dy &
                          + (jac(i, j, k + 1) * met(i, j, k + 1, 2, 3) &
                          * vU - jac(i, j, k - 1) &
                          * met(i, j, k - 1, 2, 3) * vD) / dz - 2.0 &
                          / 3.0 * ((jacEdgeR * var(i, j, k, 2) - jacEdgeL &
                          * var(i - 1, j, k, 2)) / dx + (jacEdgeF &
                          * var(i, j, k, 3) - jacEdgeB &
                          * var(i, j - 1, k, 3)) / dy + (jacEdgeU &
          ...
          stressTensTFC = (0.5 * (vU - vD) / dz + 0.5 &
                          * (jac(i, j + 1, k) * wF - jac(i, j - 1, k) &
                          * wB) / dy + (met23EdgeU &
                          * vertWindTFC(i, j, k, var) - met23EdgeD &
                          * vertWindTFC(i, j, k - 1, var)) / dz) &
                          / jac(i, j, k)
      else if(mu == 3 .and. nu == 3) then
          stressTensTFC = (2.0 * (vertWindTFC(i, j, k, var) &
                          - vertWindTFC(i, j, k - 1, var)) / dz - 2.0 &
                          / 3.0 * ((jacEdgeR * var(i, j, k, 2) - jacEdgeL &
                          * var(i - 1, j, k, 2)) / dx + (jacEdgeF &
                          * var(i, j, k, 3) - jacEdgeB &
                          * var(i, j - 1, k, 3)) / dy + (jacEdgeU &
