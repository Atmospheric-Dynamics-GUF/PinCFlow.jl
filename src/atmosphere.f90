module atmosphere_module
  ...
  subroutine init_atmosphere
    ...
    if(compute_cloudcover) then
      ...
      if(topography) then
        allocate(z2TFC(sizeX2, sizeY2, sizeZ2), stat = allocstat)
        if(allocstat /= 0) stop "init.f90: could not allocate z2TFC"
      else
  ...
  subroutine setup_topography
    ...
    if(compute_cloudcover .and. rayTracer) then
      do ix = 1, nx
        do jy = 1, ny
          do kz = 1, SizeZ
            ...
            do ii = 1, NSCX
              do jj = 1, NSCY
                do kk = 1, NSCZ
                  ...
                  z2TFC(i2, j2, k2) = zTFC(ix, jy, kz) + (- dz / 2.0 + (kk &
                      &- 0.5) * dzsc) * jac(ix, jy, kz)
                end do
              end do
            end do
