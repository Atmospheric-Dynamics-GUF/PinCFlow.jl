module wkb_module
  ...
  use muscl_module
  use ice_module, ONLY:Psat_ice, smooth_shapiro_sgs
  use mpi
  ...
  subroutine setup_wkb(ray, ray_var3D, var, diffusioncoeff, waveAmplitudes, &
    ...
    type(ice_rayType), dimension(:, :, :), allocatable, intent(out) :: &
        &ray_varIce
    type(ice_rayType2), dimension(:, :, :), allocatable, intent(out), optional &
        &:: ray_cloud
    ...
    if(include_ice) then
      ...
      if(raytracer) then
        ...
        if(compute_cloudcover) then
          ...
          allocate(ray_cloud(1 - nbxSC:nxNSCX + nbxSC, 1 - nbySC:nyNSCY &
              &+ nbySC, 1 - nbzSC:nzNSCZ + nbzSC), stat = allocstat)
                ...
                do jy2 = 1, NSCY
                  ...
                  do ix = 1, nx
                    do ix2 = 1, NSCX
                      ...
                      ray_cloud(ik, jl, km)%Ni = var%ICE(ix, jy, kz, inN)
                      ray_cloud(ik, jl, km)%Qi = var%ICE(ix, jy, kz, inQ)
                      ray_cloud(ik, jl, km)%Qv = var%ICE(ix, jy, kz, inQv)
            ...
            do ix = 1, nx
              do iwm = 1, NWM_WP
                ...
                fld_amp(ix, jy, kz, iwm) = (amp_wkb / wnrm_init) ** 2 &
                    ...
                    &** 2) * omi_notop(ix, jy, kz) * rhoStratTFC(ix, jy, kz)
        ...
        do ix = 1, nx
          ...
          ray_loop: do iRay = 1, nRay(ix, jy, kz)
            ...
            if(include_ice .and. update_phase) then
              ...
              dpRay(iRay, ix, jy, kz) = - dt * omir + alphaRK(rkStage) &
                  &* dpRay(iRay, ix, jy, kz)
  ...
  subroutine orographic_source(var, ray, time, dt)
    ...
    ! Launch ray volumes according to the lower boundary condition for
    ! waves.
      ...
      do ix = 1, nx
        ...
        do i_sfc = 1, n_sfc
          ...
          if(.not. steady_state .and. launch_algorithm == "scale") then
            ...
            if(topography) then
              !*** CHANGE  ***
              print *, 'wadr rescaled in wkb.f90 orographic_source'
              stop
              wadr = wadr * dt * cgrz / jac(ix, jy, kz) / dz / 9. !Factor 9 added
            else
  ...
  subroutine calc_ice(ray, var, ray_varIce, ray_cloud)
    ...
    type(ice_rayType2), dimension(1 - nbxSC:nxNSCX + nbxSC, 1 - nbySC:nyNSCY &
        &+ nbySC, 1 - nbxSC:nzNSCZ + nbzSC), intent(out), optional :: ray_cloud
        ...
        do ixrv = 0, nx + 1
          ...
          do iRay = 1, nRay(ixrv, jyrv, kzrv)
            ...
            if(apply) then
              ...
              !                * dz)) then
              print *, 'ERROR IN calc_ice: RAY VOLUME', iRay, 'in cell', ixrv, &
                  &jyrv, kzrv, 'TOO LOW'
              stop
            end if
            ...
            if(topography .and. compute_cloudcover) then
                      ...
                      do jj = 1, NSCY
                        ...
                        do kk = 1, NSCZ
                          ...
                          if(sizeY > 1) then
                            ...
                            fcpspy = dwnrl * dyi / dysc
                          else
                            fcpspy = 1.0
                          end if

                          dzi = (min((zr + dzr * 0.5), zsc + dzsc * jac(ix, &
                              &jy, kz) * 0.5) - max((zr - dzr * 0.5), zsc &
                              &- dzsc * jac(ix, jy, kz) * 0.5))

                          fcpspz = dwnrm * dzi / jac(ix, jy, kz) / dzsc
                          ...
                          if(gauss_smoothing) then
                            ...
                            if(abs(dxx) .le. dxr / 2. .and. abs(dyy) .le. dyr &
                                &/ 2. .and. abs(dzz) .le. dzr / 2.) then

                              amprw = sqrt(abs(omir * ray(iRay, ixrv, jyrv, &
                                  &kzrv)%dens) * 2. * wnrh ** 2 / (wnrh ** 2 &
                          ...
                          !superimpose fields
                          ray_cloud(ii2, jj2, kk2)%wwp = ray_cloud(ii2, jj2, &
                              &kk2)%wwp + wPrime
                          ray_cloud(ii2, jj2, kk2)%epp = ray_cloud(ii2, jj2, &
                              &kk2)%epp + expPrime
                          ray_cloud(ii2, jj2, kk2)%thp = ray_cloud(ii2, jj2, &
                              &kk2)%thp + thetaPrime
                        end do ! kk
                      end do ! ii
                    end do ! jj
                  ...
                  do jy = jymin, jymax
                    ...
                    do ix = ixmin, ixmax
                      ...
                      if(fcpsar .gt. fcpsar_max(ix, jy, kz) .and. wadr .gt. 0) &
                        ...
                        !TEST***
                        !compare wave action
                        !var%OPT(ix, jy, kz, 3) = 9999 !wPrime !dphi
                        ...
                        do ii = 1, NSCX
                          ...
                          do jj = 1, NSCY
                            ...
                            do kk = 1, NSCZ
                              ...
                              dzi = (min((zr + dzr * 0.5), zsc + dzsc * 0.5) &
                                  &- max((zr - dzr * 0.5), zsc - dzsc * 0.5))

                              fcpspz = dwnrm * dzi / dzsc
                              ...
                              if(gauss_smoothing) then
                                ...
                                ! dens = N exp{-x**2 / (2 \sigma)}
                                dens_gauss = nn * vv * ray(iRay, ixrv, jyrv, &
                                    &kzrv)%dens * exp(- (dxx ** 2 / sigdex &
                                    &+ dyy ** 2 / sigdey + dzz ** 2 / sigdez) &
                                    &/ 2.)
                              ...
                              !superimpose fields
                              ray_cloud(ii2, jj2, kk2)%wwp = ray_cloud(ii2, &
                                  &jj2, kk2)%wwp + wPrime
                              ray_cloud(ii2, jj2, kk2)%epp = ray_cloud(ii2, &
                                  &jj2, kk2)%epp + expPrime
                              ray_cloud(ii2, jj2, kk2)%thp = ray_cloud(ii2, &
                                  &jj2, kk2)%thp + thetaPrime
                            end do ! kk
                          end do ! ii
                        end do ! jj
                      end if ! compute_cloudcover
    ...
    !if (compute_cloudcover) then
    !do iray = 1, 1
    !  call smooth_shapiro_sgs(4.0, ray_cloud)
    ! print*, maxval(abs(ray_cloud(:, :, :)%wwp))
    !end do
    !endif
    if(reconstruct_gw_field == 1) then
      !***************************************
