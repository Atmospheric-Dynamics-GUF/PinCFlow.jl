module wkb_module
  ...
  ! initial ray field for reset
  type(rayType), dimension(:), allocatable :: ray0
  ...
  ! subgrid wave number field
  real, dimension(:,:,:), allocatable, private :: waveNumber
  integer, dimension(:,:), allocatable, private :: nRayPerSubCell
...
!          stop "Stopping."
  ...
  subroutine reset_ray(ray)
    !--------------------------------------------
    ! resets rays to original position with 
    ! wave number 
    ! - periodic bc in x and z
    ! - re-init of wave number at lower boundary
    !--------------------------------------------
    ...
    ! local variables
    integer :: i,k,isub,ksub,iRay
    logical :: right, top
    real    :: kk,mm,kTot,cg,cgx,cgz,omegaInt

    
    ! reset rays to initial state ray0
    ray(:) = ray0(:)

    ! obtain k and m from surrouinding subcell
    rayloop: do iRay = 1, nRay
       ...
       ! subgrid cell index for ray position
       if( right ) then
          isub = 2*i
       else
          isub = 2*i-1
       ...
       ! subgrid wave number
       kk = waveNumber(isub,ksub,1)
       mm = waveNumber(isub,ksub,2)
       ...
       ! intrinsic frequency
       omegaInt = omiSign * NN * kk / kTot   !xxx omega < 0 acc. Ulrich Achatz for jet 
       
       ! save values to ray
       ray(iRay)%m = mm
       ray(iRay)%cgx = cgx
       ray(iRay)%cgz = cgz
       ray(iRay)%omega = omegaInt
  ...
  subroutine calc_waveNumber(ray)
    !--------------------------------------------
    ! calculates the wave number in 
    ! the subgrid cells
    !--------------------------------------------
    ...
    ! init wave number field
    waveNumber(:,:,:) = 0.0


    ! Add up wave numbers over subgrid, running over all rays
    rayloop: do iRay = 1, nRay
       ...
       waveNumber(isub,ksub,1) = waveNumber(isub,ksub,1) + kk
       waveNumber(isub,ksub,2) = waveNumber(isub,ksub,2) + mm
       nRayPerSubCell(isub,ksub) = nRayPerSubCell(isub,ksub) + 1

    end do rayloop

    ! wave number mean over sub grid cells
    subgrid_loop: do ksub = 1, 2*nz
       do isub = 1, 2*nx
          
          n = nRayPerSubCell(isub,ksub)
  ...
  end subroutine calc_waveNumber
  ...
  subroutine setup_wkb(ray, waveAct, waveActOld, Psi)
    ...
    ! field of rays - initial field
    allocate( ray0(nRay), stat=allocstat)
    if(allocstat /= 0) stop "setup_wkb: could not allocate ray0"
    ...
    ! subgrid wave number field
    allocate( waveNumber(2*nx,2*nz,2), stat=allocstat)
    if(allocstat /= 0) stop "setup_wkb: could not allocate waveNumber"

    allocate( nRayPerSubCell(2*nx,2*nz), stat=allocstat)
    if(allocstat /= 0) stop "setup_wkb: could not allocate nRayPerSubCell"
    ...
    do iRay = 1,nRay      ! replace i by iRay!!!

       kHor = ray(iRay)%k
       kTot = sqrt(kHor**2 + ray(iRay)%m**2)
...
!          waveAct(i,j,k) = dabs(0.5*rhoStrat(k)*bAmp**2 / N2 / omegaInt)
  ...
  subroutine finish_wkb(ray, waveAct,waveActOld, Psi)
    ...
    deallocate( waveNumber, stat=allocstat)
    if(allocstat /= 0) stop "finish_wkb: could not deallocate waveNumber"
