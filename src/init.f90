...
contains
  ...
  subroutine setup (var,var0,var1,varG,flux,flux0, &
       ...
       & dTheta,dPStrat,drhoStrat,w_0,dIce,dTracer)
    ...
    if (include_tracer) then
       if (nVar /= 8) then
          stop "init.f90: nVar must be set to 8, or else this will cause issues"
       end if
    ...
    ! allocate dTracer **IK**
    if (include_tracer) then
       allocate(dTracer(-nbx:nx+nbx,-nby:ny+nby,-nbz:nz+nbz),stat=allocstat)
...
!!$    end if
    ...
    if(topography .and. spongeTFC) then
       allocate(alphaTFC(0:(nx + 1), 0:(ny + 1), 0:(nz + 1)), &
            stat=allocstat)
    ...
    if(topography) then
       if(model == "WKB" .or. .not. fluctuationMode &
            .or. poissonSolverType /= "bicgstab" .or. reconstType /= "MUSCL" &
            .or. musclType /= "muscl1" .or. fluxType /= "upwind" .or. heatingONK14 &
            .or. TurbScheme .or. rayTracer .or. pressureScaling .or. dens_relax &
            .or. include_ice) &
            then
          stop "Terrain-following coordinates not implemented for chosen &
               setup!"
       end if
    end if

    ! TFC FJ
    ! **IK** removed due to compilation difficulties.
    ! Safety switch for test cases.
!!$    if(topography) then
!!$        if(.not. any(testCase == (/"sinus", "uniform_theta", &
!!$        "monochromeWave", "wavePacket", "mountainwave", "Robert_Bubble", &
!!$        "coldBubble", "smoothVortex", "atmosphereatrest", &
!!$        "SkamarockKlemp94", "hotBubble", "hotBubble2D", &
!!$        "hotBubble3D", "baroclinic_LC", "baroclinic_ID"/))) then
!!$            stop "Terrain-following coordinates not implemented for chosen &
!!$            testCase"
!!$        end if
!!$    end if
    ...
    case( "Boussinesq" )
       ...
       if (include_tracer) then ! **IK**
          stop "init.f90: Boussinesq not possible. &
               & Currently only pseudo_incompressible available."
       end if
  ...
  subroutine initialise (var)
    ...
    real :: pEdgeR, pEdgeL, pFEdgeR, pFEdgeL, &
         pEdgeF, pEdgeB, pREdgeF, pREdgeB
    real :: piUEdgeR, piDEdgeR, piUEdgeL, piDEdgeL, &
         piFUEdgeR, piFDEdgeR, piFUEdgeL, piFDEdgeL, &
         piUEdgeF, piDEdgeF, piUEdgeB, piDEdgeB, &
    ...
    ! on default there is no initial tracer in the atmosphere (correct? **IK**)
    if (include_tracer) var(:,:,:,iVart) = 0.0 ! **IK**
    !---------------------------------------------------------------
    ...
    case( "monochromeWave" )
       ...
       do k = 1,nz
          ...
          do i = 1,nx
             ...
             if(topography) then
                var(i, j, k, 4) = var(i, j, k, 4) / jac(i, j, k) &
          ...
          do j = 1,ny
             do i = 1,nx
                ...
                if (fluctuationMode) then
                   if(topography) then
                      ! TFC FJ
                      var(i, j, k, 1) = var(i, j, k, 1) / rhoRef &
                           - rhoStratTFC(i, j, k)
                   else
                ...
                do i = 1, nz
                   do k = 0, nz + 1
                      ...
                      if (zloc < z_trpp) then
                         ...
                         the_env_pp(i, j, k) &
                              = thet0 * (1.e0 + ntrp ** 2.0 / g_ndim * zloc) &
                              + 0.5 * deltht * fstht
                      else
                         ! above tropopause
                         ...
                         the_env_pp(i, j, k) &
                              = thet0 * (1.0e0 + nstr ** 2.0 / g_ndim * zloc) &
                              + zeta_baro * (- thet0 * (nstr ** 2.0 - ntrp &
                              ** 2.0) / g_ndim * z_trpp + 0.5 * deltht * fstht &
                              * (1.0 + 5.0 * (z_trpp - zloc) / z_trpp))
                      end if

                      dens_env_pp(i, j, k) = pStratTFC(i, j, k) &
                           / the_env_pp(i, j, k)

                      var(i, j, k, 1) = dens_env_pp(i, j, k) &
                           - rhoStratTFC(i, j, k)
                   end do
                end do
             else
          ...
          do j = 1, ny
             do i = 1, nx
                ...
                if(topography) then
                   ! TFC FJ
                   var(i, j, 0, 5) &
                        = - 0.25 * 0.5 * (jac(i, j, 0) + jac(i, j, 1)) * dz * Ma2 &
                        * kappa * rho / (0.5 * (pStratTFC(i, j, 0) &
             ...
             do j = 1,ny
                do i = 1,nx
                   ...
                   if(topography) then
                      ! TFC FJ
                      var(i, j, k + 1, 5) &
                           = var(i, j, k, 5) + 0.5 * 0.5 * (jac(i, j, k) &
                           + jac(i, j, k + 1)) * dz * Ma2 * kappa * rho / (0.5 &
          ...
          do j=1,ny
             ...
             if(topography) then
                ...
                do i = 1, nx
                   ...
                   tempev &
                        = max(tp_strato, tp_srf_trp - tpdiffhor_tropo &
                        * s2_strtd(j))
                   var(i, j, 0, 5) = 1.0 + 0.5 * dz * 0.5 * (jac(i, j, 0) &
                        + jac(i, j, 1)) * kappa / tempev
                   var(i, j, 1, 5) = 1.0 - 0.5 * dz * 0.5 * (jac(i, j, 0) &
                        + jac(i, j, 1)) * kappa / tempev
                   ! Set potential temperature at the surface.
                   do k = 0, 1
                      tempev &
                           = max(tp_strato, var(i, j, k, 5) * (tp_srf_trp &
                           - tpdiffhor_tropo * s2_strtd(j) &
                           - ptdiffvert_tropo / kappa &
                   ...
                   if (fluctuationMode) then
                      if(topography) then
                         ...
                         rho_int_00 = rho_int_00 + 0.5 * (rhoStratTFC(i, j, k) &
                              + rhoStratTFC(i, j + 1, k))
                         rho_int_pm = rho_int_pm + 0.5 &
                              * (rhoStratTFC(i + 1, j, k) &
                              + rhoStratTFC(i + 1, j - 1, k))
                         rho_int_p0 = rho_int_p0 + 0.5 &
                              * (rhoStratTFC(i + 1, j, k) &
                              + rhoStratTFC(i + 1, j + 1, k))
                         rho = rho + rhoStratTFC(i, j, k)
                      else
                   ...
                   if (f_Coriolis_y(j) /= 0.0) then
                      if(topography) then
                         ...
                         pEdgeB = 0.5 * (pStratTFC(i, j, k) / jac(i, j, k) &
                              + pStratTFC(i, j - 1, k) / jac(i, j - 1, k))
                         pREdgeF = 0.5 * (pStratTFC(i + 1, j, k) &
                              / jac(i + 1, j, k) &
                              + pStratTFC(i + 1, j + 1, k) &
                              / jac(i + 1, j + 1, k))
                         pREdgeB = 0.5 * (pStratTFC(i + 1, j, k) &
                              / jac(i + 1, j, k) &
                              + pStratTFC(i + 1, j - 1, k) &
                              / jac(i + 1, j - 1, k))
                         ...
                         piDEdgeF = 0.5 * (jac(i, j, k - 1) &
                              ...
                              * met(i, j + 1, k - 1, 2, 3) &
                              * var(i, j + 1, k - 1, 5))
                         piUEdgeB = 0.5 * (jac(i, j, k + 1) &
                              * met(i, j, k + 1, 2, 3) &
                              * var(i, j, k + 1, 5) &
                              + jac(i, j - 1, k + 1) &
                              * met(i, j - 1, k + 1, 2, 3) &
                              * var(i, j - 1, k + 1, 5))
                         piDEdgeB = 0.5 * (jac(i, j, k - 1) &
                              * met(i, j, k - 1, 2, 3) &
                              * var(i, j, k - 1, 5) &
                              + jac(i, j - 1, k - 1) &
                              * met(i, j - 1, k - 1, 2, 3) &
                              * var(i, j - 1, k - 1, 5))
                         piRUEdgeF = 0.5 * (jac(i + 1, j, k + 1) &
                              * met(i + 1, j, k + 1, 2, 3) &
                              * var(i + 1, j, k + 1, 5) &
                              + jac(i + 1, j + 1, k + 1) &
                              * met(i + 1, j + 1, k + 1, 2, 3) &
                              * var(i + 1, j + 1, k + 1, 5))
                         piRDEdgeF = 0.5 * (jac(i + 1, j, k - 1) &
                              * met(i + 1, j, k - 1, 2, 3) &
                              * var(i + 1, j, k - 1, 5) &
                              + jac(i + 1, j + 1, k - 1) &
                              * met(i + 1, j + 1, k - 1, 2, 3) &
                              * var(i + 1, j + 1, k - 1, 5))
                         piRUEdgeB = 0.5 * (jac(i + 1, j, k + 1) &
                              * met(i + 1, j, k + 1, 2, 3) &
                              * var(i + 1, j, k + 1, 5) &
                              + jac(i + 1, j - 1, k + 1) &
                              * met(i + 1, j - 1, k + 1, 2, 3) &
                              * var(i + 1, j - 1, k + 1, 5))
                         piRDEdgeB = 0.5 * (jac(i + 1, j, k - 1) &
                              * met(i + 1, j, k - 1, 2, 3) &
                              * var(i + 1, j, k - 1, 5) &
                              + jac(i + 1, j - 1, k - 1) &
                              * met(i + 1, j - 1, k - 1, 2, 3) &
                              * var(i + 1, j - 1, k - 1, 5))
                         piGrad = 0.25 * (pEdgeF / rho_int_00 &
                              * ((jac(i, j + 1, k) * var(i, j + 1, k, 5) &
                              - jac(i, j, k) * var(i, j, k, 5)) / dy &
                              + (piUEdgeF - piDEdgeF) * 0.5 / dz) + pEdgeB &
                              / rho_int_0m * ((jac(i, j, k) &
                              * var(i, j, k, 5) - jac(i, j - 1, k) &
                              * var(i, j - 1, k, 5)) / dy + (piUEdgeB &
                              - piDEdgeB) * 0.5 / dz) + pREdgeF &
                              / rho_int_p0 * ((jac(i + 1, j + 1, k) &
                              * var(i + 1, j + 1, k, 5) - jac(i + 1, j, k) &
                              * var(i + 1, j, k, 5)) / dy + (piRUEdgeF &
                              - piRDEdgeF) * 0.5 / dz) + pREdgeB &
                              / rho_int_pm * ((jac(i + 1, j, k) &
                              * var(i + 1, j, k, 5) - jac(i + 1, j - 1, k) &
                              * var(i + 1, j - 1, k, 5)) / dy + (piRUEdgeB &
                              - piRDEdgeB) * 0.5 / dz))
                         ! Compute zonal wind.
                         var(i, j, k, 2) &
                              = - uRef / f_Coriolis_y(j) / lRef / (Ma2 * kappa) &
                              * piGrad
                      else
                         var(i,j,k,2) &
       ...
       if(topography) then
          ...
          do i = 1, nx
             do j = 1, ny
                ...
                do k = 1, nz
                   z_tr_diff_tfc(i, j, k) &
                        = z_tr_dim - heightTFC(i, j, k) * lRef
                end do
                k_2_tfc(i, j) = int(minloc(z_tr_diff_tfc(i, j, :), dim = 1))
                z_2_tfc(i, j) = heightTFC(i, j, k_2_tfc(i, j)) * lRef
                theta_bar_0_tfc(i, j) &
                     = thetaStratTFC(i, j, k_2_tfc(i, j)) * thetaRef

                ! Set alpha_t_tfc.
                if(sizeX > 1 .and. sizeY == 1) then
                   alpha_t_tfc(i, j) &
                        = u_strength * f_Coriolis_dim &
                        * theta_bar_0_tfc(i, j) * Lx_end * bar_sigma_x &
                        / (dTh_atm * g * z_2_tfc(i, j) * pi)
                else
                   alpha_t_tfc(i, j) &
                        = u_strength * f_Coriolis_dim &
                        * theta_bar_0_tfc(i, j) * Ly_end * bar_sigma_y &
                        / (dTh_atm * g * z_2_tfc(i, j) * pi)
                end if
             end do
          end do
                   ...
                   do j = 1, ny
                      do k = 1, nz
                         ...
                         if(z_dim .lt. z_2_tfc(i, j)) then
                            F_0_tfc(i, j, k) &
                                 = alpha_t_tfc(i, j) * (2.0 * z_dim &
                                 * sin(pi * z_dim / (2.0 * z_2_tfc(i, j))) &
                                 - z_dim ** 2.0 / z_2_tfc(i, j))
                         else
                            F_0_tfc(i, j, k) &
                                 = alpha_t_tfc(i, j) * (2.0 * (z_dim - L_z) &
                                 * sin(pi * (z_dim - L_z) / (2.0 &
                                 * (z_2_tfc(i, j) - L_z))) - (z_dim - L_z) &
                                 ** 2.0 / (z_2_tfc(i, j) - L_z)) &
                                 * z_2_tfc(i, j) / (z_2_tfc(i, j) - L_z)
                         end if
                         F_a = F_0_tfc(i, j, k)
                         ...
                         if (sizeX > 1 .and. sizeY == 1) then
                            ...
                            if (x_dim < Lx_end * (0.25 - 0.5 &
                                 * bar_sigma_x)) then
                               term_a = 1.0
                            else if(x_dim >= Lx_end * (0.25 - 0.5 &
                                 * bar_sigma_x) .and. x_dim < Lx_end * (0.25 &
                                 + 0.5 * bar_sigma_x)) then
                               term_a = 0.5 * (1.0 - sin(pi * ((x_dim &
                                    - 0.25 * Lx_end) / (Lx_end &
                                    * bar_sigma_x))))
                            else if(x_dim >= Lx_end * (0.25 + 0.5 &
                                 * bar_sigma_x) .and. x_dim < Lx_end * (0.75 &
                                 - 0.5 * bar_sigma_x)) then
                               term_a = 0.0
                            else if(x_dim >= Lx_end * (0.75 - 0.5 &
                                 * bar_sigma_x) .and. x_dim < Lx_end * (0.75 &
                                 + 0.5 * bar_sigma_x)) then
                               term_a = 0.5 * (1.0 + sin(pi * ((x_dim &
                                    - 0.75 * Lx_end) / (Lx_end &
                            ...
                            if (balance_eq == "QG") then
                               stop "ERROR: balance_eq == QG not &
                                    provided"
                            else
                               streamfunc = g * F_a * term_b &
                                    / f_Coriolis_dim
                               pi_pr_xz_tfc(i, j, k) &
                                    = f_Coriolis_dim * streamfunc / (cp &
                                    * thetaRef * thetaStratTFC(i, j, k))
                            ...
                            if (y_dim < Ly_end * (0.25 - 0.5 &
                                 * bar_sigma_y)) then
                               term_a = 1.0
                            else if(y_dim >= Ly_end * (0.25 - 0.5 &
                                 * bar_sigma_y) .and. y_dim < Ly_end * (0.25 &
                                 + 0.5 * bar_sigma_y)) then
                               term_a = 0.5 * (1.0 - sin(pi * ((y_dim &
                                    - 0.25 * Ly_end) / (Ly_end &
                                    * bar_sigma_y))))
                            else if(y_dim >= Ly_end * (0.25 + 0.5 &
                                 * bar_sigma_y) .and. y_dim < Ly_end * (0.75 &
                                 - 0.5 * bar_sigma_y)) then
                               term_a = 0.0
                            else if(y_dim >= Ly_end * (0.75 - 0.5 &
                                 * bar_sigma_y) .and. y_dim < Ly_end * (0.75 &
                                 + 0.5 * bar_sigma_y)) then
                               term_a = 0.5 * (1.0 + sin(pi * ((y_dim &
                                    - 0.75 * Ly_end) / (Ly_end &
                      ...
                      do i = 1,nx
                         ...
                         if (fluctuationMode) then
                            if(topography) then
                               ! TFC FJ
                               rho_int_m0 &
                                    = rho_int_m0 + 0.5 * (rhoStratTFC(i, j, k) &
                                    + rhoStratTFC(i - 1, j, k)) * rhoRef
                               rho_int_00 &
                                    = rho_int_00 + 0.5 * (rhoStratTFC(i, j, k) &
                                    + rhoStratTFC(i + 1, j, k)) * rhoRef
                               rho_int_mp &
                                    = rho_int_mp + 0.5 &
                                    * (rhoStratTFC(i, j + 1, k) &
                         ...
                         if(topography) then
                            ...
                            pEdgeL = 0.5 * (pStratTFC(i, j, k) / jac(i, j, k) &
                                 + pStratTFC(i - 1, j, k) &
                                 / jac(i - 1, j, k))
                            pFEdgeR = 0.5 * (pStratTFC(i, j + 1, k) &
                                 / jac(i, j + 1, k) &
                                 + pStratTFC(i + 1, j + 1, k) &
                                 / jac(i + 1, j + 1, k))
                            pFEdgeL = 0.5 * (pStratTFC(i, j + 1, k) &
                                 / jac(i, j + 1, k) &
                                 + pStratTFC(i - 1, j + 1, k) &
                                 / jac(i - 1, j + 1, k))
                            ...
                            piDEdgeR = 0.5 * (jac(i, j, k - 1) &
                                 ...
                                 * met(i + 1, j, k - 1, 1, 3) &
                                 * var(i + 1, j, k - 1, 5))
                            piUEdgeL = 0.5 * (jac(i, j, k + 1) &
                                 * met(i, j, k + 1, 1, 3) &
                                 * var(i, j, k + 1, 5) &
                                 + jac(i - 1, j, k + 1) &
                                 * met(i - 1, j, k + 1, 1, 3) &
                                 * var(i - 1, j, k + 1, 5))
                            piDEdgeL = 0.5 * (jac(i, j, k - 1) &
                                 * met(i, j, k - 1, 1, 3) &
                                 * var(i, j, k - 1, 5) &
                                 + jac(i - 1, j, k - 1) &
                                 * met(i - 1, j, k - 1, 1, 3) &
                                 * var(i - 1, j, k - 1, 5))
                            piFUEdgeR = 0.5 * (jac(i, j + 1, k + 1) &
                                 * met(i, j + 1, k + 1, 1, 3) &
                                 * var(i, j + 1, k + 1, 5) &
                                 + jac(i + 1, j + 1, k + 1) &
                                 * met(i + 1, j + 1, k + 1, 1, 3) &
                                 * var(i + 1, j + 1, k + 1, 5))
                            piFDEdgeR = 0.5 * (jac(i, j + 1, k - 1) &
                                 * met(i, j + 1, k - 1, 1, 3) &
                                 * var(i, j, k - 1, 5) &
                                 + jac(i + 1, j + 1, k - 1) &
                                 * met(i + 1, j + 1, k - 1, 1, 3) &
                                 * var(i + 1, j + 1, k - 1, 5))
                            piFUEdgeL = 0.5 * (jac(i, j + 1, k + 1) &
                                 * met(i, j + 1, k + 1, 1, 3) &
                                 * var(i, j + 1, k + 1, 5) &
                                 + jac(i - 1, j + 1, k + 1) &
                                 * met(i - 1, j + 1, k + 1, 1, 3) &
                                 * var(i - 1, j + 1, k + 1, 5))
                            piFDEdgeL = 0.5 * (jac(i, j + 1, k - 1) &
                                 * met(i, j + 1, k - 1, 1, 3) &
                                 * var(i, j + 1, k - 1, 5) &
                                 + jac(i - 1, j + 1, k - 1) &
                                 * met(i - 1, j + 1, k - 1, 1, 3) &
                                 * var(i - 1, j + 1, k - 1, 5))
                            piGrad = 0.25 * (pEdgeR / rho_int_00 &
                                 * ((jac(i + 1, j, k) &
                                 * var(i + 1, j, k, 5) - jac(i, j, k) &
                                 * var(i, j, k, 5)) / dx + (piUEdgeR &
                                 - piDEdgeR) * 0.5 / dz) + pEdgeL &
                                 / rho_int_m0 * ((jac(i, j, k) &
                                 * var(i, j, k, 5) - jac(i - 1, j, k) &
                                 * var(i - 1, j, k, 5)) / dx + (piUEdgeL &
                                 - piDEdgeL) * 0.5 / dz) + pFEdgeR &
                                 / rho_int_0p * ((jac(i + 1, j + 1, k) &
                                 * var(i + 1, j + 1, k, 5) &
                                 - jac(i, j + 1, k) &
                                 * var(i, j + 1, k, 5)) / dx + (piFUEdgeR &
                                 - piFDEdgeR) * 0.5 / dz) + pFEdgeL &
                                 / rho_int_mp * ((jac(i, j + 1, k) &
                                 * var(i, j + 1, k, 5) &
                                 - jac(i - 1, j + 1, k) &
                                 * var(i - 1, j + 1, k, 5)) / dx &
                                 + (piFUEdgeL - piFDEdgeL) * 0.5 / dz))
                            ! Compute meridional wind.
                            var(i, j, k, 3) &
                                 = 1.0 / f_Coriolis_dim * cp * thetaRef * rhoRef &
                                 / (uRef * lRef) * piGrad
                         else
                            var(i,j,k,3) &
                                 =  1.0/f_Coriolis_dim &
                         ...
                         if(topography) then
                            ! TFC FJ
                            the_env_pp(i, j, k) = pStratTFC(i, j, k) / rho &
                                 * rhoRef
                         else
                      ...
                      do i = 1,nx
                         ...
                         if (fluctuationMode) then
                            if(topography) then
                               ! TFC FJ
                               rho_int_0m &
                                    = rho_int_0m + 0.5 * (rhoStratTFC(i, j, k) &
       ...
       if(topography) then
          ...
          do k = 1, nz
             do j = 1, ny
                do i = 1, nx
                   balance(i, j, k) &
                        = uRef * (var(i, j, k + 1, 2) - var(i, j, k, 2)) &
                        / (jac(i, j, k) * dz * lRef) + g / (jac(i, j, k) &
                        * f_Coriolis_dim * lRef &
                        * thetaStratTFC(i, j, k)) * ((jac(i, j + 1, k) &
                        * var(i, j + 1, k, 6) - jac(i, j, k) &
                        * var(i, j, k, 6)) / dy + (jac(i, j, k + 1) &
                        * met(i, j, k + 1, 2, 3) * var(i, j, k + 1, 6) &
                   ...
                   balance1(i, j, k) &
                        ...
                        * met(i, j, k, 2, 3) * var(i, j, k, 5)) / dz) &
                        / (jac(i, j, k) * kappa * lRef)

                   balance2(i, j, k) &
                        = thetaRef * Rsp * var(i, j, k, 6) &
                        * (var(i, j, k + 1, 5) - var(i, j, k, 5)) &
                        / (jac(i, j, k) * kappa * dz * lRef) - g &
                        * (var(i, j, k, 6) - thetaStratTFC(i, j, k)) &
                        / thetaStratTFC(i, j, k)

                   balance3(i, j, k) &
                        = g * (var(i, j, k, 6) - thetaStratTFC(i, j, k)) &
                        / thetaStratTFC(i, j, k)
    ...
    case("zparabel")
       ...
       !------------------------------------------------------
    case ('isothermal_and_steady_wind') ! **IK** test case
    ...
    subroutine init_GWP(Psi,kk,mm,ll)
      ...
      do k = 1,nz
         ...
         do i = 1,nx
            ...
            if(topography) then
               ! TFC FJ
               theta11_r = Fr2 * thetaStratTFC(i + 1, j, k) &
                    * Psi(i + 1, j, k, 3, 1)
               theta11_c = Fr2 * thetaStratTFC(i, j, k) &
                    * Psi(i, j, k, 3, 1)
               theta11_l = Fr2 * thetaStratTFC(i - 1, j, k) &
                    * Psi(i - 1, j, k, 3, 1)
               theta11_t = Fr2 * thetaStratTFC(i, j, k + 1) &
                    * Psi(i, j, k + 1, 3, 1)
               theta11_b = Fr2 * thetaStratTFC(i, j, k - 1) &
                    * Psi(i, j, k - 1, 3, 1)
            else
            ...
            if(topography) then
               ...
               du10_dx = 0.5 * (jac(i + 1, j, k) * u10_r - jac(i - 1, j, k) &
                    * u10_l) / dx / jac(i, j, k) + 0.5 &
                    * (jac(i, j, k + 1) * met(i, j, k + 1, 1, 3) * u10_t &
                    - jac(i, j, k - 1) * met(i, j, k - 1, 1, 3) * u10_b) &
                    / dz / jac(i, j, k)
               du10_dz = 0.5 * (u10_t - u10_b) / dz / jac(i, j, k)
               dw10_dx = 0.5 * (jac(i + 1, j, k) * w10_r - jac(i - 1, j, k) &
                    * w10_l) / dx / jac(i, j, k) + 0.5 &
                    * (jac(i, j, k + 1) * met(i, j, k + 1, 1, 3) * w10_t &
                    - jac(i, j, k - 1) * met(i, j, k - 1, 1, 3) * w10_b) &
                    / dz / jac(i, j, k)
               dw10_dz = 0.5 * (w10_t - w10_b) / dz / jac(i, j, k)
               dpi0_dz = 0.5 * (pi0_t - pi0_b) / dz / jac(i, j, k)
               dtheta11_dx = 0.5 * (jac(i + 1, j, k) * theta11_r &
                    - jac(i - 1, j, k) * theta11_l) / dx &
                    / jac(i, j, k) + 0.5 * (jac(i, j, k + 1) &
                    * met(i, j, k + 1, 1, 3) * theta11_t &
                    - jac(i, j, k - 1) * met(i, j, k - 1, 1, 3) &
                    * theta11_b) / dz / jac(i, j, k)
