module flux_module
  ...
  real, dimension(:,:,:),     allocatable :: rhoBar, rhopBar, &
          & rhoOld, rhopOld
    ...
    case( 'MUSCL' )
       ...
       case( "muscl1" )
                     ...
                     do jy = - nby, ny + nby
                         do kz = 0, nz + 1
                             if(pStratTFC(ix, jy, kz) == 0.0) then
                                 print*, "ERROR in rec. rho: pStratTFC = 0 &
                                 at k = ", kz
                                 stop
                             end if
                             rhoBar(ix, jy, kz) &
                             = var(ix, jy, kz, 1) / pStratTFC(ix, jy, kz)
                         end do
                     end do
                 end do
             else
                 ...
                 do ix = - nbx, nx + nbx
                     do jy = - nby, ny + nby
                         do kz = 0, nz + 1
                             ...
                             rhopBar(ix, jy, kz) &
                             = var(ix, jy, kz, 6) / pStratTFC(ix, jy, kz)
                         end do
                     end do
                 end do
             else
             ...
             if (fluctuationMode ) then
                 if(topography) then
                     ...
                     do ix = - nbx, nx + nbx - 1
                         do jy = - nby, ny + nby
                             do kz = 0, nz + 1
                                 rhoEdgeR = 0.5 * (var(ix, jy, kz, 1) &
                                            + var(ix + 1, jy, kz, 1) &
                                            + rhoStratTFC(ix, jy, kz) &
                                            + rhoStratTFC(ix + 1, jy, kz))
                                 pEdgeR = 0.5 * (pStratTFC(ix, jy, kz) &
                                          + pStratTFC(ix + 1, jy, kz))
                                 uBar(ix, jy, kz) = var(ix, jy, kz, 2) &
                                                    * rhoEdgeR / pEdgeR
                             end do
                         end do
                     end do
                 ...
                 do kz = 0, nz+1
                     do ix = -nbx,nx+nbx-1
                         ...
                         ! GBcorr: to be consistent with current rouine: momentumFlux
                           ! * 0.5*(var(ix,:,kz,1) + var(ix+1,:,kz,1))
                           * 0.5*(var(ix,:,kz,1) + var(ix+1,:,kz,1))/Pstrat(kz)
                     end do
                 end do
             end if
             ...
             if (fluctuationMode ) then
                 if(topography) then
                     ...
                     do ix = - nbx, nx + nbx
                         do jy = - nby, ny + nby - 1
                             do kz = 0, nz + 1
                                 rhoEdgeF = 0.5 * (var(ix, jy, kz, 1) &
                                            + var(ix, jy + 1, kz, 1) &
                                            + rhoStratTFC(ix, jy, kz) &
                                            + rhoStratTFC(ix, jy + 1, kz))
                                 pEdgeF = 0.5 * (pStratTFC(ix, jy, kz) &
                                          + pStratTFC(ix, jy + 1, kz))
                                 vBar(ix, jy, kz) = var(ix, jy, kz, 3) &
                                                    * rhoEdgeF / pEdgeF
                             end do
                         end do
                     end do
             ...
             if (fluctuationMode ) then
                 if(topography) then
                     ...
                     do ix = - nbx, nx + nbx
                         do jy = - nby, ny + nby
                             do kz = 0, nz + 1
                                 rhoEdgeU = 0.5 * (var(ix, jy, kz, 1) &
                                            + var(ix, jy, kz + 1, 1) &
                                            + rhoStratTFC(ix, jy, kz) &
                                            + rhoStratTFC(ix, jy, kz + 1))
                                 pEdgeU = 0.5 * (pStratTFC(ix, jy, kz) &
                                          + pStratTFC(ix, jy, kz + 1))
                                 wBar(ix, jy, kz) = wBar(ix, jy, kz) &
                                                    * rhoEdgeU / pEdgeU
                             end do
                         end do
                     end do
                     ...
                     do kz = 0, nz+1
                         ...
                         ! GBcorr: to be consistent with current rouine: momentumFlux
                           ...
                           ! + rhoStratTilde(kz))
                           * (  0.5*(var(:,:,kz,1) + var(:,:,kz+1,1)) &
                           + rhoStratTilde(kz))/PstratTilde(kz)
                           ! FS:
                 ...
                 do kz = 0, nz+1
                     ...
                     ! GBcorr: to be consistent with current rouine: momentumFlux
                       ! * 0.5*(var(:,:,kz,1) + var(:,:,kz+1,1))
                       * 0.5*(var(:,:,kz,1) + var(:,:,kz+1,1))/PstratTilde(kz)
                       ! FS:
          ...
          case ( "tracer" )
             ...
             tracerBar = 0.0
             if(topography) then
                 stop "fluxes.f90: topography and tracer not implemented yet"
             else
                 tracerBar(:,:,:) = var(:,:,:,iVart)
             end if
       ...
       do j = 1,ny
          do i = 0,nx
             ...
             case( "ILES" )
                ...
                if( fluctuationMode ) then
                   fRho = flux_aldm(rhoL + rhoStrat(k),&
                        &           rhoR + rhoStrat(k),uSurf,&
       ...
       do j = 0,ny
          do i = 1,nx
             ...
             case( "ILES" )
                ...
                if( fluctuationMode ) then
                   gRho = flux_aldm(rhoB+rhoStrat(k), &
                        &           rhoF+rhoStrat(k),vSurf,&
       ...
       do j = 1,ny
          do i = 1,nx
             ...
             case( "ILES" )
                ...
                if( fluctuationMode ) then
                   hRho = flux_aldm(rhoD+rhoStratTilde(k), &
                        &           rhoU+rhoStratTilde(k),wSurf,&
       ...
       do j = 0,ny
          do i = 1,nx
             ...
             if(TurbScheme)then
                coef_t  &
                = coef_t &
                  + 0.5*( var(i,j,k,7) + var(i,j+1,k,7) ) & !UA
                    * delta_hs/Pr_turb
             end if
       ...
       do j = 1,ny
          do i = 1,nx
             ...
             if(TurbScheme)then
                coef_t  &
                = coef_t &
                  + 0.5*( var(i,j,k,7) + var(i,j,k+1,7) ) & !UA
                    * delta_vs/Pr_turb
             end if
...
!   variables for the turbulence scheme
  ...
  !------------------------------------------------------------------------
  subroutine tracerFlux (vara,var,flux,fluxmode)
    real, dimension(-nbx:nx+nbx,-nby:ny+nby,-nbz:nz+nbz,nVar), &
         & intent(in) :: vara , var
    character(len=*), intent(in) :: fluxmode
    ...
    real  :: fRhoTracer, gRhoTracer, hRhoTracer
       ...
       do j = 1,ny
          do i = 0,nx
             ...
             case( "central" )
                ...
                if (fluctuationMode) then                  
                   tracerL = (var(i,j,k,1) + rhoStrat(k))*var(i,j,k,iVart)
                   tracerR = (var(i+1,j,k,1) + rhoStrat(k))*var(i+1,j,k,iVart)
                else
                   tracerL = var(i,j,k,1)*var(i,j,k,iVart)
                   tracerR = var(i+1,j,k,1)*var(i+1,j,k,iVart)
                end if
                ...
                fRhoTracer = uSurf * 0.5 * (tracerL + tracerR)

             case default
                stop "fluxes.f90: only fluxType central implemented for tracer."
             end select

             flux(i,j,k,1,iVart) = fRhoTracer
       ...
       do j = 0,ny
          do i = 1,nx
             ...
             case( "central" )
                ...
                if (fluctuationMode) then
                   tracerF = (var(i,j+1,k,1)+rhoStrat(k))*var(i,j+1,k,iVart)
                   tracerB = (var(i,j,k,1)+rhoStrat(k))*var(i,j,k,iVart)
                else
                   tracerF = var(i,j+1,k,1)*var(i,j+1,k,iVart)
                   tracerB = var(i,j,k,1)*var(i,j,k,iVart)
                end if
                ...
                gRhoTracer = vSurf * 0.5 * (tracerL + tracerR)

             case default
                stop "fluxes.f90: only fluxType central implemented for tracer."
             end select
             flux(i,j,k,2,iVart) = gRhoTracer
          end do
       end do
    end do
       ...
       do j = 1,ny
          do i = 1,nx
             ...
             case( "central" )
                ...
                if (fluctuationMode) then
                   tracerU = (var(i,j,k+1,1)+rhoStrat(k))*var(i,j,k+1,iVart)
                   tracerD = (var(i,j,k,1)+rhoStrat(k))*var(i,j,k,iVart)
                else
                   tracerU = var(i,j,k+1,1)*var(i,j,k+1,iVart)
                   tracerD = var(i,j,k,1)*var(i,j,k,iVart)
                end if
                ...
                hRhoTracer = wSurf * 0.5 * (tracerD + tracerU)
...
!-----------------------------------------------------------------------!
  ...
  subroutine momentumFlux (vara,var,flux,fluxmode,PStrata, PStratTildea)
    ...
    ! TFC FJ
    real :: pEdgeR, pREdgeR, pEdgeF, pREdgeF, pEdgeU, pREdgeU, &
            pFEdgeR, pFEdgeF, pFedgeU, pUEdgeR, pUEdgeF, pUEdgeU
             ...
             case( "upwind", "ILES" )
                ...
                case( "upwind" )
                   if(topography) then
                       ...
                       pEdgeR = 0.5 * (jac(i, j, k) * pStratTFC(i, j, k) &
                                + jac(i + 1, j, k) * pStratTFC(i + 1, j, k))
                       pREdgeR = 0.5 * (jac(i + 1, j, k) &
                                 * pStratTFC(i + 1, j, k) + jac(i + 2, j, k) &
                                 * pStratTFC(i + 2, j, k))
                       if(fluxmode == "nln") then
                           uSurf = 0.5 * (pEdgeR * var(i, j, k, 2) &
                                   + pREdgeR * var(i + 1, j, k, 2))
                       else if(fluxmode == "lin") then
                           uSurf = 0.5 * (pEdgeR * vara(i, j, k, 2) &
                                   + pREdgeR * vara(i + 1, j, k, 2))
                       else
                           stop "ERROR: wrong fluxmode"
                       end if
                   else
             ...
             case( "upwind", "ILES" )
                ...
                case( "upwind" )
                   if(topography) then
                       ...
                       pEdgeF = 0.5 * (jac(i, j, k) * pStratTFC(i, j, k) &
                                + jac(i, j + 1, k) * pStratTFC(i, j + 1, k))
                       pREdgeF = 0.5 * (jac(i + 1, j, k) &
                                 * pStratTFC(i + 1, j, k) &
                                 + jac(i + 1, j + 1, k) &
                                 * pStratTFC(i + 1, j + 1, k))
                       if(fluxmode == "nln") then
                           vSurf = 0.5 * (pEdgeF * var(i, j, k, 3) &
                                   + pREdgeF * var(i + 1, j, k, 3))
                       else if(fluxmode == "lin") then
                           vSurf = 0.5 * (pEdgeF * vara(i, j, k, 3) &
                                   + pREdgeF * vara(i + 1, j, k, 3))
                       else
                           stop "ERROR: wrong fluxmode"
                       end if
                   else
             ...
             case( "upwind", "ILES" )
                ...
                case( "upwind" )
                   if(topography) then
                       ...
                       pEdgeU = 0.5 * (jac(i, j, k) * pStratTFC(i, j, k) &
                                + jac(i, j, k + 1) * pStratTFC(i, j, k + 1))
                       pREdgeU = 0.5 * (jac(i + 1, j, k) &
                                 * pStratTFC(i + 1, j, k) &
                                 + jac(i + 1, j, k + 1) &
                                 * pStratTFC(i + 1, j, k + 1))
                       if(fluxmode == "nln") then
                           wSurf = 0.5 * (pEdgeU * var(i, j, k, 4) &
                                   + pREdgeU * var(i + 1, j, k, 4))
                       else if(fluxmode == "lin") then
                           wSurf = 0.5 * (pEdgeU * vara(i, j, k, 4) &
                                   + pREdgeU * vara(i + 1, j, k, 4))
                       else
                           stop "ERROR: wrong fluxmode"
                       end if
                   else
             ...
             case( "upwind", "ILES" )
                ...
                case( "upwind" )
                   if(topography) then
                       ...
                       pEdgeR = 0.5 * (jac(i, j, k) * pStratTFC(i, j, k) &
                                + jac(i + 1, j, k) * pStratTFC(i + 1, j, k))
                       pFEdgeR = 0.5 * (jac(i, j + 1, k) &
                                 * pStratTFC(i, j + 1, k) &
                                 + jac(i + 1, j + 1, k) &
                                 * pStratTFC(i + 1, j + 1, k))
                       if(fluxmode == "nln") then
                           uSurf = 0.5 * (pEdgeR * var(i, j, k, 2) &
                                   + pFEdgeR * var(i, j + 1, k, 2))
                       else if(fluxmode == "lin") then
                           uSurf = 0.5 * (pEdgeR * vara(i, j, k, 2) &
                                   + pFEdgeR * vara(i, j + 1, k, 2))
                       else
                           stop "ERROR: wrong fluxmode"
                       end if
                   else
             ...
             case( "upwind", "ILES" )
                ...
                case( "upwind" )
                   if(topography) then
                       ...
                       pEdgeF = 0.5 * (jac(i, j, k) * pStratTFC(i, j, k) &
                                + jac(i, j + 1, k) * pStratTFC(i, j + 1, k))
                       pFEdgeF = 0.5 * (jac(i, j + 1, k) &
                                 * pStratTFC(i, j + 1, k) + jac(i, j + 2, k) &
                                 * pStratTFC(i, j + 2, k))
                       if(fluxmode == "nln") then
                           vSurf = 0.5 * (pEdgeF * var(i, j, k, 3) &
                                   + pFEdgeF * var(i, j + 1, k, 3))
                       else if(fluxmode == "lin") then
                           vSurf = 0.5 * (pEdgeF * vara(i, j, k, 3) &
                                   + pFEdgeF * vara(i, j + 1, k, 3))
                       else
                           stop "ERROR: wrong fluxmode"
                       end if
                   else
             ...
             case( "upwind", "ILES" )
                ...
                case( "upwind" )
                   if(topography) then
                       ...
                       pEdgeU = 0.5 * (jac(i, j, k) * pStratTFC(i, j, k) &
                                + jac(i, j, k + 1) * pStratTFC(i, j, k + 1))
                       pFEdgeU = 0.5 * (jac(i, j + 1, k) &
                                 * pStratTFC(i, j + 1, k) &
                                 + jac(i, j + 1, k + 1) &
                                 * pStratTFC(i, j + 1, k + 1))
                       if(fluxmode == "nln") then
                           wSurf = 0.5 * (pEdgeU * var(i, j, k, 4) &
                                   + pFEdgeU * var(i, j + 1, k, 4))
                       else if(fluxmode == "lin") then
                           wSurf = 0.5 * (pEdgeU * vara(i, j, k, 4) &
                                   + pFEdgeU * vara(i, j + 1, k, 4))
                       else
                           stop "ERROR: wrong fluxmode"
                       end if
                   else
             ...
             case( "upwind", "ILES" )
                ...
                case( "upwind" )
                   if(topography) then
                       ...
                       pEdgeR = 0.5 * (jac(i, j, k) * pStratTFC(i, j, k) &
                                + jac(i + 1, j, k) * pStratTFC(i + 1, j, k))
                       pUEdgeR = 0.5 * (jac(i, j, k + 1) &
                                 * pStratTFC(i, j, k + 1) &
                                 + jac(i + 1, j, k + 1) &
                                 * pStratTFC(i + 1, j, k + 1))
                       if(fluxmode == "nln") then
                           uSurf = 0.5 * (pEdgeR * var(i, j, k, 2) &
                                   + pUEdgeR * var(i, j, k + 1, 2))
                       else if(fluxmode == "lin") then
                           uSurf = 0.5 * (pEdgeR * vara(i, j, k, 2) &
                                   + pUEdgeR * vara(i, j, k + 1, 2))
                       else
                           stop "ERROR: wrong fluxmode"
                       end if
                   else
                       ...
                       if (fluxmode == "nln") then
                          usurf &
                          = 0.5 &
                            *(  var(i,j,k  ,2)*Pstrat(k) &
                              + var(i,j,k+1,2)*Pstrat(k+1))
                       else if (fluxmode == "lin") then
                          usurf &
                          = 0.5 &
             ...
             case( "upwind", "ILES" )
                ...
                case( "upwind" )
                   if(topography) then
                       ...
                       pEdgeF = 0.5 * (jac(i, j, k) * pStratTFC(i, j, k) &
                                + jac(i, j + 1, k) * pStratTFC(i, j + 1, k))
                       pUEdgeF = 0.5 * (jac(i, j, k + 1) &
                                 * pStratTFC(i, j, k + 1) &
                                 + jac(i, j + 1, k + 1) &
                                 * pStratTFC(i, j + 1, k + 1))
                       if(fluxmode == "nln") then
                           vSurf = 0.5 * (pEdgeF * var(i, j, k, 3) &
                                   + pUEdgeF * var(i, j, k + 1, 3))
                       else if(fluxmode == "lin") then
                           vSurf = 0.5 * (pEdgeF * vara(i, j, k, 3) &
                                   + pUEdgeF * vara(i, j, k + 1, 3))
                       else
                           stop "ERROR: wrong fluxmode"
                       end if
                   else
                       ...
                       if (fluxmode == "nln") then
                          vsurf &
                          = 0.5 &
                            *(  var(i,j,k  ,3)*Pstrat(k) &
                              + var(i,j,k+1,3)*Pstrat(k+1))
                       else if (fluxmode == "lin") then
                          vsurf &
                          = 0.5 &
             ...
             case( "upwind", "ILES" )
                ...
                case( "upwind" )
                   if(topography) then
                       ...
                       pEdgeU = 0.5 * (jac(i, j, k) * pStratTFC(i, j, k) &
                                + jac(i, j, k + 1) * pStratTFC(i, j, k + 1))
                       pUEdgeU = 0.5 * (jac(i, j, k + 1) &
                                 * pStratTFC(i, j, k + 1) + jac(i, j, k + 2) &
                                 * pStratTFC(i, j, k + 2))
                       if(fluxmode == "nln") then
                           wSurf = 0.5 * (pEdgeU * var(i, j, k, 4) &
                                   + pUEdgeU * var(i, j, k + 1, 4))
                       else if(fluxmode == "lin") then
                           wSurf = 0.5 * (pEdgeU * vara(i, j, k, 4) &
                                   + pUEdgeU * vara(i, j, k + 1, 4))
                       else
                           stop "ERROR: wrong fluxmode"
                       end if
                   else
                       ...
                       if (fluxmode == "nln") then
                          wsurf &
                          = 0.5 &
                            *(  var(i,j,k  ,4)*PstratTilde(k) &
                              + var(i,j,k+1,4)*PstratTilde(k+1))
                       else if (fluxmode == "lin") then
                          wsurf &
                          = 0.5 &
       ...
       do k = 1,nz
          do j = 1,ny
             do i = -1,nx
                ...
                if(topography) then
                    ! TFC FJ
                    fRhoU_visc = coef_v * jac(i + 1, j, k) &
                                 * stressTensTFC(i + 1, j, k, 1, 1, var)
                else
       ...
       do k = 1,nz
          do j = 0,ny
             do i = 0,nx
                ...
                if(topography) then
                    ! TFC FJ
                    gRhoU_visc = coef_v * 0.25 * (jac(i, j, k) &
                                 * stressTensTFC(i, j, k, 1, 2, var) &
                                 + jac(i + 1, j, k) &
                                 * stressTensTFC(i + 1, j, k, 1, 2, var) &
                                 + jac(i, j + 1, k) &
                                 * stressTensTFC(i, j + 1, k, 1, 2, var) &
                                 + jac(i + 1, j + 1, k) &
       ...
       do k = 0,nz
          do j = 1,ny
             do i = 0,nx
                ...
                if(topography) then
                    ! TFC FJ
                    stressTens13 = met(i, j, k, 1, 3) &
                                   * stressTensTFC(i, j, k, 1, 1, var) &
                                   + met(i, j, k, 2, 3) &
                                   * stressTensTFC(i, j, k, 1, 2, var) &
                                   + stressTensTFC(i, j, k, 1, 3, var) &
                                   / jac(i, j, k)
                    stressTens13R = met(i + 1, j, k, 1, 3) &
                                    * stressTensTFC(i + 1, j, k, 1, 1, var) &
                                    + met(i + 1, j, k, 2, 3) &
                                    * stressTensTFC(i + 1, j, k, 1, 2, var) &
                                    + stressTensTFC(i + 1, j, k, 1, 3, var) &
                                    / jac(i + 1, j, k)
                    stressTens13U = met(i, j, k + 1, 1, 3) &
                                    * stressTensTFC(i, j, k + 1, 1, 1, var) &
                                    + met(i, j, k + 1, 2, 3) &
                                    * stressTensTFC(i, j, k + 1, 1, 2, var) &
                                    + stressTensTFC(i, j, k + 1, 1, 3, var) &
                                    / jac(i, j, k + 1)
                    stressTens13RU = met(i + 1, j, k + 1, 1, 3) &
                                     * stressTensTFC(i + 1, j, k + 1, 1, 1, &
                                     var) + met(i + 1, j, k + 1, 2, 3) &
                                     * stressTensTFC(i + 1, j, k + 1, 1, 2, &
                                     var) + stressTensTFC(i + 1, j, k + 1, 1, &
                                     3, var) / jac(i + 1, j, k + 1)
                    hRhoU_visc = coef_v * 0.25 * (jac(i, j, k) * stressTens13 &
                                 + jac(i + 1, j, k) * stressTens13R &
                                 + jac(i, j, k + 1) * stressTens13U &
       ...
       do k = 1,nz
          do j = 0,ny
             do i = 0,nx
                ...
                if(topography) then
                    ! TFC FJ
                    fRhoV_visc = coef_v * 0.25 * (jac(i, j, k) &
                                 * stressTensTFC(i, j, k, 2, 1, var) &
                                 + jac(i + 1, j, k) &
                                 * stressTensTFC(i + 1, j, k, 2, 1, var) &
                                 + jac(i, j + 1, k) &
                                 * stressTensTFC(i, j + 1, k, 2, 1, var) &
                                 + jac(i + 1, j + 1, k) &
       ...
       do k = 0,nz
          do j = 0,ny
             do i = 1,nx
                ...
                if(topography) then
                    ! TFC FJ
                    stressTens23 = met(i, j, k, 1, 3) &
                                   * stressTensTFC(i, j, k, 2, 1, var) &
                                   + met(i, j, k, 2, 3) &
                                   * stressTensTFC(i, j, k, 2, 2, var) &
                                   + stressTensTFC(i, j, k, 2, 3, var) &
                                   / jac(i, j, k)
                    stressTens23F = met(i, j + 1, k, 1, 3) &
                                    * stressTensTFC(i, j + 1, k, 2, 1, var) &
                                    + met(i, j + 1, k, 2, 3) &
                                    * stressTensTFC(i, j + 1, k, 2, 2, var) &
                                    + stressTensTFC(i, j + 1, k, 2, 3, var) &
                                    / jac(i, j + 1, k)
                    stressTens23U = met(i, j, k + 1, 1, 3) &
                                    * stressTensTFC(i, j, k + 1, 2, 1, var) &
                                    + met(i, j, k + 1, 2, 3) &
                                    * stressTensTFC(i, j, k + 1, 2, 2, var) &
                                    + stressTensTFC(i, j, k + 1, 2, 3, var) &
                                    / jac(i, j, k + 1)
                    stressTens23FU = met(i, j + 1, k + 1, 1, 3) &
                                     * stressTensTFC(i, j + 1, k + 1, 2, 1, &
                                     var) + met(i, j + 1, k + 1, 2, 3) &
                                     * stressTensTFC(i, j + 1, k + 1, 2, 2, &
                                     var) + stressTensTFC(i, j + 1, k + 1, 2, &
                                     3, var) / jac(i, j + 1, k + 1)
                    hRhoV_visc = coef_v * 0.25 * (jac(i, j, k) * stressTens23 &
                                 + jac(i, j + 1, k) * stressTens23F &
                                 + jac(i, j, k + 1) * stressTens23U &
                                 + jac(i, j + 1, k + 1) * stressTens23FU)
                else
       ...
       do k = 0,nz
          do j = 1,ny
             do i = 0,nx
                ...
                if(topography) then
                    ! TFC FJ
                    fRhoW_visc = coef_v * 0.25 * (jac(i, j, k) &
                                 * stressTensTFC(i, j, k, 3, 1, var) &
                                 + jac(i + 1, j, k) &
                                 * stressTensTFC(i + 1, j, k, 3, 1, var) &
                                 + jac(i, j, k + 1) &
                                 * stressTensTFC(i, j, k + 1, 3, 1, var) &
                                 + jac(i + 1, j, k + 1) &
                                 * stressTensTFC(i + 1, j, k + 1, 3, 1, var))
                else
       ...
       do k = 0,nz
          do j = 0,ny
             do i = 1,nx
                ...
                if(topography) then
                    ! TFC FJ
                    gRhoW_visc = coef_v * 0.25 * (jac(i, j, k) &
                                 * stressTensTFC(i, j, k, 3, 2, var) &
                                 + jac(i, j + 1, k) &
                                 * stressTensTFC(i, j + 1, k, 3, 2, var) &
                                 + jac(i, j, k + 1) &
  ...
  subroutine init_fluxes
    ...
    ! rhopBar
    allocate(rhopBar(-nbx:nx+nbx,-nby:ny+nby,-nbz:nz+nbz),stat=allocstat)
    ...
    !UAB
    ! rhoTilde_mom
    allocate(rhoTilde_mom(-nbx:nx+nbx,-nby:ny+nby,-nbz:nz+nbz,1:3,0:1),&
         & stat=allocstat)
    if(allocstat /= 0) stop "fluxes.f90: could not allocate rhoTilde"
    !UAE

    ! rhopTilde
    allocate(rhopTilde(-nbx:nx+nbx,-nby:ny+nby,-nbz:nz+nbz,1:3,0:1),&
         & stat=allocstat)
  ...
  subroutine terminate_fluxes
    ...
    if(topography) then
        ...
        if(allocstat /= 0) stop "terminate_fluxes: dealloc of uOldTFC &
        failed"

        deallocate(vOldTFC, stat = allocstat)
        if(allocstat /= 0) stop "terminate_fluxes: dealloc of vOldTFC &
        failed"

        deallocate(wOldTFC, stat = allocstat)
        if(allocstat /= 0) stop "terminate_fluxes: dealloc of wOldTFC &
        failed"
  ...
  subroutine reconstructionTestTFC(var)
      ...
      real, dimension((- nbx):(nx + nbx), (- nby):(ny + nby), &
      (- nbz):(nz + nbz), 1:3, 0:1) :: rhoTilde_tfc, rhopTilde_tfc, &
      uTilde_tfc, vTilde_tfc, wTilde_tfc
      ...
      call reconstruction(var, "rho")
      topography = .true.
      print*, "Reconstruction difference (rho):", &
      maxval(abs(rhoTilde - rhoTilde_tfc))
      ...
      call reconstruction(var, "rhop")
      topography = .true.
      print*, "Reconstruction difference (rhop):", &
      maxval(abs(rhopTilde - rhopTilde_tfc))
      ...
      call reconstruction(var, "uvw")
      topography = .true.
      print*, "Reconstruction difference (u):", &
      maxval(abs(uTilde - uTilde_tfc))
      print*, "Reconstruction difference (v):", &
      maxval(abs(vTilde - vTilde_tfc))
      print*, "Reconstruction difference (w):", &
      maxval(abs(wTilde - wTilde_tfc))
  ...
  subroutine momentumFluxTestTFC(var, flux, RKStage)
      ...
      if(.not. divergenceTest) then
          ...
          call momentumFlux(var, var, flux, "nln", pStrat, pStratTilde)
          topography = .true.
          print*, "Momentum flux difference (nln):", &
          maxval(abs(flux_tfc - flux))
          ...
          call momentumFlux(var, var, flux, "lin", pStrat, pStratTilde)
          topography = .true.
          print*, "Momentum flux difference (lin):", &
          maxval(abs(flux_tfc - flux))
          ...
          do k = 0, nz
              do j = 1, ny
                  do i = 1, nx
                      ...
                      ! jacEdgeU = 0.5 * (jac(i, j, k) + jac(i + 1, j, k))
                      sumLoc &
                      = sumLoc + fluxDiffW(i, j, k) * jacEdgeU * dx * dy * dz
                      sumLocSquared &
                      = sumLocSquared + (fluxDiffW(i, j, k) * jacEdgeU * dx &
                        * dy * dz) ** 2.0
                  end do
              end do
          end do

          call mpi_allreduce(sumLoc, sumGlob, 1, mpi_double_precision, &
          mpi_sum, comm, ierror)
          ...
          if (master) then
              ...
              if(.not. testTFC) then
                  ...
                  open(42, file = "momentum_flux_divergence_error.dat", &
  ...
  subroutine massFluxTestTFC(var, flux)
      ...
      call massFlux(var, var, flux, "nln", pStrat, pStratTilde)
      topography = .true.
      print*, "Mass flux difference (nln):", &
      maxval(abs(flux_tfc - flux))
      ...
      call massFlux(var, var, flux, "lin", pStrat, pStratTilde)
      topography = .true.
      print*, "Mass flux difference (lin):", &
      maxval(abs(flux_tfc - flux))
