        ...
        elseif upd_mod == "rhs"
            if int_mod == "impl"
                ...
                for k in 1:nz, j in 1:ny, i in 1:nx
                    ...
                    buoy = 1.0 /
                           (facw +
                            rhostrattfc[i, j, k] / rho * bvsstrattfc[i, j, k] * dt^2) *
            ...
            elseif int_mod == "expl"
                for k in 1:nz, j in 1:ny, i in 1:nx
                    ...
                    wvrt = 0.5 *
                           (vertWind(i, j, k, model) + vertWind(i, j, k - 1, model))
                    buoy = -g_ndim * rhop / rho
                    buoy -= dt * rhostrattfc[i, j, k] / rho * bvsstrattfc[i, j, k] *
                            wvrt
                    var.rhop[i, j, k] = -buoy * rho / g_ndim
                end
            else
                    ...
                    if mmp_mod == "lhs"
                        ...
                        volForce = volForce +
                                   ...
                                   ((rhoOld[i, j, k] + rhostrattfc[i, j, k]) * vC +
                                    (rhoOld[i + 1, j, k] + rhostrattfc[i + 1, j, k]) *
                                    vR)
                        F = -fluxDiff + volForce
                    end
