...
function linOpr(sIn, Ls, opt, hortot, model)
  ...
  (;
    ...
    ard_b,
    alu_b,
  ) = model.operator.op_fields
  (; ald_b, afu_b, afd_b, abu_b, abd_b, auu_b, add_b, aruu_b, ardd_b, aluu_b) =
    model.operator.op_fields
    ...
    for k in 1:nz
      for j in 1:ny
        for i in 1:nx
          ...
          # Compute RHS.
          bu =
            (pEdgeR * uR - pEdgeL * uL) / dx / jac[i, j, k] * c.ma^2.0 * c.kappa
          bv =
            (pEdgeF * vF - pEdgeB * vB) / dy / jac[i, j, k] * c.ma^2.0 * c.kappa
          bw =
            (pEdgeU * wU - pEdgeD * wD) / dz / jac[i, j, k] * c.ma^2.0 * c.kappa
          # @show uR, vF, wU
...
function bicgstab(b_in, dt, model, sol, nIter, errFlag, opt)
  ...
  errFlag = true
  return println("BICGSTAB: max iteration reached.")
  # nIter = maxIt
...
function correctorStep(model, dt, opt, facray, facprs)
  ...
  # TODO...
  corX = OffsetArray(
    zeros(Float64, nx + 2nbx + 1, ny + 2nby + 1, nz + 4),
    (-nbx):(nx + nbx),
    (-nby):(ny + nby),
    -1:(nz + 2),
  )
  ...
  @. var.pip[0:(nx + 1), 0:(ny + 1), 0:(nz + 1)] =
    var.pip[0:(nx + 1), 0:(ny + 1), 0:(nz + 1)] +
    dp[0:(nx + 1), 0:(ny + 1), 0:(nz + 1)]
    ...
    for k in 1:nz
      for j in 1:ny
        for i in 0:nx
          ...
          if (
            model.parameters.boundaries.spongelayer &&
            model.parameters.boundaries.sponge_uv
          )
            facu =
          ...
          model.variables.prognostic_fields.u[i, j, k] =
            model.variables.prognostic_fields.u[i, j, k] + du
        end
      end
    end
  elseif (opt == "expl")
    ...
    for k in k0:k1
      for j in 1:ny
        for i in 1:nx
          ...
          dw =
            -facprs * dt /
            (facw + rhostrattfcEdgeU / rhoEdge * bvsstw * dt^2.0) * pGradZ -
      ...
      for j in 1:ny
        for i in 1:nx
          ...
          fcscal_bd =
            sqrt(pstrattfc(i, j - 1, k - 1)^2.0 / rhostrattfc(i, j - 1, k - 1))
          fcscal_uu =
            sqrt(pstrattfc(i, j, k + 2)^2.0 / rhostrattfc(i, j, k + 2))
          fcscal_dd =
            sqrt(pstrattfc(i, j, k - 2)^2.0 / rhostrattfc(i, j, k - 2))
          fcscal_ruu =
          ...
          elseif (k == nz && zboundaries == "solid_wall")
            ARD =
              -jacInv / dx * pEdgeRDiv / rhoEdgeR * pEdgeRGra * met13EdgeR /
              dz -
              jacInv / dz * pEdgeDDiv / rhoEdgeD *
              pEdgeDGra *
              met13EdgeD *
          ...
          if (k == 1 && zboundaries == "solid_wall")
            ALU =
              -jacInv / dx * pEdgeLDiv / rhoEdgeL * pEdgeLGra * met13EdgeL /
              dz -
              jacInv / dz * pEdgeUDiv / rhoEdgeU *
              pEdgeUGra *
              met13EdgeU *
          ...
          elseif (k == nz && zboundaries == "solid_wall")
            AFD =
              -jacInv / dy * pEdgeFDiv / rhoEdgeF * pEdgeFGra * met23EdgeF /
              dz -
              jacInv / dz * pEdgeDDiv / rhoEdgeD *
              pEdgeDGra *
              met23EdgeD *
          ...
          if (k == 1 && zboundaries == "solid_wall")
            ABU =
              -jacInv / dy * pEdgeBDiv / rhoEdgeB * pEdgeBGra * met23EdgeB /
              dz -
              jacInv / dz * pEdgeUDiv / rhoEdgeU *
              pEdgeUGra *
              met23EdgeU *
    ...
    for k in 1:nz
      for j in 1:ny
        for i in 1:nx
          ...
          pUEdgeRGra =
            0.5 * (pstrattfc(i, j, k + 1) + pstrattfc(i + 1, j, k + 1))
          pUEdgeLGra =
            0.5 * (pstrattfc(i, j, k + 1) + pstrattfc(i - 1, j, k + 1))
          pUEdgeFGra =
            0.5 * (pstrattfc(i, j, k + 1) + pstrattfc(i, j + 1, k + 1))
          pUEdgeBGra =
            0.5 * (pstrattfc(i, j, k + 1) + pstrattfc(i, j - 1, k + 1))
          pDEdgeRGra =
            0.5 * (pstrattfc(i, j, k - 1) + pstrattfc(i + 1, j, k - 1))
          ...
          # Compute density coefficients.
          rhostrattfcEdgeR =
            0.5 * (rhostrattfc[i, j, k] + rhostrattfc[i + 1, j, k])
          rhostrattfcEdgeL =
            0.5 * (rhostrattfc[i, j, k] + rhostrattfc(i - 1, j, k))
          rhostrattfcEdgeF =
            0.5 * (rhostrattfc[i, j, k] + rhostrattfc(i, j + 1, k))
          rhostrattfcEdgeB =
            0.5 * (rhostrattfc[i, j, k] + rhostrattfc(i, j - 1, k))
          rhostrattfcEdgeU =
            (
          ...
          rhoEdgeR =
            0.5 * (var.rho[i, j, k] + var.rho[i + 1, j, k]) + rhostrattfcEdgeR
          rhoEdgeL =
            0.5 * (var.rho[i, j, k] + var.rho(i - 1, j, k)) + rhostrattfcEdgeL
          rhoEdgeF =
            0.5 * (var.rho[i, j, k] + var.rho(i, j + 1, k)) + rhostrattfcEdgeF
          rhoEdgeB =
            0.5 * (var.rho[i, j, k] + var.rho(i, j - 1, k)) + rhostrattfcEdgeB
          rhoEdgeU =
            (
              jac(i, j, k + 1) * var.rho[i, j, k] +
              jac[i, j, k] * var.rho(i, j, k + 1)
          ...
          if (model.parameters.boundaries.spongelayer)
            if (model.parameters.boundaries.sponge_uv)
              ...
              facUEdgeR =
                facUEdgeR +
                dt * 0.5 * (kr_sp_tfc(i, j, k + 1) + kr_sp_tfc(i + 1, j, k + 1))
              facUEdgeL =
                facUEdgeL +
                dt * 0.5 * (kr_sp_tfc(i, j, k + 1) + kr_sp_tfc(i - 1, j, k + 1))
              facUEdgeF =
                facUEdgeF +
                dt * 0.5 * (kr_sp_tfc(i, j, k + 1) + kr_sp_tfc(i, j + 1, k + 1))
              facUEdgeB =
                facUEdgeB +
                dt * 0.5 * (kr_sp_tfc(i, j, k + 1) + kr_sp_tfc(i, j - 1, k + 1))
              facDEdgeR =
          ...
          impVerEdgeU =
            1.0 /
            (facEdgeU + rhostrattfcEdgeU / rhoEdgeU * bvsStratEdgeU * dt^2.0)
          impVerEdgeD =
            1.0 /
            (facEdgeD + rhostrattfcEdgeD / rhoEdgeD * bvsStratEdgeD * dt^2.0)
