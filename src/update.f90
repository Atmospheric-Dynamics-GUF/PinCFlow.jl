          ...
          do k = kSponge, nz
             do j = 1,ny
                do i = 1,nx
                   ...
                   rho_old = var(i,j,k,1)
                   alpha = c4_strtd(j)*spongeAlphaZ*(z(k)-zSponge)/spongeDz
                   beta = 1./(1.+alpha*0.5*dt)**2
                   ...
                   do i = 1, nx
                       ...
                       if(lateralSponge) then
                           ...
                           if(x(i00 + i) <= xSponge0) then
                               alpha = alpha + spongeAlphaX * sin(0.5 * pi &
                                       * (xSponge0 - x(i00 + i)) / (xSponge0 &
                                       - lx(0))) ** 2.0
                           else if(x(i00 + i) >= xSponge1) then
                               alpha = alpha + spongeAlphaX * sin(0.5 * pi &
                                       * (x(i00 + i) - xSponge1) / (lx(1) &
                                       - xSponge1)) ** 2.0
                           end if
                           ! Meridional sponge.
                           if(y(j00 + j) <= ySponge0) then
                               alpha = alpha + spongeAlphaY * sin(0.5 * pi &
                                       * (ySponge0 - y(j00 + j)) / (ySponge0 &
                                       - ly(0))) ** 2.0
                           else if(y(j00 + j) >= ySponge1) then
                               alpha = alpha + spongeAlphaY * sin(0.5 * pi &
                                       * (y(j00 + j) - ySponge1) / (ly(1) &
                                       - ySponge1)) ** 2.0
                           end if
                       end if
                       ! Vertical sponge.
                       if(heightTFC(i, j, k) >= zSponge) then
                           alpha = alpha + spongeAlphaZ * sin(0.5 * pi &
                                   * (heightTFC(i, j, k) - zSponge) / (lz(1) &
                                   - zSponge)) ** 2.0
                       end if
...
!---------------------------------------------------------------------
  ...
  subroutine wind_ip (var,xip,yip,zip,wind_recon,u_ip,v_ip,w_ip)
    ...
    if (wind_recon == 'u' .or. wind_recon == 'v') then
       ...
       u_ip &
       = (  (x(i+i00 + 1) + dx/2. - xip) &
            *(  (y(j+j00 + 1) - yip)*var(i,j,k,2) &
              + (yip - y(j+j00))*var(i,j+1,k,2))&
          + (xip - x(i+i00) - dx/2.) &
            *(  (y(j+j00 + 1) - yip)*var(i+1,j,k,2) &
              + (yip - y(j+j00))*var(i+1,j+1,k,2)))&
         /(dx*dy)
       ...
       v_ip &
       =(  (x(i+i00 + 1) - xip) &
           *(  (y(j+j00 + 1) + dy/2. - yip)*var(i,j,k,3) &
             + (yip - y(j+j00) - dy/2.)*var(i,j+1,k,3))&
         + (xip - x(i+i00)) &
           *(  (y(j+j00 + 1) + dy/2. - yip)*var(i+1,j,k,3) &
             + (yip - y(j+j00) - dy/2.)*var(i+1,j+1,k,3)))&
        /(dx*dy)
       ...
       =0.5 &
        *(  (  (x(i+i00 + 1) - xip) &
                *(  (y(j+j00 + 1) - yip)*var(i,j,k,4) &
                  + (yip - y(j+j00))*var(i,j+1,k,4))&
             + (xip - x(i+i00)) &
               *(  (y(j+j00 + 1) - yip)*var(i+1,j,k,4) &
                 + (yip - y(j+j00))*var(i+1,j+1,k,4)))&
            /(dx*dy) &
          + (  (x(i+i00 + 1) - xip) &
                *(  (y(j+j00 + 1) - yip)*var(i,j,k-1,4) &
                  + (yip - y(j+j00))*var(i,j+1,k-1,4))&
             + (xip - x(i+i00)) &
               *(  (y(j+j00 + 1) - yip)*var(i+1,j,k-1,4)  &
                 + (yip - y(j+j00))*var(i+1,j+1,k-1,4)))&
            /(dx*dy))
       ...
       = 0.5 &
         *(  (  (x(i+i00 + 1) + dx/2. - xip) &
             ...
             /(dx*dy) &
           + (  (x(i+i00 + 1) + dx/2. - xip) &
                *(  (y(j+j00 + 1) - yip)*var(i,j,k+1,2) &
                  + (yip - y(j+j00))*var(i,j+1,k+1,2))&
              + (xip - x(i+i00) - dx/2.) &
                *(  (y(j+j00 + 1) - yip)*var(i+1,j,k+1,2) &
                  + (yip - y(j+j00))*var(i+1,j+1,k+1,2)))&
             /(dx*dy))
       ...
       = 0.5 &
         *(  (  (x(i+i00 + 1) - xip) &
             ...
             /(dx*dy) &
           + (  (x(i+i00 + 1) - xip) &
                *(  (y(j+j00 + 1) + dy/2. - yip)*var(i,j,k+1,3) &
                  + (yip - y(j+j00) - dy/2.)*var(i,j+1,k+1,3))&
              + (xip - x(i+i00)) &
                *(  (y(j+j00 + 1) + dy/2. - yip)*var(i+1,j,k+1,3) &
                  + (yip - y(j+j00) - dy/2.)*var(i+1,j+1,k+1,3)))&
             /(dx*dy))
...
!---------------------------------------------------------------------
  ...
  subroutine momentumPredictor_nc (var,flux,force,dt,q,m,mmp_mod,int_mod, &
                                 & facray)
    !-------------------------------------
    ...
    real :: rho, rhop, rhou, rhov, rhow, facu, facv, facw, facr, &
            pstw, pstw_0, buoy
    real :: rho10, rho01
    ...
    real :: u_ip, v_ip, w_ip, &
            u_ip_n, v_ip_n, w_ip_n, u_ip_t, v_ip_t, w_ip_t, &
            u_rp, v_rp, w_rp, &
            u_rp_n, v_rp_n, w_rp_n, u_rp_t, v_rp_t, w_rp_t
             ...
             do i = i0,i1
                ...
                if (mmp_mod == "tot") then
                   ...
                   if (TestCase == "baroclinic_LC")then!FS
                      piGrad = piGrad + kappaInv*MaInv2 &
                           * (PStrat(k) - pStrat_0(k)) &
                           * (var_env(i+1,j,k,5)-var_env(i,j,k,5))/dx
                   end if
          ...
          do k = 1,nz
             do j = 1,ny
                do i = i0,i1
                   ...
                   if (TestCase == "baroclinic_LC") then
                      rhou_e &
                      = 0.5 * (var_env(i,j,k,1) + var_env(i+1,j,k,1))
                      if( fluctuationMode ) then
                   ...
                   if (TestCase == "baroclinic_LC")then !FS
                      piGrad &
                      = piGrad + kappaInv*MaInv2 &
                                 * (Pstrat(k)/rhou - Pstrat_0(k)/rhou_e) &
                                 * (piR_e-piL_e)/dx
                   end if
             ...
             do j = 1,ny
                do i = i0,i1
                   ...
                   if (TestCase == "baroclinic_LC") then
                      ...
                      rhov0m_e &
                      =  0.5 * (var_env(i,j,k,1) + var_env(i,j-1,k,1))
                      rhov00_e &
                      =  0.5 * (var_env(i,j+1,k,1) + var_env(i,j,k,1))
                      rhov1m_e &
                      =  0.5 * (var_env(i+1,j,k,1) + var_env(i+1,j-1,k,1))
                      rhov10_e &
                      =  0.5 * (var_env(i+1,j+1,k,1) + var_env(i+1,j,k,1))
                   ...
                   if (TestCase == "baroclinic_LC") then
                      ...
                      =   piGrady &
                        + kappaInv*MaInv2 &
                          * 0.25 &
                          * (  (Pstrat(k)/rhov0m - Pstrat_0(k)/rhov0m_e) &
                               * (  var_env(i,j,k,5) &
                                  - var_env(i,j-1,k,5))/dy &
                             + (Pstrat(k)/rhov00 - Pstrat_0(k)/rhov00_e) &
                               * (  var_env(i,j+1,k,5) &
                                  - var_env(i,j,k,5))/dy &
                             + (Pstrat(k)/rhov1m - Pstrat_0(k)/rhov1m_e) &
                               * (  var_env(i+1,j,k,5) &
                                  - var_env(i+1,j-1,k,5))/dy &
                             + (Pstrat(k)/rhov10 - Pstrat_0(k)/rhov10_e) &
                               * (  var_env(i+1,j+1,k,5) &
                                  - var_env(i+1,j,k,5))/dy)
                     else
                      piGrady &
                   ...
                   = 1.0/facu &
                     * (  uhorx &
                        + dt*(f_cor_nd(j)*vhory + volfcx/rhou - piGradx) + dt**2*f_cor_nd(j)**2*backgroundFlow_dim(1)/uRef)
                   else
                   uAst &
                   = 1.0/facu &
                     * (  uhorx &
                        + dt*(f_cor_nd(j)*vhory + volfcx/rhou - piGradx))
                   end if
                   !UAE
             ...
             do i = 1,nx
                ...
                if (mmp_mod == "tot") then
                   ...
                   if (TestCase == "baroclinic_LC")then !FS
                      piGrad = piGrad + kappaInv*MaInv2 &
                           * (PStrat(k) - pStrat_0(k)) &
                           * (var_env(i,j+1,k,5) - var_env(i,j,k,5))/dy
                   end if
             ...
             do j = j0,j1
                do i = 1,nx
                   ...
                   if (TestCase == "baroclinic_LC")then !FS
                      piGrad &
                      = piGrad + kappaInv*MaInv2 &
                                 * (Pstrat(k)/rhov - Pstrat_0(k)/rhov_e) &
                                 * (piF_e-piB_e)/dy
                   end if
                   ...
                   if (testCase == "SkamarockKlemp94")then
                      vAst &
                   = vhory + dt*(-f_cor_v*(uhorx-backgroundFlow_dim(1)/uRef) - piGrad + volfcy/rhov)
                   else
                      vAst &
                   = vhory + dt*(-f_cor_v*uhorx - piGrad + volfcy/rhov)
                   end if
                   ...
                   if (TestCase == "baroclinic_LC") then
                      if (background == "HeldSuarez") then
                         ! Rayleigh damping
                         vAst &
                         = vAst - dt* 0.5*(kv_hs(j,k) + kv_hs(j+1,k))*vhory
                      end if
                   end if
             ...
             do j = j0,j1
                do i = 1,nx
                   ...
                   if (TestCase == "baroclinic_LC") then
                      ...
                      = 0.5 * (var_env(i  ,j  ,k,1) + var_env(i-1,j  ,k,1))
                      rhou00_e &
                      = 0.5 * (var_env(i+1,j  ,k,1) + var_env(i  ,j  ,k,1))
                      rhoum1_e &
                      = 0.5 * (var_env(i  ,j+1,k,1) + var_env(i-1,j+1,k,1))
                      rhou01_e &
                      = 0.5 * (var_env(i+1,j+1,k,1) + var_env(i  ,j+1,k,1))
                   ...
                   if (TestCase == "baroclinic_LC") then
                      ...
                      =   piGradx &
                        + kappaInv*MaInv2 &
                        * 0.25 &
                        * (  (Pstrat(k)/rhou00 - Pstrat_0(k)/rhou00_e)&
                             * (  var_env(i+1,j  ,k,5) &
                                - var_env(i  ,j  ,k,5))/dx &
                           + (Pstrat(k)/rhoum0 - Pstrat_0(k)/rhoum0_e)&
                             * (  var_env(i  ,j  ,k,5) &
                                - var_env(i-1,j  ,k,5))/dx &
                           + (Pstrat(k)/rhou01 - Pstrat_0(k)/rhou01_e)&
                             * (  var_env(i+1,j+1,k,5) &
                                - var_env(i  ,j+1,k,5))/dx &
                           + (Pstrat(k)/rhoum1 - Pstrat_0(k)/rhoum1_e)&
                             * (  var_env(i  ,j+1,k,5) &
                                - var_env(i-1,j+1,k,5))/dx)
                     else
                      piGradx &
                   ...
                   if (TestCase == "baroclinic_LC") then
                      if (background == "HeldSuarez") then
                         ! Rayleigh damping
                         facv &
                         = facv + dt* 0.5*(kv_hs(j,k) + kv_hs(j+1,k))
                      end if
                   end if
                   ...
                   if (testCase == "SkamarockKlemp94")then
                      vAst &
                           = 1.0 /facv &
                           * (  vhory &
                           + dt*(- f_cor_v*(uhorx-backgroundFlow_dim(1)/uRef) + volfcy/rhov - piGrady))
                   else
                      vAst &
                           = 1.0 /facv &
                           * (  vhory &
                           + dt*(- f_cor_v*uhorx + volfcy/rhov - piGrady))
                   end if
                   !UAE
          ...
          do j = 1,ny
             do i = 1,nx
                ...
                if (mmp_mod == "tot") then
                   ...
                   if (TestCase == "baroclinic_LC")then !FS
                      piGrad = piGrad + 0.5* kappaInv*MaInv2* &
                           (Pstrat(k)+Pstrat(k+1)-pStrat_0(k)-pStrat_0(k+1)) &
                ...
                if (TestCase == "baroclinic_LC") then
                   if (background == "HeldSuarez") then
                      ...
                      volForce &
                           = volforce - 0.5*(kw_hs(k)+kw_hs(k+1))*rhoM_1*var(i,j,k,4)
                   end if
                end if
                ...
                do i = 1,nx
                   ...
                   if (TestCase == "baroclinic_LC") then
                      ...
                      if( fluctuationMode ) then
                         ...
                         rhow_e &
                         =  rhow_e + 0.5*(rhoStrat_0(k) + rhoStrat_0(k+1))
                      end if
                   end if
                   ...
                   if (TestCase == "baroclinic_LC")then !FS
                      piGrad &
                      =   piGrad &
                        + kappaInv*MaInv2 * (pstw/rhow - pstw_e/rhow_e) &
                          * (piU_e-piD_e)/dz
                   end if
                   ...
                   if (TestCase == "baroclinic_LC") then
                      ...
                      = - g_ndim &
                          * 0.5 &
                          * (  rhopOld(i,j,k)/rho000 &
                             - var_env(i,j,k,6)/rho000_e &
                             + rhopOld(i,j,k+1)/rho001 &
                             - var_env(i,j,k+1,6)/rho001_e)
                     else
                      buoy  &
                      = - g_ndim &
                   ...
                   if (spongeLayer) then
                      wAst = wAst - dt*0.5*(kr_sp_w(j,k)+kr_sp_w(j,k+1))*wvert
                   end if
                ...
                do i = 1,nx
                   ...
                   if (TestCase == "baroclinic_LC") then
                      ...
                      = 1.0 &
                        ...
                        /(  facw &
                          + rhoStratTilde(k)/rhow * pstw/pstw_0 &
                            * bvsstw * dt**2) &
                        * (  wvert - dt * piGrad &
                           - dt * g_ndim &
                             * 0.5*(  rhopOld(i,j,k)/rho000 &
                                    ...
                                    + dt &
                                      !* (  rhoStrat(k)/Pstrat(k) &
                                      * (  rhoStrat(k)/Pstrat_0(k) &
                                           * heat0/rho000 &
                                         !+ rhoStrat(k+1)/Pstrat(k+1) &
                                         + rhoStrat(k+1)/Pstrat_0(k+1) &
                                           * heat1/rho001)))
                     else
                      wAst &
                      = 1.0 &
...
!---------------------------------------------------------------------
  ...
  subroutine momentumPredictor (var,flux,force,dt,q,m,mmp_mod,int_mod, &
    ...
    real :: piEdgeR, piUEdgeR, piUUEdgeR, piDEdgeR, piDDEdgeR, &
            piEdgeF, piUEdgeF, piUUEdgeF, piDEdgeF, piDDEdgeF, &
            piREdgeU, piLEdgeU, piFEdgeU, piBEdgeU
             ...
             do i = i0,i1
                ...
                if (mmp_mod == "tot") then
                   ...
                   if(topography) then
                       ...
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           piUUEdgeR = 0.5 * (jac(i, j, k + 2) &
                                       * met(i, j, k + 2, 1, 3) &
                                       * var(i, j, k + 2, 5) &
                                       + jac(i + 1, j, k + 2) &
                                       * met(i + 1, j, k + 2, 1, 3) &
                                       * var(i + 1, j, k + 2, 5))
                           piUEdgeR = 0.5 * (jac(i, j, k + 1) &
                                      * met(i, j, k + 1, 1, 3) &
                                      * var(i, j, k + 1, 5) &
                                      + jac(i + 1, j, k + 1) &
                                      * met(i + 1, j, k + 1, 1, 3) &
                           ...
                           piEdgeR = 0.5 * (jac(i, j, k) * met(i, j, k, 1, 3) &
                                     * var(i, j, k, 5) + jac(i + 1, j, k) &
                                     * met(i + 1, j, k, 1, 3) &
                                     * var(i + 1, j, k, 5))
                           piGrad = kappaInv * MaInv2 * pEdgeR &
                                    * ((jac(i + 1, j, k) * piR - jac(i, j, k) &
                                    * piL) / dx + (- piUUEdgeR + 4.0 &
                                    * piUEdgeR - 3.0 * piEdgeR) * 0.5 / dz)
                       else if(k == nz .and. zBoundary == "solid_wall") then
                           piDDEdgeR = 0.5 * (jac(i, j, k - 2) &
                                       * met(i, j, k - 2, 1, 3) &
                                       * var(i, j, k - 2, 5) &
                                       + jac(i + 1, j, k - 2) &
                                       * met(i + 1, j, k - 2, 1, 3) &
                                       * var(i + 1, j, k - 2, 5))
                           piDEdgeR = 0.5 * (jac(i, j, k - 1) &
                                      * met(i, j, k - 1, 1, 3) &
                                      * var(i, j, k - 1, 5) &
                                      + jac(i + 1, j, k - 1) &
                                      * met(i + 1, j, k - 1, 1, 3) &
                ...
                if (TestCase == "baroclinic_LC") then
                   if (background == "HeldSuarez") then
                      ...
                      volForce &
                      = volForce &
                        - kv_hs(j,k)*rhoM_1*(var(i,j,k,2) - var_env(i,j,k,2))
                ...
                if(topography .and. mmp_mod == "lhs") then
                    ...
                    if(testCase == "baroclinic_LC") then
                        vC = vC - 0.5 * (var_env(i, j, k, 3) &
                             + var_env(i, j - 1, k, 3))
                        vR = vR - 0.5 * (var_env(i + 1, j, k, 3) &
                             + var_env(i + 1, j - 1, k, 3))
                    end if
                    volForce = volForce + 0.5 * f_cor_nd(j) &
                       ...
                       if(topography) then
                           ...
                           if(k == 1 .and. zBoundary == "solid_wall") then
                               ...
                               piUEdgeR = 0.5 * (jac(i, j, k + 1) &
                                          ...
                                          * met(i + 1, j, k + 1, 1, 3) &
                                          * var_env(i + 1, j, k + 1, 5))
                               piEdgeR = 0.5 * (jac(i, j, k) &
                                         * met(i, j, k, 1, 3) &
                                         * var_env(i, j, k, 5) &
                                         + jac(i + 1, j, k) &
                                         * met(i + 1, j, k, 1, 3) &
                                         * var_env(i + 1, j, k, 5))
                               piGrad = piGrad + kappaInv * MaInv2 * (pEdgeR &
                                        / rhou - pEdgeR / rhou_e) &
                                        * ((jac(i + 1, j, k) * piR_e &
                                        - jac(i, j, k) * piL_e) / dx &
                                        + (- piUUEdgeR + 4.0 * piUEdgeR &
             ...
             do j = 1,ny
                do i = i0,i1
                       ...
                       if(topography) then
                           ...
                           if(k == 1 .and. zBoundary == "solid_wall") then
                               ...
                               piEdgeR = 0.5 * (jac(i, j, k) &
                                         ...
                                         * met(i + 1, j, k, 1, 3) &
                                         * var_env(i + 1, j, k, 5))
                               piGradX = piGradX + kappaInv * MaInv2 * (pEdgeR &
                                         / rhou - pEdgeR / rhou_e) &
                                         * ((jac(i + 1, j, k) * piR_e &
                                         - jac(i, j, k) * piL_e) / dx &
                                         + (- piUUEdgeR + 4.0 * piUEdgeR &
                   ...
                   if(topography) then
                       ! TFC FJ
                       ! Coriolis force is integrated on LHS.
                       uAst = 1.0 / facu * (uhorx + dt * (- piGradX &
                              + volfcx / rhou))
                   else
                       if (testCase == "SkamarockKlemp94")then
                           uAst &
                           = 1.0/(facu*facv + (f_cor_nd(j)*dt)**2) &
                             * (  facv * (uhorx + dt*(volfcx/rhou - piGradx)) &
                             + f_cor_nd(j)*dt &
                             * (vhory + dt*(volfcy/rhou - piGrady)) &
                             + f_cor_nd(j)**2*dt**2*backgroundFlow_dim(1)/uRef)
                       else
                           uAst &
                           = 1.0/(facu*facv + (f_cor_nd(j)*dt)**2) &
                             * (  facv * (uhorx + dt*(volfcx/rhou - piGradx)) &
             ...
             do i = 1,nx
                ...
                if (mmp_mod == "tot") then
                   ...
                   if(topography) then
                       ...
                       ! Compute pressure gradient component.
                       if(k == 1 .and. zBoundary == "solid_wall") then
                           piUUEdgeF = 0.5 * (jac(i, j, k + 2) &
                                       * met(i, j, k + 2, 2, 3) &
                                       * var(i, j, k + 2, 5) &
                                       + jac(i, j + 1, k + 2) &
                                       * met(i, j + 1, k + 2, 2, 3) &
                                       * var(i, j + 1, k + 2, 5))
                           piUEdgeF = 0.5 * (jac(i, j, k + 1) &
                                      * met(i, j, k + 1, 2, 3) &
                                      * var(i, j, k + 1, 5) &
                                      + jac(i, j + 1, k + 1) &
                                      * met(i, j + 1, k + 1, 2, 3) &
                           ...
                           piEdgeF = 0.5 * (jac(i, j, k) * met(i, j, k, 2, 3) &
                                     * var(i, j, k, 5) + jac(i, j + 1, k) &
                                     * met(i, j + 1, k, 2, 3) &
                                     * var(i, j + 1, k, 5))
                           piGrad = kappaInv * MaInv2 * pEdgeF &
                                    * ((jac(i, j + 1, k) * piF - jac(i, j, k) &
                                    * piB) / dy + (- piUUEdgeF + 4.0 &
                                    * piUEdgeF - 3.0 * piEdgeF) * 0.5 / dz)
                       else if(k == nz .and. zBoundary == "solid_wall") then
                           piDDEdgeF = 0.5 * (jac(i, j, k - 2) &
                                       * met(i, j, k - 2, 2, 3) &
                                       * var(i, j, k - 2, 5) &
                                       + jac(i, j + 1, k - 2) &
                                       * met(i, j + 1, k - 2, 2, 3) &
                                       * var(i, j + 1, k - 2, 5))
                           piDEdgeF = 0.5 * (jac(i, j, k - 1) &
                                      * met(i, j, k - 1, 2, 3) &
                                      * var(i, j, k - 1, 5) &
                                      + jac(i, j + 1, k - 1) &
                                      * met(i, j + 1, k - 1, 2, 3) &
                ...
                if(topography .and. mmp_mod == "lhs") then
                    ...
                    if(testCase == "SkamarockKlemp94") then
                        ...
                        uC = uC - 0.5 * (var_env(i, j, k, 2) &
                             + var_env(i - 1, j, k, 2))
                        uF = uF - 0.5 * (var_env(i, j + 1, k, 2) &
                             + var_env(i - 1, j + 1, k, 2))
                    end if
                    volForce = volForce - 0.5 * (f_cor_nd(j) &
                               * (rhoOld(i, j, k) + rhoStratTFC(i, j, k)) &
                               * uC + f_cor_nd(j + 1) * (rhoOld(i, j + 1, k) &
                               + rhoStratTFC(i, j + 1, k)) * uF)
                end if
             ...
             do j = j0,j1
                do i = 1,nx
                       ...
                       if(topography) then
                           ...
                           if(k == 1 .and. zBoundary == "solid_wall") then
                               ...
                               piUEdgeF = 0.5 * (jac(i, j, k + 1) &
                                          ...
                                          * met(i, j + 1, k + 1, 2, 3) &
                                          * var_env(i, j + 1, k + 1, 5))
                               piEdgeF = 0.5 * (jac(i, j, k) &
                                         * met(i, j, k, 2, 3) &
                                         * var_env(i, j, k, 5) &
                                         + jac(i, j + 1, k) &
                                         * met(i, j + 1, k, 2, 3) &
                                         * var_env(i, j + 1, k, 5))
                               piGrad = piGrad + kappaInv * MaInv2 * (pEdgeF &
                                        / rhov - pEdgeF / rhov_e) &
                                        * ((jac(i, j + 1, k) * piF_e &
                                        - jac(i, j, k) * piB_e) / dy &
                                        + (- piUUEdgeF + 4.0 * piUEdgeF &
                   ...
                   if(topography) then
                       ...
                       if (testCase == "SkamarockKlemp94")then
                           vAst &
                           = vhory + dt*(-f_cor_v*(uhorx-backgroundFlow_dim(1) &
                             / uRef) - piGrad + volfcy/rhov)
                       else
                           vAst &
                           = vhory + dt*(-f_cor_v*uhorx - piGrad + volfcy/rhov)
                       end if
             ...
             do j = j0,j1
                do i = 1,nx
                       ...
                       if(topography) then
                           ...
                           if(k == 1 .and. zBoundary == "solid_wall") then
                               ...
                               piEdgeF = 0.5 * (jac(i, j, k) &
                                         ...
                                         * met(i, j + 1, k, 2, 3) &
                                         * var_env(i, j + 1, k, 5))
                               piGradY = piGradY + kappaInv * MaInv2 * (pEdgeF &
                                         / rhov - pEdgeF / rhov_e) &
                                         * ((jac(i, j + 1, k) * piF_e &
                                         - jac(i, j, k) * piB_e) / dy &
                                         + (- piUUEdgeF + 4.0 * piUEdgeF &
                   ...
                   if(topography) then
                       ! TFC FJ
                       ! Coriolis force is integrated on LHS.
                       vAst = 1.0 / facv * (vhory + dt * (- piGradY &
                              + volfcy / rhov))
                   else
                       if (testCase == "SkamarockKlemp94")then
                           vAst &
                           = 1.0 &
                             ...
                             + (0.5*(f_cor_nd(j) + f_cor_nd(j+1))*dt)**2) &
                             * (- 0.5*(f_cor_nd(j) + f_cor_nd(j+1))*dt &
                             * ((uhorx-backgroundFlow_dim(1)/uRef) + dt &
                             * (volfcx/rhov - piGradx)) &
                             + facu * (vhory + dt * (volfcy/rhov - piGrady)))
                       else
                           vAst &
             ...
             do i = 1,nx
                ...
                if(topography) then
                    ...
                    do ll = 0, 1
                        do mm = 0, 1
                            ...
                            fluxDiffU(ll, mm) = (fR - fL) / dx &
                                                + (gF - gB) / dy &
                                                + (hU - hD) / dz
                            jacEdgeR = 0.5 * (jac(i - ll, j, k + mm) &
                                       + jac(i + 1 - ll, j, k + mm))
                    ...
                    do ll = 0, 1
                        do mm = 0, 1
                            ...
                            fluxDiffV(ll, mm) = (fR - fL) / dx &
                                                + (gF - gB) / dy &
                                                + (hU - hD) / dz
                            jacEdgeF = 0.5 * (jac(i, j - ll, k + mm) &
                                       + jac(i, j + 1 - ll, k + mm))
                    ...
                    fluxDiff = trafoTFC(i, j, k, fluxDiffU(0, 0), &
                               fluxDiffU(0, 1), fluxDiffU(1, 0), &
                               fluxDiffU(1, 1), fluxDiffV(0, 0), &
                               fluxDiffV(0, 1), fluxDiffV(1, 0), &
                               fluxDiffV(1, 1), fluxDiff, "tfc")
                end if
                ...
                if (mmp_mod == "tot") then
                   ...
                   if(topography) then
                       ...
                       piREdgeU = 0.5 * (jac(i + 1, j, k) &
                                  * met(i + 1, j, k, 3, 1) &
                                  * var(i + 1, j, k, 5) + jac(i + 1, j, k + 1) &
                                  * met(i + 1, j, k + 1, 3, 1) &
                                  * var(i + 1, j, k + 1, 5))
                       piLEdgeU = 0.5 * (jac(i - 1, j, k) &
                                  * met(i - 1, j, k, 3, 1) &
                                  * var(i - 1, j, k, 5) + jac(i - 1, j, k + 1) &
                                  * met(i - 1, j, k + 1, 3, 1) &
                                  * var(i - 1, j, k + 1, 5))
                       piFEdgeU = 0.5 * (jac(i, j + 1, k) &
                                  * met(i, j + 1, k, 3, 2) &
                                  * var(i, j + 1, k, 5) + jac(i, j + 1, k + 1) &
                                  * met(i, j + 1, k + 1, 3, 2) &
                                  * var(i, j + 1, k + 1, 5))
                       piBEdgeU = 0.5 * (jac(i, j - 1, k) &
                                  * met(i, j - 1, k, 3, 2) &
                                  * var(i, j - 1, k, 5) + jac(i, j - 1, k + 1) &
                                  * met(i, j - 1, k + 1, 3, 2) &
                                  * var(i, j - 1, k + 1, 5))
                       ...
                       chris23EdgeU = 0.5 * (pStratTFC(i, j, k) &
                                      * chris(i, j, k, 2, 3) &
                                      * met(i, j, k, 2, 3) * var(i, j, k, 5) &
                                      + pStratTFC(i, j, k + 1) &
                                      * chris(i, j, k + 1, 2, 3) &
                                      * met(i, j, k + 1, 2, 3) &
                                      * var(i, j, k + 1, 5))
                       ! Compute pressure gradient component.
                       piGrad = kappaInv * MaInv2 * pEdgeU &
                                * ((piREdgeU - piLEdgeU) * 0.5 / dx &
                                + (piFEdgeU - piBEdgeU) * 0.5 / dy &
                                + (jac(i, j, k + 1) * met(i, j, k + 1, 3, 3) &
                                * var(i, j, k + 1, 5) - jac(i, j, k) &
                                * met(i, j, k, 3, 3) * var(i, j, k, 5)) / dz) &
                                + kappaInv * MaInv2 * (chris11EdgeU &
                   ...
                   if (TestCase == "baroclinic_LC") then
                      ...
                      if(topography) then
                          ! TFC FJ
                          volForce = volForce + FrInv2 * drho_e / (0.5 &
                ...
                if(topography .and. mmp_mod == "lhs") then
                    ...
                    if(testCase == "SkamarockKlemp94") then
                        ...
                        vC = vC - 0.5 * (var_env(i, j, k, 3) &
                             + var_env(i, j - 1, k, 3))
                        vU = vU - 0.5 * (var_env(i, j, k + 1, 3) &
                             + var_env(i, j - 1, k + 1, 3))
                        uC = uC - 0.5 * (var_env(i, j, k, 2) &
                             + var_env(i - 1, j, k, 2))
                        uU = vU - 0.5 * (var_env(i, j, k + 1, 2) &
                             + var_env(i - 1, j, k + 1, 2))
                    end if
                    volForce = volForce + 0.5 * f_cor_nd(j) &
                               * (met(i, j, k, 1, 3) * (rhoOld(i, j, k) &
                               + rhoStratTFC(i, j, k)) * vC &
                               + met(i, j, k + 1, 1, 3) * (rhoOld(i, j, k + 1) &
                               + rhoStratTFC(i, j, k + 1)) * vU) - 0.5 &
                               * f_cor_nd(j) * (met(i, j, k, 2, 3) &
                               * (rhoOld(i, j, k) + rhoStratTFC(i, j, k)) * uC &
                               + met(i, j, k + 1, 2, 3) * (rhoOld(i, j, k + 1) &
                               + rhoStratTFC(i, j, k + 1)) * uU)
                end if
                ...
                do i = 1,nx
                   ...
                   if(topography) then
                       ...
                       piGrad = kappaInv * MaInv2 * pEdgeU / rhow &
                                ...
                                * met(i, j, k, 3, 3) &
                                * var(i, j, k, 5)) / dz) &
                                + kappaInv * MaInv2 / rhow * (chris11EdgeU &
                                + chris22EdgeU + 2.0 * (chris13EdgeU &
                                + chris23EdgeU))
                       if(testCase == "baroclinic_LC") then
                           var(:, :, :, 5) = var(:, :, :, 5) &
                   ...
                   if (TestCase == "baroclinic_LC")then !FS
                       if(topography) then
                           ...
                           chris23EdgeU = 0.5 * (pStratTFC(i, j, k) &
                                          ...
                                          * var_env(i, j, k + 1, 5))
                           ! Compute pressure gradient component.
                           piGrad = piGrad + kappaInv * MaInv2 * (pEdgeU &
                                    / rhow - pEdgeU / rhow_e) &
                                    * ((piREdgeU - piLEdgeU) * 0.5 / dx &
                                    + (piFEdgeU - piBEdgeU) * 0.5 / dy &
                                    + (jac(i, j, k + 1) &
                                    * met(i, j, k + 1, 3, 3) &
                                    * var_env(i, j, k + 1, 5) - jac(i, j, k) &
                                    * met(i, j, k, 3, 3) &
                                    * var_env(i, j, k, 5)) / dz) &
                                    + kappaInv * MaInv2 * (1.0 / rhow - 1.0 &
                                    / rhow_e) * (chris11EdgeU + chris22EdgeU &
                   ...
                   if (TestCase == "baroclinic_LC") then
                       if(topography) then
                           ...
                           buoy = - g_ndim * 0.5 * (rhopOld(i, j, k) / rho000 &
                                  / jac(i, j, k) - var_env(i, j, k, 6) &
                                  / rho000_e / jac(i, j, k) &
                                  + rhopOld(i, j, k + 1) / rho001 &
                                  / jac(i, j, k + 1) - var_env(i, j, k + 1, 6) &
                                  / rho001_e / jac(i, j, k + 1))
                       else
                           buoy  &
                           = - g_ndim &
                             * 0.5 &
                   ...
                   if (spongeLayer) then
                       if(topography .and. spongeTFC) then
                           ! TFC FJ
                           wAst = wAst - dt * 0.5 * (alphaTFC(i, j, k) &
                                  + alphaTFC(i, j, k + 1)) * wvert
                       else
                           wAst = wAst - dt * 0.5 * (kr_sp_w(j, k) &
                                  + kr_sp_w(j, k + 1)) * wvert
                       end if
                   end if
          ...
          do k = k0,k1
             ...
             do j = 1,ny
                do i = 1,nx
                   ...
                   if(topography) then
                       if(testCase == "baroclinic_LC") then
                           buoy = - g_ndim * 0.5 * (var(i, j, k, 6) / rho000 &
                                  / jac(i, j, k) - var_env(i, j, k, 6) &
                                  / rho000_e / jac(i, j, k) &
                                  + var(i, j, k + 1, 6) / rho001 &
                                  / jac(i, j, k + 1) - var_env(i, j, k + 1, 6) &
                                  / rho001_e / jac(i, j, k + 1))
                       else
                           buoy = - g_ndim * 0.5 * (var(i, j, k, 6) / rho000 &
                                  / jac(i, j, k) + var(i, j, k + 1, 6) &
                                  / rho001 / jac(i, j, k + 1))
                       end if
                   end if
                   ...
                   if(topography) then
                       ...
                       uC = 0.5 * (var(i, j, k, 2) &
                            + var(i - 1, j, k, 2))
                       uU = 0.5 * (var(i, j, k + 1, 2) &
                            + var(i - 1, j, k + 1, 2))
                       vC = 0.5 * (var(i, j, k, 3) &
                            + var(i, j - 1, k, 3))
                       vU = 0.5 * (var(i, j, k + 1, 3) &
                            + var(i, j - 1, k + 1, 3))
                       wAst = 1.0 / (facw + rhoStratEdgeU / rhow &
                              * bvsstw * dt ** 2.0) * (wvert - dt &
                              * piGrad + dt * buoy + rhoStratEdgeU &
                              / rhow * bvsstw * dt ** 2.0 * (0.5 &
                              * (met(i, j, k, 1, 3) * uC &
                              + met(i, j, k + 1, 1, 3) * uU) + 0.5 &
                              * (met(i, j, k, 2, 3) * vC &
                              + met(i, j, k + 1, 2, 3) * vU)))
                   else
                       if (TestCase == "baroclinic_LC") then
          ...
          do k = 1,nz
             do j = 1,ny
                do i = 1,nx
                   ...
                   if(topography) then
                       ! TFC FJ
                       wvrt = 0.5 * (vertWindTFC(i, j, k, var) &
                   ...
                   if(topography) then
                       ...
                       F &
                       = - fluxDiff &
                         + PStrat(k)/PStrat_0(k) * rhoStrat(k)/g_ndim &
                         * bvsStrat(k)*wvrt &
                   ...
                   do i = 1,nx
                      ...
                      if( fluctuationMode ) then
                          if(topography) then
                              ...
                              rhow = rhow + 0.5 * (rhoStratTFC(i, j, k) &
                                     + rhoStratTFC(i, j, k + 1))
                      ...
                      if(topography) then
                          ...
                          piLEdgeU = 0.5 * (jac(i - 1, j, k) &
                                     ...
                                     * met(i - 1, j, k + 1, 1, 3) &
                                     * var(i - 1, j, k + 1, 5))
                          piREdgeD = 0.5 * (jac(i + 1, j, k) &
                                     * met(i + 1, j, k, 1, 3) &
                                     * var(i + 1, j, k, 5) &
                                     + jac(i + 1, j, k - 1) &
                                     * met(i + 1, j, k - 1, 1, 3) &
                                     * var(i + 1, j, k - 1, 5))
                          piLEdgeD = 0.5 * (jac(i - 1, j, k) &
                                     * met(i - 1, j, k, 1, 3) &
                                     * var(i - 1, j, k, 5) &
                                     + jac(i - 1, j, k - 1) &
                                     * met(i - 1, j, k - 1, 1, 3) &
                          ...
                          piBEdgeU = 0.5 * (jac(i, j - 1, k) &
                                     ...
                                     * met(i, j - 1, k + 1, 2, 3) &
                                     * var(i, j - 1, k + 1, 5))
                          piFEdgeD = 0.5 * (jac(i, j + 1, k) &
                                     * met(i, j + 1, k, 2, 3) &
                                     * var(i, j + 1, k, 5) &
                                     + jac(i, j + 1, k - 1) &
                                     * met(i, j + 1, k - 1, 2, 3) &
                                     * var(i, j + 1, k - 1, 5))
                          piBEdgeD = 0.5 * (jac(i, j - 1, k) &
                                     * met(i, j - 1, k, 2, 3) &
                                     * var(i, j - 1, k, 5) &
                                     + jac(i, j - 1, k - 1) &
                                     * met(i, j - 1, k - 1, 2, 3) &
                          ...
                          piGradZEdgeU = kappaInv * MaInv2 * pEdgeU / rhow &
                                         ...
                                         * met(i, j, k, 3, 3) &
                                         * var(i, j, k, 5)) / dz) &
                                         + kappaInv * MaInv2 / rhow &
                                         * (chris11EdgeU &
                                         + chris22EdgeU + 2.0 &
                                         * (chris13EdgeU &
                                         + chris23EdgeU))
                          piGradZEdgeD = kappaInv * MaInv2 * pEdgeD &
                                         / rhowm &
                                         * (0.5 * (piREdgeD &
                                         - piLEdgeD) / dx + 0.5 &
                                         * (piFEdgeD - piBEdgeD) &
                          ...
                          if(testCase == "baroclinic_LC") then
                              ...
                              chris23EdgeD = 0.5 * (pStratTFC(i, j, k) &
                                             ...
                                             * var_env(i, j, k - 1, 5))
                              ! Compute pressure gradients.
                              piGradZEdgeU = kappaInv * MaInv2 * (pEdgeU &
                                             / rhow - pEdgeU / rhow_e) &
                                             * (0.5 * (piREdgeU &
                                             - piLEdgeU) / dx + 0.5 &
                                             * (piFEdgeU - piBEdgeU) &
                              ...
                              ! Interpolate.
                              piGrad = piGrad + 0.5 * (piGradZEdgeU &
                                       + piGradZEdgeD)
                          end if
                      else
                          if (TestCase == "baroclinic_LC") then
                              ...
                              = 0.5*(var_env(i,j,k,1) + var_env(i,j,k+1,1))
                              rhowm_e &
                              = 0.5*(var_env(i,j,k-1,1) + var_env(i,j,k,1))

                              if( fluctuationMode ) then
                                  rhow_e &
                                  =   rhow_e &
                                    + 0.5*(rhoStrat_0(k) + rhoStrat_0(k+1))
                                  rhowm_e &
                                  = rhowm_e &
                                    + 0.5*(rhoStrat_0(k-1) + rhoStrat_0(k))
                             end if

                             piGrad &
                             = piGrad &
                               + kappaInv*MaInv2 &
                                 * 0.5*(  (pstw/rhow - pstw_e/rhow_e) &
                                          * (  var_env(i,j,k+1,5) &
                                             - var_env(i,j,  k,5))/dz &
                                        + (pstwm/rhowm - pstwm_e/rhowm_e) &
                                          * (  var_env(i,j,k  ,5) &
                                             - var_env(i,j,k-1,5))/dz)
                          else
                              piGrad &
                      ...
                      if (TestCase == "baroclinic_LC") then
                          ...
                          if(topography) then
                              ...
                              buoy = - g_ndim * (var(i, j, k, 6) / rho &
                                     - var_env(i, j, k, 6) / rho_e)
                              buoy = 1.0 / (facw + rhoStratTFC(i, j, k) / rho &
                                     * bvsStratTFC(i, j, k) * dt ** 2.0) &
                                     * (- rhoStratTFC(i, j, k) / rho &
                                     * bvsStratTFC(i, j, k) * dt &
                                     * jac(i, j, k) * (wvrt - dt * piGrad) &
                                     + facw * buoy + rhoStratTFC(i, j, k) &
                                     / rho * bvsStratTFC(i, j, k) * dt &
                                     * jac(i, j, k) * facw * 0.5 &
                                     * (met(i, j, k, 1, 3) * (var(i, j, k, 2) &
                                     + var(i - 1, j, k, 2)) &
                                     + met(i, j, k, 2, 3) * (var(i, j, k, 3) &
                                     + var(i, j - 1, k, 3))))
                          else
                              buoy &
                              = 1.0 &
                   ...
                   do i = 1,nx
                      ...
                      if(topography) then
                          ...
                          = - g_ndim * rhop/rho &
                            ...
                            - dt * (  rhoStrat(k)/rho * PStrat(k)/PStrat_0(k) &
                            * bvsStrat(k)*wvrt &
                            + g_ndim/rho &
                            !* rhoStrat(k)/Pstrat(k) * heat_flc)
                            * rhoStrat(k)/Pstrat_0(k) * heat_flc)
    ...
    do k = 1,nz
       do j = 1,ny
          do i = 1,nx
             ...
             if(topography) then
                stop "update.f90: topography not implemented in tracerUpdate"
             end if
             ...
             ! update density
             var(i,j,k,iVart) = var(i,j,k,iVart) + beta(m) * q(i,j,k) / rho(i,j,k)

             ! avoid negative values for tracer
             if ((var(i,j,k,iVart).lt. 0.0)) then
                var(i,j,k,iVart) = 0.0
             end if
          end do
       end do
    ...
    call setBoundary (var,flux,"var")
     ...
     do k = 1,nz !theta
       do j = 1,ny
          do i = 1,nx
             var(i,j,k,1) &
                  = (var(i,j,k,1)-rhoStrat(k))/PStrat(k) !- dens_env_pp(i, j, k) + rhoStrat(k)
...
!---------------------------------------------------------------------
  ...
  ! w0_mod = 'Almgrenetal08'
    ...
    if (w0_mod == 'Almgrenetal08') then
       ...
       call mpi_allreduce(sum_local2(1),sum_global2(1),&
            nz-1+1,&
            mpi_double_precision,mpi_sum,comm,ierror)
    ...
    if (w0_mod == 'Almgrenetal08') then
       do k = 1,nz
          ...
          expo = exp(-g_ndim*(rho_bar(k)+rhoStrat_s(k)) / (gamma*press0(k)) * z(k))
    ...
    if (w0_mod == 'Almgrenetal08') then
       ...
       do k = 1,nz-1
          expo &
               = exp(g_ndim*(rho_bar(k)+rhoStrat_s(k)) / (gamma*press0(k)) &
               * 0.5*(z(k) + z(k+1)))
    ...
    do k = 1,nz
       ...
       if (PStrat(k) <= 0.) then
          print*,'ERROR in BGstate_update: PStrat(',k,') =',PStrat(k), &
                & '<=0.'
          stop
       end if
    ...
    do k = 1,nz
       if (k == 1) then
          bvsStrat(k) &
          = g_ndim/thetaStrat(k) * (thetaStrat(k+1) - thetaStrat(k))/dz
         else if (k == nz) then
          bvsStrat(k) &
          = g_ndim/thetaStrat(k) * (thetaStrat(k) - thetaStrat(k-1))/dz
         else
          bvsStrat(k) &
          = g_ndim/thetaStrat(k) &
            * (thetaStrat(k+1) - thetaStrat(k-1))/(2.0 * dz)
  ...
  subroutine momentumPredictorTestTFC(var, flux, force, dMom, int_mod)
      ...
      real, dimension((- 1):nx, (- 1):ny, (- 1):nz, 3, nVar), &
      intent(in) :: flux
      real, dimension(0:(nx + 1), 0:(ny + 1), 0:(nz + 1), 3), &
      intent(in) :: force
      real, dimension((- nbx):(nx + nbx), (- nby):(ny + nby), &
      ...
      if(int_mod == "expl") then
          ...
          "expl", 1.0)
          topography = .false.
          call momentumPredictor(var, flux, force, 1.0, dMom, 1, "tot", &
          "expl", 1.0)
          topography = .true.
          print*, "Momentum predictor difference: ", &
          ...
          "expl", 1.0)
          topography = .false.
          call momentumPredictor(var, flux, force, 1.0, dMom, 1, "lhs", &
          "expl", 1.0)
          topography = .true.
          ...
          "impl", 1.0)
          topography = .false.
          call momentumPredictor(var, flux, force, 1.0, dMom, 1, "rhs", &
          "impl", 1.0)
          topography = .true.
  ...
  subroutine massUpdateTestTFC(var, flux, dRho, int_mod)
      ...
      if(int_mod == "expl") then
          call massUpdate(var_tfc, flux, 1.0, dRho, 1, "rho", "tot", &
          "expl", 1.0)
          call massUpdate(var_tfc, flux, 1.0, dRho, 1, "rhop", "tot", &
          "expl", 1.0)
          topography = .false.
          call massUpdate(var, flux, 1.0, dRho, 1, "rho", "tot", &
          "expl", 1.0)
          call massUpdate(var, flux, 1.0, dRho, 1, "rhop", "tot", &
          "expl", 1.0)
          topography = .true.
          print*, "Mass update difference: ", &
          maxval(abs(var_tfc - var))
      else if(int_mod == "impl") then
          call massUpdate(var_tfc, flux, 1.0, dRho, 1, "rho", "lhs", &
          "expl", 1.0)
          call massUpdate(var_tfc, flux, 1.0, dRho, 1, "rhop", "lhs", &
          "expl", 1.0)
          topography = .false.
          call massUpdate(var, flux, 1.0, dRho, 1, "rho", "lhs", &
          "expl", 1.0)
          call massUpdate(var, flux, 1.0, dRho, 1, "rhop", "lhs", &
          "expl", 1.0)
          topography = .true.
          print*, "Mass update difference (lhs, expl): ", &
          maxval(abs(var_tfc - var))

          var_tfc = var
          wOldTFC = var(:, :, :, 4)
          call massUpdate(var_tfc, flux, 1.0, dRho, 1, "rhop", "rhs", &
          "impl", 1.0)
          topography = .false.
          call massUpdate(var, flux, 1.0, dRho, 1, "rhop", "rhs", &
          "impl", 1.0)
          topography = .true.
          print*, "Mass update difference (rhs, impl): ", &
          maxval(abs(var_tfc - var))
          ...
          "expl", 1.0)
          topography = .true.
          print*, "Mass update difference (rhs, expl): ", &
          maxval(abs(var_tfc - var))
      end if
