module ice_sub_module
  ...
  subroutine output_ice(i, j, k, iVar, var, field_prc)
    ...
    if(master .and. i == 1 .and. j == 1 .and. k == 1 .and. iVar == inN) then
      ...
      print "(a)", repeat("-", 80)
      print *, ""
      print *, "max value N in master =", maxval(real(var%ICE(:, :, :, inN)))
      print *, "max value Q in master =", maxval(real(var%ICE(:, :, :, inQ)))
      print *, "max value Qv in master =", maxval(real(var%ICE(:, :, :, inQv)))
      print *, ""
    end if
    ...
    if(model == "pseudo_incompressible") then
      ...
      print *, ' iceSources works only with model == pseudo_incompressible '
      stop
    ...
    if(iVar .eq. inN) then
      ...
      field_prc(i, j) = real(var%ICE(i, j, k, iVar) / rho / mRef, kind = 4) ! N_v = \rho n
    ...
    elseif(iVar .eq. inQv) then
      ...
      field_prc(i, j) = real(var%ICE(i, j, k, iVar) / rho, kind = 4) !Q_v = \rho q_v
      ...
      print *, 'wrong value for iVar in output_ice in ice_sub.f90'
      !field_prc(i, j) = real(var(i, j, k, iVar), kind = 4)
