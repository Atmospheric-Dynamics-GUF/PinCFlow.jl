module output_netCDF_module
  ...
  subroutine create_netCDF
    ...
    if(compute_cloudcover) then
      ...
      ! z axis
      if(topography) then
        call handle_err(nf90_def_var(ncid, 'z2', nf90_float, [dimidx2, &
            &dimidy2, dimidz2], ncidz2))
      else
  ...
  subroutine write_netCDF_background(iOutput)
    ...
    if(iOutput == 0) then
      ...
      if(compute_cloudcover) then
        ...
        if(topography) then
          call handle_err(nf90_put_var(ncid, ncidz2, z2TFC(1:sizeX2, 1:ny &
              &* sizeY2, 1:sizeZ2) * lRef, start = [1], count = countNC2))
        else
          call handle_err(nf90_put_var(ncid, ncidz2, z2(1:sizeZ2) * lRef, &
              &start = [1], count = [sizeZ2]))
        end if
      end if
  ...
  subroutine write_netCDF(iOutput, iTime, time, cpuTime, var, ray_var3D, &
    ...
    integer :: i, j, k, isc, jsc, ksc, ii, jj, kk
            ...
            do jsc = 1, NSCY
              ...
              do i = 1, nx
                do isc = 1, NSCX
                  ...
                  sc_ice_field(ii, jj, kk)%Ni = ray_cloud(ii, jj, kk)%Ni &
                      &/ rho(i, j, k)
                  ...
                  !    &/ rho(i, j, k)
                  !w
                  sc_ice_field(ii, jj, kk)%Qv = ray_cloud(ii, jj, kk)%wwp * uref

                  !Si
                  sc_ice_field(ii, jj, kk)%Qi = ray_cloud(ii, jj, kk)%si

                  !theta'
                  !sc_ice_field(ii, jj, kk)%Qi = ray_cloud(ii, jj, kk)%thp &
                  !    &* thetaRef
                  !pi'
                  !sc_ice_field(ii, jj, kk)%Qv = ray_cloud(ii, jj, kk)%epp
                end do
              end do
            end do
          end do
