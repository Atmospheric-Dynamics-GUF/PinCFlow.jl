...
contains
  ...
  subroutine SetUpHypre
    ...
    call HYPRE_StructGridSetExtents(grid_hypre, &
            & (/ (icoord)*nx+1, (jcoord)*ny+1 , 1 /), &
            & (/ (icoord+1)*nx , (jcoord+1)*ny , nz /), ierr_hypre)

    call HYPRE_StructGridSetPeriodic(grid_hypre,&
            & (/npts_x_periodic_hypre, npts_y_periodic_hypre, &
            & npts_z_periodic_hypre/),ierr_hypre)
    ...
    if (timeScheme == "semiimplicit") then
       ...
       do ihp=0,10
          call HYPRE_StructStencilSetElement( stencil_i, ihp, &
                  & offsets_i( (ihp+1), : ), ierr_hypre)
       end do
      else
       offsets_e(1,:) = (/ 0, 0, 0/)
       offsets_e(2,:) = (/-1, 0, 0/)
       ...
       do ihp=0,6
          call HYPRE_StructStencilSetElement( stencil_e, ihp, &
                  & offsets_e( (ihp+1), : ), ierr_hypre)
       end do
    end if
    ...
    if (timeScheme == "semiimplicit") then
       call HYPRE_StructMatrixCreate(mpi_comm_world, grid_hypre, &
                                   & stencil_i, A_hp_i, ierr_hypre)
      else
       call HYPRE_StructMatrixCreate(mpi_comm_world, grid_hypre, &
                                   & stencil_e, A_hp_e, ierr_hypre)
    end if
    ...
    if (timeScheme == "semiimplicit") then
       call HYPRE_StructHybridCreate(mpi_comm_world, solver_hp_i, &
                                   & ierr_hypre)
      else
       call HYPRE_StructHybridCreate(mpi_comm_world, solver_hp_e, &
                                   & ierr_hypre)
    end if
    ...
    if (timeScheme == "semiimplicit") then
       call HYPRE_StructHybridSetConvergenc(solver_hp_i, tolCond, &
                                          & ierr_hypre)
      else
       call HYPRE_StructHybridSetConvergenc(solver_hp_e, tolCond, &
                                          & ierr_hypre)
    end if
  ...
  subroutine test_hypre
    ...
    integer ierr_t
    integer npts_x_periodic_t, npts_y_periodic_t, &
            npts_z_periodic_t
    integer ihp
    ...
    call HYPRE_StructGridSetPeriodic(grid_t,&
            & (/npts_x_periodic_t, npts_y_periodic_t, &
            & npts_z_periodic_t/),ierr_t)

    ! Add boxes to the grid
    call HYPRE_StructGridSetExtents(grid_t, &
            & (/ (icoord)*nx+1, (jcoord)*ny+1 , 1 /), &
            & (/ (icoord+1)*nx , (jcoord+1)*ny , nz /), ierr_t)
    ...
    do ihp=0,10
    !do ihp=0,6
       call HYPRE_StructStencilSetElement( stencil_t, ihp, &
                                         & offsets_t( (ihp+1), : ), ierr_t)
    end do
    ...
    call HYPRE_StructMatrixCreate(mpi_comm_world, grid_t, &
                                & stencil_t, A_hp_t, ierr_t)
    ...
    call HYPRE_StructHybridSetup(solver_hp_t, A_hp_t, b_hp_t, x_hp_t, &
                               & ierr_t)
    ...
    call HYPRE_StructHybridSolve(solver_hp_t, A_hp_t, b_hp_t, x_hp_t, &
                               & ierr_t)
    ...
    call HYPRE_StructHybridGetNumIterati( solver_hp_t, nIter, &
                                        & ierr_t )
