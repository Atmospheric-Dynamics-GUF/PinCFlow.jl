             ...
             do j = 1, ny
                ...
                do i = 1, nx
                          ...
                          select case(model)
                             case( "pseudo_incompressible" )
                                 if(topography) then
                                     ! TFC FJ
                                     var(i, j, k, iVar) &
                                     = (pStratTFC(i, j, k) / field_prc(i, j)) &
                                       / rhoRef - rhoStratTFC(i, j, k)
                                 else
                                     var(i,j,k,iVar) &
                                     = (Pstrat(k)/field_prc(i,j))/rhoRef &
                                       - rhoStrat(k)
                                 end if
                           ...
                           case( "Boussinesq" )
                             ...
                             ! Boussinesq: density fluctuations are stored in
                             ! var(:, :, :, 6)!
                             var(i, j, k, iVar) = - field_prc(i, j) / theta00 &
                                                  * rho00 / thetaRef
                  ...
                  !---------------------------------
                           ...
                           stop "tec360: unknown iVar in read_data"
                         end if
                   end select ! iVar
                end do ! i
             end do ! j
