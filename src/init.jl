...
function initialize_values(
    ...
    u = OffsetArray(
        zeros(Float64, nx + 1 + 2nbx, ny + 1 + 2nby, nz + 1 + 2nbz),
        -nbx:nx+nbx,
        -nby:ny+nby,
        -nbz:nz+nbz,
    )
    ...
    ndim = 3
    flux_u = OffsetArray(
        zeros(Float64, nx + 1 + 2nbx, ny + 1 + 2nby, nz + 1 + 2nbz, ndim),
        -nbx:nx+nbx,
        -nby:ny+nby,
        -nbz:nz+nbz,
        1:ndim,
    ...
    flux_v, flux_w, flux_rho, flux_exner, flux_rhop =
        (copy(flux_u), copy(flux_u), copy(flux_u), copy(flux_u), copy(flux_u))
    ...
    rhoTilde = OffsetArray(
        zeros(Float64, nx + 1 + 2nbx, ny + 1 + 2nby, nz + 1 + 2nbz, ndim, 2),
        -nbx:nx+nbx,
        -nby:ny+nby,
        -nbz:nz+nbz,
        1:ndim,
    ...
    uTilde, vTilde, wTilde, rhopTilde =
        (copy(rhoTilde), copy(rhoTilde), copy(rhoTilde), copy(rhoTilde))
    ...
    s_aux_field_lin_opr = OffsetArray(s_aux_field_lin_opr_, 0:nx+1, 0:ny+1, 0:nz+1)
