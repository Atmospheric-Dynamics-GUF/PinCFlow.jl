...
!----------------------------------------------------------------------------------
  ...
  subroutine setHalos( var, option )
    ...
    case( "var" )
     ...
     !---------------------------------------------
     ! tracer copied from ice
     !---------------------------------------------
     if ( include_tracer .and. updateTracer ) then
          ! slice size
          sendcount = nbx*(ny+2*nby+1)*nz
          recvcount = sendcount
          ...
          do i = 1,nbx
             xSliceLeft_send (i,-nby:ny+nby,1:nz) &
           & = var(i,-nby:ny+nby,1:nz,iVart)
             xSliceRight_send(i,-nby:ny+nby,1:nz) &
           & = var(nx-nbx+i,-nby:ny+nby,1:nz,iVart)
          end do
          ...
          if ( idim > 1 ) then
             ...
             do i = 1,nbx
                ! right halos
                var(nx+i,-nby:ny+nby,1:nz,iVart) &
              & = xSliceRight_recv(i,-nby:ny+nby,1:nz)

                ! left halos
                var(-nbx+i,-nby:ny+nby,1:nz,iVart) &
              & = xSliceLeft_recv(i,-nby:ny+nby,1:nz)
     ...
     if ( include_tracer .and. updateTracer ) then
          ...
          do j = 1, nby
             ySliceBack_send(-nbx:nx+nbx,j,1:nz) &
           & = var(-nbx:nx+nbx,j,1:nz,iVart)
             ySliceForw_send(-nbx:nx+nbx,j,1:nz) &
           & = var(-nbx:nx+nbx,ny-nby+j,1:nz,iVart)
          end do
          ...
          if( jdim > 1 ) then
             ...
             do j = 1, nby
              ...
              & = ySliceForw_recv(-nbx:nx+nbx,j,1:nz)
                ...
                ! left halos
                var(-nbx:nx+nbx,-nby+j,1:nz,iVart) &
              & = ySliceBack_recv(-nbx:nx+nbx,j,1:nz)
       ...
       !if( updateMass ) then
          ...
          if( idim > 1 ) then
             ...
             call mpi_sendrecv(x1SliceRight_send(1,1), sendcount, &
                  & mpi_double_precision, dest, tag, &
                  & x1SliceLeft_recv(1,1), recvcount, &
                  & mpi_double_precision, &
                  & source, mpi_any_tag, comm, sts_left, ierror)
             ...
             call mpi_sendrecv(x1SliceLeft_send(1,1), sendcount, &
                  & mpi_double_precision, dest, tag, &
                  & x1SliceRight_recv(1,1), recvcount, &
                  & mpi_double_precision, &
                  & source, mpi_any_tag, comm, sts_right, ierror)
       ...
       !if( updateMass ) then
          ...
          if( jdim > 1 ) then
             ...
             call mpi_sendrecv(y1SliceForw_send(1,1), sendcount, &
                  & mpi_double_precision, dest, tag, &
                  & y1SliceBack_recv(1,1), recvcount, &
                  & mpi_double_precision, &
                  & source, mpi_any_tag, comm, sts_back, ierror)
             ...
             call mpi_sendrecv(y1SliceBack_send(1,1), sendcount, &
                  & mpi_double_precision, dest, tag, &
                  & y1SliceForw_recv(1,1), recvcount, &
                  & mpi_double_precision, &
                  & source, mpi_any_tag, comm, sts_right, ierror)
