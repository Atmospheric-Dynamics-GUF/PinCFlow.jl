...
levels = np.linspace(uLS.min(), uLS.max())
colormap = "seismic"
...
levels = np.linspace(vLS.min(), vLS.max())
...
levels = np.linspace(wLS.min(), wLS.max())  
...
levels = np.linspace(tLS.min(), tLS.max())
pplot11 = axes[1, 1].contourf(xx, zz, tLS, levels, cmap = colormap)
fig.colorbar(pplot11, ax = axes[1, 1])
plt.tight_layout()
fig.savefig("../results/2DWP90_msgwam.pdf")

 
# Create difference plot...
if (not (tLS == tLSref).all() or not (uLS == uLSref).all() or not (vLS == vLSref).all() or not (wLS == wLSref).all()):
  fig, axes = plt.subplots(nrows = 2, ncols = 2, figsize = (8, 6))
  ...
  axes[0, 0].set_title(r'Large-scale zonal wind $u \, \left[\mathrm{m \,' \
      r's^{- 1}}\right]$')
  levels = np.linspace(deltau.min(), deltau.max())
  ...
  fig.colorbar(pplot01, ax = axes[0, 1])
  levels = np.linspace(deltav.min(), deltav.max())
  axes[1, 0].set_title(r'Large-scale vertical wind $w \, \left[\mathrm{m \,' \
      r's^{- 1}}\right]$')
  ...
  levels = np.linspace(deltat.min(), deltat.max())
  pplot11 = axes[1, 1].contourf(xx, zz, deltat, levels, cmap = colormap)
  fig.colorbar(pplot11, ax = axes[1, 1])
  plt.tight_layout()
  fig.savefig("../results/2DWP90_msgwam_difference.pdf")
