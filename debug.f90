  ...
  subroutine checkSymm1D(phi, nx)
    ...
    do m = 1, nx/2
       ...
       if( identic(phiL, phiR) ) then 
          ...
          print*,"i_left, i_right,j, k, dphi: ",nx/2-m+1,nx/2+m,dphi
          symmetric = .false.
       end if
...
! ----------------------------------------------------------
  ...
  subroutine checkSymmTilde1D(phiTilde, nx)
    ...
    print*,"debug.f90/checkSymmTilde1D: monitoring phiTilde at nx/2 left and nx/2+1 right:", &
         & nx/2, nx/2+1, &
         & phiTilde(nx/2,-1), phiTilde(nx/2+1,1)
    ...
    do m = 1, nx/2
       do lambda = -1,1,2
          ...
          if( identic(phiL,phiR) ) then 
             ...
             print*,"i_left, i_right,j, k, dphi: ",nx/2-m+1,nx/2+m,dphiTilde
             symmetric = .false.
          end if
...
! ----------------------------------------------------------
  ...
  subroutine initSymmTilde(phiTilde)
    ...
    ! warning
    print*,"debug.f90/initSymmTilde: phiTilde is reset randomly! collocated = ", collocated
...
! ----------------------------------------------------------
  ...
  subroutine initSymmetry(var)
    ...
    ! make symmetric
    do m = 1, nx/2+nbx
       var(nx/2+m,:,:,1) = var(nx/2-m+1,:,:,1)   ! rho
       var(nx/2+m,:,:,3) = var(nx/2-m+1,:,:,3)   ! v
       var(nx/2+m,:,:,4) = var(nx/2-m+1,:,:,4)   ! w
       var(nx/2+m,:,:,5) = var(nx/2-m+1,:,:,5)   ! pi'

       ! u: staggered and antisymmetric
       var(nx/2+m,:,:,2) = -var(nx/2-m,:,:,2)   ! u
    end do
...
! ----------------------------------------------------------
  ...
  subroutine initSymmFlux(flux)
    ...
    ! make anti-symmetric
    do m = 1, nx/2
       flux(nx/2+m,:,:,1,1) = -flux(nx/2-m,:,:,1,1)   ! frho
       flux(nx/2+m,:,:,1,3) = -flux(nx/2-m,:,:,1,3)   ! fv
       flux(nx/2+m,:,:,1,4) = -flux(nx/2-m,:,:,1,4)   ! fw
    end do

    ! staggered u + symmetric because of rho*u^2
    do m = 0, nx/2
       flux(nx/2+m,:,:,1,2) = flux(nx/2-m-1,:,:,1,2)   ! fu
    end do
...
! ----------------------------------------------------------
  ...
  subroutine checkSymmTilde(phiTilde, pos)
    ...
    ! warning
    print*,"debug.f90/checkSymmTilde: checking phiTilde for symmetry at pos", pos 
             ...
             do m = 1, nx/2+1                   ! Note: symmetry only for 1. ghost cell assured
                do lambda = 0,1
                   ...
                   if( identic(phiL,phiR) ) then 
                      ...
                      print*,"i_left, i_right,j,k, dphi, phiL, phiR: ", &
                           & nx/2-m+1,nx/2+m,j,k,dphi, phiL, phiR
                      symmetric = .false.
                   end if
       ...
       do j = -1,ny
          ...
          do m = 1, nx/2
             ...
             if(dfrho .ne. 0.0) then 
                print*,"i_left, i_right,j,k, dfrho: ",nx/2-m,nx/2+m,j,k,dfrho
                symmetric = .false.
             end if

             dfv = flux(nx/2+m,j,k,1,3) + flux(nx/2-m,j,k,1,3)   ! fv
             if(dfv .ne. 0.0) then 
                print*,"i_left, i_right,j,k, dfv: ", nx/2-m,nx/2+m,j,k,dfv
                symmetric = .false.
             end if

             dfw = flux(nx/2+m,j,k,1,4) + flux(nx/2-m,j,k,1,4)   ! fw
             if(dfw .ne. 0.0) then 
                print*,"i_left, i_right,j,k, dfw: ", nx/2-m,nx/2+m,j,k,dfw
                symmetric = .false.
             end if
          ...
          do m = 0, nx/2
             ...
             if(dfu .ne. 0.0) then 
                print*,"i_left, i_right,j,k, dfu: ", nx/2-m-1,nx/2+m,j,k,dfu
                symmetric = .false.
             end if
          end do
       ...
       do j = -nby, ny+nby
          do m = 1, nx/2+nbx
             ...
             if( identic(rhoR,rhoL) ) then 
                ...
                symmetric = .false.
                print*,"i_left, i_right,j,k, drho: ",nx/2-m+1,nx/2+m,j,k,drho
             end if
             ...
             if( identic(vR,vL) ) then 
                ...
                print*,"i_left, i_right,j,k, dv: ", nx/2-m+1,nx/2+m,j,k,dv
                symmetric = .false.
             end if
             ...
             if( identic(wR, wL) ) then
                ...
                print*,"i_left, i_right,j,k, dw: ", nx/2-m+1,nx/2+m,j,k,dw
                symmetric = .false.
             end if
             ...
             if( identic(pL,pR) ) then
                ...
                print*,"i_left, i_right,j,k, dp: ", nx/2-m+1,nx/2+m,j,k,dp
                symmetric = .false.
             end if
             ...
             if( identic(uR,-uL) ) then
                ...
                print*,"i_left, i_right,j,k, du: ", nx/2-m,nx/2+m,j,k,du
                symmetric = .false.
             end if
