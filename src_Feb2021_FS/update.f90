...
!---------------------------------------------------------------------
  ...
  subroutine momentumPredictor_nc (var0,var,flux,force,dt,q,m,mmp_mod,int_mod, &
  !subroutine momentumPredictor (var,flux,force,dt,q,m,mmp_mod,int_mod, &
    ...
    !      f_cor_nd(j) = f_Coriolis_dim*tRef
    ...
    if (mmp_mod == "tot" .or. mmp_mod == "lhs") then
       ...
       if (int_mod == "expl") then
          do k = k0,k1
             ...
             do j = 1,ny
                do i = 1,nx
                   ...
                   if (spongeLayer) then
                      wAst = wAst - dt*0.5*(kr_sp(k)+kr_sp(k+1))*wvert
                   end if
          ...
          if (heatingONK14 .or. TurbScheme .or. rayTracer) then
             call heat_w0(var0,var,flux,dt,heat,S_bar,w_0,rhopw_bar)
            else
             heat = 0.
             S_bar = 0.
             w_0 = 0.
          ...
          do k = k0,k1
             ...
             do j = 1,ny
                do i = 1,nx
                   ...
                   if (spongeLayer) then
                      facw = facw + dt*0.5*(kr_sp(k)+kr_sp(k+1))
                   end if
...
!---------------------------------------------------------------------
  ...
  !UAC subroutine momentumPredictor (var,flux,force,dt,q,m,mmp_mod,int_mod)
  !subroutine momentumPredictor_wc (var,flux,force,dt,q,m,mmp_mod,int_mod, &
  subroutine momentumPredictor (var0,var,flux,force,dt,q,m,mmp_mod,int_mod, &
                              & facray)
    !----------------------------------
    !  calculates the velocities u^*
    !----------------------------------
             ...
             do j = 1,ny
                do i = i0,i1
                   ...
                   if (TestCase == "baroclinic_LC")then !FS
                      !UAC
                      !piGrad = piGrad + kappaInv*MaInv2 &
                      !     * (PStrat(k)/rhou - pStrat_0(k)/(rhoStrat_0(k)+0.5*(var_env(i,j,k,1)+var_env(i+1,j,k,1)))) &
                      !     * (var_env(i+1,j,k,5)- var_env(i,  j,k,5))/dx
                      piGrad &
                      = piGrad + kappaInv*MaInv2 &
                                 * (Pstrat(k)/rhou - Pstrat_0(k)/rhou_e) &
          ...
          do k = 1,nz
             do j = 1,ny
                do i = i0,i1
                   ...
                   !UAD
                   !if (TestCase == "baroclinic_LC") then !FS
                   !   piGrady = piGrady + kappaInv*MaInv2 &
                   !       * 0.25 &
                   !       * (  (Pstrat(k)/rhov0m - pStrat_0(k)/(rhoStrat_0(k)+0.5*(var_env(i,j,k,1)+var_env(i,j-1,k,1))))&
                   !          * (var_env(i,j,k,5) - var_env(i,j-1,k,5))/dy &
                   !        + (Pstrat(k)/rhov00- pStrat_0(k)/(rhoStrat_0(k)+0.5*(var_env(i,j+1,k,1)+var_env(i,j,k,1)))) &
                   !          * (var_env(i,j+1,k,5) - var_env(i,j,k,5))/dy &
                   !        + (Pstrat(k)/rhov1m - pStrat_0(k)/(rhoStrat_0(k)+0.5*(var_env(i+1,j,k,1)+var_env(i+1,j-1,k,1))))&
                   !          * (var_env(i+1,j,k,5) - var_env(i+1,j-1,k,5))/dy &
                   !        + (Pstrat(k)/rhov10 - pStrat_0(k)/(rhoStrat_0(k)+0.5*(var_env(i+1,j+1,k,1)+var_env(i+1,j,k,1))))&
                   !          * (var_env(i+1,j+1,k,5) - var_env(i+1,j,k,5))/dy)
                   !end if
             ...
             do j = j0,j1
                do i = 1,nx
                   ...
                   if (TestCase == "baroclinic_LC")then !FS
                      !UAC
                      !piGrad = piGrad + kappaInv*MaInv2 &
                      !     * (PStrat(k)/rhov - pStrat_0(k)/(rhoStrat_0(k)+0.5*(var_env(i,j,k,1)+var_env(i,j+1,k,1)))) &
                      !     * (var_env(i,j+1,k,5) - var_env(i,  j,k,5))/dy
                      piGrad &
                      = piGrad + kappaInv*MaInv2 &
                                 * (Pstrat(k)/rhov - Pstrat_0(k)/rhov_e) &
          ...
          do k = 1,nz
             do j = j0,j1
                do i = 1,nx
                   ...
                   !UAD
                   !if (TestCase == "baroclinic_LC") then !FS
                   !   piGradx = piGradx + kappaInv*MaInv2 &
                   !      * 0.25 &
                   !     * (  (Pstrat(k)/rhou00 - pStrat_0(k)/(rhoStrat_0(k)+0.5*(var_env(i+1,j,k,1)+var_env(i,j,k,1)))) &
                   !          * (var_env(i+1,j,k,5) - var_env(i,j,k,5))/dx &
                   !        + (Pstrat(k)/rhoum0 - pStrat_0(k)/(rhoStrat_0(k)+0.5*(var_env(i,j,k,1)+var_env(i-1,j,k,1))))&
                   !          * (var_env(i,j,k,5) - var_env(i-1,j,k,5))/dx &
                   !        + (Pstrat(k)/rhou01 - pStrat_0(k)/(rhoStrat_0(k)+0.5*(var_env(i+1,j+1,k,1)+var_env(i,j+1,k,1)))) &
                   !          * (var_env(i+1,j+1,k,5) - var_env(i,j+1,k,5))/dx &
                   !        + (Pstrat(k)/rhoum1 - pStrat_0(k)/(rhoStrat_0(k)+0.5*(var_env(i,j+1,k,1)+var_env(i-1,j+1,k,1)))) &
                   !          * (var_env(i,j+1,k,5) - var_env(i-1,j+1,k,5))/dx)  
                   !end if
                   ...
                   ! vAst &
                   ! = 1.0/(facu*facv + (0.5*(f_cor_nd(j)+f_cor_nd(j+1))*dt)**2) &
                   !   * (- 0.5*(f_cor_nd(j)+f_cor_nd(j+1))*dt &
                   !        * (uhorx + dt * (volfcx/rhov - piGradx)) &
                   !      + facu * (vhory + dt * (volfcy/rhov - piGrady)))
             ...
             do j = 1,ny
                do i = 1,nx
                   ...
                   if (TestCase == "baroclinic_LC")then !FS
                      !UAC
                      !piGrad = piGrad + kappaInv*MaInv2 &
                      !     * (pstw/rhow - (0.5*(pStrat_0(k)+pStrat_0(k+1)))/(0.5*(var_env(i,j,k,1)+rhoStrat_0(k)+var_env(i,j,k+1,1)+rhoStrat_0(k+1)))) &
                      !     * (var_env(i,j,k+1,5) - var_env(i,  j,k,5))/dz
                      piGrad &
                      =   piGrad &
                        + kappaInv*MaInv2 * (pstw/rhow - pstw_e/rhow_e) &
             ...
             do j = 1,ny
                do i = 1,nx
                   ...
                   if (TestCase == "baroclinic_LC") then !FS
                      ...
                      !piGrad = piGrad + kappaInv*MaInv2 &
                      !     * (pstw/rhow  &
                      !     - (0.5*(pStrat_0(k)+pStrat_0(k+1))/(0.5*(var_env(i,j,k,1)+rhoStrat_0(k)+var_env(i,j,k+1,1)+rhoStrat_0(k+1))))) &
                      !     * (var_env(i,j,k+1,5) - var_env(i,j,k,5))/dz
                      piGrad &
                      =   piGrad &
                        + kappaInv*MaInv2 * (pstw/rhow - pstw_e/rhow_e) &
...
!--------------------------------------------------------------------------
  ...
  subroutine massUpdate (var0,var,flux,dt,q,m,upd_var,upd_mod,int_mod,facray)
    !-----------------------------
             ...
             do k = 1,nz
                ...
                do j = 1,ny
                   do i = 1,nx
                      ...
                      !if (TestCase == "baroclinic_LC") then !FS
                      !   piGrad = piGrad + kappaInv*MaInv2 &
                      !        * 0.5*(  (pstw/rhow - (0.5*(pStrat_0(k)+pStrat_0(k+1))/(0.5*(var_env(i,j,k,1)+rhoStrat_0(k)+var_env(i,j,k+1,1)+rhoStrat_0(k+1))))) &
                      !        * (var_env(i,j,k+1,5)-var_env(i,j,k,5))/dz &
                      !        + ( pstwm/rhowm - (0.5*(pStrat_0(k)+pStrat_0(k-1))/(0.5*(var_env(i,j,k,1)+rhoStrat_0(k)+var_env(i,j,k-1,1)+rhoStrat_0(k-1))))) &
                      !        * (var_env(i,j,k,5)-var_env(i,j,k-1,5))/dz)
                      ...
                      !UAB
                      if (spongeLayer) facw = facw + dt*kr_sp(k)
                      !UAE
...
!---------------------------------------------------------------------
  ...
  subroutine BGstate_update(var0,var,flux,dt,m,q_P,q_rho,int_mod,heating_switch, init_switch)
  ...
  ! in/out variables
    ...
    integer, intent(in) :: heating_switch, init_switch

    real, dimension(-nbx:nx+nbx,-nby:ny+nby,-nbz:nz+nbz) :: heat
    real, dimension(-nbz:nz+nbz) :: S_bar, PStratold
    integer :: i,j,k
    real, dimension(1:nz) :: sum_local, sum_global, sum_local2, sum_global2
    real, dimension(1:nz+2) :: sum_local_init, sum_global_init
    real, dimension(-nbz:nz+nbz) :: press0, rhow_bar, rhopw_bar
    real, dimension(-nbz:nz+nbz) :: divPw, divrhow !UA

    real, dimension(-nbz:nz+nbz) :: w_0 !UA

    !UAB
    real, dimension(-nbz:nz+nbz) :: w_mean !UA
    !UAE
    ...
    if (init_switch == 0) then
       ...
       do k = 0,nz+1 
          do j = 1,ny
             do i = 1,nx
                ...
                the_env_pp(i,j,k) = Pstrat(k)/rho 
                sum_local_init(k+1) = sum_local_init(k+1) + (1./the_env_pp(i,j,k))
                     
             end do
          end do
       end do

       call mpi_allreduce(sum_local_init(1),sum_global_init(1),&
            nz+2+1,&
            mpi_double_precision,mpi_sum,comm,ierror)
       sum_global_init = sum_global_init/(sizeX*sizeY)
       
       thetaStrat(0:nz+1) = 1./sum_global_init(1:nz+2)

       pistrat(0) = 1 + 0.5*dz*kappa/(0.5*(thetaStrat(0)+thetaStrat(1)))
       pistrat(1) = 1 - 0.5*dz*kappa/(0.5*(thetaStrat(0)+thetaStrat(1)))

       p_env_pp(:,:,0) = 1 + 0.5*dz*kappa/(0.5*(the_env_pp(:,:,0)+the_env_pp(:,:,1))) 
       p_env_pp(:,:,1) = 1 - 0.5*dz*kappa/(0.5*(the_env_pp(:,:,0)+the_env_pp(:,:,1)))

       do k = 2,nz+1
          pistrat(k) = pistrat(k-1) - dz*kappa/thetaStrat(k-1)
          p_env_pp(:,:,k) = p_env_pp(:,:,k-1) - dz*kappa/the_env_pp(:,:,k-1)
       end do
        ...
        do k = 0,nz+1 
          ...
          do j = 1,ny
             do i = 1,nx
                rho = pStrat(k)/the_env_pp(i,j,k)
                if( fluctuationMode )  then
                   var(i,j,k,1) = rho - rhoStrat(k)
                else   
                   var(i,j,k,1) = rho 
                end if
                var(i,j,k,5) =0. !FSFeb2021test ! p_env_pp(i,j,k) - pistrat(k)
             end do
          end do
       end do
       ...
       thetaStrat(nz+2) = thetaStrat(nz+1)
       pStrat(nz+2) = pStrat(nz+1)
       rhoStrat(nz+2) = rhoStrat(nz+1)
       pistrat(nz+2) = pistrat(nz+1)
       ...
       do k = 1,nz-1
          do j = 1,ny
             do i = 1,nx
                ...
                sum_local2(k) = sum_local2(k) + flux(i,j,k,3,1)
             end do
          end do
       end do
       call mpi_allreduce(sum_local(1),sum_global(1),&
       ...
       rhopw_bar(1:nz) = sum_global(1:nz)
       rhow_bar(1:nz) = sum_global2(1:nz)
       
       
        rhopw_bar = 0. !FSFeb2021test
       rhow_bar = 0.
       ...
       sum_n &
            = sum_n + &
            exp(-g_ndim*rhoStrat(1)/(gamma*press0(1))*z(1)*0.5) * &
            (- S_bar(1)/PStrat(1) + 0.5*rhow_bar(1) * g_ndim/(gamma*press0(1)))
       sum_d = sum_d + &
            exp(-g_ndim*rhoStrat(1)/(gamma*press0(1))*z(1)*0.5) * &
            1./(gamma*press0(1))
       ...
       do k = 2,nz-1
          sum_n &
               = sum_n + &
               exp(-g_ndim*rhoStrat(k)/(gamma*press0(k))*0.5*(z(k)+z(k+1))) * &
               (- S_bar(k)/PStrat(k) &
               + 0.5*(rhow_bar(k) + rhow_bar(k-1)) * g_ndim/(gamma*press0(k))) 
          sum_d = sum_d +  &
               exp(-g_ndim*rhoStrat(k)/(gamma*press0(k))*0.5*(z(k)+z(k+1))) * &
               1./(gamma*press0(k))
       end do
       ...
       sum_n &
            = sum_n + &
            exp(-g_ndim*rhoStrat(nz)/(gamma*press0(nz))*z(nz)*0.5) * &
            ( - S_bar(nz)/PStrat(nz) &
            + 0.5*rhow_bar(nz-1) * g_ndim/(gamma*press0(nz))) 
       sum_d = sum_d + &
            exp(-g_ndim*rhoStrat(nz)/(gamma*press0(nz))*z(nz)*0.5) * &
            1./(gamma*press0(nz))
       ...
       !    !UAB
       !    !if (spongeLayer) then
       !    !   w_0(k) = w_0(k)/(1. + dt*0.5*(kr_sp(k)+kr_sp(k+1)))
       !    !end if
       !    !UAE
       ! end do
       ...
       w_0(1) &
            = dz &
            * exp(-g_ndim*rhoStrat(1)/(gamma*press0(1))*0.5*z(1)) * &
            (- S_bar(1)/Pstrat(1) &
            + (-dptopdt + g_ndim * 0.5*rhow_bar(1))/(gamma*press0(1)))
       
       do k = 2,nz-1
          w_0(k) &
               = w_0(k-1) &
               + dz * exp(-g_ndim*rhoStrat(k)/(gamma*press0(k))*0.5*(z(k)+z(k+1))) & !z_k+1/2 ?
               *(- S_bar(k)/Pstrat(k) &
               + (-dptopdt &
               + g_ndim * 0.5*(rhow_bar(k) + rhow_bar(k-1))) &
               /(gamma*press0(k)))
       end do
       ...
       !UAB
       !! update w so that its horizontal mean agrees with w_0
       
       !! calculate horizontal mean of w
       !do k = 1,nz
       ...
       !w_mean(1:nz) = sum_global(1:nz)
       
       !do k=1,nz
       !   var(:,:,k,4) = var(:,:,k,4) - w_mean(k) + w_0(k)
       !end do
       !UAE
       ...
       do k=1,nz
          divPw(k) = (PstratTilde(k)*w_0(k) - PstratTilde(k-1)*w_0(k-1))/dz
           divrhow(k)  & 
           = (rhoStratTilde(k)*w_0(k)+rhopw_bar(k)-rhoStratTilde(k-1)*w_0(k-1)-rhopw_bar(k-1))/dz 
       end do
       ...
       rhoStrat(1:nz) = rhoStrat(1:nz)!sum_global(1:nz) !FSFeb2021test
       ...
       do k = 1,nz
          if(int_mod == "expl")then
             ...
             Pstrat(k) = Pstrat(k) + beta(m) * q_P(k) !PI_old auskommentiert
             ...
             rhostrat(k) = rhostrat(k) + beta(m) * q_rho(k) !PI_old auskommentiert
          else if (int_mod == "impl")then
             if (heating_switch == 0) then
                PStrat(k) = PStrat(k) - dt*divPw(k) - dt*S_bar(k)  !PI_old auskommentiert
                 rhoStrat(k) = rhoStrat(k) - dt*divrhow(k)  !PI_old auskommentiert
             else
                PStrat(k) = PStrat00(k) - dt*divPw(k) - dt*S_bar(k)
                rhoStrat(k) = rhoStrat00(k) - dt*divrhow(k)
             end if
          else
          ...
          if (rhoStrat(k) <= 0.) then
             print*,'ERROR in BGstate_update: rhoStrat(',k,') =', &
                  & rhoStrat(k),'<=0.'
             stop
          end if
          !UAE
          
          !update thetaStrat and piStrat !FS
          thetaStrat(k) = PStrat(k)/rhoStrat(k)
       ...
       thetaStrat(nz+1) = thetaStrat(nz)
       pStrat(nz+1) = pStrat(nz)
       rhoStrat(nz+1) = rhoStrat(nz)
       pistrat(nz+1) = pistrat(nz)
