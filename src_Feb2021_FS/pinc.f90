program pinc_prog
  ...
  integer :: heating_switch, init_switch !FS
  ...
  ! 2) read input.f90
    call setup(var,var0,var1,varG,flux,force,source,dRho,dRhop,dMom,dTheta, &
  ...
  if (zero_initial_state) then
     if (fluctuationMode) then
        ...
        print*,'ERROR: zero_initial_state = .true. requires &
  ...
  if (heatingONK14 .or. TurbScheme .or. rayTracer) then
     ...
     heating_switch = 0
     init_switch = 0
     call BGstate_update(var1,var,flux,0.,RKstage,dPStrat,drhoStrat, &
          & "impl",heating_switch, init_switch) 
  end if
  ...
  if (restart) then
     ...
     !UAC
     !call read_profile (iIn, PStrat,"/scratch/atmodynamics/schmid/barLC_H135km/Pstrat_in.dat")
     !call read_profile (iIn, rhoStrat,"/scratch/atmodynamics/schmid/barLC_H135km/rhostrat_in.dat")
     !call read_profile (iIn, thetaStrat,"/scratch/atmodynamics/schmid/barLC_H135km/thetastrat_in.dat")
     !call read_profile (iIn, bvsStrat,"/scratch/atmodynamics/schmid/barLC_H135km/bvsstrat_in.dat")
  ...
  !testb
  !print*,'Sponge is between k =',kSponge,'and k =',nz
  !print*,'Sponge begins at z = ',zSponge * lRef
  !teste
  ...
  20 continue
     ...
     if (timeScheme == "semiimplicit") then
        ...
        do RKstage = 1, nStages
           ...
           !              & "rhop", "lhs", "expl")
           call massUpdate(var0, var, flux, 0.5*dt, dRho, RKstage, &
                         & "rho", "tot", "expl",1.)
         
           call massUpdate(var0, var,flux, 0.5*dt, dRhop, RKstage, &
                         & "rhop", "lhs", "expl",1.)
           !UAE
           ...
           !                     & RKstage, "lhs", "expl")
           call momentumPredictor(var0, var, flux, force, 0.5*dt, dMom, &
                                & RKstage, "lhs", "expl", 1.)
           !UAE
        end do
        ...
        rhopOld = var(:,:,:,1)  ! rhopOld for momentum predictor
        ...
        !                     & "impl")
        call momentumPredictor(var0, var,flux,force,0.5*dt,dMom,RKstage,"rhs",&
                             & "impl", 1.)
        !UAE
        ...
        !if (heating) then
           ...
           heating_switch = 0
           init_switch = 1
           call BGstate_update(var0,var,flux,0.5*dt,RKstage,dPStrat,drhoStrat, &
                             & "impl",heating_switch, init_switch)
        end if
        ...
        call massUpdate(var1, var,flux,0.5*dt,dRhop,RKstage,&
                      & "rhop","rhs","expl",1.)
        call setHalos( var, "var" )
        call setBoundary (var, flux, "var")
        !UAE
        ...
        !call momentumPredictor(var,flux,force,0.5*dt,dMom,RKstage,"rhs",&
        !                     & "expl")
        call momentumPredictor(var1, var,flux,force,0.5*dt,dMom,RKstage,"rhs",&
                             & "expl", 1.)
        !UAE
        ...
        do RKstage = 1, nStages
           ...
           !             & "rhop", "lhs", "expl")
           call massUpdate(var1, var, flux, dt, dRho, RKstage, &
                        & "rho", "tot", "expl",1.)

           call massUpdate(var1, var, flux, dt, dRhop, RKstage, &
                        & "rhop", "lhs", "expl",1.)
           !UAE
           ...
           call momentumPredictor(var1, var, flux, force, dt, dMom, RKstage, &
                                & "lhs", "expl", 1.)
        ...
        if (heatingONK14 .or. TurbScheme .or. rayTracer) then
           ...
           heating_switch = 1
           init_switch = 1
           call BGstate_update(var1,var,flux,dt,RKstage,dPStrat,drhoStrat, &
                             & "impl",heating_switch,init_switch) !FS 0.5*dt -> dt
           call setHalos( var, "var" )
           call setBoundary (var, flux, "var")
        end if
...
!FSE
       ...
       450    continue
           ...
           if( updateMass ) then
              ...
              !call massUpdate(var, flux, dt, dRho, RKstage, &
              !              & "rho", "tot", "expl")
              call massUpdate(var0, var, flux, dt, dRho, RKstage, &
                            & "rho", "tot", "expl",1.)
              !UAE
              if (testCase /= 'baroclinic_LC') then
              ...
              if (auxil_equ) then
                 ...
                 !call massUpdate(var, flux, dt, dRhop, RKstage, &
                 !              & "rhop", "tot", "expl")
                 call massUpdate(var0, var, flux, dt, dRhop, RKstage, &
                               & "rhop", "tot", "expl",1.)
                 !UAE
                 if (testCase /= 'baroclinic_LC') then
           ...
           if ( predictMomentum ) then
              ...
              !                     & "tot", "expl")
              call momentumPredictor(var0, var, flux, force, dt, dMom, RKstage, &
                                   & "tot", "expl", 1.)
              !UAE
              if (testCase /= 'baroclinic_LC') then
           ...
           !UAE
              ...
              init_switch = 1
              call BGstate_update(var0,var,flux,dt,RKstage,dPStrat,drhoStrat, &
                                & "expl",heating_switch, init_switch)
