...
contains
  ...
  subroutine setup (var,var0,var1,varG,flux,force,source,dRho,dRhop,dMom, &
  ...
  !UAE
    ...
    if (TestCase == "baroclinic_LC") then
       ...
       if (background == "HeldSuarez") then
          ...
          allocate(kw_hs(0:ny+1,0:nz+1), stat=allocstat)
       ...
       !UAE
          ...
          do j = 1, ny
             ...
             if (yloc < ymin) then
                ...
                c2_strtd(j) &
                = 1.!FS!cos((yloc - (yjets + 0.5*dy_hs))/dy_hs * 0.5*pi)**2
                ...
                c2_strtd(j) &
                = 1.!FS!cos((yloc - (yjetn - 0.5*dy_hs))/dy_hs * 0.5*pi)**2
          ...
          do k=0,nz+1
            ...
            !  Rayleigh BL drag and Sponge:
              if (z(k) <= 5.e3/lRef) then
                   kv_hs(:,k) = kf_hs*facsig
                   !kv_hs(j,k) = (1.+cos(pi*z(k)/(5.e3/lRef)))*tRef/(6.*86400.)
                   kw_hs(:,k) = 0.
                   !print*, z(k)*lRef, kv_hs(k)/tRef
                else if (z(k) > 5.e3/lRef .and. z(k) < z(nz+1 - ceiling(spongeHeight*real(nz+1)))/lRef) then
                   kv_hs(:,k) = 0.
                   kw_hs(:,k) = 0.
                else if (z(k) >= z(nz+1 - ceiling(spongeHeight*real(nz+1)))/lRef) then
                   kw_hs(:,k) = tRef*spongeAlphaZ_dim*sin(pi/2. * (z(k)-z(nz+1 - ceiling(spongeHeight*real(nz+1))))/(z(nz+1)-z(nz+1 - ceiling(spongeHeight*real(nz+1)))))**2
                   kv_hs(:,k) = 0.!kw_hs(:,k)
                end if
                ! else if (z(k) <= 65.e3/lRef .and. z(k) > 60.e3/lRef) then
                !    kv_hs(j,k) = (1.-cos(pi*(z(k)-60.e3/lRef)/(5.e3/lRef)))*tRef/(4.*86400.)
                ! else if (z(k) > 65.e3/lRef) then
                !    kv_hs(j,k) = tRef/(2.*86400.)
                ! end if
          ...
          do j=1,ny
             ...
             do k = 2, nz+1!nz-ceiling((0.85)*real(nz)) !nz+1 !FS
                do i=1,nx
                   pistar &
                   ...
                   if (k == nz-ceiling((0.85)*real(nz)))then
            ...
            !  do k = nz+1-ceiling((0.85)*real(nz)), nz+1 !FS
            !     do i=1,nx
            !        pistar &
            !        = var(i,j,k-2,5) - 2.0*dz * kappa/the_env_pp(i,j,k-1)
          ...
          ! do k = (nz - ceiling(0.5*real(nz)))+1, nz+1 !FS
          !    the_env_pp(:,:,k) = the_env_pp(:,:,nz - ceiling(0.5*real(nz)))
          !    var(:,:,k,5) = var(:,:,nz - ceiling(0.5*real(nz)),5)
          ! end do
       ...
       do k=0,nz+1
          do j=1,ny
             do i = 1,nx
                ...
                yloc = y(j+j00)
                f_Coriolis_y(j) = f_Coriolis_dim!FSJuly2020*sin(pi*yloc/ymax)
