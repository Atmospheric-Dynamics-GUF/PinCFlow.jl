...
contains
  ...
  subroutine Corrector( var,flux,dMom,dt,errFlagBicg,nIter,m,opt,facray)
    ! -------------------------------------------------
    ...
    call poissonSolver( rhs, var, dt, errFlagBicg, nIter, m, opt, facray )
    ...
    call correctorStep (var, dMom, dt, m, opt, facray)
...
!
 ...
 !============================================================== 
  ...
  !subroutine val_PsIn(var, dt, opt, facray)
    ...
    real :: pStratU, pStratD, rhoEdge
    !UAC real :: AL,AR, AB,AF, AD,AU, ALB,ALF, ARB,ARF, AC
       ...
       do k = 1,nz
          ...
          do j = 1,ny
             do i = 1,nx
                ...
                rhoEdge = 0.5*( var(i+1,j,k,1) + var(i,j,k,1) )
                !UAC rhoEdge = rhoEdge + rhoStrat(k)
                if (fluctuationMode) rhoEdge = rhoEdge + rhoStrat(k)
                !UAE
                ...
                !UAC
                !acontr = - 0.25 * dxy * pStrat(k)**2/rhoEdge &
                !      * (-f_cor_nd(j-1)*dt)/(facu*facv + (f_cor_nd(j-1)*dt)**2)
                acontr = 0.25 * dxy * pStrat(k)**2/rhoEdge &
                         * f_cor_v*dt/(facu*facv + (f_cor_v*dt)**2)
                !UAE
                ...
                if (k == nz) then
                   ...
                   rhoEdge = 0.5*( var(i,j,k+1,1) + var(i,j,k,1) )
                   !UAC rhoEdge = rhoEdge + rhoStratTilde(k)
                   if (fluctuationMode) then
                      rhoEdge = rhoEdge + rhoStratTilde(k)
                   end if
                   !UAE
                ...
                if (k == 1) then
                   ...
                   rhoEdge = 0.5*( var(i,j,k,1) + var(i,j,k-1,1) )
                   !UAC rhoEdge = rhoEdge + rhoStratTilde(k-1)
                   if (fluctuationMode) then
                      rhoEdge = rhoEdge + rhoStratTilde(k-1)
                   end if
                   !UAC
...
!------------------------------------------------------------------
 ...
 subroutine heat_w0(var,flux,dt,heat,S_bar,w_0)
  ...
  ! in/out variables
    ...
    real, dimension(-nbz:nz+nbz), intent(out) :: S_bar!,rhopw_bar
    real, intent(in) :: dt
  ...
  !UAC subroutine heat_w0(var,flux,heat,S_bar,w_0)
  subroutine heat_w0_Feb2021(var0,var,flux,dt,heat,S_bar,w_0,rhopw_bar)
  ...
  ! in/out variables
    ...
    real, dimension(-nbz:nz+nbz), intent(out) :: rhopw_bar

    integer :: i,j,k
    real, dimension(1:nz) :: sum_local, sum_global, sum_local2, sum_global2
    real, dimension(-nbz:nz+nbz) :: press0 , rhow_bar
    ...
    do k = 2,nz-1
       w_0(k) &
       = w_0(k-1) &
         + dz * exp(-g_ndim*rhoStrat(k)/(gamma*press0(k))*0.5*(z(k)+z(k+1))) &
           *(- S_bar(k)/Pstrat(k) &
             + (-dptopdt &
  ...
  end subroutine heat_w0_Feb2021
