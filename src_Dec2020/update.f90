...
!---------------------------------------------------------------------
 ...
 subroutine momentumPredictor (var,flux,force,dt,q,m,mmp_mod,int_mod)
             ...
             do i = 1,nx
                ...
                if (TestCase == "baroclinic_LC") then
                   if (background == "HeldSuarez") then
                      ...
                      volForce &
                           = volforce - 0.5*(kw_hs(j,k)+kw_hs(j,k+1))*rhoM_1*var(i,j,k,4)
                   end if
                end if
                ...
                do i = 1,nx
                   ...
                   if (TestCase == "baroclinic_LC") then
                      if (background == "HeldSuarez") then
                         ...
                         wAst = wAst - dt*0.5*(kw_hs(j,k)+kw_hs(j,k+1))*wvert
                      end if
                   end if
                ...
                do i = 1,nx
                   ...
                   if (TestCase == "baroclinic_LC") then
                      if (background == "HeldSuarez") then
                         ...
                         facw = facw + dt*0.5*(kw_hs(j,k)+kw_hs(j,k+1))
                      end if
                   end if
  ...
  subroutine momentumPredictor_FSDec2020 (var,flux,force,dt,q,m,mmp_mod,int_mod)
    !----------------------------------
    !  calculates the velocities u^*
    !----------------------------------
    ...
    if (TestCase == "baroclinic_LC")then 
       ...
       do j = 0,ny+1
          yloc = y(j+j00)
          f_cor_nd(j) = f_Coriolis_dim*tRef!FSJuly2020*sin(pi*yloc/ymax)
       end do
             ...
             do i = 1,nx
                ...
                if (TestCase == "baroclinic_LC") then
                   if (background == "HeldSuarez") then
                      ...
                      volForce &
                      = volForce &
                        - 0.5*(kv_hs(j,k)+kv_hs(j+1,k))*rhoM_1*(var(i,j,k,3) - var_env(i,j,k,3))
          ...
          do k = 1,nz
             do j = j0,j1
                do i = 1,nx
                   ...
                   vAst &
                  ! = vhory + dt*(-0.5*(f_cor_nd(j)+f_cor_nd(j+1))*uhorx - piGrad + volfcy/rhov)
                  = vhory + dt*(-f_cor_nd(j)*uhorx - piGrad + volfcy/rhov)
                ...
                do i = 1,nx
                   ...
                   if (TestCase == "baroclinic_LC") then
                      ...
                      buoy &
                           = - g_ndim &
                           * 0.5 &
                           * (rhopOld(i,j,k)/rho000 - var_env(i,j,k,6)/(var_env(i,j,k,1)+rhoStrat_0(k)) & 
                           +  rhopOld(i,j,k+1)/rho001 - var_env(i,j,k,6)/(var_env(i,j,k+1,1)+rhoStrat_0(k+1)))
                     else
                      buoy  &
                      = - g_ndim &
                          * 0.5*(  rhopOld(i,j,k  )/rho000 &
                   ...
                   if (TestCase == "baroclinic_LC") then
                      if (background == "HeldSuarez") then
                         ...
                         wAst &
                              = wAst - dt*0.5*(kw_hs(j,k)+kw_hs(j,k+1))*wvert
                      end if
                   end if
                ...
                do i = 1,nx
                   ...
                   if (TestCase == "baroclinic_LC") then
                      if (background == "HeldSuarez") then
                         ...
                         facw &
                              = facw + dt*0.5*(kw_hs(j,k)+kw_hs(j,k+1))
                      end if
                   end if
                   ...
                   if(testCase /= "baroclinic_LC")then !FSDec2020
                   wAst &
                   = 1.0 &
                     /(  facw*facr &
                       + rhoStratTilde(k)/rhow * bvsstw * dt**2) &
                     * (  facr * (wvert - dt * piGrad) &
                      ...
                      wAst &
                           ...
                           * (  facr * (wvert - dt * piGrad) &
                           - dt * g_ndim &
                           * 0.5*(  rhoOld(i,j,k)/rho000 - var_env(i,j,k,6)/(var_env(i,j,k,1)+rhoStrat_0(k)) & 
                           + rhoOld(i,j,k+1)/rho001 - var_env(i,j,k+1,6)/(var_env(i,j,k+1,1)+rhoStrat_0(k+1))&
                           + dt &
                           * (  rhoStrat(k)/Pstrat(k) &
                           * heat0/rho000 &
                           + rhoStrat(k+1)/Pstrat(k+1) &
  ...
  end subroutine momentumPredictor_FSDec2020
...
!--------------------------------------------------------------------------
  ...
  subroutine massUpdate_FSDec2020 (var,flux,dt,q,m,upd_var,upd_mod,int_mod)
    !-----------------------------
                   ...
                   do i = 1,nx
                      ...
                      if (TestCase /= "baroclinic_LC") then !FSDec2020
                         ...
                         buoy & 
                              ...
                              *(wvrt - dt * piGrad) &
                              - facw  &
                              * (  ((rho-rhoStrat(k))* g_ndim/rho -  &
                              ( var_env(i,j,k,6)*g_ndim/(var_env(i,j,k,1)+rhoStrat_0(k)))) &
                              + dt &
                              * (  rhoStrat(k)/Pstrat(k) * heat_flc &
                              + alprlx * rho_p)))

                         var(i,j,k,6) = -buoy*rho/g_ndim + var_env(i,j,k,6)
                      end if
                ...
                do j = 1,ny
                   do i = 1,nx
                      ...
                      var(i,j,k,6) = -buoy * rho/g_ndim
                          ...
                          buoy &
                      = - g_ndim * ( (rho-rhoStrat(k))/rho - var_env(i,j,k,6)/(var(i,j,k,1)+rhoStrat_0(k))  ) &
                        - dt &
                          * (  rhoStrat(k)/rho * bvsStrat(k)*wvrt &
                             + g_ndim/rho &
                               * (  rhoStrat(k)/Pstrat(k) * heat_flc &
  ...
  end subroutine massUpdate_FSDec2020
...
!---------------------------------------------------------------------
  ...
  !UAB 200413
  subroutine BGstate_update(var,flux,dt,m,q_P,q_rho,int_mod,heating_switch)
  !UAE 200413
