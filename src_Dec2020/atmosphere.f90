...
!   achatze
    ...
    case( "pseudo_incompressible", "WKB" )
       ...
       case( 'HeldSuarez' )
          ...
          do k = 2,nz+2!nz-ceiling((0.85)*real(nz))!nz+2 !FS
      ...
      !    do k = nz+1-ceiling((0.85)*real(nz)), nz+2 !FS
!              pistar = pistrat(k-2) - 2.0*dz * kappa/thetaStrat(k-1) 
! &
!              T_bar &
!              =  tp_sponge + pistar*(0.5*tpdiffhor_tropo &
!                        - ptdiffvert_tropo/kappa * log(pistar))!FS 0.5->1
...
!                        - ptdiffvert_tropo/kappa * log(pistrat(k)))!FS 0.5->1
...
!           do k = (nz - ceiling(0.5*real(nz)))+1,nz+2 !FS
!              thetaStrat(k) = thetaStrat(nz - ceiling(0.5*real(nz)))
!              pStrat(k) = pStrat(nz - ceiling(0.5*real(nz)))
!              piStrat(k) = piStrat(nz - ceiling(0.5*real(nz)))
!              rhoStrat(k) = rhoStrat(nz - ceiling(0.5*real(nz)))
