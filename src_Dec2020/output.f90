...
!            write(40,rec=irc_prc) field_prc
  ...
  subroutine read_data( &
    ...
    if(master) then
       open(40,file='/scratch/atmodynamics/schmid/barLC_50kmx500m_60days/pf_all.dat',form="unformatted",access='direct',&
            & recl=SizeX*SizeY)
             ...
             do j = 1, ny
                ...
                do i = 1, nx
                   ...
                   select case (iVar) 
                      ...
                      case(6) ! potential temperature fluctuation theta' 
                              ! (deviation from background, Boussinesq)
                          ...
                          select case(model) 
                             case( "pseudo_incompressible" )
                             !for calcPVinversion
                             var(i,j,k,iVar) = field_prc(i,j)/thetaRef - thetaStrat(k)!(Pstrat(k)/field_prc(i,j))/rhoRef - rhoStrat(k)
