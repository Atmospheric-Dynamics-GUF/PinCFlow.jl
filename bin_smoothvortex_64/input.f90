...
&domain
  ...
  sizeX = 64, !1 !16,                  ! nb of global grid cells
  sizeY = 64, !64,!48,!64,
  sizeZ = 1,
  nbx = 2,                  ! nb. of ghost cells
  nby = 2,
  nbz = 1,
  lx_dim =    0., 1. ! domain lenths in m
  ly_dim =   0, 1.!-1.5e7, 1.5e7!-2.e7, 2.e7
  lz_dim =    0.0, 0.015625!1.
...
&solverList
  ...
  cfl = 0.45
  cfl_wave = 0.05                 ! passage rate of phase throuh a cell
  dtMax_dim =  1.!29.1e-4!0.00078125            ! max time step in s
...
! shap_dts_fac = 5.               ! horizontal-Shapro-filter damping time 
                                  ! scale 
  ...
  heatingONK14 = .false. !.false. !.true.           ! pseudo-incompressible dynamics with 
...
&poissonSolverList
  ...
  tolPoisson = 1.0e-12          ! abort criterion
  ...
  maxIterADI = 10!10               ! nb of iterations for ADI preconditioner
...
&atmosphereList
  ...
  background = "smoothV"         ! const-N    -> set N_BruntVaisala_dim
  ...
  z_tr_dim = 12000.0             ! m 
                                 ...
                                 ! (need for baroclinic)
  theta_tr_dim = 706.098             ! K 
                                 ! const pot. temp. of troposphere 
                                 ! (need for baroclinic)


  theta0_dim = 706.098!300               ! K      
  ...
  press0_dim =  101625.0         ! ground pressure (at z=0) in Pa:
                                 ! 101325.0 for z = 0 bottom of atmosphere
                                 ! 101.3250 for z = 0 at appr 60km
  
  N_BruntVaisala_dim = 0.!0.018     ! Brunt-Vaisala frequency for 
                                 ! 1) "const-N" atmosphere in 1/s
                                 ! 2) "unifrom" Boussinesq 
  
  backgroundFlow_dim =  1.0, 1.0, 0.0 !m/s
...
&boundaryList
  ...
  spongeLayer = .false. !.false.     ! sponge with relaxation to background
    !sponge_uv = .false.      ! sponge applied to horizontal winds as well
                           ! (switch used in semi-implicit time stepping)
  spongeHeight = 0.3      ! relative height of sponge layer
  spongeAlphaZ_dim =  1.4e-4 !7.e-5 !1.4e-4 !2.8e-4 !1.7e-3 ! relaxation rate coeff in 1/s
   ! spongeAlphaZ_fac = 0.5 ! relaxation rate coeff in 1/dt 
...
&outputList
  ...
  outputTimeDiff =  1.   ! output every ... seconds
  maxTime =  2.           ! stop after maxTime seconds
  ...
  varIn = 1,1,1,1,1,0,0,0    ! 1 = output, 0 = no output 
...
&baroclinic_LC
  ...
  jwdth_dim = 2.4e6!5.e6             ! half (!) jet width (m)

  add_ptptb = .true. !.true.            ! add local potential-temperature 
                                ! perturbation
  ptptb_x_dim = 2.5e6            ! x coordinate of local 
                                ! potential-temperature perturbation (m)
  ptptb_y_dim = 5.e6!7.5e6 !10.e6            ! y coordinate of local 
                                ! potential-temperature perturbation (m)
  ...
  ptptb_dz_dim = 2.e3           ! vertical width of local 
                                ! potential-temperature perturbation  (m)
  ptptb_amp_dim = 3.0  !3.0       ! amplitute of local potential-temperature 
                                ! perturbation (K)
