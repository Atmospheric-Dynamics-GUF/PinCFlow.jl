...
&variables
  ...
  include_testoutput = .true. ! test output
  include_testoutput_2 = .false.
&end
...
&solverList
  ...
  dtMax_dim = 1. !3.6e3               ! max time step in s
...
&outputList
  ...
  outputTimeDiff =  100.0  ! output every ... seconds
  maxTime = 1200.0          ! stop after maxTime seconds
...
&wavePacket
  ...
  lambdaX_dim = 10.e3      ! wave length in x direction in m
                          ! lambdaX = 0.0 --> infinite wavelength
  lambdaY_dim = 0         ! wave length in y direction in m
                          ! lambday = 0.0 --> infinite wavelength
  lambdaZ_dim = -2.e3     ! vertical wave length in m

  amplitudeFactor = 0. !0.19   ! normalilized buoyancy amplitude

  xCenter_dim = 5.0e3     ! center of wave packet in x direction in m

  yCenter_dim = 5.0e3     ! center of wave packet in y direction in m
  zCenter_dim = 5.0e3      ! center of wave packet in z direction in m

  sigma_dim = 6.0e3      ! vertical width of Gaussian wavepacket in m

  sigma_hor_dim = 0.      ! cosine distribution width
                          ! (in x direction, 0 means infinity)
  sigma_hor_yyy_dim = 0.  ! cosine distribution width
...
!  amplitudeFactor_sp = 0.1, 0. !0.12,
  amplitudeFactor_sp = 0.12, 0.12,
  !amplitudeFactor_sp = 0. , 0.19, 
  omiSign_sp = 1, 1,

  lambdaX_dim_sp = 10.e3, 10.e3,
  lambdaY_dim_sp = 0., 0.,  
  lambdaZ_dim_sp = -2.e3, 2.e3, 

  sigma_hor_dim_sp = 0., 0., 
  sigma_hor_yyy_dim_sp = 0., 0., 
  sigma_dim_sp = 6.e3, 6.e3, 

  xCenter_dim_sp = 5.e3, 5.e3,
  yCenter_dim_sp = 5.e3, 5.e3,
  zCenter_dim_sp = 7.e3, 9.e3
...
&ICELIST
 ...
 INIT_QV =  1.E-4,
 INIT_M_ICE      =  1.E-16,
 RADIUS_SOLUTION =  75.E-9,
 SIGMA_R =  1.5,
 T_NUC   =  196.0,
 P_NUC   =  2.0E+4,
 DT_ICE  =  1.0E-3,
 NUC_APPROX_TYPE = 'linFit',
 ISSR_TOP        =  0.000000000000000E+000,
...
&tracerList
  tracerSetup = 'increase_in_z_tracer' !"gaussian_tracer_2D"
&end
