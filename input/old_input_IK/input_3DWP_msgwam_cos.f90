...
&outputList
  ...
  outputTimeDiff = 10800.0  ! output every ... seconds
  maxTime        = 10800.0  ! stop after maxTime seconds
...
&LagrangeRayTracing
  ...
  yrmax_dim = 3.e6,        ! right bound of initial rays (in y dir.) (m)
  ...
  sigwpy_dim = 0.e0        ! width of the wave packet in hor. (y-dir.) (m);
                           ! (0 means infinity)
  sigwpz_dim = 10.e3,       ! width of the wave packet in vertical (m);

  branchr = -1,            ! frequency branch (dispersion relation)
  !presently not used:
  lindUinit = .false.,     ! ind. wind already at initial time (true/false)

  mountainHeight_wkb_dim = 0.0 ! WKB mountain height (m)
  mountainWidth_wkb_dim = 0.0  ! WKB mountain half-width (m)
