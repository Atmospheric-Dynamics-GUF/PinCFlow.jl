...
Every commit to `main` that changes the code must also be released as a new version of the package. This means that such a commit must update the version numbers in `Project.toml`, `docs/Project.toml` and `examples/Project.toml` and add release notes to `NEWS.md`. Following the interpretation of [semantic versioning](https://julialang.github.io/Pkg.jl/dev/compatibility/#Version-specifier-format-1), bug fixes that don't change the signatures of top-level objects call for a new patch, breaking changes to the Public API (anything exported by the main module) call for a new major version, and everything in between calls for a new minor version. Once the commit has been pushed, navigate to it on GitHub and add the comment `@JuliaRegistrator register()`. For a new major version, the comment must mention where the breaking changes are documented (an example of this can be found [here](https://github.com/Atmospheric-Dynamics-GUF/PinCFlow.jl/commit/6049b83416104195e0575516d4d12f5614a6e0b1#commitcomment-170569001)).
