...
$$\begin{align*}
    1. && \rho'^n & = \rho^n - \bar{\rho},\\
    2. && \left(\rho^\#, \rho'^\#, \hat{\boldsymbol{u}}^\#\right) & = \mathrm{L}_{\Delta t / 2} \left(\rho^n, \rho'^n, \hat{\boldsymbol{u}}^n, \hat{\boldsymbol{u}}^n, \alpha_\mathrm{R}^{n + 1}\right)\\
    3. && \left(\rho'^{n + 1 / 2}, \hat{\boldsymbol{u}}^{n + 1 / 2}, \pi'^{n + 1 / 2}\right) & = \mathrm{RI}_{\Delta t / 2} \left(\rho^\#, \rho'^\#, \hat{\boldsymbol{u}}^\#, \pi'^n, \beta_\mathrm{R}^{n + 1}\right)\\
    4. && \left(\rho'^*, \hat{\boldsymbol{u}}^*\right) & = \mathrm{RE}_{\Delta t / 2} \left(\rho^n, \rho'^n, \hat{\boldsymbol{u}}^n, \pi'^{n + 1 / 2}\right)\\
    5. && \left(\rho^{**}, \rho'^{**}, \hat{\boldsymbol{u}}^{**}\right) & = \mathrm{L}_{\Delta t} \left(\rho^n, \rho'^*, \hat{\boldsymbol{u}}^*, \hat{\boldsymbol{u}}^{n + 1 / 2}, \alpha_\mathrm{R}^{n + 1}\right)\\
    6. && \left(\rho'^{n + 1}, \hat{\boldsymbol{u}}^{n + 1}, \pi'^{n + 1}\right) & = \mathrm{RI}_{\Delta t / 2} \left(\rho^{**}, \rho'^{**}, \hat{\boldsymbol{u}}^{**}, \pi'^{n + 1 / 2}, 2 \beta_\mathrm{R}^{n + 1}\right),
\end{align*}$$
...
where the operators $\mathrm{L}$, $\mathrm{RI}$ and $\mathrm{RE}$ perform an explicit integration of the left-hand sides, an implicit integration of the right-hand sides and an explicit integration of the right-hand sides, each over the time step indicated in its subscript, respectively. The superscripts represent various time levels between those before ($n$) and after ($n + 1$) the current time step $\Delta t = t^{n + 1} - t^n$. In $\mathrm{L}$, the fourth argument is the velocity by which the prognostic variables are transported ([Benacchio & Klein, 2019](https://doi.org/10.1175/mwr-d-19-0073.1); [Schmid et al., 2021](https://doi.org/10.1175/MWR-D-21-0126.1)). A complete description of the exact implementation of these steps follows here.
 ...
 1. The left-hand sides are integrated over $\Delta t / 2$ with a low-storage RK3 scheme ([Williamson, 1980](https://doi.org/10.1016/0021-9991(80)90033-9)). Fractional implicit Euler steps are used to integrate the Rayleigh-damping terms of the LHS sponge. At every RK3 stage $m$, the following updates are performed.
     ...
     1. Density update:
        ...
        $$\begin{align*}
            q^{\rho, m + 1} & = - \frac{\Delta t}{2 J} \left(\frac{\partial J \rho^m u^n}{\partial \hat{x}} + \frac{\partial J \rho^m v^n}{\partial \hat{y}} + \frac{\partial J \rho^m \hat{w}^n}{\partial \hat{z}}\right) + \left(\alpha_\mathrm{RK} q^\rho\right)^m,\\
            \rho^{m + 1} & = \rho^m + \beta_\mathrm{RK}^m q^{\rho, m + 1},\\
            \rho^{m + 1} & \rightarrow \left(1 + \alpha_\mathrm{R}^{n + 1} f_\mathrm{RK}^m \frac{\Delta t}{2}\right)^{- 1} \left(\rho^{m + 1} + \alpha_\mathrm{R}^{n + 1} f_\mathrm{RK}^m \frac{\Delta t}{2} \bar{\rho}\right)
        \end{align*}$$
     ...
     1. Wind update:
        ...
        $$\begin{align*}
            ...
            q^{\rho \hat{w}, m + 1} & = \frac{\Delta t}{2} \left[- \mathcal{A}^{\rho \hat{w}, m, n} + \mathcal{V}^{\rho \hat{w}, m} + \mathcal{X}^{\rho \hat{w}, m} + G^{13} f \left(\rho v\right)^m - G^{23} f \left(\rho u\right)^m\right] + \left(\alpha_\mathrm{RK} q^{\rho \hat{w}}\right)^m\\
            \hat{w}^{m + 1} & = \left(\rho^{- 1}\right)^{m + 1} \left[\left(\rho \hat{w}\right)^m + \beta_\mathrm{RK}^m q^{\rho \hat{w}, m + 1}\right],\\
            \hat{\boldsymbol{u}}^{m + 1} & \rightarrow \left(1 + \alpha_\mathrm{R}^{n + 1} f_\mathrm{RK}^m \frac{\Delta t}{2}\right)^{- 1} \left(\hat{\boldsymbol{u}}^{m + 1} + \alpha_\mathrm{R}^{n + 1} f_\mathrm{RK}^m \frac{\Delta t}{2} \hat{\boldsymbol{u}}_\mathrm{R}^{m + 1}\right)
        \end{align*}$$
    ...
    $$\begin{align*}
        \mathcal{A}^{\rho u, m, n} & = \frac{1}{J} \left[\frac{\partial J \left(\rho u\right)^m u^n}{\partial \hat{x}} + \frac{\partial J \left(\rho u\right)^m v^n}{\partial \hat{y}} + \frac{\partial J \left(\rho u\right)^m \hat{w}^n}{\partial \hat{z}}\right],\\
        \mathcal{A}^{\rho v, m, n} & = \frac{1}{J} \left[\frac{\partial J \left(\rho v\right)^m u^n}{\partial \hat{x}} + \frac{\partial J \left(\rho v\right)^m v^n}{\partial \hat{y}} + \frac{\partial J \left(\rho v\right)^m \hat{w}^n}{\partial \hat{z}}\right],\\
        \mathcal{A}^{\rho \hat{w}, m, n} & = G^{13} \mathcal{A}^{\rho u, m, n} + G^{23} \mathcal{A}^{\rho v, m, n} + \frac{1}{J^2} \left[\frac{\partial J \left(\rho w\right)^m u^n}{\partial \hat{x}} + \frac{\partial J \left(\rho w\right)^m v^n}{\partial \hat{y}} + \frac{\partial J \left(\rho w\right)^m \hat{w}^n}{\partial \hat{z}}\right].
    \end{align*}$$
 ...
 1. The right-hand sides are integrated over $\Delta t / 2$ with an implicit Euler step. The divergence constraint is then enforced by solving a Poisson equation for Exner-pressure differences $\Delta \pi'$, which are used to correct the wind, the density fluctuations and the Exner-pressure itself. The details of these substeps are as follows.
     ...
     1. Predictor step:
        ...
        $$\begin{align*}
            ...
            \hat{w}^{n + 1 / 2} & = \left[1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2} + \frac{\bar{\rho}}{\rho^\#} \frac{\left(N \Delta t\right)^2}{4}\right]^{- 1}\\
            & \quad \times \left[\hat{w}^\# + \frac{\Delta t}{2} \left(- c_p \frac{P}{\rho^\#} \mathcal{P}^{\rho \hat{w}, n} + \frac{b'^\#}{J} + \frac{F^{\rho \hat{w}, n + 1}}{\rho^\#}\right) \vphantom{\frac{\bar{\rho}}{\rho^\#} \frac{\left(N \Delta t\right)^2}{4}}\right.\\
            & \qquad \quad + \left.\frac{\bar{\rho}}{\rho^\#} \frac{\left(N \Delta t\right)^2}{4} \left(G^{1 3} u^{n + 1 / 2} + G^{2 3} v^{n + 1 / 2}\right)\right],\\
            \rho'^{n + 1 / 2} & = - \frac{\rho^\#}{g} \left[1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2} + \frac{\bar{\rho}}{\rho^\#} \frac{\left(N \Delta t\right)^2}{4}\right]^{- 1}\\
            & \quad \times \left\{- \frac{\bar{\rho}}{\rho^\#} N^2 \frac{\Delta t}{2} J \left[\hat{w}^\# + \frac{\Delta t}{2} \left(- c_p \frac{P}{\rho^\#} \mathcal{P}^{\rho \hat{w}, n} + \frac{F^{\rho \hat{w}, n + 1}}{\rho^\#}\right)\right]\right.\\
            & \qquad \quad + \left(1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2}\right) b'^\# + \frac{\bar{\rho}}{\rho^\#} N^2 \frac{\Delta t}{2} J \left(1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2}\right)\\
            & \qquad \quad \times \left.\left(G^{1 3} u^{n + 1 / 2} + G^{2 3} v^{n + 1 / 2}\right) \vphantom{\frac{\bar{\rho}}{\rho^\#} N^2 \frac{\Delta t}{2}}\right\}
        \end{align*}$$

     1. Poisson equation:

        $$\begin{align*}
            & \frac{1}{J} \left(\frac{\partial J P u^{n + 1 / 2}}{\partial \hat{x}} + \frac{\partial J P v^{n + 1 / 2}}{\partial \hat{y}} + \frac{\partial J P \hat{w}^{n + 1 / 2}}{\partial \hat{z}}\right)\\
            & \quad = \frac{1}{J} \left\{\frac{\partial J P \mathcal{C}^{\rho u, \#, n + 1}}{\partial \hat{x}} + \frac{\partial J P \mathcal{C}^{\rho v, \#, n + 1}}{\partial \hat{y}} \vphantom{\left(\frac{\bar{\rho}}{\rho^\#} \frac{\left(N \Delta t\right)^2}{4}\right)^{- 1}}\right.\\
            & \qquad \qquad + \frac{\partial}{\partial \hat{z}} \left[\left(1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2} + \frac{\bar{\rho}}{\rho^\#} \frac{\left(N \Delta t\right)^2}{4}\right)^{- 1}\right.\\
            & \qquad \qquad \qquad \quad \times \left(\frac{\Delta t}{2} c_p \frac{J P^2}{\rho^\#} \mathcal{D}^{\rho \hat{w}} + J P \frac{\bar{\rho}}{\rho^\#} \frac{\left(N \Delta t\right)^2}{4}\right.\\
            & \qquad \qquad \qquad \qquad \quad \times \left.\left.\left.\left(G^{13} \mathcal{C}^{\rho u, \#, n + 1} + G^{23} \mathcal{C}^{\rho v, \#, n + 1}\right) \vphantom{\frac{\bar{\rho}}{\rho^\#} \frac{\left(N \Delta t\right)^2}{4}}\right) \vphantom{\left(\frac{\bar{\rho}}{\rho^\#} \frac{\left(N \Delta t\right)^2}{4}\right)^{- 1}}\right]\right\}
        \end{align*}$$
     ...
     1. Corrector step:
        ...
        $$\begin{align*}
            ...
            \hat{w}^{n + 1 / 2} & \rightarrow \hat{w}^{n + 1 / 2} - \left[1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2} + \frac{\bar{\rho}}{\rho^\#} \frac{\left(N \Delta t\right)^2}{4}\right]^{- 1}\\
            & \quad \times \left[\frac{\Delta t}{2} c_p \frac{P}{\rho^\#} \mathcal{D}^{\rho \hat{w}} + \frac{\bar{\rho}}{\rho^\#} \frac{\left(N \Delta t\right)^2}{4} \left(G^{1 3} \mathcal{C}^{\rho u, \#, n + 1} + G^{23} \mathcal{C}^{\rho v, \#, n + 1}\right)\right],\\
            \rho'^{n + 1 / 2} & \rightarrow \rho'^{n + 1 / 2} + \frac{\rho^\#}{g} \left[1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2} + \frac{\bar{\rho}}{\rho^\#} \frac{\left(N \Delta t\right)^2}{4}\right]^{- 1}\\
            & \quad \times \left[- \frac{\bar{\rho}}{\rho^\#} \frac{\left(N \Delta t\right)^2}{4} J c_p \frac{P}{\rho^\#} \mathcal{D}^{\rho \hat{w}} + \frac{\bar{\rho}}{\rho^\#} N^2 \frac{\Delta t}{2} J \left(1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2}\right)\right.\\
            & \qquad \quad \times \left.\left(G^{1 3} \mathcal{C}^{\rho u, \#, n + 1} + G^{2 3} \mathcal{C}^{\rho v, \#, n + 1}\right) \vphantom{\frac{\bar{\rho}}{\rho^\#} \frac{\left(N \Delta t\right)^2}{4}}\right],\\
    ...
    $$\begin{align*}
        \mathcal{D}^{\rho u} & = \left(\frac{\partial \Delta \pi'}{\partial \hat{x}} + G^{13} \frac{\partial \Delta \pi'}{\partial \hat{z}}\right),\\
        \mathcal{D}^{\rho v} & = \left(\frac{\partial \Delta \pi'}{\partial \hat{y}} + G^{23} \frac{\partial \Delta \pi'}{\partial \hat{z}}\right),\\
        \mathcal{D}^{\rho \hat{w}} & = \left(G^{13} \frac{\partial \Delta \pi'}{\partial \hat{x}} + G^{23} \frac{\partial \Delta \pi'}{\partial \hat{y}} + G^{33} \frac{\partial \Delta \pi'}{\partial \hat{z}}\right)
    \end{align*}$$
 ...
 1. The right-hand sides (without the Rayleigh-damping terms) are integrated over $\Delta t / 2$ with an explicit Euler step. Therein, the Exner-pressure fluctuations $\pi'^{n + 1 / 2}$ are used to compute the pressure gradient. The exact updates are
    ...
    $$\begin{align*}
        ...
        \hat{w}^* & = \hat{w}^n + \frac{\Delta t}{2} \left(- c_p \frac{P}{\rho^n} \mathcal{P}^{\rho \hat{w}, n + 1 / 2} + \frac{b'^n}{J} + \frac{F^{\rho \hat{w}, n + 1}}{\rho^n}\right),\\
        \rho'^* & = - \frac{\rho^n}{g} \left[b'^n - \frac{\Delta t}{2} \bar{\rho} N^2 \left(\frac{w}{\rho}\right)^n\right].
    \end{align*}$$
...
$$\begin{align*}
    1. && \rho'^n & = \rho^n - \frac{P^n}{\bar{\theta}},\\
...
(see [Benacchio & Klein, 2019](https://doi.org/10.1175/mwr-d-19-0073.1); [Chew et al., 2022](https://doi.org/10.1175/MWR-D-21-0175.1)). Another detailed description follows here.
 ...
 1. The left-hand sides are integrated over $\Delta t / 2$ with the low-storage RK3 scheme. Fractional implicit Euler steps are used to integrate the Rayleigh-damping terms of the LHS sponge. The updates at every RK3 stage are as follows.
     ...
     1. Density-fluctuations update:
        ...
        $$\begin{align*}
            q^{\rho', m + 1} & = - \frac{\Delta t}{2} \left[\frac{1}{J} \left(\frac{\partial J \rho'^m u^n}{\partial \hat{x}} + \frac{\partial J \rho'^m v^n}{\partial \hat{y}} + \frac{\partial J \rho'^m \hat{w}^n}{\partial \hat{z}}\right) + \frac{F^{P, n + 1}}{\bar{\theta}}\right] + \left(\alpha_\mathrm{RK} q^{\rho'}\right)^m,\\
            \rho'^{m + 1} & = \rho'^m + \beta_\mathrm{RK}^m q^{\rho', m + 1},\\
            \rho'^{m + 1} & \rightarrow \left(1 + \alpha_\mathrm{R}^{n + 1} f_\mathrm{RK}^m \frac{\Delta t}{2}\right)^{- 1} \left\{\rho'^{m + 1} + \alpha_\mathrm{R}^{n + 1} f_\mathrm{RK}^m \frac{\Delta t}{2} \bar{\rho} \left[1 - \frac{\left(P / \rho\right)^{m + 1}}{\bar{\theta}}\right]\right\}
        \end{align*}$$

     1. Mass-weighted potential-temperature update:

        $$\begin{align*}
            q^{P, m + 1} & = - \frac{\Delta t}{2} \left\{\frac{1}{J} \left[\frac{\partial J \left(P u\right)^n}{\partial \hat{x}} + \frac{\partial J \left(P v\right)^n}{\partial \hat{y}} + \frac{\partial J \left(P \hat{w}\right)^n}{\partial \hat{z}}\right] - F^{P, n + 1}\right\}\\
            & \quad + \left(\alpha_\mathrm{RK} q^P\right)^m,\\
            P^{m + 1} & = P^m + \beta_\mathrm{RK}^m q^{P, m + 1},\\
            P^{m + 1} & \rightarrow \left(1 + \alpha_\mathrm{R}^{n + 1} f_\mathrm{RK}^m \frac{\Delta t}{2}\right)^{- 1} P^{m + 1} \left(1 + \alpha_\mathrm{R}^{n + 1} f_\mathrm{RK}^m \frac{\Delta t}{2} \frac{\bar{\rho}}{\rho^{m + 1}}\right)
        \end{align*}$$
 ...
 1. The right-hand sides are integrated over $\Delta t / 2$ with an implicit Euler step. The divergence constraint is then enforced by solving a Poisson equation for Exner-pressure differences $\Delta \pi'$, which are used to correct the wind, the density fluctuations and the Exner-pressure itself. In principle, this is the same as in pseudo-incompressible mode, however, instead of $\hat{\boldsymbol{u}}$, the predictor step updates $\hat{\boldsymbol{U}} = J P \hat{\boldsymbol{u}}$. The details are as follows.
     ...
     1. Predictor step:
        ...
        $$\begin{align*}
            ...
            \hat{W}^{n + 1 / 2} & = \left[1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2} + \frac{\left(P /  \rho\right)^\#}{\bar{\theta}} \frac{\left(N \Delta t\right)^2}{4}\right]^{- 1}\\
            & \quad \times \left\{\hat{W}^\# + \frac{\Delta t}{2} J P^\# \left[- c_p \left(\frac{P}{\rho}\right)^\# \mathcal{P}^{\rho \hat{w}, n} + \frac{b'^\#}{J} + \frac{F^{\rho \hat{w}, n + 1}}{\rho^\#}\right]\right.\\
            & \qquad \quad + \left.J P^\#\frac{\left(P /  \rho\right)^\#}{\bar{\theta}} \frac{\left(N \Delta t\right)^2}{4} \left[G^{1 3} \frac{U^{n + 1 / 2}}{J P^\#} + G^{2 3} \frac{V^{n + 1 / 2}}{J P^\#}\right]\right\},\\
            \rho'^{n + 1 / 2} & = - \frac{\rho^\#}{g} \left[1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2} + \frac{\left(P /  \rho\right)^\#}{\bar{\theta}} \frac{\left(N \Delta t\right)^2}{4}\right]^{- 1}\\
            & \quad \times \left\{- \frac{\left(P /  \rho\right)^\#}{\bar{\theta}} N^2 \frac{\Delta t}{2} J \left[\frac{\hat{W}^\#}{J P^\#} + \frac{\Delta t}{2} \left(- c_p \left(\frac{P}{\rho}\right)^\# \mathcal{P}^{\rho \hat{w}, n} + \frac{F^{\rho \hat{w}, n + 1}}{\rho^\#}\right)\right]\right.\\
            & \qquad \quad  + \left(1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2}\right) b'^\# + \frac{\left(P /  \rho\right)^\#}{\bar{\theta}} N^2 \frac{\Delta t}{2} J \left(1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2}\right)\\
     ...
     1. Poisson equation:
        ...
        $$\begin{align*}
            & \frac{1}{J} \left[\frac{\partial U^{n + 1 / 2}}{\partial \hat{x}} + \frac{\partial V^{n + 1 / 2}}{\partial \hat{y}} + \frac{\partial \hat{W}^{n + 1 / 2}}{\partial \hat{z}}\right] - F^{P, n + 1}\\
            & \quad = - \left(\frac{\partial P}{\partial \pi'}\right)^\# \frac{2 \Delta \pi'}{\Delta t}\\
            & \qquad + \frac{1}{J} \left\{\frac{\partial J P^\# \mathcal{C}^{\rho u, \#, n + 1}}{\partial \hat{x}} + \frac{\partial J P^\# \mathcal{C}^{\rho v, \#, n + 1}}{\partial \hat{y}} \vphantom{\left[\left(\frac{\left(P /  \rho\right)^\#}{\bar{\theta}}\right)^{- 1}\right]}\right.\\
            & \qquad \qquad \quad + \frac{\partial}{\partial \hat{z}} \left[\left(1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2} + \frac{\left(P /  \rho\right)^\#}{\bar{\theta}} \frac{\left(N \Delta t\right)^2}{4}\right)^{- 1}\right.\\
            & \qquad \qquad \qquad \qquad \times \left(\frac{\Delta t}{2} J P^\# c_p \left(\frac{P}{\rho}\right)^\# \mathcal{D}^{\rho \hat{w}} + J P^\# \frac{\left(P /  \rho\right)^\#}{\bar{\theta}} \frac{\left(N \Delta t\right)^2}{4}\right.\\
            & \qquad \qquad \qquad \qquad \qquad \times \left.\left.\left.\left(G^{13} \mathcal{C}^{\rho u, \#, n + 1} + G^{23} \mathcal{C}^{\rho v, \#, n + 1}\right)\vphantom{\left(\frac{P^2}{\rho}\right)^\#}\right) \vphantom{\left(\frac{\left(P /  \rho\right)^\#}{\bar{\theta}}\right)^{- 1}}\right]\right\}
        \end{align*}$$
     ...
     1. Corrector step:
        ...
        $$\begin{align*}
            ...
            \hat{W}^{n + 1 / 2} & = \left\{\hat{W}^{n + 1 / 2} - \left[1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2} + \frac{\left(P /  \rho\right)^\#}{\bar{\theta}} \frac{\left(N \Delta t\right)^2}{4}\right]^{- 1}\right.\\
            & \qquad \qquad \qquad \times \left[\frac{\Delta t}{2} J P^\# c_p \left(\frac{P}{\rho}\right)^\# \mathcal{D}^{\rho \hat{w}} + J P^\# \frac{\left(P /  \rho\right)^\#}{\bar{\theta}} \frac{\left(N \Delta t\right)^2}{4}\right.\\
            & \qquad \qquad \qquad \qquad \times \left.\left.\left(G^{1 3} \mathcal{C}^{\rho u, \#, n + 1} + G^{23} \mathcal{C}^{\rho v, \#, n + 1}\right) \vphantom{\left(\frac{P^2}{\rho}\right)^\#}\right] \vphantom{\left[\frac{\left(P /  \rho\right)^\#}{\bar{\theta}}\right]^{- 1}}\right\},\\
            \rho'^{n + 1 / 2} & \rightarrow \rho'^{n + 1 / 2} + \frac{\rho^\#}{g} \left[1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2} + \frac{\left(P /  \rho\right)^\#}{\bar{\theta}} \frac{\left(N \Delta t\right)^2}{4}\right]^{- 1}\\
            & \quad \times \left[- \frac{\left(P /  \rho\right)^\#}{\bar{\theta}} \frac{\left(N \Delta t\right)^2}{4} J c_p \left(\frac{P}{\rho}\right)^\# \mathcal{D}^{\rho \hat{w}}\right.\\
            & \qquad \quad + \left.\frac{\left(P /  \rho\right)^\#}{\bar{\theta}} N^2 \frac{\Delta t}{2} J \left(1 + \beta_\mathrm{R}^{n + 1} \frac{\Delta t}{2}\right)  \left(G^{1 3} \mathcal{C}^{\rho u, \#, n + 1} + G^{2 3} \mathcal{C}^{\rho v, \#, n + 1}\right)\right],\\
            \pi'^{n + 1 / 2} & = \pi'^n + \Delta \pi'
        \end{align*}$$
 ...
 1. The right-hand sides (without the Rayleigh-damping terms) are integrated over $\Delta t / 2$ with an explicit Euler step. In contrast to the pseudo-incompressible mode, the Exner-pressure fluctuations $\pi'^n$ are used to compute the pressure gradient. They are themselves also updated with the left-hand side of the prognostic equation for $P$. Thus, one has
    ...
    $$\begin{align*}
        ...
        \hat{W}^* & = \left\{\hat{W}^n + \frac{\Delta t}{2} J P^n \left[- c_p \left(\frac{P}{\rho}\right)^n \mathcal{P}^{\rho \hat{w}, n} + \frac{b'^n}{J} + \frac{F^{\rho \hat{w}, n + 1}}{\rho^n}\right]\right\},\\
        \rho'^* & = - \frac{\rho^n}{g} \left[b'^n - \frac{\Delta t}{2} N^2 \frac{\left(P / \rho\right)^n}{\bar{\theta}} \frac{W^n}{J P^n}\right],\\
        \pi'^* & = \pi'^n - \frac{\Delta t}{2} \left(\frac{\partial \pi'}{\partial P}\right)^n \left\{\frac{1}{J} \left[\frac{\partial U^n}{\partial \hat{x}} + \frac{\partial V^n}{\partial \hat{y}} + \frac{\partial \hat{W}^n}{\partial \hat{z}}\right] - F^{P, n + 1}\right\}.
    \end{align*}$$
...
$$\begin{align*}
    ...
    q^{k, m + 1} & = \Delta t \left(\frac{\partial \Omega^{m, n}}{\partial \hat{x}} + G^{13} \frac{\partial \Omega^{m, n}}{\partial \hat{z}}\right) + \alpha_\mathrm{RK}^m q^{k, m}, & k^{m + 1} & = k^m + \beta_\mathrm{RK}^m q^{k, m + 1},\\
    q^{l, m + 1} & = \Delta t \left(\frac{\partial \Omega^{m, n}}{\partial \hat{y}} + G^{23} \frac{\partial \Omega^{m, n}}{\partial \hat{z}}\right) + \alpha_\mathrm{RK}^m q^{l, m}, & l^{m + 1} & = l^m + \beta_\mathrm{RK}^m q^{l, m + 1},\\
    q^{m, m + 1} & = \frac{\Delta t}{J} \frac{\partial \Omega^{m, n}}{\partial \hat{z}} + \alpha_\mathrm{RK}^m q^{m, m}, & m^{m + 1} & = m^m + \beta_\mathrm{RK}^m q^{m, m + 1},
\end{align*}$$

where $\Omega^{m, n} = \Omega \left(\boldsymbol{x}^m, \boldsymbol{k}^m, \hat{\boldsymbol{u}}^n\right)$. At the end of every RK3 stage, the Rayleigh damping of the LHS sponge is applied to the phase-space wave-action density with a fractional implicit Euler step, i.e.
