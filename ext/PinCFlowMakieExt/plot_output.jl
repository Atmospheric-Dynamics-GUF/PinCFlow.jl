    ...
    for (variable, i, j, k, n) in fields
        ...
        if variable in ray_volume_properties
            ...
            if nx > 1 && nz > 1
                ...
                rects = vec(Rect.(x0 .- w ./ 2, y0 .- h ./ 2, w, h))
                poly!(ax, rects, color = :purple)
                ...
                #     colormap = cgrad(colormap; categorical = true),
                #     marker = Rect,
                #     markersize = (10, 1), #markersize = collect(
            ...
            # Get the label.
            #label = LaTeXString(attrs(data[variable])["label"])
            label= "Test" #L"%$(variable)\ [\mathrm{units}]"
